...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(ui->sourceComboBox, SIGNAL(currentIndexChanged(QString)), this,
          SLOT(SourceChanged(QString)));
  connect(ui->targetComboBox, SIGNAL(currentIndexChanged(QString)), this,
          SLOT(TargetChanged(QString)));
  ...
  connect(ui->sqlServerWindowsAuthCheckBox, SIGNAL(stateChanged(int)), this,
          SLOT(SqlServerWindowsAuthChecked(int)));
  connect(ui->s_sqlServerWindowsAuthCheckBox, SIGNAL(stateChanged(int)), this,
          SLOT(SqlServerSWindowsAuthChecked(int)));

  connect(this, SIGNAL(UpdateLogSignal()), this, SLOT(UpdateLog()),
          Qt::QueuedConnection);

  connect(ui->s_sqlServerTestConnectionButton, SIGNAL(pressed()), this,
          SLOT(TestSourceConnection()));
  connect(ui->s_oracleTestConnectionButton, SIGNAL(pressed()), this,
          SLOT(TestSourceConnection()));
  connect(ui->s_db2TestConnectionButton, SIGNAL(pressed()), this,
          SLOT(TestSourceConnection()));
  connect(ui->s_ifmxTestConnectionButton, SIGNAL(pressed()), this,
          SLOT(TestSourceConnection()));
  connect(ui->s_mysqlTestConnectionButton, SIGNAL(pressed()), this,
          SLOT(TestSourceConnection()));
  connect(ui->s_aseTestConnectionButton, SIGNAL(pressed()), this,
          SLOT(TestSourceConnection()));
  connect(ui->s_asaTestConnectionButton, SIGNAL(pressed()), this,
          SLOT(TestSourceConnection()));

  connect(ui->t_sqlServerTestConnectionButton, SIGNAL(pressed()), this,
          SLOT(TestTargetConnection()));
  connect(ui->t_db2TestConnectionButton, SIGNAL(pressed()), this,
          SLOT(TestTargetConnection()));
  connect(ui->t_oracleTestConnectionButton, SIGNAL(pressed()), this,
          SLOT(TestTargetConnection()));
  connect(ui->t_mysqlTestConnectionButton, SIGNAL(pressed()), this,
          SLOT(TestTargetConnection()));
  connect(ui->t_pgTestConnectionButton, SIGNAL(pressed()), this,
          SLOT(TestTargetConnection()));

  connect(ui->tablesFileButton, SIGNAL(pressed()), this,
          SLOT(SelectTablesFile()));
...
int MainWindow::GetDbType(QString name) {
  ...
  else if (name.compare("Oracle") == 0)
    type = SQLDATA_ORACLE;
  else if (name.compare("IBM DB2") == 0)
    type = SQLDATA_DB2;
  else if (name.compare("MySQL") == 0)
    type = SQLDATA_MYSQL;
  else if (name.compare("MariaDB") == 0)
    type = SQLDATA_MYSQL;
  else if (name.compare("PostgreSQL") == 0)
    type = SQLDATA_POSTGRESQL;
  else if (name.compare("Sybase ASE") == 0)
    type = SQLDATA_SYBASE;
  else if (name.compare("Informix") == 0)
    type = SQLDATA_INFORMIX;
  else if (name.compare("Sybase SQL Anywhere") == 0)
    type = SQLDATA_ASA;
...
void MainWindow::SourceChanged(QString source) {
  ...
  if (_source_type == SQLDATA_SQL_SERVER)
    ui->sourceConnectionsTabWidget->insertTab(1, ui->sqlServerSourceTab,
                                              "Source - SQL Server");
  else if (_source_type == SQLDATA_ORACLE)
    ui->sourceConnectionsTabWidget->insertTab(1, ui->oracleSourceTab,
                                              "Source - Oracle");
  else if (_source_type == SQLDATA_DB2)
    ui->sourceConnectionsTabWidget->insertTab(1, ui->db2SourceTab,
                                              "Source - IBM DB2");
  else if (_source_type == SQLDATA_MYSQL)
    ui->sourceConnectionsTabWidget->insertTab(1, ui->mysqlSourceTab,
                                              "Source - MySQL");
  else if (_source_type == SQLDATA_SYBASE)
    ui->sourceConnectionsTabWidget->insertTab(1, ui->sybaseSourceTab,
                                              "Source - Sybase ASE");
  else if (_source_type == SQLDATA_INFORMIX)
    ui->sourceConnectionsTabWidget->insertTab(1, ui->informixSourceTab,
                                              "Source - Informix");
  else if (_source_type == SQLDATA_ASA)
    ui->sourceConnectionsTabWidget->insertTab(1, ui->asaSourceTab,
                                              "Source - Sybase SQL Anywhere");
...
void MainWindow::TargetChanged(QString target) {
  ...
  if (_target_type == SQLDATA_SQL_SERVER)
    ui->targetConnectionsTabWidget->insertTab(1, ui->sqlServerTargetTab,
                                              "Target - SQL Server");
  else if (_target_type == SQLDATA_ORACLE)
    ui->targetConnectionsTabWidget->insertTab(1, ui->oracleTargetTab,
                                              "Target - Oracle");
  else if (_target_type == SQLDATA_DB2)
    ui->targetConnectionsTabWidget->insertTab(1, ui->db2TargetTab,
                                              "Target - IBM DB2");
  else if (_target_type == SQLDATA_MYSQL)
    ui->targetConnectionsTabWidget->insertTab(1, ui->mysqlTargetTab, title);
  else if (_target_type == SQLDATA_POSTGRESQL)
    ui->targetConnectionsTabWidget->insertTab(1, ui->pgTargetTab,
                                              "Target - PostgreSQL");
...
void MainWindow::RunValidation() {
  ...
  } else if (ui->validationOptionComboBox->currentIndex() == 1) {
    _cmdline += "rows";
...
void MainWindow::GetSourceConnectionString(std::string &source) {
  ...
  else if (_source_type == SQLDATA_SQL_SERVER)
...
void MainWindow::TestConnection(std::string &conn) {
  ...
  // Try to connect
  int rc = _sqlDataCmd.TestConnection(conn, error, driver_loaded, search_path,
                                      &time_spent);
  ...
  if (search_path.empty() == false) {
    ...
    for (std::list<std::string>::iterator i = search_path.begin();
         i != search_path.end(); i++)
      text.append("\n  ").append((*i).c_str());
  }
...
void MainWindow::GetMysqlConnectionString(std::string &conn, bool source) {
  ...
  if ((source &&
       _source.compare(QString("MariaDB"), Qt::CaseInsensitive) == 0) ||
      (!source &&
       _target.compare(QString("MariaDB"), Qt::CaseInsensitive) == 0))
    conn = "mariadb,";

  QLineEdit *user =
      source ? ui->s_mysqlUserNameLineEdit : ui->mysqlUserNameLineEdit;
  QLineEdit *pwd =
      source ? ui->s_mysqlPasswordLineEdit : ui->mysqlPasswordLineEdit;
  QComboBox *srv =
      source ? ui->s_mysqlServerNameComboBox : ui->mysqlServerNameComboBox;
  QLineEdit *port = source ? ui->s_mysqlPortLineEdit : ui->mysqlPortLineEdit;
  QComboBox *db =
      source ? ui->s_mysqlDatabaseComboBox : ui->mysqlDatabaseComboBox;
...
void MainWindow::UpdateTransferLog(SqlDataReply &reply) {
  ...
  if (reply._cmd_subtype == SQLDATA_CMD_NUMBER_OF_TABLES) {
    ...
    if (_total_tables > 0) {
      ...
      ui->transferStatusComboBox->setCurrentIndex(
          ui->transferStatusComboBox->count() - 1);
    } else {
      // Enable transfer and validate commands
      ui->transferButton->setEnabled(true);
      ui->validateButton->setEnabled(true);

      ui->transferStatusComboBox->addItem(
          QString(reply._s_name).append("No tables found."));
      ui->transferStatusComboBox->setCurrentIndex(
          ui->transferStatusComboBox->count() - 1);
    }
    ...
    if (reply._cmd_subtype == SQLDATA_CMD_ALL_COMPLETE) {
      ...
      if (reply._int1 != 0) {
        ...
        speed_bytes =
            ((double)reply._t_bigint1) / ((double)reply._int1) * 1000.0;
      }
      ...
      ui->transferSpeedBytesEdit->setText(
          QString(speed_bytes_fmt).append("/sec"));
      ui->transferSpeedRowsEdit->setText(
          QString::number(speed_rows).append(" rows/sec"));
      ...
      ui->transferStatusComboBox->addItem(
          QString(reply._s_name).append("Transfer complete."));
      ui->transferStatusComboBox->setCurrentIndex(
          ui->transferStatusComboBox->count() - 1);
  ...
  if (reply._cmd_subtype == SQLDATA_CMD_STARTED) {
    ...
    ui->transferStatusComboBox->addItem(
        QString(reply._s_name).append(" started."));
    ui->transferStatusComboBox->setCurrentIndex(
        ui->transferStatusComboBox->count() - 1);
  } else if (reply._cmd_subtype == SQLDATA_CMD_OPEN_CURSOR) {
    item->setText("Open cursor");
    ui->transferStatusComboBox->addItem(
        QString(reply._s_name).append(" cursor open."));
    ui->transferStatusComboBox->setCurrentIndex(
        ui->transferStatusComboBox->count() - 1);
  } else if (reply._cmd_subtype == SQLDATA_CMD_DROP) {
    item->setText("Drop target");
    ui->transferStatusComboBox->addItem(
        QString(reply._s_name).append(" target table dropped."));
    ui->transferStatusComboBox->setCurrentIndex(
        ui->transferStatusComboBox->count() - 1);
    ...
    if (reply._cmd_subtype == SQLDATA_CMD_COMPLETE_WITH_DDL_ERROR) {
      ...
      ui->transferStatusComboBox->addItem(
          QString(reply._s_name).append(" failed."));
      ...
      ui->transferStatusComboBox->setCurrentIndex(
          ui->transferStatusComboBox->count() - 1);
    } else if (reply._cmd_subtype == SQLDATA_CMD_TRUNCATE) {
      item->setText("Truncate target");
      ui->transferStatusComboBox->addItem(
          QString(reply._s_name).append(" target table truncated."));
      ui->transferStatusComboBox->setCurrentIndex(
          ui->transferStatusComboBox->count() - 1);
    } else if (reply._cmd_subtype == SQLDATA_CMD_CREATE) {
      item->setText("Create target");
      ui->transferStatusComboBox->addItem(
          QString(reply._s_name).append(" target table created."));
      ui->transferStatusComboBox->setCurrentIndex(
          ui->transferStatusComboBox->count() - 1);

      _ddl_executed++;
      ui->ddlExecEdit->setText(QString::number(_ddl_executed));
    } else if (reply._cmd_subtype == SQLDATA_CMD_IN_PROGRESS) {
      item->setText("In Progress");
    } else
      // DDL command
      if (reply._cmd_subtype == SQLDATA_CMD_ADD_DEFAULT ||
          reply._cmd_subtype == SQLDATA_CMD_ADD_CHECK_CONSTRAINT ||
          reply._cmd_subtype == SQLDATA_CMD_ADD_PRIMARY_KEY ||
          reply._cmd_subtype == SQLDATA_CMD_ADD_UNIQUE_KEY ||
          reply._cmd_subtype == SQLDATA_CMD_ADD_FOREIGN_KEY ||
          reply._cmd_subtype == SQLDATA_CMD_ADD_COMMENT ||
          reply._cmd_subtype == SQLDATA_CMD_CREATE_INDEX ||
          reply._cmd_subtype == SQLDATA_CMD_CREATE_SEQUENCE ||
      ...
      } else if (reply._cmd_subtype == SQLDATA_CMD_COMPLETE) {
        if (reply.rc == 0) {
          ...
          ui->transferStatusComboBox->addItem(
              QString(reply._s_name).append(" data transfer complete."));
        } else {
          item->setText("Failed");
  ...
  if (reply._cmd_subtype == SQLDATA_CMD_IN_PROGRESS ||
    ...
    if (reply._t_bigint1 != 0 && reply._t_int2 != 0)
      speed_bytes =
          ((double)reply._t_bigint1) / ((double)reply._t_int2) * 1000.0;
...
void MainWindow::UpdateValidationRowCount(SqlDataReply &reply) {
  ...
  if (reply._cmd_subtype == SQLDATA_CMD_NUMBER_OF_TABLES) {
    ...
    ui->validation_totalNumberTablesEdit->setText(
        QString::number(_total_tables));

    ui->validationStatusComboBox->addItem("Validating tables.");
    ui->validationStatusComboBox->setCurrentIndex(
        ui->validationStatusComboBox->count() - 1);
  } else
    // All operations are completed
    if (reply._cmd_subtype == SQLDATA_CMD_ALL_COMPLETE) {
      char time_fmt[21];
      ...
      ui->rowsDifferenceEdit->setText(
          QString::number(abs(reply._s_int1 - reply._t_int1)));
      ...
      ui->validationStatusComboBox->setCurrentIndex(
          ui->validationStatusComboBox->count() - 1);
    } else if (reply._cmd_subtype == SQLDATA_CMD_STARTED) {
      int idx = GetTableIndex(reply._s_name);
      ...
      if (reply._cmd_subtype == SQLDATA_CMD_COMPLETE) {
        ...
        if (reply._s_rc == -1 || reply._t_rc == -1) {
          ...
          if (reply._s_int1 != reply._t_int1) {
            ...
            ui->notEqualRowsCountEdit->setText(
                QString::number(++_not_equal_tables));
          } else {
            item->setText("Ok");
        ...
        if (item != NULL)
          item->setText(QString(time_fmt) + " (" + s_time_fmt + ", " +
                        t_time_fmt + ")");
        ...
        ui->rowsDifferenceEdit->setText(
            QString::number(abs(_source_all_rows - _target_all_rows)));
...
void MainWindow::UpdateRawLog() {
  ...
  // Handle each message
  for (std::list<QString>::iterator i = _raw_log_replies.begin();
       i != _raw_log_replies.end(); i++)
...
int MainWindow::GetTableIndex(const char *name) {
  ...
  for (std::list<std::string>::iterator i = _avail_tables.begin();
       i != _avail_tables.end(); i++, k++) {
...
void MainWindow::ConsoleCallbackS(void *object, const char *format,
                                  va_list args) {
  if (object == NULL)
    return;
...
void MainWindow::SetTransferOptions() {
  ...
  if (ui->createTablesRadioButton->isChecked() == true)
    _cmdline += "create";
  else if (ui->recreateTablesRadioButton->isChecked() == true)
    _cmdline += "recreate";
  else if (ui->truncateTablesRadioButton->isChecked() == true)
    _cmdline += "truncate";
  else
    _cmdline += "none";
}
...
void MainWindow::InitTransferLogView() {
  // Set headers
  QStringList headers;
  headers << "Table Name" << "Status" << "Rows Read" << "Rows Written"
...
void MainWindow::InitValidationLogView() {
  // Set headers
  QStringList headers;
  headers << "Table Name" << "Status" << "Rows in Source" << "Rows in Target"
          << "Validation Time";
...
void MainWindow::LoadOptions() {
  ...
  // Initial active tab
  ui->mainTabWidget->setCurrentIndex(
      _settings->value(INI_MAINTAB_INDEX, 0).toInt());

  // Source and target database types
  int sourceIdx =
      ui->sourceComboBox->findText(_settings->value(INI_SOURCE).toString());
  int targetIdx =
      ui->targetComboBox->findText(_settings->value(INI_TARGET).toString());

  // First set indexes to -1 to force calling signals when first item will be
  // set
  ...
  QStringList oracleList =
      _settings->value(INI_ORACLE_CONNECTIONS).toStringList();
  ...
  ui->oracleSidComboBox->setCurrentIndex(
      _settings->value(INI_LAST_ORACLE_CONNECTION).toInt());
  ...
  QStringList sqlServerList =
      _settings->value(INI_SQL_SERVER_CONNECTIONS).toStringList();
  ...
  ui->sqlServerNameComboBox->setCurrentIndex(
      _settings->value(INI_LAST_SQL_SERVER_CONNECTION).toInt());
  ...
  // MySQL server names
  QStringList mysqlList =
      _settings->value(INI_MYSQL_CONNECTIONS).toStringList();
  ...
  QString userName;
  ui->oracleUserNameLineEdit->setText(
      _settings->value(INI_ORACLE_USERNAME).toString());
  ui->t_oracleUserNameLineEdit->setText(
      _settings->value(INI_ORACLE_T_USERNAME).toString());
  ui->sqlServerUserNameLineEdit->setText(
      _settings->value(INI_SQL_SERVER_USERNAME).toString());
  ui->s_sqlServerUserNameLineEdit->setText(
      _settings->value(INI_SQL_SERVER_S_USERNAME).toString());
  ui->mysqlUserNameLineEdit->setText(
      _settings->value(INI_MYSQL_USERNAME).toString());
  ui->s_mysqlUserNameLineEdit->setText(
      _settings->value(INI_MYSQL_S_USERNAME).toString());
  ui->pgUserNameLineEdit->setText(_settings->value(INI_PG_USERNAME).toString());
  ui->aseUserNameLineEdit->setText(
      _settings->value(INI_ASE_USERNAME).toString());
  ui->asaUserNameLineEdit->setText(
      _settings->value(INI_ASA_USERNAME).toString());
  ...
  ui->db2UserNameLineEdit->setText(
      _settings->value(INI_DB2_USERNAME).toString());
  ui->t_db2UserNameLineEdit->setText(
      _settings->value(INI_DB2_T_USERNAME).toString());

  bool save_ora_password =
      _settings->value(INI_ORACLE_SAVE_PASSWORD, false).toBool();
  bool save_ora_t_password =
      _settings->value(INI_ORACLE_T_SAVE_PASSWORD, false).toBool();
  bool save_sql_password =
      _settings->value(INI_SQL_SERVER_SAVE_PASSWORD, false).toBool();
  bool save_sql_s_password =
      _settings->value(INI_SQL_SERVER_S_SAVE_PASSWORD, false).toBool();
  bool save_mysql_password =
      _settings->value(INI_MYSQL_SAVE_PASSWORD, false).toBool();
  bool save_mysql_s_password =
      _settings->value(INI_MYSQL_S_SAVE_PASSWORD, false).toBool();
  bool save_pg_password =
      _settings->value(INI_PG_SAVE_PASSWORD, false).toBool();
  bool save_ase_password =
      _settings->value(INI_ASE_SAVE_PASSWORD, false).toBool();
  bool save_asa_password =
      _settings->value(INI_ASA_SAVE_PASSWORD, false).toBool();
  bool save_ifmx_password =
      _settings->value(INI_IFMX_SAVE_PASSWORD, false).toBool();
  bool save_db2_password =
      _settings->value(INI_DB2_SAVE_PASSWORD, false).toBool();
  bool save_db2_t_password =
      _settings->value(INI_DB2_T_SAVE_PASSWORD, false).toBool();

  if (save_ora_password == true)
    ui->oraclePasswordLineEdit->setText(
        DecodePassword(_settings->value(INI_ORACLE_PASSWORD).toString()));

  if (save_ora_t_password == true)
    ui->t_oraclePasswordLineEdit->setText(
        DecodePassword(_settings->value(INI_ORACLE_T_PASSWORD).toString()));

  if (save_sql_password == true)
    ui->sqlServerPasswordLineEdit->setText(
        DecodePassword(_settings->value(INI_SQL_SERVER_PASSWORD).toString()));

  if (save_sql_s_password == true)
    ui->s_sqlServerPasswordLineEdit->setText(
        DecodePassword(_settings->value(INI_SQL_SERVER_S_PASSWORD).toString()));

  if (save_mysql_password == true)
    ui->mysqlPasswordLineEdit->setText(
        DecodePassword(_settings->value(INI_MYSQL_PASSWORD).toString()));

  if (save_mysql_s_password == true)
    ui->s_mysqlPasswordLineEdit->setText(
        DecodePassword(_settings->value(INI_MYSQL_S_PASSWORD).toString()));

  if (save_pg_password == true)
    ui->pgPasswordLineEdit->setText(
        DecodePassword(_settings->value(INI_PG_PASSWORD).toString()));

  if (save_ase_password == true)
    ui->asePasswordLineEdit->setText(
        DecodePassword(_settings->value(INI_ASE_PASSWORD).toString()));

  if (save_asa_password == true)
    ui->asaPasswordLineEdit->setText(
        DecodePassword(_settings->value(INI_ASA_PASSWORD).toString()));

  if (save_ifmx_password == true)
    ui->ifmxPasswordLineEdit->setText(
        DecodePassword(_settings->value(INI_IFMX_PASSWORD).toString()));

  if (save_db2_password == true)
    ui->db2PasswordLineEdit->setText(
        DecodePassword(_settings->value(INI_DB2_PASSWORD).toString()));

  if (save_db2_t_password == true)
    ui->t_db2PasswordLineEdit->setText(
        DecodePassword(_settings->value(INI_DB2_T_PASSWORD).toString()));
  ...
  ui->sqlServerWindowsAuthCheckBox->setChecked(
      _settings->value(INI_SQL_SERVER_AUTH, false).toBool());
  ui->s_sqlServerWindowsAuthCheckBox->setChecked(
      _settings->value(INI_SQL_SERVER_S_AUTH, false).toBool());

  // SQL Server database list
  QStringList sqlServerDatabases =
      _settings->value(INI_SQL_SERVER_DATABASES).toStringList();
  ui->sqlServerDatabaseComboBox->addItems(sqlServerDatabases);
  ui->sqlServerDatabaseComboBox->setCurrentIndex(
      _settings->value(INI_LAST_SQL_SERVER_DATABASE).toInt());

  // SQL Server (as source) database list
  sqlServerDatabases =
      _settings->value(INI_SQL_SERVER_S_DATABASES).toStringList();
  ui->s_sqlServerDatabaseComboBox->addItems(sqlServerDatabases);
  ui->s_sqlServerDatabaseComboBox->setCurrentIndex(
      _settings->value(INI_LAST_SQL_SERVER_S_DATABASE).toInt());

  // MySQL database list
  QStringList mysqlDatabases =
      _settings->value(INI_MYSQL_DATABASES).toStringList();
  ...
  QStringList asaDatasources =
      _settings->value(INI_ASA_DATASOURCES).toStringList();
  ui->asaDatasourceComboBox->addItems(asaDatasources);

  // Informix database list
  QStringList ifmxDatabases =
      _settings->value(INI_IFMX_DATABASES).toStringList();
  ...
  ui->tablesComboBox->setCurrentIndex(
      _settings->value(INI_LAST_TABLE_LIST).toInt());
  ...
  ui->tablesExcludeComboBox->setCurrentIndex(
      _settings->value(INI_LAST_EXCL_TABLE_LIST).toInt());

  // Schema name mapping
  QStringList schemaMappingList =
      _settings->value(INI_SCHEMA_MAPPING_LIST).toStringList();

  if (schemaMappingList.isEmpty() == false)
    ui->schemaMappingComboBox->addItems(schemaMappingList);

  ui->schemaMappingComboBox->setCurrentIndex(
      _settings->value(INI_LAST_SCHEMA_MAPPING_LIST).toInt());

  // Load to the default schema option
  ui->defaultSchemaCheckBox->setChecked(
      _settings->value(INI_DEFAULT_SCHEMA, true).toBool());

  // Table creation options
  ui->createTablesRadioButton->setChecked(
      _settings->value(INI_CREATE_TABLE, false).toBool());
  ui->recreateTablesRadioButton->setChecked(
      _settings->value(INI_RECREATE_TABLE, true).toBool());
  ui->truncateTablesRadioButton->setChecked(
      _settings->value(INI_TRUNCATE_TABLE, false).toBool());
  ui->loadExistingRadioButton->setChecked(
      _settings->value(INI_LOAD_TO_EXISTING, false).toBool());

  // Validation options
  ui->validationOptionComboBox->setCurrentIndex(
      _settings->value(INI_VALIDATION_OPTION).toInt());

  ui->sessionsSpinBox->setValue(
      _settings->value(INI_CONCURRENT_SESSIONS, 4).toInt());
}
...
void MainWindow::SaveOptions() {
  ...
  _settings->setValue(INI_LAST_ORACLE_CONNECTION,
                      ui->oracleSidComboBox->currentIndex());
  _settings->setValue(INI_LAST_SQL_SERVER_CONNECTION,
                      ui->sqlServerNameComboBox->currentIndex());
  ...
  _settings->setValue(INI_ORACLE_T_USERNAME,
                      ui->t_oracleUserNameLineEdit->text());
  _settings->setValue(INI_ORACLE_PASSWORD,
                      (save_ora_password == true)
                          ? EncodePassword(ui->oraclePasswordLineEdit->text())
                          : "");
  _settings->setValue(INI_ORACLE_T_PASSWORD,
                      (save_ora_t_password == true)
                          ? EncodePassword(ui->t_oraclePasswordLineEdit->text())
                          : "");
  ...
  _settings->setValue(INI_SQL_SERVER_USERNAME,
                      ui->sqlServerUserNameLineEdit->text());
  _settings->setValue(INI_SQL_SERVER_S_USERNAME,
                      ui->s_sqlServerUserNameLineEdit->text());
  _settings->setValue(
      INI_SQL_SERVER_PASSWORD,
      (save_sql_password == true)
          ? EncodePassword(ui->sqlServerPasswordLineEdit->text())
          : "");
  _settings->setValue(
      INI_SQL_SERVER_S_PASSWORD,
      (save_sql_s_password == true)
          ? EncodePassword(ui->s_sqlServerPasswordLineEdit->text())
          : "");
  ...
  _settings->setValue(INI_SQL_SERVER_AUTH,
                      ui->sqlServerWindowsAuthCheckBox->isChecked());
  _settings->setValue(INI_SQL_SERVER_S_AUTH,
                      ui->s_sqlServerWindowsAuthCheckBox->isChecked());
  SaveComboBoxToIni(INI_SQL_SERVER_DATABASES, ui->sqlServerDatabaseComboBox);
  SaveComboBoxToIni(INI_SQL_SERVER_S_DATABASES,
                    ui->s_sqlServerDatabaseComboBox);
  _settings->setValue(INI_LAST_SQL_SERVER_DATABASE,
                      ui->sqlServerDatabaseComboBox->currentIndex());
  _settings->setValue(INI_LAST_SQL_SERVER_S_DATABASE,
                      ui->s_sqlServerDatabaseComboBox->currentIndex());

  _settings->setValue(INI_MYSQL_USERNAME, ui->mysqlUserNameLineEdit->text());
  _settings->setValue(INI_MYSQL_S_USERNAME,
                      ui->s_mysqlUserNameLineEdit->text());
  _settings->setValue(INI_MYSQL_PASSWORD,
                      (save_mysql_password == true)
                          ? EncodePassword(ui->mysqlPasswordLineEdit->text())
                          : "");
  _settings->setValue(INI_MYSQL_S_PASSWORD,
                      (save_mysql_s_password == true)
                          ? EncodePassword(ui->s_mysqlPasswordLineEdit->text())
                          : "");
  ...
  _settings->setValue(INI_PG_PASSWORD,
                      (save_pg_password == true)
                          ? EncodePassword(ui->pgPasswordLineEdit->text())
                          : "");
  ...
  _settings->setValue(INI_ASE_PASSWORD,
                      (save_ase_password == true)
                          ? EncodePassword(ui->asePasswordLineEdit->text())
                          : "");
  ...
  _settings->setValue(INI_ASA_PASSWORD,
                      (save_asa_password == true)
                          ? EncodePassword(ui->asaPasswordLineEdit->text())
                          : "");
  ...
  _settings->setValue(INI_IFMX_PASSWORD,
                      (save_ifmx_password == true)
                          ? EncodePassword(ui->ifmxPasswordLineEdit->text())
                          : "");
  ...
  _settings->setValue(INI_DB2_PASSWORD,
                      (save_db2_password == true)
                          ? EncodePassword(ui->db2PasswordLineEdit->text())
                          : "");
  _settings->setValue(INI_DB2_T_PASSWORD,
                      (save_db2_t_password == true)
                          ? EncodePassword(ui->t_db2PasswordLineEdit->text())
                          : "");
  ...
  if (ui->tablesExcludeComboBox->currentText().isEmpty() == false)
    _settings->setValue(INI_LAST_EXCL_TABLE_LIST,
                        ui->tablesExcludeComboBox->currentIndex());
  else
    _settings->setValue(INI_LAST_EXCL_TABLE_LIST, -1);

  if (ui->schemaMappingComboBox->currentText().isEmpty() == false)
    _settings->setValue(INI_LAST_SCHEMA_MAPPING_LIST,
                        ui->schemaMappingComboBox->currentIndex());
  else
    _settings->setValue(INI_LAST_SCHEMA_MAPPING_LIST, -1);

  _settings->setValue(INI_DEFAULT_SCHEMA,
                      ui->defaultSchemaCheckBox->isChecked());

  // Table list or file
  _settings->setValue(INI_TABLE_LIST_RADIO,
                      ui->tablesListRadioButton->isChecked());
  _settings->setValue(INI_TABLE_FILE_RADIO,
                      ui->tablesFileRadioButton->isCheckable());

  // Save table creation options
  _settings->setValue(INI_CREATE_TABLE,
                      ui->createTablesRadioButton->isChecked());
  _settings->setValue(INI_RECREATE_TABLE,
                      ui->recreateTablesRadioButton->isChecked());
  _settings->setValue(INI_TRUNCATE_TABLE,
                      ui->truncateTablesRadioButton->isChecked());
  _settings->setValue(INI_LOAD_TO_EXISTING,
                      ui->loadExistingRadioButton->isChecked());

  _settings->setValue(INI_VALIDATION_OPTION,
                      ui->validationOptionComboBox->currentIndex());
...
void MainWindow::SelectTablesFile() {
  QString file = QFileDialog::getOpenFileName(
      this, "Select a File with List of Tables", "");
...
void MainWindow::SaveComboBoxToIni(char const *optionName,
                                   QComboBox *comboBox) {
