...
void File::GetPathFromDirectoryAndFile(std::string &path, const char *dir,
                                       const char *file) {
...
void File::SplitDirectoryAndFile(const char *path, std::string &dir,
                                 std::string &file) {
  if (path == NULL)
    return;
...
void File::CreateDirectories(const char *path) {
  ...
  while (path[i]) {
    ...
    // path[i] points either to separator or end of string (in the last case, we
    // also need to create last directory
    dir.assign(path, i);
...
#if defined(WIN32) || defined(WIN64)
  // open the file
  // if _S_IWRITE is not set, the Read Only file is created (at least on
  // Windows)
  int fileh = _open(file, _O_CREAT | _O_RDWR | _O_BINARY | _O_TRUNC,
                    _S_IREAD | _S_IWRITE);
#else
  // open the file
