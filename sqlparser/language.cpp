...
void SqlParser::ConvertIdentifier(Token *token, int type, int scope) {
  ...
  // if(_spl_current_declaring_cursor != NULL && ConvertCursorParameter(token)
  // == true) 	return;
  ...
  if ((type == SQL_IDENT_COLUMN || type == SQL_IDENT_COLUMN_SINGLE) &&
      Source(SQL_ORACLE) == true && Target(SQL_MYSQL) == true) {
    ConvertColumnIdentifier(token, scope);
    return;
  } else if (type == SQL_IDENT_COLUMN_SINGLE && Source(SQL_TERADATA) == true &&
             Target(SQL_ESGYNDB) == true) {
  ...
  if (type == SQL_IDENT_COLVAR) {
  ...
  } else if (_source == SQL_POSTGRESQL) {
    TokenStr ident;
    size_t len = 0;
    ...
    if (_target == SQL_SQL_SERVER) {
      ...
      if (Target(SQL_ORACLE, SQL_DB2, SQL_POSTGRESQL, SQL_GREENPLUM) == true) {
        ...
        // Change quote to backtick for MySQL (" is not suppored by default in
        // MySQL)
        if (Target(SQL_MARIADB, SQL_MYSQL)) {
          if (*cur == '"' || *cur == '[') {
            ...
            *ident = '`';
            ...
            size_t i = 1;
            for (i = 1; i < token->len - 1; i++) {
              // Open or close quote is faced inside, can be qualified
              // identifier
...
void SqlParser::ConvertObjectIdentifier(Token *token, int scope) {
  ...
  if (_spl_package != NULL && _target != SQL_ORACLE &&
      (scope == SQL_SCOPE_FUNC || scope == SQL_SCOPE_PROC))
  ...
  if (parts > 1)
    ...
    if (!_option_set_explicit_schema.empty()) {
      TokenStr schema;
      schema.Append(_option_set_explicit_schema.c_str(), L"",
                    _option_set_explicit_schema.length());
      schema.Append(".", L".", 1);
      schema.Append(ident);
      ident.Set(schema);
    }
...
bool SqlParser::ConvertCursorParameter(Token *token) {
  ...
  if (Target(SQL_SQL_SERVER, SQL_MARIADB, SQL_MYSQL) == true &&
      _spl_cursor_params.GetCount() > 0) {
    for (ListwmItem *i = _spl_cursor_params.GetFirst(); i != NULL;
         i = i->next) {
...
bool SqlParser::ConvertImplicitRecordVariable(Token *token) {
  ...
  // Sybase ADS allows referencing field of previously declared cursor in
  // declaration of another cursor
  if (!exists && Source(SQL_SYBASE_ADS) &&
...
bool SqlParser::ConvertImplicitRecordVariable(Token *token, Token *rec) {
  // Check for rec. prefix
  if (rec != NULL && Token::Compare(rec, token, rec->len) &&
      token->Compare(".", L".", rec->len, 1)) {
    // Change to @rec_field in SQL Server, rec_field in MySQL
...
bool SqlParser::ConvertImplicitForRecordVariable(Token *token) {
  ...
  if (token == NULL || _source != SQL_DB2 ||
      _spl_implicit_record_fields.GetCount() == 0)
    return false;
  ...
  if (i != NULL && i->value != NULL && i->value2 != NULL) {
    ...
    if (Token::Compare(token, rec, rec->len) == false) {
      ...
      if (IsScope(SQL_SCOPE_INSERT_VALUES) == true ||
          ...
          (IsScope(SQL_SCOPE_ASSIGNMENT_RIGHT_SIDE) == true &&
...
bool SqlParser::ConvertTriggerNewOldColumn(Token *token) {
  ...
  if (_spl_new_correlation_name != NULL &&
      Token::Compare(_spl_new_correlation_name, token,
                     _spl_new_correlation_name->len) == true &&
      token->Compare(".", L".", _spl_new_correlation_name->len, 1) == true) {
    // :new.column in Oracle, but not in WHEN condition
    if (_target == SQL_ORACLE &&
        IsScope(SQL_SCOPE_TRG_WHEN_CONDITION) == false) {
      TokenStr ident(":", L":", 1);
      ident.Append(token);
    ...
    if (_spl_new_correlation_name == NULL &&
        Token::Compare(token, "NEW", L"NEW", 1, 3) == true &&
      ...
      if (_spl_old_correlation_name != NULL &&
          Token::Compare(_spl_old_correlation_name, token,
                         _spl_old_correlation_name->len) == true &&
          token->Compare(".", L".", _spl_old_correlation_name->len, 1) ==
              true) {
        ...
        exists = true;
      } else if (_spl_old_correlation_name == NULL &&
                 Token::Compare(token, "OLD", L"OLD", 1, 3) == true &&
                 token->Compare(".", L".", 4, 1) == true) {
        // :old.column in Oracle, but not in WHEN condition
        if (_target == SQL_ORACLE &&
            IsScope(SQL_SCOPE_TRG_WHEN_CONDITION) == false) {
...
// Convert declared local temporary table (Sybase ASA), table variable (SQL
// Server)
...
bool SqlParser::ConvertSessionTemporaryTable(Token *token) {
  ...
  if ((_source == SQL_DB2 && _target != SQL_DB2 &&
       token->Compare("SESSION.", L"SESSION.", 0, 8)) ||
      // In Sybase ASE temporary table reference includes TEMPDB.. qualifier
      (_source == SQL_SYBASE && _target != SQL_SYBASE &&
       token->Compare("TEMPDB..", L"TEMPDB..", 0, 8))) {
    TokenStr name;
...
void SqlParser::ConvertParameterIdentifier(Token *ref, Token *decl) {
  ...
  // Set value of the referenced parameter to the value changed at the
  // declaration
...
void SqlParser::ConvertVariableIdentifier(Token *ref, Token *decl) {
  ...
  // Set value of the referenced variable to the value changed at the
  // declaration
  Token::Change(ref, decl);
...
bool SqlParser::ConvertToTsqlVariable(Token *name) {
  ...
  *ident = '@';
  ...
  for (size_t i = 0; i < name->len; i++) {
    // Check for a special character to be replaced by @ at first position
...
bool SqlParser::ConvertOraclePseudoColumn(Token *token) {
  ...
  if (len > 8 && token->Compare(".nextval", L".nextval", len - 8, 8) == true) {
    ...
    // .currval reference
    if (len > 8 &&
        token->Compare(".currval", L".currval", len - 8, 8) == true) {
...
bool SqlParser::ParseColumnConstraints(Token *create, Token *table_name,
                                       Token *column, Token *type,
                                       Token *type_end, Token **id_col,
        ...
        if (cns->Compare("NULL", L"NULL", 4) == true) {
          ...
          if (cns->Compare("NOT", L"NOT", 3) == true) {
            ...
            if (TOKEN_CMP(next, "NULL") == true) {
              ...
              if (TOKEN_CMP(next, "LOGGED") == true ||
                  TOKEN_CMP(next, "COMPACT") == true) {
                if (_target != SQL_DB2)
                  Token::Remove(cns, next);
              } else
                ...
                if (TOKEN_CMP(next, "CASESPECIFIC") == true) {
                  if (Target(SQL_TERADATA, SQL_ESGYNDB) == false)
                    ...
                    // NOT CASESPECIFIC must go right after data type before NOT
                    // NULL/DEFAULT in EsgynDB
                    if (_source != SQL_ESGYNDB && _target == SQL_ESGYNDB) {
                      AppendFirst(type_end, " NOT CASESPECIFIC",
                                  L" NOT CASESPECIFIC", 17, next);
                      Token::Remove(cns, next);
                    }
                }
            ...
            if (cns->Compare("CHECK", L"CHECK", 5) == true) {
              ...
              if (cns->Compare("IDENTITY", L"IDENTITY", 8) == true) {
                ...
                if (_target == SQL_ORACLE)
                  Token::Remove(cns);
                else if (_target == SQL_POSTGRESQL) {
                  Token *seq_name =
                      AppendIdentifier(table_name, "_seq", L"_seq", 4);
                ...
                if (cns->Compare("ROWGUIDCOL", L"ROWGUIDCOL", 10) == true) {
                  // GUIDs are not automatically generated, so remove the
                  // keyword
                  if (_target != SQL_SQL_SERVER)
                    Token::Remove(cns);
                  ...
                  // DB2 GENERATED ALWAYS | BY DEFAULT AS IDENTITY
                  else if (cns->Compare("GENERATED", L"GENERATED", 9) == true) {
                    ParseDb2GeneratedClause(create, table_name, column, cns,
                                            id_col, id_start, id_inc,
                                            id_default);
                    ...
                    if (cns->Compare("FOR", L"FOR", 3) == true) {
                      ...
                      if (Token::Compare(type, "BIT", L"BIT", 3) == true ||
                          Token::Compare(type, "SBCS", L"SBCS", 4) == true ||
                      ...
                      if (cns->Compare("CCSID", L"CCSID", 5) == true) {
                        ...
                        else if (cns->Compare("COMPACT", L"COMPACT", 7) ==
                                 true) {
                          // Remove for other databases
                          if (_target != SQL_DB2)
                            Token::Remove(cns);
                        ...
                        // DB2 INLINE LENGTH clause for LOB data types; Sybase
                        // ASA INLINE
                        else if (cns->Compare("INLINE", L"INLINE", 6) == true) {
                          /*Token *length */ (void)GetNextWordToken(
                              "LENGTH", L"LENGTH", 6);
                          ...
                          if (cns->Compare("PREFIX", L"PREFIX", 6) == true) {
                            ...
                            if (cns->Compare("COMPUTE", L"COMPUTE", 7) ==
                                true) {
                              ...
                              if (open != NULL) {
                                ...
                                // Compute columns in SQL Server does not
                                // include data type and NULL/NOT NULL
                                if (_target == SQL_SQL_SERVER) {
                            ...
                            // Check for AUTO_INCREMENT clause in MySQL
                            else if (cns->Compare("AUTO_INCREMENT",
                                                  L"AUTO_INCREMENT",
                                                  14) == true) {
                              ...
                              if (_target == SQL_SQL_SERVER)
                                ...
                                if (_target == SQL_ORACLE)
                                  ...
                                  // For PostgreSQL and Greenplum, add DEFAULT
                                  // nextval
                                  if (_target == SQL_POSTGRESQL ||
                                      _target == SQL_GREENPLUM) {
                                    Token *seq_name = AppendIdentifier(
                                        table_name, "_seq", L"_seq", 4);

                                    // Generate CREATE SEQUENCE before CREATE
                                    // TABLE
                                    Prepend(create, "CREATE SEQUENCE ",
                                            L"CREATE SEQUENCE ", 16);
                                    PrependCopy(create, seq_name);
                                    Prepend(create, ";\n\n", L";\n\n", 3);

                                    // Generate DEFAULT nextval('tablename_seq')
                                    // clause
                                    Token::Change(cns, "DEFAULT ", L"DEFAULT ",
                                                  8);
                                    Append(cns, "nextval ('", L"nextval ('",
                                           10);
                              ...
                              // UNSIGNED in MySQL
                              if (cns->Compare("UNSIGNED", L"UNSIGNED", 8) ==
                                  true) {
                                ...
                                // COLLATE in MySQL
                                if (cns->Compare("COLLATE", L"COLLATE", 7) ==
                                    true) {
                                  ...
                                  if (cns->Compare("CHARACTER", L"CHARACTER",
                                                   9) == true) {
                                    Token *set =
                                        GetNextWordToken("SET", L"SET", 3);
                                    ...
                                    if (Target(SQL_SQL_SERVER, SQL_ORACLE,
                                               SQL_HIVE) == true)
                                      Token::Remove(cns, name);
                                    else
                                      // EsgynDB does not support LATIN charset
                                      if (_target == SQL_ESGYNDB &&
                                          TOKEN_CMP(name, "LATIN") == true)
                                        Token::Remove(cns, name);

                                    num++;
                                  } else
                                    // COMMENT in MySQL
                                    if (cns->Compare("COMMENT", L"COMMENT",
                                                     7) == true) {
                                      // Get string literal
                                      ...
                                      // ON UPDATE CURRENT_TIMESTAMP clause in
                                      // MySQL
                                      if (cns->Compare("ON", L"ON", 2) ==
                                          true) {
                                        Token *update = GetNextWordToken(
                                            "UPDATE", L"UPDATE", 6);
                                        Token *current = GetNextWordToken(
                                            "CURRENT_TIMESTAMP",
                                        ...
                                        if (cns->Compare("CASESPECIFIC",
                                          ...
                                          if (cns->Compare("COMPRESS",
                                                           L"COMPRESS",
                                                           8) == true) {
                                            ParseTeradataCompressClauseDefaultExpression(
                                                cns);
                                            num++;
                                          } else
                                            // TITLE 'text' clause in Teradata
                                            if (TOKEN_CMP(cns, "TITLE") ==
                                                true) {
                                              Token *value =
                                                  GetNextStringToken();
                                              ...
                                              // FORMAT 'format' clause in
                                              // Teradata
                                              if (TOKEN_CMP(cns, "FORMAT") ==
                                                  true) {
                                                Token *value =
                                                    GetNextStringToken();
...
bool SqlParser::ParseDefaultExpression(Token *type, Token *type_end,
                                       Token *token, int pos) {
  if (token == NULL)
    return false;
  ...
  // DB2 DEFAULT maybe without the expression and terminated by , ) or CHECK ()
  // or FOR BIT | SBCS | MIXED DATA
  if (first->Compare(',', L',') == true || first->Compare(')', L')') == true ||
      first->Compare("CHECK", L"CHECK", 5) == true ||
      first->Compare("FOR", L"FOR", 3) == true) {
    PushBack(first);
    first = NULL;
  } else
  ...
  if (first != NULL && first->type == TOKEN_FUNCTION) {
    ...
    if (_target == SQL_ESGYNDB && TOKEN_CMP(first, "CURRENT_DATE") == false &&
        TOKEN_CMP(first, "CURRENT_TIMESTAMP") == false &&
        TOKEN_CMP(first, "CURRENT_TIME") == false) {
  ...
  // In Oracle and EsgynDB unlike DB2, MySQL DEFAULT must go right after data
  // type
  ...
  // In DB2 default value can be omitted, add the value explicitly depending on
  // the data type
  if (first == NULL && _target != SQL_DB2) {
    // Specify DEFAULT '' for CHAR, VARCHAR and CLOB
    if (type->Compare("CHAR", L"CHAR", 4) == true ||
        type->Compare("CHAR", L"CHAR", 4))
      Append(append_default_value, " ''", L" ''", 3);
    else
      // Specify DEFAULT 0 for INTEGER, DECIMAL and NUMERIC
      if (type->Compare("INTEGER", L"INTEGER", 7) == true ||
          type->Compare("DECIMAL", L"DECIMAL", 7) == true ||
        ...
        if (type->Compare("TIMESTAMP", L"TIMESTAMP", 9) == true) {
          if (_target == SQL_SQL_SERVER)
            Append(append_default_value, " GETDATE()", L" GETDATE()", 10,
                   token);
          else if (_target == SQL_ORACLE)
            Append(append_default_value, " SYSTIMESTAMP", L" SYSTIMESTAMP", 13,
                   token);
          else
            Append(append_default_value, " CURRENT_TIMESTAMP",
                   L" CURRENT_TIMESTAMP", 18, token);
        } else
          ...
          if (type->Compare("DATE", L"DATE", 4) == true) {
            if (_target == SQL_SQL_SERVER)
              ...
              Append(append_default_value, " CURRENT_DATE", L" CURRENT_DATE",
                     13, token);
          } else
            // Default for CLOB
            if (type->Compare("CLOB", L"CLOB", 4) == true) {
              if (_target == SQL_ORACLE)
                Append(append_default_value, " EMPTY_CLOB()", L" EMPTY_CLOB()",
                       13, token);
              else
                Append(append_default_value, " ''", L" ''", 3);
            }
...
bool SqlParser::FormatDefaultExpression(Token *datatype, Token *exp,
                                        Token *default_) {
...
bool SqlParser::ParseStandaloneColumnConstraints(Token *alter,
                                                 Token *table_name,
                                                 ListW &pkcols,
                                                 ListWM *inline_indexes) {
  bool exists = false;

  // In Oracle constraint can be enclosed with () i.e. ALTER TABLE ADD (PRIMARY
  // KEY (c1,c2))
  Token *open = GetNext('(', L'(');
  ...
  while (true) {
    ...
    if (ParseKeyConstraint(alter, table_name, cns, pkcols, false,
                           inline_indexes) == true)
      exists_now = true;
    else
...
// Inline named constraint can be specified in SQL Server, Oracle
bool SqlParser::ParseInlineColumnConstraint(Token *type, Token *type_end,
                                            Token *constraint, int num) {
  // CONSTRAINT keyword before the name
  if (constraint == NULL ||
      Token::Compare(constraint, "CONSTRAINT", L"CONSTRAINT", 10) == false)
    return false;
  ...
  if (ParseKeyConstraint(NULL, NULL, cns, pkcols, true, NULL) == true)
    ...
    if (ParseForeignKey(cns) == true)
      ...
      if (ParseCheckConstraint(cns) == true)
        exists = true;
      else if (cns->Compare("DEFAULT", L"DEFAULT", 7) == true) {
...
bool SqlParser::ParseKeyConstraint(Token * /*alter*/, Token *table_name,
                                   Token *cns, ListW &pkcols, bool inline_cns,
                                   ListWM *inline_indexes) {
  if (cns == NULL)
    return false;
    ...
    if (cns->Compare("UNIQUE", L"UNIQUE", 6) == true) {
      ...
      if (index != NULL) {
        ...
        if (_target != SQL_MYSQL) {
          ...
          // Remove INDEX or KEY keyword for other databases (UNIQUE constraint
          // will be used)
          Token::Remove(index);
        }
      }
      ...
      if (cns->Compare("INDEX", L"INDEX", 5) == true ||
          cns->Compare("KEY", L"KEY", 3) == true) {
        index = true;
        index_name = GetNextIdentToken();
  ...
  while (true) {
    ...
    // Oracle does not allow specifying ASC, DESC in key constraint, but if it
    // is index it will be moved to a separate CREATE INDEX, so ASC and DESC can
    // be used
  ...
  if (unique == false && index == true && inline_indexes != NULL) {
    ...
    // Inline indexes will be moved, so delete comma at the end of previous
    // column
  ...
  while (true) {
    ...
    if (next->Compare("USING", L"USING", 5) == true) {
      ...
      if (Token::Compare(key_option, "INDEX", L"INDEX", 5)) {
        ...
        if (Token::Compare(key_option, "BTREE", L"BTREE", 5) ||
            Token::Compare(key_option, "HASH", L"HASH", 4)) {
          // Remove for other databases
          if (_target != SQL_MYSQL)
...
bool SqlParser::ParseForeignKey(Token *foreign) {
  ...
  while (true) {
    ...
    if (action->Compare("DELETE", L"DELETE", 6) == true)
      on_delete = true;
    else if (action->Compare("UPDATE", L"UPDATE", 6) == true)
      on_update = true;
    ...
    if (on_delete == true || on_update == true) {
      ...
      if (what->Compare("NO", L"NO", 2) == true) {
        ...
        // Oracle does not support ON DELETE NO ACTION and ON UPDATE NO ACTION
        // clause, but this is the default behavior (do not allow to delete
        // parent rows if child rows exist)
        if (_target == SQL_ORACLE)
...
bool SqlParser::ParseExpression(Token *first, int prev_operator) {
  ...
  // Some databases (Sybase ADS i.e.) support boolean values in expressions and
  // their negation like SELECT NOT True
  if (Source(SQL_SYBASE_ADS) && TOKEN_CMP(first, "NOT")) {
    ...
    // Logical expressions (without operand) converted to BIT in SQL Server, but
    // NOT 1 is not allowed
  ...
  if (first->Compare('(', L'(') == true) {
    ...
    if (Token::Compare(next, "SELECT", L"SELECT", 6) == true) {
      ...
      if (!select_pattern)
        ParseSelectStatement(next, 0, SQL_SEL_EXP, NULL, NULL, NULL, NULL, NULL,
                             NULL, NULL, NULL);
    ...
    if (ParseDatetimeLiteral(first) == true)
      ...
      if (ParseNamedVarExpression(first) == true)
        ...
        // CASE expression (check CASE and IF before function as it can contain
                  ...
                  if (ParseFunctionWithoutParameters(first) == true)
                    ...
                    if (ParseBooleanLiteral(first))
                      ...
                      if (first->type == TOKEN_WORD ||
                          first->type == TOKEN_IDENT) {
                        // In COBOL EXEC SQL block all variables/parameters are
                        // prefixed with :
                        if (_source_app == APP_COBOL && _level == LEVEL_SQL &&
                            _cobol != NULL)
                        ...
                        if (first->type == TOKEN_IDENT ||
                            first->type == TOKEN_STRING)
                          exists = true;

  if (exists == false)
    return false;

  // Parse operators (they will be processed recursively by calling
  // ParseExpression on the second operand)
  ...
  // End of expression with negation of boolean value (without operator i.e. NOT
  // TRUE, not NOT 1=2)
  if (_not != NULL) {
    if (Target(SQL_SQL_SERVER))
...
bool SqlParser::ParseDatetimeLiteral(Token *token) {
  ...
  if (TOKEN_CMP(token, "TIMESTAMP") == true ||
      TOKEN_CMP(token, "DATE") == true) {
...
// Named variable and expression: param => expr (Oracle), param := expr
// (PostgreSQL)
...
bool SqlParser::ParseBooleanExpression(int scope, Token *stmt_start,
                                       int *conditions_count, int *rowlimit,
                                       Token *prev_open,
                                       bool *bool_operator_no_exists) {
  int count = 0;
  ...
  if (open != NULL) {
    ...
    if (select != NULL) {
      ...
      if (!select_pattern)
        ParseSelectStatement(select, 0, SQL_SEL_EXP, NULL, &select_list_end,
    ...
    // Do not exit now as AND or OR condition can follow, or it parsed (exp) >
    // exp2
  }
  ...
  if (open == NULL) {
    ...
    if (first->Compare("EXISTS", L"EXISTS", 6) == true ||
        first->Compare("NOT", L"NOT", 3) == true) {
      PushBack(first);
      first = NULL;
    } else {
      bool ora_join = false;
  ...
  if (spec_and == false) {
    ...
    // Check for ) than means subexpression, not boolean expression - (exp) >
    // exp2 i.e.
    ...
    if (Token::Compare(op, ',', L',') == true) {
      // List of columns in the left part of (a, b, c) IN ... Open ( is taken at
      // higher level
      if (prev_open != NULL) {
    ...
    if (Token::Compare(op, "NOT", L"NOT", 3) == true) {
      ...
      if (op != NULL && op->Compare('(', L'(')) {
        ...
        // Subquery
        if (select != NULL)
          ParseSelectStatement(select, 0, SQL_SEL_EXP, NULL, NULL, NULL, NULL,
                               NULL, NULL, NULL, NULL);
        else
          ParseBooleanExpression(scope, stmt_start, conditions_count, rowlimit);
        ...
        if (!TOKEN_CMP(op, "LIKE") && !TOKEN_CMP(op, "IN") &&
            !TOKEN_CMP(op, "BETWEEN") && !TOKEN_CMP(op, "EXISTS") &&
            !TOKEN_CMP(op, "MATCHES")) {
          ParseExpression(op);
          op = GetNextToken();
        }
      }
  ...
  if (spec_and == true || TOKEN_CMPC(op, ')'))
    ...
    // Possible boolean function or expression without following operator (check
    // for terminating word)
          ...
          if (op->Compare("IN", L"IN", 2) == true) {
            ...
            if (op->Compare("EXISTS", L"EXISTS", 6) == true) {
              ...
              if (open != NULL) {
                ...
                // Subquery
                if (Token::Compare(next, "SELECT", L"SELECT", 6) == true)
                  ParseSelectStatement(next, 0, SQL_SEL_EXISTS_PREDICATE, NULL,
                                       NULL, NULL, NULL, NULL, NULL, NULL,
                                       NULL);
  ...
  if (unary == false) {
    if (op->chr != 0 || op->wchr != 0) {
      ...
      if (_source == SQL_TERADATA &&
          ParseTeradataComparisonOperator(op) == true) {
      }
      // Unknown or no operator
        ...
        // Sybase ADS supports boolean (true/false) data type, so boolean
        // expression can be without operator (i.e. it can be just AND TRUE)
        if ((open == NULL || bool_op_not_exists) && Source(SQL_SYBASE_ADS)) {
          // Boolean is converted to 1/0 in SQL Server that cannot go without
          // boolean operator, so we create a boolean expression by adding = 1
          if (Target(SQL_SQL_SERVER))
...
bool SqlParser::ParseBooleanAndOr(int scope, Token *stmt_start,
                                  int *conditions_count, int *rowlimit) {
  bool exists = false;
  ...
  while (true) {
    ...
    if (Token::Compare(next, "AND", L"AND", 3) == false &&
        Token::Compare(next, "OR", L"OR", 2) == false) {
      PushBack(next);
      break;
    }
...
bool SqlParser::ParseInPredicate(Token *in) {
  ...
  if (select != NULL)
    ParseSelectStatement(select, 0, SQL_SEL_IN_PREDICATE, NULL, NULL, NULL,
  ...
  // Return true if at least IN ( parsed. Other inside can be complex select and
  // ) can be missed so returning false if ) not found will mean double parsing
  return true;
}

// Parser a procedural block
bool SqlParser::ParseBlock(int type, bool frontier, int scope,
                           int *result_sets) {
  ...
  while (true) {
    ...
    if (token->Compare("BEGIN", L"BEGIN", 5) == true) {
      ...
      if (block == true) {
        ...
        if (end != NULL) {
          ...
          if (semi == NULL) {
            // In SQL Server, Sybase ASE BEGIN END can go without ; while MySQL,
            // MariaDB require BEGIN END;
    ...
    if (frontier == true) {
      ...
      if (token->Compare("END", L"END", 3) == true) {
        ...
        // For procedural block check for next CREATE PROCEDURE, FUNCTION,
        // TRIGGER and exit to avoid stack overflow in case of parser issues and
        // deep nested calls
        if (type == SQL_BLOCK_PROC &&
            token->Compare("CREATE", L"CREATE", 6) == true) {
          Token *next = GetNextToken();

          if (Token::Compare(next, "PROCEDURE", L"PROCEDURE", 9) ||
              Token::Compare(next, "FUNCTION", L"FUNCTION", 8) ||
          ...
          // IF block can be terminated with ELSEIF, ELSIF, ELSE or ENDIF
          // keyword
          if (type == SQL_BLOCK_IF && (token->Compare("ELSEIF", L"ELSEIF", 6) ||
                                       token->Compare("ELSIF", L"ELSIF", 5) ||
                                       token->Compare("ELSE", L"ELSE", 4) ||
                                       token->Compare("ENDIF", L"ENDIF", 5))) {
            PushBack(token);
            break;
          } else
            // CASE block can be terminated with WHEN or ELSE keyword
            if (type == SQL_BLOCK_CASE &&
                (token->Compare("WHEN", L"WHEN", 4) ||
                 token->Compare("ELSE", L"ELSE", 4))) {
              PushBack(token);
              break;
            } else
              // REPEAT block terminated with UNTIL
              if (type == SQL_BLOCK_REPEAT &&
                  token->Compare("UNTIL", L"UNTIL", 5) == true) {
                PushBack(token);
                break;
              } else
                // Oracle EXCEPTION block terminated by WHEN for next condition
                if (type == SQL_BLOCK_EXCEPTION &&
                    token->Compare("WHEN", L"WHEN", 4) == true) {
                  PushBack(token);
                  break;
                } else
                  // In SQL Server, Sybase procedure (not function) body can be
                  // terminated by GO without BEGIN-END block
                  if (Source(SQL_SQL_SERVER, SQL_SYBASE) == true &&
                      token->Compare("GO", L"GO", 2) == true) {
                    PushBack(token);
                    break;
                  } else
                    // Sybase ADS TRY ... CATCH
                    if ((type == SQL_BLOCK_TRY && TOKEN_CMP(token, "CATCH")) ||
                        (type == SQL_BLOCK_CATCH && TOKEN_CMP(token, "END"))) {
                      PushBack(token);
                      break;
                    }
      ...
      // In SQL Server, Sybase procedure (not function) body can be terminated
      // by GO without BEGIN-END block
      if (Source(SQL_SQL_SERVER, SQL_SYBASE) == true &&
          token->Compare("GO", L"GO", 2) == true) {
        PushBack(token);
        break;
...
// Oracle PL/SQL assignment statement variable := exp, in PostgreSQL var = exp
// can be used
bool SqlParser::ParseAssignmentStatement(Token *variable) {
  ...
  if (colon == NULL) {
    ...
    if (prev->Compare(';', L';') == false &&
        prev->Compare("BEGIN", L"BEGIN", 5) == false &&
        prev->Compare("THEN", L"THEN", 4) == false &&
        prev->Compare("ELSE", L"ELSE", 4) == false && !TOKEN_CMP(prev, "DO"))
      return false;
  }
  ...
  // Sybase ADS supports assignment of boolean expression to logical value i.e a
  // = 1=2;
  if (Source(SQL_SYBASE_ADS) && decl_var != NULL &&
      decl_var->data_subtype == TOKEN_DT2_BOOL) {
    bool bool_op_not_exists = false;

    ParseBooleanExpression(SQL_BOOL_ASSIGN, NULL, NULL, NULL, NULL,
                           &bool_op_not_exists);

    // SQL Server does not allow boolean expression if assignment (= <> etc.)
    // but simple assignment a = true is fine (converted to a = 1 using BIT data
    // type)
...
bool SqlParser::ParseStringConcatenation(Token *first, int prev_operator) {
  ...
  if (_target == SQL_SQL_SERVER) {
    ...
    if (first->nullable == true) {
      ...
      // Set not null to first token to avoid nested ISNULL when multiple
      // concatenations specified
    ...
    if (Target(SQL_MARIADB, SQL_MYSQL)) {
      ...
      if (first->nullable == true) {
        ...
        // Set not null to first token to avoid nested IFNULL when multiple
        // concatenations specified
        first->nullable = false;
      }
...
bool SqlParser::ParseAdditionOperator(Token *first, int prev_operator) {
  ...
  if (Source(SQL_SQL_SERVER, SQL_SYBASE) == true) {
    // Number always has priority in SQL Server: both '5'+ 5 and 5 + '5' give
    // result 10, but if at least one string literal contains not a number ','
    // i.e. we can consider + as string concatenation
    ...
    if (string_concat == true) {
      ...
      if (Target(SQL_ORACLE, SQL_POSTGRESQL) == true) {
        ...
        if (Target(SQL_MARIADB, SQL_MYSQL)) {
          // If it is first expression add CONCAT( before, and ) after last
          // expression
          if (prev_operator != SQL_OPERATOR_PLUS)
            Prepend(first, "CONCAT(", L"CONCAT(", 7);

          // Sybase ASE treats NULLs in + operator as empty string (unlike SQL
          // Server) MySQL, MariaDB CONCAT gives NULL if any expression is NULL
          if (_source == SQL_SYBASE) {
    ...
    if (_source == SQL_DB2 && second->data_type == TOKEN_DT_INTERVAL) {
      ...
      // Check for a +/- chain of interval expressions: for example, + expr
      // MONTH - expr DAYS + ...
...
// Check for a +/- chain of interval expressions: for example, + expr MONTH -
// expr DAYS + ...
...
// num UNITS DAY | SECOND etc. operator in Informix (interval in days, seconds
// etc.) num DAY | SECOND etc. in DB2
bool SqlParser::ParseUnitsOperator(Token *first) {
  ...
  if (_target == SQL_ORACLE) {
    ...
    // For SQL Server, remove field keyword as it will replaced by DATEADD for
    // +/- expressions
    if (_target == SQL_SQL_SERVER)
      Token::Remove(field);
    else
  ...
  else if (month != NULL || months != NULL)
    first->data_subtype = TOKEN_DT2_INTVL_MON;
  else if (minute != NULL || minutes != NULL)
...
bool SqlParser::ParseSubtractionOperator(Token *first) {
  ...
  if (_source == SQL_ORACLE && first->data_type == TOKEN_DT_DATETIME &&
      second->data_type == TOKEN_DT_DATETIME) {
...
bool SqlParser::ParseLabelDeclaration(Token *word, bool outer_sp_block) {
  // Make sure it is DB2, Teradata, MySQL to avoid confusion with := assignment
  // in Oracle, PostgreSQL i.e.
  ...
  // Label
  if (semi != NULL ||
      (name->len > 1 && name->Compare(':', L':', name->len - 1) == true))
    exists = true;
  ...
  if (outer_sp_block == true) {
    ...
    // In Oracle, outer label is allowed only in anonymous blocks, remove it;
    // also remove for other databases
