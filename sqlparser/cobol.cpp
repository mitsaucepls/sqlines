...
bool Cobol::ParseProcedureDivision(Token *procedure) {
  ...
  if (_target == SQL_ORACLE && _proc_start != NULL && _proc_name != NULL &&
      _proc_using_params.GetCount() > 0) {
    TokenStr params;
    ...
    for (ListwItem *i = _proc_using_params.GetFirst(); i != NULL; i = i->next) {
      ...
      for (ListwmItem *j = _proc_linkage_params.GetFirst(); j != NULL;
           j = j->next) {
...
void Cobol::ParseStatement(int stmt_scope) {
  // Not that statements not followed by . make up the single statement with the
  // subsequent statements until .
  while (true) {
    bool exists = false;
    Token *next = GetNextToken();
    ...
    else if (TOKEN_CMP(next, "IF"))
      exists = ParseIfStatement(next);
    else if (TOKEN_CMP(next, "MOVE"))
      exists = ParseMoveStatement(next);
    else if (TOKEN_CMP(next, "PROG-END") || TOKEN_CMP(next, "GOBACK")) {
      Token *dot = TOKEN_GETNEXT('.');
    ...
    if (dot != NULL) {
      // Every statement must terminate itself (sometimes statements just
      // removed so no delimiter is required)
      Token::Remove(dot);
      break;
    } else
      // True block of IF statement can be ended by ELSE (still without . in the
      // last statement before ELSE)
      if (stmt_scope == CBL_SCOPE_IF_TRUE && LOOKNEXT("ELSE"))
        break;
  }
}

// Parse data section that can contain variable definitions, COPY and INCLUDE
// commands
...
bool Cobol::ParseVariableSection(int section_scope, Token *parent_record,
                                 ListWM *record_items) {
  bool exists = false;
  ...
  while (true) {
    ...
    if (next->IsNumeric() == true) {
      ...
      if (entry_num != NULL) {
        ...
        // If within a record, change period for the previous item to comma for
        // Oracle; it will be removed for last item later
      ...
      // If period follows the name it means record definition, otherwise
      // variable definition
      ...
      if (dot != NULL) {
        ...
        // All variables from linkage section are moved to parameter list, but
        // we will comment the declarations, not remove them to preserve
        // variable length, code comments etc.
...
// Record declaration
bool Cobol::ParseRecordDeclaration(int section_scope, Token *parent_record,
                                   Token *num, Token *name, Token *dot) {
  if (name == NULL)
    return false;
  ...
  if (Source(SQL_DB2))
    special_case = ParseDb2RecordDeclaration(section_scope, num, name, dot,
                                             end_dot, &record_items);
  ...
  if (!special_case && _target_app == APP_PLSQL) {
    ...
    // If it is not top level record, move TYPE definition before top level
    // recod
    if (parent_record != NULL) {
      ...
      // Specify a record variable inside the record instead full nested record
      // definition
...
bool Cobol::ParseDb2RecordDeclaration(int section_scope, Token *num,
                                      Token *name, Token *dot, Token *end_dot,
                                      ListWM *record_items) {
  ...
  if (record_items->GetCount() == 2) {
    ...
    if (type1 != NULL && type2 != NULL &&
        (TOKEN_CMP(type1, "S9") || TOKEN_CMP(type1, "9")) &&
        TOKEN_CMP(type2, "X")) {
  ...
  // All variables from linkage section are moved to parameter list, but we will
  // comment the declarations, not remove them to preserve variable length, code
  // comments etc.
...
bool Cobol::ParseVariableDeclaration(int section_scope, Token *num, Token *name,
                                     ListWM *record_items) {
  if (name == NULL)
    return false;
...
bool Cobol::ParsePicFormat(Token *pic, Token **type, Token **len,
                           Token **var_end) {
  if (pic == NULL)
    return false;
    ...
    if (TOKEN_CMP(next, "INCLUDE")) {
      ...
      if (TOKEN_CMP(next, "DECLARE")) {
        ...
        if (TOKEN_CMP(next, "SELECT")) {
          ...
          _sqlparser->SetLevel(LEVEL_SQL);
          _sqlparser->ParseSelectStatement(next, 0, 0, NULL, NULL, NULL, NULL,
...
void Cobol::ConvertIdentifier(Token *name, int type) {
  ...
  // Add v_ or p_ prefix to avoid conflicts with column names as COBOL uses :var
  // syntax in SQL
...
Token *Cobol::GetNextWordToken(const char *str, const wchar_t *wstr,
                               size_t len) {
  if (_sqlparser != NULL)
...
Token *Cobol::Append(Token *token, const char *str, const wchar_t *wstr,
...
Token *Cobol::Prepend(Token *token, const char *str, const wchar_t *wstr,
                      size_t len, Token *format) {
  if (_sqlparser != NULL)
...
Token *Cobol::PrependCopy(Token *token, Token *first, Token *last,
