...
bool SqlParser::ParseInformixReturns(Token *returns) {
  ...
  while (true) {
    ...
    // For Oracle it is unknown yet whether data type will be used as OUT
    // parameter and length must be removed or OBJECT and length must be
    // preserved; so parse as columns and preserve sizes

    // For other databases parameters and columns both preserve sizes, so it is
    // not important how to parse data type
...
// Replace RETURNING values with OUT parameters or table function when
// converting from Informix
void SqlParser::InformixConvertReturning(Token *create, Token *procedure) {
  ...
  if (_spl_return_with_resume == 0) {
    // Single scalar value is returned; use function in Oracle, SQL Server,
    // PostgreSQL
    ...
    // Multiple values are returned
      ...
      if (_target == SQL_ORACLE) {
        ...
        for (ListwItem *i = _spl_returning_datatypes.GetFirst(); i != NULL;
             i = i->next) {
  ...
  // Convert procedure with RETURN WITH RESUME to a table function
    ...
    if (_target == SQL_ORACLE) {
      ...
      // Define row object
      PrependNoFormat(create, "-- Row and table types for pipelined function",
                      L"-- Row and table types for pipelined function", 45);
      Prepend(create, "\n", L"\n", 1);
      Prepend(create, "CREATE OR REPLACE TYPE ", L"CREATE OR REPLACE TYPE ",
              23);
...
void SqlParser::InformixGetReturningName(Token *retvalue, TokenStr &name,
                                         const char *prefix,
                                         const wchar_t *wprefix, size_t plen,
                                         int num, bool always_prefix) {
  if (retvalue == NULL)
    return;
...
bool SqlParser::ParseInformixIntoTable(Token *select) {
  ...
  if (_target == SQL_ORACLE) {
    // Temporary table
    if (temp != NULL || scratch != NULL) {
      // If inside a procedural block, use insert as temporary table is created
      ...
      if (_spl_scope != 0) {
        ...
        Prepend(select, "CREATE GLOBAL TEMPORARY TABLE ",
                L"CREATE GLOBAL TEMPORARY TABLE ", 30);
        PrependCopy(select, table_name);
...
bool SqlParser::InformixPatternAssignLastSerial(Token *let) {
  ...
  Token *sqlerrd1 =
      GetNextWordToken(open, "\"sqlca.sqlerrd1\"", L"\"sqlca.sqlerrd1\"", 16);
...
bool SqlParser::ParseInformixMatchesPredicate(Token *pre_first, Token *first,
