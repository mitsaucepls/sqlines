...
bool SqlParser::ParseOraclePackage(Token *token) {
  ...
  if (TOKEN_CMP_PART0(token, "DBMS_OUTPUT.")) {
    ...
    exists = true;
  } else if (TOKEN_CMP_PART0(token, "APEX_") ||
             TOKEN_CMP_PART0(token, "DBMS_") ||
             TOKEN_CMP_PART0(token, "SYS.DBMS_") ||
             TOKEN_CMP_PART0(token, "DEBUG_") ||
             TOKEN_CMP_PART0(token, "CTX_") || TOKEN_CMP_PART0(token, "HTF_") ||
             TOKEN_CMP_PART0(token, "HTP_") || TOKEN_CMP_PART0(token, "ORD_") ||
             TOKEN_CMP_PART0(token, "OWA_") || TOKEN_CMP_PART0(token, "SDO_") ||
             TOKEN_CMP_PART0(token, "SEM_") || TOKEN_CMP_PART0(token, "UTL_") ||
             TOKEN_CMP_PART0(token, "WPG_")) {
    // Define the package name
    if (TOKEN_CMP_PART0(token, "SYS."))
      Str::GetSubtringUntil(token->str + 4, '.', (int)token->len - 4,
                            package_name);
    else
            ...
            if (next->Compare("PCTINCREASE", L"PCTINCREASE", 11) == true) {
              ...
              if (next->Compare("FREELISTS", L"FREELISTS", 9) == true) {
                ...
                if (next->Compare("FREELIST", L"FREELIST", 8) == true) {
                  /*Token *groups */ (void)GetNextWordToken("GROUPS", L"GROUPS",
                                                            6);
                  Token *num = GetNextToken();
                  ...
                  // BUFFER_POOL name
                  if (next->Compare("BUFFER_POOL", L"BUFFER_POOL", 11) ==
                      true) {
                    Token *value = GetNextToken();
                    ...
                    // FLASH_CACHE name
                    if (next->Compare("FLASH_CACHE", L"FLASH_CACHE", 11) ==
                        true) {
                      Token *value = GetNextToken();
                      ...
                      if (next->Compare("CELL_FLASH_CACHE", L"CELL_FLASH_CACHE",
                                        16) == true) {
                        Token *value = GetNextToken();
  ...
  while (true) {
    ...
    if (next->Compare("TABLESPACE", L"TABLESPACE", 10) == true) {
      ...
      if (next->Compare("DISABLE", L"DISABLE", 7) == true ||
          next->Compare("ENABLE", L"ENABLE", 6) == true) {
...
bool SqlParser::ParseOraclePartitions(Token *token) {
  ...
  if (local == NULL) {
    ...
    if (subpartition != NULL) {
      ...
      Token *subpartition2 =
          GetNextWordToken("SUBPARTITION", L"SUBPARTITION", 12);
    ...
    // Greenplum does not support partitions in indexes, so set removed flags to
    // remove definitions
...
bool SqlParser::ParseOraclePartitionsBy(Token *token) {
  ...
  Token *partition =
      token->Compare("PARTITION", L"PARTITION", 9) ? token : NULL;
  Token *subpartition = NULL;

  if (partition == NULL)
    subpartition =
        token->Compare("SUBPARTITION", L"SUBPARTITION", 12) ? token : NULL;
  ...
  if (subpartition != NULL) {
    Token *subpartitions =
        GetNextWordToken("SUBPARTITIONS", L"SUBPARTITIONS", 13);
    Token *num = NULL;
...
bool SqlParser::ParseOraclePartition(Token *partition, Token *subpartition) {
  // Both partition and subpartition parameters can be NULL for LOCAL
  // partitioned indexes
  Token *open = GetNextCharToken('(', L'(');
...
bool SqlParser::ParseOracleVariableDeclarationBlock(Token *declare) {
  ...
  // Remove DECLARE for SQL Server, MySQL as it will be specified before each
  // variable
  if (Token::Compare(declare, "DECLARE", L"DECLARE", 7) == true) {
    if (Target(SQL_SQL_SERVER, SQL_MARIADB, SQL_MYSQL) == true)
      Token::Remove(declare);
  } else
    // PostgreSQL, Netezza required DECLARE keyword before outer declaration
    // section after AS keyword
    if (_source == SQL_ORACLE && Target(SQL_POSTGRESQL, SQL_NETEZZA) == true) {
      ...
      if (_target == SQL_NETEZZA) {
        ...
        for (ListwItem *p = _spl_parameters.GetFirst(); p != NULL;
             p = p->next) {
          Append(declare, "\n", L"\n", 1);
  ...
  // Last token of last declared variable (not cursor) in the current PL/SQL
  // DECLARE block
  ...
  while (true) {
    ...
    if (TOKEN_CMP(name, "BEGIN") ||
        (_spl_package != NULL &&
         (TOKEN_CMP(name, "PROCEDURE") || TOKEN_CMP(name, "FUNCTION"))) ||
    ...
    // CURSOR cur IS SELECT definition, PRAGMA or forward declaration for
    // exception
    if (ParseOracleCursorDeclaration(name, &cursors) ||
        ParseOracleObjectType(name) || ParseOracleObjectSubtype(name) ||
        ParseOraclePragma(name) || ParseOracleException(name) ||
        ParseOracleProcSpec(name) || ParseOracleFuncSpec(name) ||
        ParseOracleNestedProcBody(name) || ParseOracleNestedFuncBody(name)) {
      exists = true;
      continue;
    }
    ...
    // Optional initialization value := value in Oracle and = value in
    // PostgreSQL
    ...
    if (colon != NULL || equal != NULL || default_ != NULL) {
      ...
      if (colon != NULL &&
          (Target(SQL_SQL_SERVER, SQL_POSTGRESQL) || _target_app == APP_JAVA))
        Token::Remove(colon);
      else
        // For Oracle add :
  ...
  // Cursors must be after all variables in MySQL and MariaDB, so move them if
  // required
  if (exists == true && last_declare_var != NULL &&
      Target(SQL_MARIADB, SQL_MYSQL) == true && cursors.GetCount() > 0) {
...
bool SqlParser::ParseOracleCursorDeclaration(Token *cursor, ListWM *cursors) {
  ...
  if (open != NULL) {
    ...
    while (true) {
      ...
      // SQL Server does support parameters, and cursor get all variable values
      // during declaration, not during OPEN cursor, so DECLARE is moved before
      // OPEN MySQL also does not support cursor parameters

      // Generate variable not substitute with actual values (especially if they
      // are constants) to increase chance that SQL execution plan is re-used
      ...
      // Save information about cursor parameters (variables will be generated
      // for MySQL i.e.)
  ...
  if (Target(SQL_SQL_SERVER, SQL_MARIADB, SQL_MYSQL) == true) {
    ...
    // For SQL Server add LOCAL since cursor are global by default and visible
    // in other scopes
    if (_target == SQL_SQL_SERVER)
  ...
  if (select != NULL) {
    ParseSelectStatement(select, 0, SQL_SEL_CURSOR, NULL, NULL, NULL, NULL,
                         NULL, NULL, NULL, NULL);
...
bool SqlParser::ParseOracleObjectType(Token *type) {
  ...
  Token *record =
      (table == NULL && ref == NULL) ? TOKEN_GETNEXTWP(is, "RECORD") : NULL;
  Token *varray = (table == NULL && ref == NULL && record == NULL)
                      ? TOKEN_GETNEXTWP(is, "VARRAY")
                      : NULL;
...
void SqlParser::OracleRemoveDataTypeSize(Token *data_type) {
  ...
  // Datatype without size specifier; target (size) can be added when INT
  // converted to NUMBER(10) i.e.
  if (Token::Compare(cur, '(', L'(') == false &&
      Token::CompareTarget(cur, "(", L"(", 1) == false)
    return;
  ...
  while (cur != NULL) {
    ...
    if (Token::Compare(cur, ')', L')') == true ||
        Token::CompareTarget(cur, ")", L")", 1) == true)
      break;
...
bool SqlParser::ParseOracleRownumCondition(Token *first, Token *op,
                                           Token *second, int *rowlimit) {
  ...
  bool rownum2 =
      (rownum1 == false) ? second->Compare("rownum", L"rownum", 6) : false;
...
// Get Oracle format string for specified string literal containing date or
// timestamp value
  ...
  if (str->len == 10) {
    ...
    if (str->len == 12) {
      ...
      if ((Token::Compare(str, '/', L'/', 3) == true &&
           Token::Compare(str, '/', L'/', 6) == true) ||
          (Token::Compare(str, '-', L'-', 3) == true &&
           Token::Compare(str, '-', L'-', 6) == true) ||
          (Token::Compare(str, '.', L'.', 3) == true &&
           Token::Compare(str, '.', L'.', 6) == true)) {
        ...
        if (n1 != -1 && n2 != -1) {
          ...
          if (mdy == true) {
            ...
            else if (Token::Compare(str, '-', L'-', 3) == true)
              format.Append("'MM-DD-YYYY'", L"'MM-DD-YYYY'", 12);
            else if (Token::Compare(str, '.', L'.', 3) == true)
              format.Append("'MM.DD.YYYY'", L"'MM.DD.YYYY'", 12);
      ...
      if (str->len >= 19) {
        ...
        if (Token::Compare(str, '-', L'-', 5) == true &&
            Token::Compare(str, '-', L'-', 8) == true &&
            Token::Compare(str, ':', L':', 14) == true &&
            Token::Compare(str, ':', L':', 17) == true && month <= 12) {
          format.Append("'YYYY-MM-DD HH24:MI:SS", L"'YYYY-MM-DD HH24:MI:SS",
                        22);
...
void SqlParser::OracleEmulateIdentity(Token *create, Token *table,
                                      Token *column, Token *last,
  ...
  Append(last, "\n\n", L"\n\n", 2);
  AppendNoFormat(last, "-- Generate ID using sequence and trigger",
                 L"-- Generate ID using sequence and trigger", 41);
  Append(last, "\n", L"\n", 1);
  ...
  Append(last, "CREATE OR REPLACE TRIGGER ", L"CREATE OR REPLACE TRIGGER ", 26,
         create);
...
void SqlParser::OracleExitHandlersToException(Token *end) {
  ...
  for (CopyPaste *i = _copypaste.GetFirstNoCurrent(); i != NULL; i = i->next) {
    if (i->type != COPY_EXIT_HANDLER_NOT_FOUND &&
        i->type != COPY_EXIT_HANDLER_SQLEXCEPTION &&
...
void SqlParser::OracleContinueHandlerForFetch(Token *fetch, Token *cursor) {
  ...
  while (cur != NULL) {
    ...
    if (found == false && _source == SQL_DB2 &&
        cur->type == COPY_CONTINUE_HANDLER_USER_DEFINED) {
      // Find SQLSTATE associated with the condition
      ListwmItem *i = Find(_spl_user_exceptions, cur->name);

      if (i != NULL &&
          Token::Compare((Token *)i->value2, "'02000'", L"'02000'", 7) == true)
        found = true;
    }
...
// Implement CONTINUE handler for NOT FOUND and SQLEXCEPTION after SELECT INTO
// in Oracle
void SqlParser::OracleContinueHandlerForSelectInto(Token *select) {
  ...
  while (cur != NULL) {
    if (cur->type != COPY_CONTINUE_HANDLER_NOT_FOUND &&
        cur->type != COPY_CONTINUE_HANDLER_SQLEXCEPTION) {
      cur = cur->next;
      continue;
    }
...
void SqlParser::OracleContinueHandlerForInsert(Token *insert) {
  ...
  while (cur != NULL) {
    ...
    if (_source == SQL_DB2 && Token::Compare(error, "'23505'", L"'23505'", 7))
      Append(end, "DUP_VAL_ON_INDEX THEN ", L"DUP_VAL_ON_INDEX THEN ", 22,
             insert);
...
// If there declarations in the body move BEGIN after last DECLARE (used for
// PostgreSQL as well)
void SqlParser::OracleMoveBeginAfterDeclare(Token *create, Token *as,
                                            Token *begin, Token *body_start) {
  ...
  // There are declarations
    ...
    if (append != NULL) {
      ...
      else if (append->prev != NULL && append->prev->IsBlank() == true)
        Append(append, "BEGIN\n", L"BEGIN\n", 6, Nvl(begin, create));
      else
        Append(append, "\nBEGIN\n", L"\nBEGIN\n", 7, Nvl(begin, create));
    }
...
void SqlParser::OracleAddOutRefcursor(Token *create, Token *name,
                                      Token *last_param) {
  // No result sets
  ...
  // Find generated cursor names for standalone SELECT statements returning a
  // result set
  for (TokenStr *t = _spl_result_set_generated_cursors.GetFirst(); t != NULL;
       t = t->next) {
    if (num > 0)
      AppendNoFormat(append, ", ", L", ", 2);
  ...
  for (ListwmItem *i = _spl_prepared_stmts_cursors_with_return.GetFirst();
       i != NULL; i = i->next) {
    Token *cursor = (Token *)i->value;
...
// Check if temporary tables need to be truncated at the beginning of block in
// Oracle
void SqlParser::OracleTruncateTemporaryTablesWithReplace() {
  for (ListwItem *i = _spl_declared_tables_with_replace.GetFirst(); i != NULL;
       i = i->next) {
    ...
    Prepend(_spl_first_non_declare, "EXECUTE IMMEDIATE 'TRUNCATE TABLE ",
            L"EXECUTE IMMEDIATE 'TRUNCATE TABLE ", 34);
...
void SqlParser::OracleMoveSelectOutofIf(Token *stmt_start, Token *select,
...
void SqlParser::OracleChangeEqualToDefault(Token *equal) {
  ...
  // INT=0 without spaces is possible, so check if we need to add spaces arounf
  // DEFAULT keyword
  ...
  else if (space_before == true && space_after == false)
    Token::Change(equal, "DEFAULT ", L"DEFAULT ", 8);
  else if (space_before == false && space_after == true)
    Token::Change(equal, " DEFAULT", L" DEFAULT", 8);
  else
    Token::Change(equal, " DEFAULT ", L" DEFAULT ", 9);
}
