...
bool SqlParser::ParseCreateTriggerOraclePattern(Token *create, Token *table,
  ...
  if (_target == SQL_SQL_SERVER) {
    ...
    if (when->Compare("BEFORE", L"BEFORE", 6) == true && insert != NULL) {
      ...
      if (exists == true) {
        ...
        Prepend(create, " ADD DEFAULT NEXT VALUE FOR ",
                L"ADD DEFAULT NEXT VALUE FOR ", 28);
...
bool SqlParser::ParseCastTimeofdayAsTimestamp(Token *cast, Token *open,
                                              Token *first) {
...
// Typical cursor check - @@FETCH_STATUS = 0 in SQL Server, @@SQLSTATUS = 0 in
// Sybase
bool SqlParser::ParseBooleanCursorCheckPattern() {
  if (!Source(SQL_SQL_SERVER, SQL_SYBASE) ||
      !Target(SQL_ORACLE, SQL_MYSQL, SQL_MARIADB, SQL_POSTGRESQL))
    return false;
    ...
    if (status != NULL) {
      ...
      if (row_found) {
        ...
        if (_target == SQL_ORACLE) {
          ...
          // Add NOT FOUND handler and variable later as it must go after all
