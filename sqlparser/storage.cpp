...
bool SqlParser::ParseTempTableOptions(Token *table_name, Token **start_out,
                                      Token **end_out, bool *no_data) {
  bool exists = false;
  ...
  while (true) {
    ...
    // ON COMMIT PRESERVE | DELETE ROWS in Oracle, DB2; ON ROLLBACK PRESERVE |
    // DELETE ROWS in DB2
    if (next->Compare("ON", L"ON", 2) == true) {
      ...
      if (_target == SQL_SQL_SERVER)
        ...
        // Oracle does not support ON ROLLBACK, but DELETE ROWS in default on
        // rollback
...
bool SqlParser::ParseStorageClause(Token *table_name, Token **id_start,
                                   Token **comment, Token *last_colname,
                                   Token *last_colend) {
  ...
  if (_source == SQL_MYSQL &&
      ParseMysqlStorageClause(table_name, id_start, comment) == true)
    return true;
  ...
  if (_source == SQL_TERADATA &&
      ParseTeradataStorageClause(last_colname, last_colend) == true)
    return true;
...
bool SqlParser::ParseMysqlStorageClause(Token *table_name, Token **id_start,
                                        Token **comment_out) {
  bool exists = false;
  ...
  if (auto_start != NULL &&
      (_target == SQL_POSTGRESQL || _target == SQL_GREENPLUM)) {
    // Try to get ;
...
bool SqlParser::ParseOracleStorageClause() {
  ...
  while (true) {
    ...
    // SEGMENT CREATION IMMEDIATE | DEFERRED (or undocumented DB CREATION
    // IMMEDIATE found in one customer)
        ...
        if (next->Compare("PCTUSED", L"PCTUSED", 7) == true) {
          ...
          if (next->Compare("INITRANS", L"INITRANS", 8) == true) {
            ...
            if (next->Compare("MAXTRANS", L"MAXTRANS", 8) == true) {
              ...
              // COMPRESS [BASIC] or COMPRESS num (for index-orginized tables
              // and indexes)
                  ...
                  if (next->Compare("NOCACHE", L"NOCACHE", 7) == true) {
                    ...
                    if (next->Compare("LOGGING", L"LOGGING", 7) == true) {
                      ...
                      if (next->Compare("NOLOGGING", L"NOLOGGING", 9) == true) {
                        ...
                        // NOPARALLEL
                        if (next->Compare("NOPARALLEL", L"NOPARALLEL", 10) ==
                            true) {
                          if (_target != SQL_ORACLE)
                            Token::Remove(next);
                          ...
                          // PARALLEL num
                          if (next->Compare("PARALLEL", L"PARALLEL", 8) ==
                              true) {
                            Token *value = GetNextNumberToken();
                            ...
                            // NOMONITORING
                            if (next->Compare("NOMONITORING", L"NOMONITORING",
                                              12) == true) {
                              if (_target != SQL_ORACLE)
                                Token::Remove(next);
                              ...
                              // TABLESPACE name
                              if (next->Compare("TABLESPACE", L"TABLESPACE",
                                                10) == true) {
                                ...
                                // STORAGE () clause
                                if (next->Compare("STORAGE", L"STORAGE", 7) ==
                                    true) {
                                  ...
                                  if (next->Compare("LOB", L"LOB", 3) == true) {
                                    if (ParseOracleLobStorageClause(next) ==
                                        true) {
                                      exists = true;
                                      continue;
                                    }
                                    ...
                                    if (ParseOraclePartitions(next) == true) {
                                      ...
                                      // COMPUTE STATISTICS
                                      if (next->Compare("COMPUTE", L"COMPUTE",
                                                        7) == true) {
                                        Token *statistics = GetNextWordToken(
                                            "STATISTICS", L"STATISTICS", 10);
                                        ...
                                        // ENABLE ROW MOVEMENT
                                        if (next->Compare("ENABLE", L"ENABLE",
                                                          6) == true) {
                                          Token *row = GetNextWordToken(
                                              "ROW", L"ROW", 3);

                                          if (row != NULL) {
                                            Token *movement = GetNextWordToken(
                                                "MOVEMENT", L"MOVEMENT", 8);
                                          ...
                                          // REVERSE index or primary key
                                          // storage attribute
                                          if (next->Compare("REVERSE",
                                                            L"REVERSE",
                                                            7) == true) {
                                            // Remove if not Oracle
                                            if (_target != SQL_ORACLE)
          ...
          if (next->Compare("CCSID", L"CCSID", 5) == true) {
            ...
            if (next->Compare("DATA", L"DATA", 4) == true) {
              ...
              if (next->Compare("AUDIT", L"AUDIT", 5) == true) {
                ...
                if (next->Compare("WITH", L"WITH", 4) == true) {
                  Token *restrict =
                      GetNextWordToken("RESTRICT", L"RESTRICT", 8);
                  Token *on = NULL;
                  Token *drop = NULL;
                  ...
                  if (next->Compare("NOT", L"NOT", 3) == true) {
                    Token *volatile_ =
                        GetNextWordToken("VOLATILE", L"VOLATILE", 8);
                    ...
                    if (next->Compare("VOLATILE", L"VOLATILE", 8) == true) {
                      ...
                      if (next->Compare("APPEND", L"APPEND", 6) == true) {
                        ...
                        if (next->Compare("PARTITION", L"PARTITION", 9) ==
                            true) {
                          Token *by = GetNextWordToken("BY", L"BY", 2);
                          Token *size =
                              GetNextWordToken(by, "SIZE", L"SIZE", 4);

                          // Partition by size
                          if (size != NULL) {
                            // Optional EVERY n G
                            Token *every =
                                GetNextWordToken("EVERY", L"EVERY", 5);
                            ...
                            exists = true;
                            continue;
                          } else if (ParseDb2PartitioningClause(next, by) ==
                                     true) {
                            exists = true;
                            continue;
                          }
                          ...
                          if (TOKEN_CMP(next, "ORGANIZE")) {
                            ...
                            /*Token *row */ (column == NULL)
                                ? TOKEN_GETNEXTWP(by, "ROW")
                                : NULL;
...
bool SqlParser::ParseTeradataStorageClause(Token *last_colname,
                                           Token *last_colend) {
  bool exists = false;
