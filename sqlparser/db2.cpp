...
bool SqlParser::ParseDb2GeneratedClause(Token *create, Token *table_name,
                                        Token *column, Token *generated,
                                        Token **id_col, Token **id_start,
                                        Token **id_inc, bool *id_default) {
  if (generated == NULL)
    return false;
        ...
        if (next->Compare("NOT", L"NOT", 3) == true) {
          ...
          if (next->Compare("COMPRESS", L"COMPRESS", 8) == true) {
            ...
            if (next->Compare("ALLOW", L"ALLOW", 5) == true) {
              /*Token *reverse */ (void)GetNextWordToken("REVERSE", L"REVERSE",
                                                         7);
              ...
              if (next->Compare("PCTFREE", L"PCTFREE", 7) == true) {
                ...
                if (_target == SQL_GREENPLUM) {
                  Token::Change(
                      next, "WITH (FILLFACTOR =", L"WITH (FILLFACTOR =", 18);
                ...
                if (next->Compare("MINPCTUSED", L"MINPCTUSED", 10) == true) {
                  ...
                  if (next->Compare("FREEPAGE", L"FREEPAGE", 8) == true) {
                    ...
                    if (next->Compare("BUFFERPOOL", L"BUFFERPOOL", 10) ==
                        true) {
                      ...
                      if (next->Compare("SPECIFICATION", L"SPECIFICATION",
                                        13) == true) {
                        /*Token *only */ (void)GetNextWordToken("ONLY", L"ONLY",
                                                                4);
                        ...
                        if (next->Compare("GBPCACHE", L"GBPCACHE", 8) == true) {
                          ...
                          if (next->Compare("CLOSE", L"CLOSE", 5) == true) {
                            ...
                            if (next->Compare("COPY", L"COPY", 4) == true) {
                              ...
                              if (next->Compare("PIECESIZE", L"PIECESIZE", 9) ==
                                  true) {
                                Token *num = GetNextToken();

                                if (num != NULL) {
                                  // Size can be optionally followed by
                                  // specifier K, M or G
                                ...
                                if (next->Compare("PADDED", L"PADDED", 6) ==
                                    true) {
                                  if (_target != SQL_DB2)
                                    Token::Remove(next);
                  ...
                  if (next->Compare("CLOSE", L"CLOSE", 5) == true) {
                    ...
                    if (next->Compare("COPY", L"COPY", 4) == true) {
                      ...
                      if (next->Compare("PIECESIZE", L"PIECESIZE", 9) == true) {
                        ...
                        if (num != NULL) {
                          // Size can be optionally followed by specifier K, M
                          // or G
                          Token *spec = Db2SizeSpecifier();
                        ...
                        if (next->Compare("LOGGED", L"LOGGED", 6) == true) {
                          ...
                          if (next->Compare("NOT", L"NOT", 3) == true) {
                            Token *logged =
                                GetNextWordToken("LOGGED", L"LOGGED", 6);

                            if (logged != NULL) {
                              if (_target == SQL_ORACLE) {
                                Token::Change(next, "NOLOGGING", L"NOLOGGING",
                                              9);
                                Token::Remove(logged);
                              } else if (_target != SQL_DB2)
                            ...
                            if (next->Compare("TRACKMOD", L"TRACKMOD", 8) ==
                                true) {
                              Token *value = GetNextToken();
                              ...
                              if (next->Compare("SEGSIZE", L"SEGSIZE", 7) ==
                                  true) {
                                Token *num = GetNextToken();
                                ...
                                // DB2 z/OS LOCKSIZE ANY | TABLESPACE | TABLE |
                                // PAGE | ROW | LOB
                                if (next->Compare("LOCKSIZE", L"LOCKSIZE", 8) ==
                                    true) {
                                  Token *value = GetNextToken();
                                  ...
                                  if (next->Compare("LOCKMAX", L"LOCKMAX", 7) ==
                                      true) {
                                    Token *value = GetNextToken();
                                    ...
                                    if (next->Compare("CCSID", L"CCSID", 5) ==
                                        true) {
                                      ...
                                      if (next->Compare("MAXROWS", L"MAXROWS",
                                                        7) == true) {
                                        Token *num = GetNextToken();
...
bool SqlParser::ParseDb2StogroupClause(Token *objname, Token *using_,
                                       Token *stogroup, int scope) {
  ...
  if (_target == SQL_ORACLE && scope == SQL_SCOPE_TABLESPACE) {
    ...
    // Use tablespace name as multiple tablespaces can be created in a single
    // storage group
...
bool SqlParser::ParseDb2FetchFirstRowOnly(Token *fetch,
                                          Token **rowlimit_soptions,
                                          int *rowlimit) {
  ...
  if (num->Compare("ROW", L"ROW", 3) == true ||
      num->Compare("ROWS", L"ROWS", 4) == true) {
    rows = num;
    num = NULL;
  }
  ...
  if (num == NULL && rowlimit != NULL)
    *rowlimit = 1;
  else if (num != NULL && rowlimit_soptions != NULL)
...
bool SqlParser::Db2CreateDatabase(Token *create, Token * /*database*/,
                                  Token * /*name*/) {
  bool exists = false;
...
Token *SqlParser::Db2SizeSpecifier() {
  ...
  if (spec->Compare("K", L"K", 1) == false &&
      spec->Compare("M", L"M", 1) == false &&
...
bool SqlParser::ParseDb2Comment(Token *comment, Token *on, Token *name) {
  ...
  if (_target == SQL_SQL_SERVER) {
    ...
    AppendNoFormat(comment, " sp_addextendedproperty 'Comment',",
                   L" sp_addextendedproperty 'Comment',", 34);
...
bool SqlParser::ConvertDb2Identifier(Token *token, int expected_type) {
  ...
  if (expected_type == SQL_IDENT_COLVAR) {
    ...
    if (_spl_scope == SQL_SCOPE_TRIGGER &&
        Token::Compare(token, "new.", L"new.", 0, 4) == true) {
...
void SqlParser::Db2TriggerCorrelatedName(Token *var) {
  ...
  if ((_spl_new_correlation_name == NULL &&
       Token::Compare(var, "NEW.", L"NEW.", 0, 4) == true) ||
      (Token::Compare(var, _spl_new_correlation_name, 3) == true &&
       Token::Compare(var, ".", L".", 3, 1) == true))
    exists = true;
...
bool SqlParser::ParseDb2ResultSetLocatorDeclaration(Token *declare, Token *name,
...
// FOR COLUMN system_name in OS/400 to specify 10-char system name to access by
// old apps (like 8-char file name in Windows for DOS apps)
