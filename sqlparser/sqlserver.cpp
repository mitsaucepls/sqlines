...
void SqlParser::SqlServerConvertRowLevelTrigger(Token *table, Token *when,
  ...
  if (delete_ != NULL) {
    ...
    Append(decl_end, "CURSOR FOR SELECT ", L"CURSOR FOR SELECT ", 18,
           _declare_format);
...
void SqlParser::SqlServerAppendSubqueryAlias(Token *append,
...
// Any Transact-SQL statement can be followed by ; and GO, any combination of
// them or nothing at all
void SqlParser::SqlServerDelimiter() {
  if (Source(SQL_SQL_SERVER, SQL_SYBASE) == false ||
      Target(SQL_SQL_SERVER, SQL_SYBASE) == true)
    return;
...
void SqlParser::SqlServerAddStmtDelimiter(bool force) {
  if (Source(SQL_SQL_SERVER, SQL_SYBASE) == false ||
      TOKEN_CMPC(GetLastToken(), ';'))
    return;

  // Add the delimiter in statements in procedural block only
  if (_spl_scope == 0 && force == false) {
    // Special case when a single statement followed by GO, then add ; even if
    // we are no inside procedure scope
  ...
  if (option->Compare("ANSI_NULLS", L"ANSI_NULLS", 10) == true) {
    ...
    if (option->Compare("ANSI_PADDING", L"ANSI_PADDING", 12) == true) {
      ...
      if (option->Compare("NOCOUNT", L"NOCOUNT", 7) == true) {
        ...
        if (option->Compare("QUOTED_IDENTIFIER", L"QUOTED_IDENTIFIER", 17) ==
            true) {
          /*Token *value */ (void)GetNextToken();
    ...
    if (option->Compare("ALLOW_PAGE_LOCKS", L"ALLOW_PAGE_LOCKS", 16) == true) {
      ...
      if (option->Compare("ALLOW_ROW_LOCKS", L"ALLOW_ROW_LOCKS", 15) == true) {
        ...
        if (option->Compare("FILLFACTOR", L"FILLFACTOR", 10) == true) {
          ...
          if (option->Compare("IGNORE_DUP_KEY", L"IGNORE_DUP_KEY", 14) ==
              true) {
            value = GetNextToken();
            ...
            if (option->Compare("PAD_INDEX", L"PAD_INDEX", 9) == true) {
              ...
              if (option->Compare("STATISTICS_NORECOMPUTE",
                                  L"STATISTICS_NORECOMPUTE", 22) == true) {
                value = GetNextToken();
...
void SqlParser::SqlServerToDateAdd(Token *op, Token *first, Token *first_end,
  ...
  else if (second->data_subtype == TOKEN_DT2_INTVL_DAY)
    PREPEND_NOFMT(first, "dd");
  else if (second->data_subtype == TOKEN_DT2_INTVL_MIN)
    PREPEND_NOFMT(first, "mi");
  else if (second->data_subtype == TOKEN_DT2_INTVL_SEC)
...
void SqlParser::ParseSqlServerExecProcedure(Token *execute, Token *name) {
  ...
  while (true) {
    ...
    // If there are no params, we can have conflict with next statement as there
    // may be no delimiters in Transact-SQL If there is at least one parameter,
    // no conflict is possible for subsequent params as comma defines whether it
    // exists or not
  ...
  // When there are no parameters MySQL, MariaDB allow as CALL name() and CALL
  // name
  if (params > 0) {
    if (Target(SQL_MARIADB, SQL_MYSQL)) {
      APPEND_NOFMT(name, "(");
...
void SqlParser::SqlServerMoveCursorDeclarations() {
  ...
  while (cursor_item != NULL) {
    ...
    // Find all OPEN statements for this cursor (the same cursor can be open
    // multiple times)
