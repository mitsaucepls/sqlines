...
// Try to define the data type by the variable or column name
// (table.column%TYPE)
...
char SqlParser::GuessType(Token *name, TokenStr &type) {
  ...
  if ((Token::Compare(name, "ID", L"ID", 2) == true) ||
    ...
    if ((len >= 7 &&
         Token::Compare(name, "BALANCE", L"BALANCE", len - 7, 7) == true) ||
        (len >= 6 &&
         Token::Compare(name, "AMOUNT", L"AMOUNT", len - 6, 6) == true)) {
      ...
      if (len > 4 &&
          Token::Compare(name, "DATE", L"DATE", len - 4, 4) == true) {
        if (Target(SQL_SQL_SERVER, SQL_MARIADB, SQL_MYSQL))
