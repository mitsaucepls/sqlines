...
void StatsSummaryItem::UpdateSingleOccurrenceStatus(StatsDetailItem *sid) {
  ...
  if (sid->conv_status == STATS_CONV_ERROR) {
    ...
    if (sid->conv_status == STATS_CONV_OK && conv_undef == 0 &&
        conv_warn == 0 && conv_error == 0) {
      conv_no_need = 0;
      conv_ok = 1;
    } else
      // No conversion is required
      if (sid->conv_status == STATS_CONV_NO_NEED && conv_undef == 0 &&
          conv_ok == 0 && conv_warn == 0 && conv_error == 0)
        conv_no_need = 1;
      else
        // No status specified, treat it as undefined conversion, and overwite
        // all other statuses except error
...
void StatsSummaryItem::UpdateSingleOccurrenceComplexity(int complexity) {
  ...
  if (complexity == STATS_CONV_VERY_HIGH) {
    ...
    complexity_medium = 0;
    complexity_high = 1;
  } else if (complexity == STATS_CONV_MEDIUM && complexity_very_high == 0 &&
             complexity_high == 0) {
    complexity_very_low = 0;
    complexity_low = 0;
    complexity_medium = 1;
  } else if (complexity == STATS_CONV_LOW && complexity_very_high == 0 &&
             complexity_high == 0 && complexity_medium == 0) {
    complexity_very_low = 0;
    complexity_low = 1;
  } else if (complexity == STATS_CONV_VERY_LOW && complexity_very_high == 0 &&
             complexity_high == 0 && complexity_medium == 0 &&
             complexity_low == 0)
...
void Stats::LogFuncCall(Token *name, Token *end, std::string &cur_file) {
  ...
  if (file != NULL) {
    ...
    fprintf(file, "%s,%d,%.*s,", cur_file.c_str(), name->line, (int)name->len,
            name->str);
    ...
    fprintf(file,
            "<sqlines>%s</sqlines>,<sqlines>%s</sqlines>,<sqlines>%s</"
            "sqlines>,<sqlines>%s</sqlines>",
            src.c_str(), tgt.c_str(), src_meta.c_str(), tgt_meta.c_str());
...
const char *Stats::GetMetaIdent(Token *name) {
  ...
  else if (name->data_subtype == TOKEN_DT2_DATETIME_SEC)
    return "expr_datetime_sec";
  else if (name->data_subtype == TOKEN_DT2_DATE)
    return "expr_date";
  else if (name->data_type == TOKEN_DT_NUMBER)
    return "expr_number";
  else if (name->data_type == TOKEN_DT_DATETIME)
    return "expr_datetime";
...
void Stats::Add(std::map<std::string, StatsItem> &map, Token *token,
                StatsItem *si, bool case_insense) {
...
void Stats::Add(std::map<std::string, StatsItem> &map, Token *start,
                StatsItem *si, Token *end, bool case_insense) {
  if (start == NULL || start->str == NULL || start->len == 0 || end == NULL)
    return;

  std::string source(start->str,
                     start->len + start->remain_size - end->remain_size);
  Add(map, source, start->t_str, si, case_insense);
}

void Stats::Add(std::map<std::string, StatsItem> &map, std::string value,
                const char *target, StatsItem *si, bool case_insense) {
  if (case_insense)
...
void Stats::Add(std::map<std::string, StatsSummaryItem> &map, std::string value,
                int conv_status, bool case_insense) {
  if (case_insense)
    std::transform(value.begin(), value.end(), value.begin(), ::toupper);
...
// Collect statistics for summary items
void Stats::Add(std::map<std::string, StatsSummaryItem> &map, Token *token,
                int conv_status) {
  if (token == NULL || token->str == NULL || token->len == 0)
    return;
...
// Collect statistics for summary items
void Stats::Add(std::map<std::string, StatsSummaryItem> &map, Token *token,
                StatsSummaryItem *item, Token *end, bool case_insense) {
  if (token == NULL || token->str == NULL || token->len == 0)
    return;
...
void Stats::Add(std::map<std::string, StatsDetailItem> &map, std::string value,
  ...
  if (i == map.end()) {
    // Create a copy as the item can be added to multiple collections with
    // different snippets
    ...
    (*i).second.count++;
    (*i).second.snippets.push_back(
        StatsSnippetItem(_source_current_file, start, end));
  }
}

// Collect statistics for detailed items
void Stats::Add(std::map<std::string, StatsDetailItem> &map, Token *token,
...
// Collect statistics for detailed items
void Stats::Add(std::map<std::string, StatsDetailItem> &map, Token *start_val,
                Token *end_val, StatsDetailItem *item, Token *start,
                Token *end) {
  if (start_val == NULL || start_val->str == NULL || end_val == NULL ||
      end_val->str == NULL)
    return;

  std::string value(start_val->str, start_val->len + start_val->remain_size -
                                        end_val->remain_size);
...
// Collect statistics for detailed items
void Stats::Add(std::map<std::string, StatsDetailItem> &map, const char *value,
...
// Collect statistics for detailed items
void Stats::Add(std::map<std::string, StatsDetailItem> &map, Token *start,
                Token *end, const char *desc, const char *note,
                const char *link, int conv_status) {
  Add(map,
      std::string(start->str,
                  start->len + start->remain_size - end->remain_size)
          .c_str(),
      desc, note, link, conv_status);
}

// Collect statistics for the specified item
void Stats::Add(std::map<std::string, int> &map, Token *token,
                bool case_insense) {
  if (token == NULL || token->str == NULL || token->len == 0)
    return;
...
void Stats::Add(std::map<std::string, int> &map, TokenStr *token,
                bool case_insense) {
  if (token == NULL)
    return;
...
void Stats::Add(std::map<std::string, int> &map, Token *start, Token *end,
                bool case_insense) {
...
void Stats::Add(std::map<std::string, int> &map, std::string value,
                bool case_insense) {
  if (case_insense)
    std::transform(value.begin(), value.end(), value.begin(), ::toupper);
...
StatsSnippetItem::StatsSnippetItem(std::string &f, Token *start, Token *end) {
  ...
  if (start->str != NULL)
    snippet.assign(start->str,
                   start->len + start->remain_size - end->remain_size);
  // Start token is a char
  else
    snippet.assign(start->next_start - 1,
                   (unsigned int)(1 + start->remain_size - end->remain_size));
}
