...
const wchar_t *g_no_alias_w[] = {L"END",    L"GO",    L"ORDER",
                                 L"SELECT", L"WHERE", NULL};
...
void SqlParser::SetLang(const char *value, bool source) {
  ...
  } else if (_stricmp(value, "cobol") == 0)
    app = APP_COBOL;
            ...
            if (_stricmp(option, "-fspmapf") == 0 && value != NULL)
              ...
              if (_stricmp(option, "-sl") == 0 && value != NULL)
                ...
                if (_stricmp(option, "-tl") == 0 && value != NULL)
                  SetLang(value, false);
                else if (_stricmp(option, "-oracle_plsql_number_mapping") ==
                             0 &&
                         value != NULL)
                  _option_oracle_plsql_number_mapping = value;
                else if (_stricmp(option, "-set_explicit_schema") == 0 &&
                         value != NULL)
                  _option_set_explicit_schema = value;
                else if (_stricmp(option, "__cur_file__") == 0 &&
                         value != NULL) {
                  _option_cur_file = value;

                  if (_stats != NULL)
                    _stats->SetSourceFile(value);
                } else if (_stricmp(option, "__eval_mode__") == 0 &&
                           value != NULL)
                  _option_eval_mode = true;
}

// Perform conversion
int SqlParser::Convert(const char *input, int size, const char **output,
...
void SqlParser::Parse(Token *token, int scope, int *result_sets) {
  ...
  if (exists == true && _source == SQL_ORACLE &&
      token->type == TOKEN_FUNCTION && token->t_type != TOKEN_STATEMENT &&
      _spl_scope == SQL_SCOPE_PROC) {
    // PostgreSQL requires PERFORM proc() from within PL/pgSQL or SELECT proc()
...
Token *SqlParser::GetNextCharToken(Token *prev, const char ch,
                                   const wchar_t wch) {
  if (prev == NULL)
    return NULL;
...
Token *SqlParser::GetNextPlusMinusAsOperatorToken(const char ch,
                                                  const wchar_t wch) {
  Token *token = GetNextToken();
  ...
  // If + or - already returned as separate token then exit, for example when
  // there is  "+ var"
  if (token->Compare(ch, wch) == true)
    return token;

  // If a number follows + or - they will be in single token as +1, but check
  // for + or - first
...
Token *SqlParser::GetNextSpecialCharToken(Token *prev, const char *str,
...
Token *SqlParser::GetNextSpecialCharToken(const char *str, const wchar_t *wstr,
                                          size_t len) {
...
Token *SqlParser::GetNextWordToken(const char *str, const wchar_t *wstr,
                                   size_t len) {
  Token *token = GetNextToken();
...
// Get next token if the previous is set
Token *SqlParser::GetNextWordToken(Token *prev, const char *str,
                                   const wchar_t *wstr, size_t len) {
  if (prev == NULL)
    return NULL;
...
bool SqlParser::GetStringLiteral(Token *token) {
  ...
  if (*cur == '\'') {
    ...
    // N'literal' in SQL Server (spaces between N and ' not allowed, N must be
    // upper case)
...
bool SqlParser::ParseComment() {
  ...
  while (true) {
    ...
    if (_remain_size >= 2 && *cur == '-' && cur[1] == '-') {
      ...
      if (Target(SQL_MARIADB, SQL_MYSQL) && _remain_size > 0 && *cur != ' ' &&
          *cur != '\t') {
        Token *space = new Token();
        space->type = TOKEN_SYMBOL;
        space->chr = ' ';
      ...
      if (_remain_size >= 2 && *cur == '/' && cur[1] == '*') {
        ...
        if (_source == SQL_INFORMIX && *cur == '{') {
          ...
          if (Source(SQL_SYBASE_ASA, SQL_SYBASE_ADS) && _remain_size >= 2 &&
              *cur == '/' && cur[1] == '/') {
            // Use 2 tokens to represent the comment
            Token *start = new Token();
            start->type = TOKEN_COMMENT;
            ...
            if (_source == SQL_MYSQL && _remain_size >= 1 && *cur == '#') {
              ...
              if (_source_app == APP_COBOL && _remain_size >= 1 &&
                  *cur == '*') {
                // Use 2 tokens to represent the comment
                Token *start = new Token();
                start->type = TOKEN_COMMENT;
...
bool SqlParser::GetWordToken(Token *token) {
  ...
  // Check for a sign for numbers in the first position (sign can go before
  // variable name -num i.e)
  if (_remain_size > 1 &&
      (*cur == '+' || *cur == '-') /*&& cur[1] >= '0' && cur[1] <= '9'*/ &&
      // Skip comment --
      cur[1] != '-') {
    char sign = *cur;
  ...
  // Identifiers starts as a word but then there is quoted part
  // SCHEMA."TABLE".COL i.e.
  ...
  while (_remain_size > 0) {
    ...
    // Check whether we meet a special character allowed in identifiers
    if (strchr(g_symbols, *cur) != NULL) {
      // @variable in SQL Server and MySQL, :new in Oracle trigger, #temp table
      // name in SQL Server
      // * meaning all columns, - in COBOL identifier, label : label name in DB2
      // $ or $$ often used as replacement markers, $ is also allowed in Oracle
      // identifiers
      ...
      if (*cur == '*' &&
          (len == 0 || (len > 0 && cur > _start && cur[-1] != '.')))
        break;

      // Check for partially quoted identifier that starts as a word then quoted
      // part follows
      if (*cur == '"' || *cur == '[') {
      ...
      if (*cur == ':') {
        // But := also means assigment in Oracle (space is not allowed between :
        // and =)
        if ((_remain_size > 1 && cur[1] == '=') ||
            // In DB2, Teradata, MySQL : used in label, and label:BEGIN (without
            // spaces) or label :BEGIN is correct, but :param can be also used
            // in scripts
            ...
            (_remain_size > 1 && cur[1] == ':') ||
            (cur > _start && cur[-1] == ':'))
          break;
      }

      // : can follow after label in DB2
      // if(*cur == ':' && len == 0 && _remain_size > 1 && Str::IsSpace(cur[1])
      // == true) 	break;

      // & used as parameter marker in scripts i.e. SQL*Plus, must be at the
      // first position
      if (*cur == '&' && len != 0)
        break;
  ...
  if (partially_quoted_identifier == true) {
    ...
    // If a single special character was selected in the right position, but no
    // more characters followed do not return as word
    if (len == 1 &&
        (strchr(g_symbols, *_next_start) != NULL ||
...
Token *SqlParser::Append(Token *token, const char *str, const wchar_t *wstr,
                         size_t len, Token *format) {
  if (token == NULL)
    return NULL;
  ...
  // If token starts with newline, add the same number of spaces before appended
  // token
  if (str != NULL && str[0] == '\n') {
    ...
    // Copy all blankes
    while (prev != NULL && (prev->Compare(' ', L' ') == true ||
                            prev->Compare('\t', L'\t') == true)) {
...
Token *SqlParser::AppendWithSpaceAfter(Token *token, const char *str,
                                       const wchar_t *wstr, size_t len,
                                       Token *format) {
...
void SqlParser::AppendFirst(Token *token, const char *str, const wchar_t *wstr,
                            size_t len, Token *format) {
  if (token == NULL)
    return;
...
Token *SqlParser::AppendNoFormat(Token *token, const char *str,
                                 const wchar_t * /*wstr*/, size_t len) {
  if (token == NULL)
    return NULL;
...
void SqlParser::AppendFirstNoFormat(Token *token, const char *str,
                                    const wchar_t * /*wstr*/, size_t len) {
  if (token == NULL)
    return;
...
Token *SqlParser::AppendCopy(Token *token, Token *app) {
  ...
  *append = *app;
  ...
  // Reallocate source if space is not in place as the original token can be
  // removed later
...
Token *SqlParser::AppendCopy(Token *token, Token *first, Token *last,
                             bool append_removed) {
...
void SqlParser::AppendSpaceCopy(Token *token, Token *first, Token *last,
                                bool append_removed) {
  if (token == NULL)
    return;
...
void SqlParser::AppendNewlineCopy(Token *token, Token *first, Token *last,
                                  size_t newlines, bool append_removed) {
...
Token *SqlParser::Prepend(Token *token, const char *str, const wchar_t *wstr,
                          size_t len, Token *format) {
  if (token == NULL)
    return NULL;
  ...
  // If token ends with newline, add the same number of spaces after prepended
  // token
...
Token *SqlParser::PrependNoFormat(Token *token, const char *str,
                                  const wchar_t * /*wstr*/, size_t len) {
  if (token == NULL)
    return NULL;
...
Token *SqlParser::PrependCopy(Token *token, Token *first, Token *last,
                              bool prepend_removed) {
  if (token == NULL || first == NULL)
    return NULL;
...
void SqlParser::PrependSpaceCopy(Token *token, Token *first, Token *last,
                                 bool prepend_removed) {
  if (token == NULL)
    return;
...
void SqlParser::PrependFirstNoFormat(Token *token, const char *str,
                                     const wchar_t * /*wstr*/, size_t len) {
  if (token == NULL)
    return;
...
void SqlParser::ChangeWithSpacesAround(Token *token, const char *new_str,
...
bool SqlParser::IsValidAlias(Token *token) {
  ...
  if (Source(SQL_ORACLE, SQL_SQL_SERVER, SQL_DB2, SQL_INFORMIX, SQL_SYBASE) ==
      false) {
  ...
  if (_source == SQL_ORACLE) {
    ...
    if (Token::Compare(token, "CONNECT", L"CONNECT", 7) == true ||
        ...
        Token::Compare(token, "START", L"START", 5) ==
            true /* START WITH after FROM */
    ...
    if (_source == SQL_SQL_SERVER || _source == SQL_SYBASE) {
      ...
      if (TOKEN_CMP(token, "ALTER") || TOKEN_CMP(token, "CLOSE") ||
          TOKEN_CMP(token, "DECLARE") || TOKEN_CMP(token, "ELSE") ||
          TOKEN_CMP(token, "END") || TOKEN_CMP(token, "EXEC") ||
          TOKEN_CMP(token, "FETCH") || TOKEN_CMP(token, "IF") ||
          TOKEN_CMP(token, "INSERT") || TOKEN_CMP(token, "GO") ||
          TOKEN_CMP(token, "GROUP") || TOKEN_CMP(token, "ORDER") ||
          TOKEN_CMP(token, "RETURN") || TOKEN_CMP(token, "SELECT") ||
          TOKEN_CMP(token, "SET") || TOKEN_CMP(token, "UNION") ||
          TOKEN_CMP(token, "UPDATE") || TOKEN_CMP(token, "WHERE"))
        alias = false;
    } else
      ...
      if (_source == SQL_DB2) {
        ...
        // DB2 allows WHERE as alias, but does not allow for example WHERE ORDER
        // BY Our parser currently does not allow WHERE as alias
        if (token->Compare("WHERE", L"WHERE", 5) == true)
          alias = false;
        else
          // GROUP BY or ORDER BY
          if (token->Compare("GROUP", L"GROUP", 5) == true ||
              token->Compare("ORDER", L"ORDER", 5) == true)
...
void SqlParser::Comment(const char *word, const wchar_t *w_word, size_t len,
