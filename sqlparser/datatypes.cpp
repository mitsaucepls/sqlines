      ...
      if (type != NULL) {
        ...
        if (like != NULL) {
          ...
          if (_target == SQL_ORACLE) {
            ...
            Token::Remove(like);
          } else if (Target(SQL_SQL_SERVER) == true) {
            TokenStr type(" ", L" ", 1);
            GuessType(col, type);
...
bool SqlParser::ParseBigintType(Token *name, int clause_scope) {
  ...
  if (_target == SQL_ORACLE) {
    ...
    if (clause_scope == SQL_SCOPE_FUNC_PARAMS ||
        clause_scope == SQL_SCOPE_PROC_PARAMS)
...
bool SqlParser::ParseBitType(Token *name) {
  ...
  if (open != NULL &&
      Target(SQL_MARIADB, SQL_MYSQL, SQL_POSTGRESQL, SQL_SYBASE_ASA) == false) {
    int size;
    int rem;
...
bool SqlParser::ParseBooleanType(Token *name) {
  ...
  if (name->Compare("BOOLEAN", L"BOOLEAN", 7) == false &&
      name->Compare("BOOL", L"BOOL", 4) == false)
    return false;
...
// CHARACTER & CHARACTER VARYING in Oracle, SQL Server, DB2, MySQL, PostgreSQL,
// Informix, Sybase ASE, Sybase ASA
bool SqlParser::ParseCharacterType(Token *name, int clause_scope) {
  ...
  if (open != NULL) {
    ...
    // For Oracle, size can be followed by length semantics CHAR or BYTE; for
    // DB2 only BYTE
    ...
    if (semantics == NULL) {
      ...
      // For Sybase ASA, size can be followed by length semantics CHAR or
      // CHARACTER
      if (semantics == NULL)
        semantics = GetNextWordToken("CHARACTER", L"CHARACTER", 9);
    }

    // If target is not Oracle, DB2 and Sybase ASA remove length semantics
    // attribute
    if (semantics != NULL && _target != SQL_ORACLE && _target != SQL_DB2 &&
        _target != SQL_SYBASE_ASA)
      Token::Remove(semantics);
    // Convert semantic CHARATER in Sybase ASA to CHAR in Oracle
    else if (Token::Compare(semantics, "CHARACTER", L"CHARACTER", 9) == true &&
             _target == SQL_ORACLE)
          ...
          if (_target == SQL_ORACLE && _source != SQL_ORACLE) {
            ...
            if (open == NULL) {
              ...
              if (_source == SQL_POSTGRESQL)
                ...
                // Size is 1 by default in SQL Server, Informix, Sybase ASE,
                // Sybase ASA
                if (Source(SQL_SQL_SERVER, SQL_INFORMIX, SQL_SYBASE,
                           SQL_SYBASE_ASA) == true)
                  Append(varying, "(1)", L"(1)", 3);
            }
          } else
            // VARCHAR is unlimited by default in PostgreSQL
            if (_target == SQL_POSTGRESQL && open == NULL &&
                _source != SQL_POSTGRESQL)
              Append(varying, "(1)", L"(1)", 3);
            else
              // Size is mandatory in MySQL
  ...
  // If target is Oracle, and used in parameter list, remove length
  // specification
  if (_target == SQL_ORACLE && (clause_scope == SQL_SCOPE_FUNC_PARAMS ||
                                clause_scope == SQL_SCOPE_PROC_PARAMS))
    Token::Remove(open, close);
...
// CHAR and CHAR VARYING in Oracle, SQL Server, DB2, MySQL, PostgreSQL,
// Informix, Sybase ASE, Sybase ASA
bool SqlParser::ParseCharType(Token *name, int clause_scope) {
  ...
  if (size != NULL && size->GetInt() <= 128 &&
      si.conv_status == STATS_CONV_NO_NEED) {
...
bool SqlParser::ParseDatetimeType(Token *name) {
  ...
  if (datetime == false && datetime_in_braces == false &&
      datetime_in_quotes == false)
    return false;
  ...
  else if (Token::Compare(first_unit, "MONTH", L"MONTH", 5) == true)
    first_month = first_unit;
  else if (Token::Compare(first_unit, "DAY", L"DAY", 3) == true)
    first_day = first_unit;
  else if (Token::Compare(first_unit, "HOUR", L"HOUR", 4) == true)
    first_hour = first_unit;
  else if (Token::Compare(first_unit, "MINUTE", L"MINUTE", 6) == true)
    first_minute = first_unit;
  else if (Token::Compare(first_unit, "SECOND", L"SECOND", 6) == true)
    first_second = first_unit;
  else if (Token::Compare(first_unit, "FRACTION", L"FRACTION", 8) == true)
  ...
  if (first_unit != NULL) {
    ...
    else if (Token::Compare(second_unit, "MONTH", L"MONTH", 5) == true)
      second_month = second_unit;
    else if (Token::Compare(second_unit, "DAY", L"DAY", 3) == true)
      second_day = second_unit;
    else if (Token::Compare(second_unit, "HOUR", L"HOUR", 4) == true)
      second_hour = second_unit;
    else if (Token::Compare(second_unit, "MINUTE", L"MINUTE", 6) == true)
      second_minute = second_unit;
    else if (Token::Compare(second_unit, "SECOND", L"SECOND", 6) == true)
      second_second = second_unit;
    else if (Token::Compare(second_unit, "FRACTION", L"FRACTION", 8) == true)
        ...
        if (_source == SQL_INFORMIX) {
          ...
          if (first_year != NULL) {
            ...
            if (second_fraction != NULL) {
              ...
              if (second_second != NULL || second_minute != NULL ||
                  second_hour != NULL) {
...
bool SqlParser::ParseDatetime2Type(Token *name) {
  ...
  if (Target(SQL_MARIADB, SQL_MYSQL)) {
    ...
    if (Target(SQL_POSTGRESQL, SQL_REDSHIFT)) {
      ...
      // Fraction is 7 by default in SQL Server, but max fraction is 6 in
      // PostgreSQL
      if (open == NULL)
        Append(name, "(6)", L"(6)", 3);
      else if (fraction_int > 6)
...
// DATE in Oracle, SQL Server, DB2, MySQL, PostgreSQL, Informix, Sybase ASE,
// Sybase ASA
bool SqlParser::ParseDateType(Token *name) {
  ...
  if (_source == SQL_ORACLE) {
    if (Target(SQL_SQL_SERVER, SQL_MARIADB, SQL_MYSQL, SQL_SYBASE))
      Token::Change(name, "DATETIME", L"DATETIME", 8);
    else if (_target == SQL_MARIADB_ORA) {
      STATS_DTL_CONV_NO_NEED(true)
      STATS_ITM_CONV_NO_NEED
    } else if (Target(SQL_DB2, SQL_POSTGRESQL, SQL_GREENPLUM, SQL_ESGYNDB))
      Token::Change(name, "TIMESTAMP(0)", L"TIMESTAMP(0)", 12);
  } else
    // Remove [] for other databases
  ...
  name->data_type = TOKEN_DT_DATETIME;
  name->data_subtype =
      (_source == SQL_ORACLE) ? TOKEN_DT2_DATETIME_SEC : TOKEN_DT2_DATE;
...
// DECIMAL and DEC in Oracle, SQL Server, DB2, MySQL, PostgreSQL, Informix,
// Sybase ASE, Sybase ASA, Teradata
bool SqlParser::ParseDecimalType(Token *name, int clause_scope) {
  ...
  if (name->Compare("DECIMAL", L"DECIMAL", 7) == true ||
    ...
    // Can be enclosed in [] for SQL Server
    if (name->Compare("[DECIMAL]", L"[DECIMAL]", 9) == true ||
        name->Compare("[DEC]", L"[DEC]", 5) == true)
...
// DOUBLE PRECISION in Oracle, SQL Server, DB2, MySQL, PostgreSQL, Informix,
...
// FLOAT in Oracle, SQL Server, DB2, MySQL, PostgreSQL, Informix, Sybase ASE,
// Sybase ASA
bool SqlParser::ParseFloatType(Token *name) {
  ...
  if (_target == SQL_ORACLE && _source != SQL_ORACLE) {
    ...
    if (_target == SQL_SQL_SERVER &&
        Source(SQL_SYBASE_ASA, SQL_SQL_SERVER) == false && open != NULL) {
      Token::Remove(open, close);
      conv = true;
    } else
      // Convert to DOUBLE in MySQL
...
bool SqlParser::ParseIntervalType(Token *name) {
  ...
  if (_target == SQL_ORACLE && _source != SQL_ORACLE) {
    ...
    if (start == false)
      ...
      // or first unit is equal to second unit (for Informix) convert to
      // NUMBER(5) in Oracle
      if (to == NULL || Token::Compare(first_unit, second_unit) == true) {
        ...
        // If second unit is specified convert to INTERVAL DAY TO SECOND in
        // Oracle, except INTERVAL YEAR TO MONTH
    ...
    if (Target(SQL_SQL_SERVER, SQL_MARIADB, SQL_MYSQL) == true) {
      ...
      STATS_ITM_CONV_ERROR
    } else if (_target == SQL_POSTGRESQL && _source != SQL_POSTGRESQL) {
      // Remove precision for PostgreSQL
      if (open != NULL)
        Token::Remove(open, close);

      // When first unit is equal to second unit (for Informix)
      // or first unit is SECOND and second unit is FRACTION (for Informix)
      // remove second unit in PostgreSQL
...
// INT in Oracle, SQL Server, DB2, MySQL, PostgreSQL, Informix, Sybase ASE,
// Sybase ASA
bool SqlParser::ParseIntType(Token *name, int clause_scope) {
  ...
  if (name->Compare("INT", L"INT", 3) == true ||
      name->Compare("INTEGER", L"INTEGER", 7) == true)
    int_ = true;
  else
    // Can be enclosed in [] for SQL Server
    if (name->Compare("[INT]", L"[INT]", 5) == true ||
        name->Compare("[INTEGER]", L"[INTEGER]", 9) == true)
      int_in_braces = true;
  ...
  if (_target_app == APP_JAVA && (clause_scope == SQL_SCOPE_FUNC_PARAMS ||
                                  clause_scope == SQL_SCOPE_PROC_PARAMS ||
    ...
    if (_target == SQL_ORACLE && _source != SQL_ORACLE) {
      ...
      // Remove [] for other databases
      else if (_target != SQL_SQL_SERVER && int_in_braces == true) {
        Token::ChangeNoFormat(name, name, 1, name->len - 2);
        conv = true;
      }
...
bool SqlParser::ParseLongType(Token *name) {
  ...
  if (Token::Compare(unit, "BINARY", L"BINARY", 6) == true)
    binary = unit;
  else if (Token::Compare(unit, "BIT", L"BIT", 3) == true) {
    bit = unit;
    varying = GetNextWordToken("VARYING", L"VARYING", 7);
  } else if (Token::Compare(unit, "NVARCHAR", L"NVARCHAR", 8) == true)
    nvarchar = unit;
  else if (Token::Compare(unit, "RAW", L"RAW", 3) == true)
    raw = unit;
  else if (Token::Compare(unit, "VARBINARY", L"VARBINARY", 9) == true)
    varbinary = unit;
  else if (Token::Compare(unit, "VARBIT", L"VARBIT", 6) == true)
    varbit = unit;
  else if (Token::Compare(unit, "VARCHAR", L"VARCHAR", 7) == true)
    varchar = unit;
  else {
    PushBack(unit);
    unit = NULL;
      ...
      if (bit != NULL && varying != NULL) {
        ...
        if (nvarchar != NULL) {
          ...
          if (raw != NULL) {
            ...
            // Size is supported in LONG RAW(n) in PL/SQL, but not in CREATE
            // TABLE DDL in Oracle
...
bool SqlParser::ParseLogicalType(Token *name) {
  ...
  else if (_target != SQL_SYBASE_ADS)
...
bool SqlParser::ParseMoneyType(Token *name) {
  ...
  } else if (Target(SQL_SQL_SERVER, SQL_POSTGRESQL) == true) {
    // Remove ""
...
// NATIONAL CHAR, NATIONAL CHARACTER in Oracle, SQL Server, DB2, MySQL, Sybase
// ASE, Sybase ASA; NATIONAL CHAR VARYING, NATIONAL CHARACTER VARYING in Oracle,
// SQL Server, DB2, MySQL, Sybase ASE, Sybase ASA; NATIONAL TEXT in SQL Server;
...
bool SqlParser::ParseNationalType(Token *name) {
  ...
  if (Token::Compare(unit, "CHAR", L"CHAR", 4) == true ||
    ...
    character = unit;
    varying = GetNextWordToken("VARYING", L"VARYING", 7);
  } else if (Token::Compare(unit, "TEXT", L"TEXT", 4) == true)
    text = unit;
  else if (Token::Compare(unit, "VARCHAR", L"VARCHAR", 7) == true)
    varchar = unit;
  ...
  if (text != NULL) {
    ...
    if (varying != NULL) {
      ...
      if (size->Compare("MAX", L"MAX", 3) == true) {
        ...
        // Convert NATIONAL CHARACTER VARYING and NATIONAL CHAR VARYING to
        // NVARCHAR2 in Oracle
        if (_target == SQL_ORACLE && _source != SQL_ORACLE) {
          Token::Remove(name, character);
          ...
          // Default size
          if (open == NULL) {
            // Size is 1 by default in SQL Server, Informix, Sybase ASE, Sybase
            // ASA
            if (Source(SQL_SQL_SERVER, SQL_INFORMIX, SQL_SYBASE,
                       SQL_SYBASE_ASA) == true)
              Append(varying, "(1)", L"(1)", 3);
...
// NCHAR and NCHAR VARYING in Oracle, SQL Server, DB2, MySQL, Informix, Sybase
// ASE, Sybase ASA
    ...
    if (varying != NULL) {
      ...
      if (varying == NULL && _target != SQL_SQL_SERVER) {
        ...
        if (_target == SQL_POSTGRESQL)
          ...
          // If size is greater than 255 convert to NVARCHAR in MySQL and
          // MariaDB
          if (Target(SQL_MARIADB, SQL_MARIADB_ORA, SQL_MYSQL)) {
            int num = 1;
...
bool SqlParser::ParseNumberType(Token *name, int clause_scope) {
  ...
  if (open == NULL ||
      (precision->Compare('*', L'*') == true && comma == NULL)) {
    ...
    if (clause_scope == SQL_SCOPE_FUNC_PARAMS ||
      ...
      if (!_option_oracle_plsql_number_mapping.empty()) {
        ...
        // Use Double class instead of primitive double type in Java to handle
        // NULLs
        if (_target_app == APP_JAVA)
...
// NUMERIC in Oracle, SQL Server, DB2, MySQL, PostreSQL, Informix, Sybase ASE,
// Sybase ASA
  ...
  if (size->Compare("MAX", L"MAX", 3) == true) {
    ...
    if (_target == SQL_ORACLE) {
      ...
      if (open == NULL) {
        ...
        if (Source(SQL_SQL_SERVER, SQL_INFORMIX, SQL_SYBASE, SQL_SYBASE_ASA) ==
            true)
          Append(name, "(1)", L"(1)", 3);
      }
    } else
...
// REAL in Oracle, SQL Server, DB2, MySQL, PostgreSQL, Informix, Sybase ASE,
// Sybase ASA
...
bool SqlParser::ParseSmalldatetimeType(Token *name) {
  ...
  if (smalldatetime == false && smalldatetime_in_braces == false &&
      smalldatetime_in_quotes == false)
    return false;
...
// SMALLINT in Oracle, SQL Server, DB2, MySQL, PostreSQL, Informix, Sybase ASE,
// Sybase ASA
...
// TEXT in SQL Server, MySQL, PostreSQL, Informix, Sybase ASE, Sybase ASA,
// Greenplum
...
bool SqlParser::ParseTimeType(Token *name) {
  ...
  if (open == NULL) {
    ...
    if (Source(SQL_DB2, SQL_MYSQL) == true &&
        Target(SQL_DB2, SQL_MYSQL) == false)
      Append(name, "(0)", L"(0)", 3);
  }
...
bool SqlParser::ParseTimetzType(Token *name) {
  ...
  if (_target == SQL_ORACLE) {
    if (open != NULL) {
      ...
      Token::Change(name, "TIMESTAMP WITH TIME ZONE",
                    L"TIMESTAMP WITH TIME ZONE", 24);
  }
...
// TIMESTAMP in Oracle, SQL Server, DB2, MySQL, PostgreSQL, Sybase ASA, Sybase
// ADS, Greenplum
...
// UNICODE CHARACTER, UNICODE CHAR, UNICODE CHARACTER VARYING, UNICODE CHAR
// VARYING in Sybase ASE
bool SqlParser::ParseUnicodeType(Token *name) {
  ...
  if (varying != NULL) {
    ...
    if (_target == SQL_ORACLE) {
      ...
      // Convert to NATIONAL CHARACTER VARYING and NATIONAL CHAR VARYING in SQL
      // Server
      if (_target == SQL_SQL_SERVER)
...
bool SqlParser::ParseUniqueidentifierType(Token *name) {
  ...
  // In SQL Server, UNIQUEIDENTIFIER is stored in 16 bytes, but on retrieval
  // returns 36 characters (GUID with dashes)
...
bool SqlParser::ParseUnsignedType(Token *name) {
  ...
  if (Token::Compare(unit, "BIGINT", L"BIGINT", 6) == true)
    bigint = unit;
  else if (Token::Compare(unit, "INT", L"INT", 3) == true ||
           Token::Compare(unit, "INTEGER", L"INTEGER", 7) == true)
    int_ = unit;
  else if (Token::Compare(unit, "SMALLINT", L"SMALLINT", 8) == true)
    smallint = unit;
  else if (Token::Compare(unit, "TINYINT", L"TINYINT", 7) == true)
    tinyint = unit;
...
// VARCHAR in Oracle, SQL Server, DB2, MySQL, PostgreSQL, Informix, Sybase ASE,
// Sybase ASA
bool SqlParser::ParseVarcharType(Token *name, int clause_scope) {
  ...
  // Size is optional for SQL Server, PostgreSQL, Informix, Sybase ASE, Sybase
  // ASA
  if (open != NULL) {
    // For SQL Server, MAX can be specified as the size
    size = GetNextToken();
    ...
    // For Sybase ASA, size can be followed by length semantics CHAR or
    // CHARACTER
    character_semantics = (char_semantics == NULL && byte_semantics == NULL)
                              ? TOKEN_GETNEXTW("CHARACTER")
                              : NULL;

    // OCTETS for DB2
    octets_semantics =
        (Nvl(character_semantics, char_semantics, byte_semantics) == NULL)
            ? TOKEN_GETNEXTW("OCTETS")
            : NULL;
    ...
    if (character_semantics != NULL) {
      if (_target == SQL_ORACLE) {
        ...
        req_eval = true;
    } else if (octets_semantics != NULL && !Target(SQL_DB2))
      req_eval = true;
  ...
  if (size->Compare("MAX", L"MAX", 3) == true) {
    ...
    if (binary == true) {
      ...
      if (_target == SQL_ORACLE && _source != SQL_ORACLE) {
        ...
        // Default size
        if (open == NULL && clause_scope != SQL_SCOPE_FUNC_PARAMS &&
            clause_scope != SQL_SCOPE_PROC_PARAMS) {
          // PostgreSQL allows not specify size
          if (_source == SQL_POSTGRESQL)
            Append(name, "(4000)", L"(4000)", 6);
        ...
        if (_target == SQL_POSTGRESQL && open == NULL &&
          ...
          if (Target(SQL_MARIADB, SQL_MYSQL) && open == NULL) {
            ...
            // When used in parameter list for a function or procedure add
            // length
            if (_source == SQL_ORACLE &&
                (clause_scope == SQL_SCOPE_FUNC_PARAMS ||
                 clause_scope == SQL_SCOPE_PROC_PARAMS))
...
bool SqlParser::ParseVarchar2Type(Token *name, int clause_scope) {
  ...
  if (_target_app == APP_JAVA && (clause_scope == SQL_SCOPE_FUNC_PARAMS ||
                                  clause_scope == SQL_SCOPE_PROC_PARAMS ||
                                  clause_scope == SQL_SCOPE_VAR_DECL ||
                                  clause_scope == SQL_SCOPE_OBJ_TYPE_DECL))
