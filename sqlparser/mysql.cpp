...
// User-defined delimiter can be specified for MySQL, often // @ (also used for
// DB2)
bool SqlParser::ParseMySQLDelimiter(Token * /*create*/) {
  ...
  // DB2 and MySQL do not require the delimiter to be on a new line, while this
  // required for other databases
  if (Target(SQL_SQL_SERVER, SQL_ORACLE) &&
      Token::Compare(char1->prev, '\n', L'\n') == false)
    Prepend(char1, "\n", L"\n", 1);
...
bool SqlParser::MySQLCheckDelimiter(char ch, wchar_t wch, Token **char1_out,
                                    Token **char2_out) {
...
void SqlParser::MySQLAddNotFoundHandler() {
  ...
  Append(append, "\nDECLARE CONTINUE HANDLER FOR NOT FOUND SET not_found = 1;",
         L"\nDECLARE CONTINUE HANDLER FOR NOT FOUND SET not_found = 1", 58,
         format);
...
bool SqlParser::MySQLMoveCursorDeclarations(Token *declare, Token *cursor_end) {
  ...
  if (_spl_first_non_declare->remain_size > declare->remain_size) {
    ...
    AppendNewlineCopy(_spl_last_declare, declare, Nvl(semi, cursor_end), 2,
                      false);
...
// MySQL SET options, i.e. SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS,
// UNIQUE_CHECKS=0;
bool SqlParser::ParseMysqlSetOptions(Token *set) {
  ...
  while (true) {
    ...
    if (option1->Compare("UNIQUE_CHECKS", L"UNIQUE_CHECKS", 13) == true ||
      ...
      // @@FOREIGN_KEY_CHECKS
      if (option1->Compare("FOREIGN_KEY_CHECKS", L"FOREIGN_KEY_CHECKS", 18) ==
              true ||
          option2->Compare("@@FOREIGN_KEY_CHECKS", L"@@FOREIGN_KEY_CHECKS",
                           20) == true)
        exists = true;
      else
        // @@SQL_MODE
...
bool SqlParser::MysqlCreateDatabase(Token *create, Token *database,
                                    Token *name) {
