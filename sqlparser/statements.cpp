                    ...
                    if (token->Compare("COMMENT", L"COMMENT", 7) == true)
                      ...
                      if (token->Compare("COMMIT", L"COMMIT", 6) == true)
                        ...
                        if (token->Compare("CREATE", L"CREATE", 6) == true)
                          exists =
                              ParseCreateStatement(token, result_sets, &proc);
                        else
                          ...
                          if (token->Compare("DECLARE", L"DECLARE", 7) == true)
                            ...
                            if (token->Compare("DEFINE", L"DEFINE", 6) == true)
                              ...
                              if ((token->Compare("DELETE", L"DELETE", 6) ==
                                   true) ||
                                  (_source == SQL_TERADATA &&
                                   TOKEN_CMP(token, "DEL") == true))
                                exists = ParseDeleteStatement(token);
                              else
                                // DELIMITER statement
                                if (token->Compare("DELIMITER", L"DELIMITER",
                                                   9) == true)
                                  exists = ParseDelimiterStatement(token);
                                else
                                  // DROP statement
                                  if (token->Compare("DROP", L"DROP", 4) ==
                                      true)
                                    exists = ParseDropStatement(token);
                                  else
                                    // EXCEPTION block
                                    if (token->Compare("EXCEPTION",
                                                       L"EXCEPTION", 9) == true)
                                      exists = ParseExceptionBlock(token);
                                    else
                                      // EXEC statement
                                      if (token->Compare("EXEC", L"EXEC", 4) ==
                                          true)
                                        exists = ParseExecStatement(token);
                                      else
                                        // EXECUTE statement
                                        if (token->Compare("EXECUTE",
                                                           L"EXECUTE",
                                                           7) == true)
                                          exists = ParseExecuteStatement(token);
                                        else
                                          // EXIT statement
                                          if (token->Compare("EXIT", L"EXIT",
                                                             4) == true)
                                            exists = ParseExitStatement(token);
                                          else
                                            // EXPORT statement
                                            if (token->Compare("EXPORT",
                                                               L"EXPORT",
                                                               6) == true)
                                              exists =
                                                  ParseExportStatement(token);
                                            else
                                              // FETCH statement
                                              if (token->Compare("FETCH",
                                                                 L"FETCH",
                                                                 5) == true)
                                                exists =
                                                    ParseFetchStatement(token);
                                              else
                                                // FOR statement
                                                if (token->Compare("FOR",
                                                                   L"FOR",
                                                                   3) == true)
                                                  exists = ParseForStatement(
                                                      token, scope);
                                                else
                                                  // FOREACH statement
                                                  if (token->Compare("FOREACH",
                                                                     L"FOREACH",
                                                                     7) == true)
                                                    exists =
                                                                                                            ...
                                                                                                            if (token
                                                                                                              ...
                                                                                                              if (token
                                                                                                                      ->Compare(
                                                                                                                          ...
                                                                                                                          9) ==
                                                                                                                  true)
                                                                                                                exists = ParseSavepointStatement(
                                                                                                                    token);
                                                                                                              else
                                                                                                                          ...
                                                                                                                          if (token
                                                                                                                            ...
                                                                                                                            if (token
                                                                                                                                    ->Compare(
                                                                                                                                        ...
                                                                                                                                        9) ==
                                                                                                                                true)
                                                                                                                              exists = ParseTerminateStatement(
                                                                                                                                  token);
                                                                                                                            else
                                                                                                                              // TRUNCATE
                                                                                                                              if (token
                                                                                                                                    ...
                                                                                                                                    if (token
                                                                                                                                      ...
                                                                                                                                      if (token
                                                                                                                                              ->Compare(
                                                                                                                                                  ...
                                                                                                                                                  5) ==
                                                                                                                                          true)
                                                                                                                                        exists = ParseWhileStatement(
                                                                                                                                            token,
                                                                                                                                            scope);
                                                                                                                                      // Label declaration
                                                                                                                                      else
                                                                                                                                        exists = ParseLabelDeclaration(
                                                                                                                                            token,
                                                                                                                                            false);
...
// CREATE statement
bool SqlParser::ParseCreateStatement(Token *create, int *result_sets,
                                     bool *proc) {
  if (create == NULL)
    return false;
          ...
          if (Token::Compare(next, "LOB", L"LOB", 3) == true) {
            ...
            if (Token::Compare(next, "SCRATCH", L"SCRATCH", 7) == true) {
              if (_target == SQL_ORACLE)
                Token::Change(next, "GLOBAL TEMPORARY", L"GLOBAL TEMPORARY",
                              16);
              ...
              if (Token::Compare(next, "EXTERNAL", L"EXTERNAL", 8) == true) {
                ...
                // For Oracle, ORGANIZATION EXTERNAL is appended after the
                // column list
                if (_target == SQL_ORACLE)
                  Token::Remove(next);
                ...
                if (Token::Compare(next, "RAW", L"RAW", 3) == true) {
                  ...
                  if (Token::Compare(next, "MULTISET", L"MULTISET", 8) ==
                      true) {
                    ...
                    // VOLATILE TABLE in Teradata (temporary table that exists
                    // until the end of session)
                    if (Token::Compare(next, "VOLATILE", L"VOLATILE", 8) ==
                        true) {
                      if (_target == SQL_ORACLE)
                        Token::Change(next, "GLOBAL TEMPORARY",
                                      L"GLOBAL TEMPORARY", 16);
  ...
  if (next->Compare("AUXILIARY", L"AUXILIARY", 9) == true ||
      next->Compare("AUX", L"AUX", 3) == true) {
        ...
        if (next->Compare("INDEX", L"INDEX", 5) == true) {
          ...
          if (next->Compare("PACKAGE", L"PACKAGE", 7) == true) {
            ...
            if (next->Compare("PROCEDURE", L"PROCEDURE", 9) == true ||
                next->Compare("PROC", L"PROC", 4) == true) {
              exists =
                  ParseCreateProcedure(create, or_, replace, next, result_sets);
                  ...
                  if (next->Compare("SEQUENCE", L"SEQUENCE", 8) == true)
                    ...
                    if (next->Compare("STOGROUP", L"STOGROUP", 8) == true)
                      ...
                      if (next->Compare("TABLE", L"TABLE", 5) == true) {
                        ...
                        if (next->Compare("TABLESPACE", L"TABLESPACE", 10) ==
                            true)
                          ...
                          if (next->Compare("TRIGGER", L"TRIGGER", 7) == true)
                            ...
                            if (next->Compare("TYPE", L"TYPE", 4) == true)
                              ...
                              if (next->Compare("VIEW", L"VIEW", 4) == true)
                                exists =
                                    ParseCreateView(create, materialized, next);
...
// ALTER statement
bool SqlParser::ParseAlterStatement(Token *alter, int *result_sets,
                                    bool *proc) {
  if (alter == NULL)
    return false;
      ...
      if (next->Compare("FUNCTION", L"FUNCTION", 8) == true) {
        if (_source != SQL_ORACLE) {
          if (_target == SQL_ORACLE)
            ...
            // In MySQL ALTER FUNCTION does not allow to change body, so change
            // to CREATE
        ...
        if (next->Compare("PROCEDURE", L"PROCEDURE", 9) == true ||
          ...
          if (_source != SQL_ORACLE) {
            if (_target == SQL_ORACLE)
              Token::Change(alter, "CREATE OR REPLACE", L"CREATE OR REPLACE",
                            17);
            else
              // In MySQL ALTER PROCEDURE does not allow to change body, so
              // change to CREATE
              if (Target(SQL_MARIADB, SQL_MYSQL))
                Token::Change(alter, "CREATE", L"CREATE", 6);
...
bool SqlParser::ParseAlterTableStatement(Token *alter, Token *table) {
  ...
  if (_source == SQL_SQL_SERVER) {
    ...
    /*Token *nocheck */ (check == NULL) ? TOKEN_GETNEXTWP(with, "NOCHECK")
                                        : NULL;
  }
...
bool SqlParser::ParseAlterIndexStatement(Token *alter, Token * /*index*/) {
  ...
  // If there are options and they are all need to removed comment the entire
  // statement
...
bool SqlParser::ParseAllocateStatement(Token *allocate) {
  ...
  // In Oracle add SYS_REFCURSOR variable instead of removed result set locator
  // declaration
  if (_target == SQL_ORACLE) {
    ...
    for (ListwmItem *i = _spl_rs_locator_procedures.GetFirst(); i != NULL;
         i = i->next) {
...
// BEGIN; or BEGIN WORK; in Informix, PostgreSQL; BEGIN TRANSACTION in
...
bool SqlParser::ParseBeginStatement(Token *begin) {
  ...
  Token *transaction = (work == NULL)
                           ? GetNextWordToken("TRANSACTION", L"TRANSACTION", 11)
                           : NULL;

  // TRAN in SQL Server and Sybase ASE
  Token *tran =
      (work == NULL && transaction == NULL) ? TOKEN_GETNEXTW("TRAN") : NULL;

  // When WORK or TRANSACTION keyword not set check for ; to not confuse with
  // BEGIN-END block
  ...
  if (_target == SQL_ORACLE ||
      (_target == SQL_POSTGRESQL && _spl_scope == SQL_SCOPE_PROC))
...
bool SqlParser::ParseCaseStatement(Token *case_, bool stmt) {
  ...
  if (else_exp != NULL && else_exp->type == TOKEN_STRING &&
      else_exp->IsNumericInString() == false)
...
bool SqlParser::ParseCloseStatement(Token *close) {
  ...
  if (_target_app == APP_JAVA) {
    ...
    if (Source(SQL_ORACLE, SQL_DB2, SQL_INFORMIX) == true &&
        _target == SQL_SQL_SERVER) {
      Token *semi = GetNextCharToken(';', L';');
      ...
      if (Source(SQL_SQL_SERVER, SQL_SYBASE) &&
          Target(SQL_SQL_SERVER, SQL_SYBASE) == false) {
  ...
  if (type->Compare("TABLE", L"TABLE", 5) == true) {
    ...
    if (type->Compare("COLUMN", L"COLUMN", 6) == true) {
      ...
      if (_target == SQL_SQL_SERVER) {
        ...
        AppendNoFormat(comment, " sp_addextendedproperty",
                       L" sp_addextendedproperty", 23);

        Token::ChangeNoFormat(is, "'MS_Description',", L"'MS_Description',",
                              17);
...
bool SqlParser::ParseCommitStatement(Token *commit) {
  ...
  if (_target_app == APP_JAVA) {
    ...
    if (_spl_scope == SQL_SCOPE_PROC &&
        Target(SQL_POSTGRESQL, SQL_NETEZZA, SQL_GREENPLUM)) {
      Comment(commit,
              Nvl(GetNextCharToken(';', L';'), work, transaction, tran));
      commented = true;
    } else
...
bool SqlParser::ParseCreateTable(Token *create, Token *token) {
  ...
  // Temporary table in SQL Server, Sybase ASE starts with #, in Sybase ASE it
  // also can start with tempdb..
  if (Token::Compare(table, '#', L'#', 0) ||
      Token::Compare(table, "tempdb..", L"tempdb..", 0, 8)) {
    if (Target(SQL_MARIADB, SQL_MYSQL))
  ...
  if (as != NULL) {
    ...
    if (select != NULL)
      ParseSelectStatement(select, 0, SQL_SEL_CREATE_TEMP_TABLE_AS, NULL, NULL,
    ...
    ParseCreateTableColumns(create, table, pkcols, &id_col, &id_start, &id_inc,
                            &id_default, &inline_indexes, &last_colname);
    ...
    // Save bookmark to the end of columns but before storage and other
    // properties
  ...
  // Add statement delimiter if not set and handle GO when source is SQL Server,
  // Sybase ASE
  SqlServerDelimiter();
  ...
  if (id_col != NULL && _target == SQL_ORACLE) {
    ...
    OracleEmulateIdentity(create, table, id_col, last, id_start, id_inc,
                          id_default);
  }

  // If source is MySQL move inline non-unique indexes to separate CREATE INDEX
  // statements
...
bool SqlParser::ParseCreateTableColumns(Token *create, Token *table_name,
                                        ListW &pkcols, Token **id_col,
                                        Token **id_start, Token **id_inc,
                                        bool *id_default,
                                        ListWM *inline_indexes,
                                        Token **last_colname) {
  bool exists = false;
  ...
  while (true) {
    // Try to parse a standalone column constraint
    bool cns = ParseStandaloneColumnConstraints(create, table_name, pkcols,
                                                inline_indexes);
    ...
    if (cns == false) {
      ...
      if (Token::Compare(type, "SERIAL", L"SERIAL", 0, 6) == true &&
          id_col != NULL)
        *id_col = column;

      Token *type_end = GetLastToken();

      ParseColumnConstraints(create, table_name, column, type, type_end, id_col,
                             id_start, id_inc, id_default);
...
bool SqlParser::ParseCreateIndex(Token *create, Token *unique, Token *index) {
  ...
  if (unique != NULL && _target == SQL_GREENPLUM) {
    ...
    if (cr_table_start != NULL && cr_table_end != NULL &&
        cr_table_end->distributed_by == false) {
      Append(cr_table_end->book, " DISTRIBUTED BY (", L" DISTRIBUTED BY (", 17,
             cr_table_start->book);
  ...
  while (true) {
    ...
    if (next->Compare("ASC", L"ASC", 3) == true ||
        next->Compare("DESC", L"DESC", 4) == true)
      next = GetNextToken();
...
// CREATE FUNCTION
bool SqlParser::ParseCreateFunction(Token *create, Token *or_, Token *replace,
                                    Token *function) {
  STATS_DECL
  ...
  if (or_ != NULL && replace != NULL) {
    ...
    if (_target == SQL_SQL_SERVER) {
      ...
      if (Target(SQL_MARIADB, SQL_MYSQL)) {
        Prepend(create, "DROP FUNCTION IF EXISTS ", L"DROP FUNCTION IF EXISTS ",
                24);
        PrependCopy(create, name);
        Prepend(create, ";\n\n", L";\n\n", 3);
  ...
  // RETURN in Oracle; RETURNS in SQL Server, DB2, PostgreSQL, MySQL, Informix;
  // RETURNING in Informix
  ...
  if (pl_sql != NULL) {
    // Replace with GO for SQL Server, as only one CREATE FUNCTION is allowed in
    // a batch
    if (_target == SQL_SQL_SERVER)
  ...
  // In case of parser error can enter to this function recursively, reset
  // variables
  ClearSplScope();
...
bool SqlParser::ParseFunctionParameters(Token *function_name) {
  ...
  while (true) {
    ...
    // In DB2 IN, OUT or INOUT precedes the name; in MySQL function keywords not
    // supported, parameter is only IN; in Sybase ASA IN precedes the name
    if (Source(SQL_DB2, SQL_SYBASE_ASA) == true) {
      ...
      if (Token::Compare(param_type, "IN", L"IN", 2) == true) {
        ...
        // For SQL Server IN is not supported, assumed by default, and OUT goes
        // after default
        if (_target == SQL_SQL_SERVER)
          Token::Remove(param_type);
      } else if (Token::Compare(param_type, "OUT", L"OUT", 3) == true)
        out = param_type;
      else if (Token::Compare(param_type, "INOUT", L"INOUT", 5) == true)
        inout = param_type;
      else
        PushBack(param_type);
    }
    ...
    // No DEFAULT for the current column, but there was a default for previous
    // column (Informix allows this in definition)
    else if (first_default != NULL) {
...
// CREATE PACKAGE
bool SqlParser::ParseCreatePackage(Token *create, Token *or_, Token *replace,
                                   Token *package) {
  if (package == NULL)
    return false;
...
bool SqlParser::ParseCreatePackageSpec(Token *create, Token * /*or_*/,
                                       Token * /*replace*/,
                                       Token * /*package*/) {
  STATS_DECL
...
// CREATE PACKAGE BODY
bool SqlParser::ParseCreatePackageBody(Token *create, Token *or_,
                                       Token *replace, Token *package,
                                       Token *body) {
  STATS_DECL
  ...
  while (true) {
    ...
    if (TOKEN_CMP(next, "PROCEDURE")) {
      Token *create = (_target_app != APP_JAVA)
                          ? Prepend(next, "CREATE ", L"CREATE ", 7)
                          : NULL;
...
// CREATE PROCEDURE
bool SqlParser::ParseCreateProcedure(Token *create, Token *or_, Token *replace,
                                     Token *procedure, int *result_sets) {
  if (procedure == NULL)
    return false;
  ...
  if (or_ != NULL && replace != NULL) {
    ...
    if (_target == SQL_SQL_SERVER) {
      ...
      if (Target(SQL_MARIADB, SQL_MYSQL)) {
        Prepend(create, "DROP PROCEDURE IF EXISTS ",
                L"DROP PROCEDURE IF EXISTS ", 25);
        PrependCopy(create, name);
        Prepend(create, ";\n\n", L";\n\n", 3);
  ...
  // External stored procedure written in C, Java, COBOL etc., no body after
  // header declaration
  ...
  // SQL Server, Oracle, Sybase require AS before body; DB2, MySQL and Informix
  // do not allow AS
  if (as == NULL)
  ...
  if (result_sets != NULL && *result_sets > 0) {
    if (_target == SQL_ORACLE) {
      if (param_count == 0)
        Append(name, " (cur OUT SYS_REFCURSOR)", L" (cur OUT SYS_REFCURSOR)",
               24);
    }
  }
  ...
  if (pl_sql != NULL) {
    ...
    if (Target(SQL_POSTGRESQL))
      ...
      // Replace with GO for SQL Server, as only one CREATE PROCEDURE is allowed
      // in a batch
      if (_target == SQL_SQL_SERVER)
...
void SqlParser::SplPostActions() {
  // Check if temporary table need to be truncated at the beginning of block in
  // Oracle
...
bool SqlParser::ParseCreateTrigger(Token *create, Token *or_, Token *trigger) {
  ...
  while (true) {
    ...
    if (operation->Compare("INSERT", L"INSERT", 6) == true)
      insert = operation;
    else if (operation->Compare("DELETE", L"DELETE", 6) == true)
      delete_ = operation;
    else if (operation->Compare("UPDATE", L"UPDATE", 6) == true) {
      update = operation;
  ...
  ParseCreateTriggerBody(create, name, table, when, insert, update, delete_,
                         &end);
...
bool SqlParser::ParseCreateTriggerBody(Token *create, Token *name, Token *table,
                                       Token *when, Token *insert,
                                       Token * /*update*/, Token * /*delete_*/,
                                       Token **end_out) {
  ...
  if (begin == NULL &&
      Target(SQL_SQL_SERVER, SQL_MARIADB, SQL_MYSQL) == false) {
  ...
  // In Oracle ; must follow END name, in SQL Server, MySQL ; is optional after
  // END
  Token *semi = GetNextCharToken(';', L';');
  ...
  // User-defined delimiter can be specified for MySQL, often // @ (also used
  // for DB2)
...
// SQL Server, DB2, MySQL DECLARE variable statement; Oracle DECLARE block
// (inner or anonymous)
bool SqlParser::ParseDeclareStatement(Token *declare) {
  ...
  while (true) {
    // Variable name of unknown type yet (local variable, cursor, table,
    // condition etc.)
    Token *name = GetNextToken();
    ...
    if (type->Compare("CURSOR", L"CURSOR", 6) == true)
      ...
      if (type->Compare("CONDITION", L"CONDITION", 9) == true ||
          type->Compare("EXCEPTION", L"EXCEPTION", 9) == true)
        ...
        if (type->Compare("TABLE", L"TABLE", 5) == true)
          ...
          if (type->Compare("RESULT_SET_LOCATOR", L"RESULT_SET_LOCATOR", 18) ==
              true)
    ...
    if (Target(SQL_ORACLE, SQL_POSTGRESQL))
      ...
      if (Target(SQL_MYSQL, SQL_MARIADB)) {
        ...
        // If the next variable are on the same line put DECLARE on this line as
        // well
...
bool SqlParser::ParseDeleteStatement(Token *delete_) {
  ...
  // DEL in Teradata
  if (_source == SQL_TERADATA && _target != SQL_TERADATA &&
      TOKEN_CMP(delete_, "DEL") == true)
  ...
  // If there is no WHERE return alias to input if no followed by ; may be it is
  // just next statement start word
...
bool SqlParser::ParseDropTableStatement(Token *drop, Token *table) {
  ...
  if (_target == SQL_ORACLE) {
    for (ListwItem *i = _spl_created_session_tables.GetFirst(); i != NULL;
         i = i->next) {
      ...
      if (Token::Compare(table_name, temp) == true) {
        ...
        if (_spl_scope == SQL_SCOPE_PROC || _spl_scope == SQL_SCOPE_FUNC ||
            _spl_scope == SQL_SCOPE_TRIGGER) {
...
// Oracle, Teradata EXECUTE IMMEDIATE; Teradata, MySQL, PostgreSQL EXECUTE
// statement; SQL Server, Sybase ASE EXECUTE/EXEC;
bool SqlParser::ParseExecuteStatement(Token *execute) {
  ...
  // Check for executing a system procedure in SQL Server, Sybase ASE (exec
  // sp_addtype i.e.)
  ...
  if (_source == SQL_SYBASE_ADS &&
      ParseSybaseExecuteProcedureStatement(execute))
    return true;
  ...
  if (immediate != NULL) {
    ...
    // In Teradata USING cannot be specified with EXECUTE IMMEDIATE, only with
    // EXECUTE
    ...
    if (_target == SQL_SQL_SERVER && (into != NULL || using_ != NULL)) {
      ...
      // Note that assignment in select list 'SELECT @var = ... ' is optional;
      // it is enough just specify OUTPUT parameters
  ...
  // EXECUTE in SQL Server, PostgreSQL, MySQL, Teradata, Sybase ASE
    ...
    // Statement ID pointing to a variable containing SQL can be specified in
    // Teradata
    ...
    // Dynamic SQL expression or variable
      ...
      if (exp == exp_end &&
          (exp->type == TOKEN_IDENT || exp->type == TOKEN_WORD)) {
...
bool SqlParser::ParseExitStatement(Token *exit) {
  ...
  if (notfound != NULL) {
    ...
    if (_target == SQL_SQL_SERVER) {
      ...
      Token::Change(when, "@@FETCH_STATUS <> 0 BREAK",
                    L"@@FETCH_STATUS <> 0 BREAK", 25);
...
bool SqlParser::ParseExportStatement(Token *export_) {
  ...
  if (select != NULL)
    ParseSelectStatement(select, 0, SQL_SEL_EXPORT, NULL, NULL, NULL, NULL,
                         NULL, NULL, NULL, NULL);
  ...
  if (_target == SQL_ORACLE) {
    ...
    // Add settings to format output, PAGESIZE tells to have one single page
...
bool SqlParser::ParseCreateType(Token *create, Token *type) {
  ...
  // Optional constraint
  ParseColumnConstraints(create, name, name, datatype, type_end, NULL, NULL,
                         NULL, NULL);
...
bool SqlParser::ParseCreateView(Token *create, Token *materialized,
                                Token *view) {
  STATS_DECL
  STATS_DTL_DECL
  ...
  if (select != NULL)
    ParseSelectStatement(select, 0, SQL_SEL_VIEW, NULL, NULL, NULL, NULL, NULL,
                         NULL, NULL, NULL);
...
// DB2 DECLARE name CONDITION FOR SQLSTATE 'value'; Sybase ASA DECLARE name
// EXCEPTION FOR SQLSTATE VALUE 'value'
bool SqlParser::ParseDeclareCondition(Token *declare, Token *name,
                                      Token *condition) {
...
bool SqlParser::ParseDeclareCursor(Token *declare, Token *name, Token *cursor) {
  ...
  if (select == NULL) {
    ...
    // In Informix, PREPARE stmt_id FROM select_string goes before DECLARE cur
    // FOR stmt_id In Teradata, cursor declared first, then PREPARE executed
    ...
    ParseSelectStatement(select, 0, SQL_SEL_CURSOR, NULL, NULL, &exp_starts,
                         &out_cols, NULL, NULL, NULL, NULL);
  ...
  if (select != NULL) {
    ...
    if (return_ == NULL) {
      ...
      if (Target(SQL_ORACLE, SQL_SQL_SERVER) && _source_app != APP_COBOL) {
        ...
        // in SQL Server, Standalone SELECT is used in place of OPEN cursor, so
        // FOR keyword is not required
        if (_target == SQL_SQL_SERVER) {
          cut_start = select;
          Token::Remove(for_);
        }
        ...
        Cut(COPY_SCOPE_PROC, COPY_CURSOR_WITH_RETURN, name, for_,
            GetLastToken());
    ...
    if (stmt_id != NULL) {
      // In Oracle declare REF CURSOR that will be used in OPEN cur FOR
      // sql_string
      if (_target == SQL_ORACLE) {
  ...
  if (cut_start == NULL && !Source(SQL_SQL_SERVER) && Target(SQL_SQL_SERVER)) {
    // In SQL Server cursor are global by default in procedures (but not in
    // functions) i.e if you call procedure multiple time you can get "already
    // exists error" if the cursor was not deallocated. We will just add LOCAL
    // option to prevent this
    if (_spl_scope == SQL_SCOPE_PROC || _spl_func_to_proc)
      APPEND(cursor, " LOCAL");

    // SQL Server takes variable values at declare time, not open cursor time,
    // so if there are any dependencies on variable we have to move the cursor
    // declaration right before OPEN cursor statements
    if (_spl_current_declaring_cursor_uses_vars)
      cut_start = declare;
  }
...
bool SqlParser::ParseDeclareHandler(Token *declare, Token *type) {
  ...
  if (Token::Compare(type, "CONTINUE", L"CONTINUE", 8) == true)
    continue_ = true;
  else if (Token::Compare(type, "EXIT", L"EXIT", 4) == true)
    exit = true;
  else if (Token::Compare(type, "UNDO", L"UNDO", 4) == true)
    undo = true;
  ...
  // Multiple comma-separated conditions can be specified NOT FOUND,
  ...
  while (true) {
    ...
    if (Token::Compare(condition, "NOT", L"NOT", 3) == true) {
      ...
      // SQLEXCEPTION
      if (Token::Compare(condition, "SQLEXCEPTION", L"SQLEXCEPTION", 12) ==
          true)
  ...
  if (_target == SQL_ORACLE) {
    ...
    if (sqlexception != NULL) {
      ...
      if (sqlstate != NULL) {
        ...
        if (_source == SQL_TERADATA) {
          ...
          if (Token::Compare(sqlstate_value, "'23505'", L"'23505'", 7) ==
              true) {
            Token::Change(sqlstate, "DUP_VAL_ON_INDEX THEN ",
                          L"DUP_VAL_ON_INDEX THEN ", 22);
    ...
    // Block will be transferred to EXCEPTION section or IF cur%NOTFOUND,
    // SQL%ROWCOUNT condition
...
bool SqlParser::ParseDeclareTable(Token *declare, Token *name, Token *table) {
  ...
  ParseCreateTableColumns(declare, name, pkcols, &id_col, &id_start, &id_inc,
                          &id_default, NULL, NULL);
  ...
  if (Target(SQL_MARIADB, SQL_MYSQL)) {
    Prepend(declare, "DROP TEMPORARY TABLE IF EXISTS ",
            L"DROP TEMPORARY TABLE IF EXISTS ", 31);
    PrependCopy(declare, name);
    Prepend(declare, ";\n", L";\n", 2);

    Token::Change(declare, "CREATE TEMPORARY TABLE", L"CREATE TEMPORARY TABLE",
                  22);
    Token::Remove(table);
  }
...
bool SqlParser::ParseDeclareGlobalTemporaryTable(Token *declare,
                                                 Token *global) {
  ...
  if (as == NULL) {
    ...
    ParseCreateTableColumns(declare, name, pkcols, NULL, NULL, NULL, NULL, NULL,
                            NULL);
  }
  // Subquery
  else
    ParseSelectStatement(select, 0, SQL_SEL_CREATE_TEMP_TABLE_AS, NULL, NULL,
                         NULL, NULL, NULL, NULL, &from_end, &where_end);
  ...
  if (_target == SQL_ORACLE) {
    ...
    // Move CREATE TEMPORARY TABLE definition before CREATE PROCEDURE, CREATE
    // FUNCTION etc.
    if (_spl_scope != 0) {
    ...
    // If SELECT is specified options must be before SELECT in Oracle, not after
    // as in DB2
...
bool SqlParser::ParseDeclareVariable(Token *declare, Token *name, Token *type,
                                     int cnt_list) {
  ...
  // In PostgreSQL DECLARE is specified once (start DECLARE section that goes
  // until BEGIN) Make sure it is not the same DECLARE keyword (when variables
  // are in a list)
    ...
    if (default_equal != NULL) {
      ...
      if (_target == SQL_ORACLE)
        ...
        if (Target(SQL_MYSQL) == true)
          ChangeWithSpacesAround(default_equal, "DEFAULT", L"DEFAULT", 7,
                                 declare);
    } else
      // SQL Server uses =
  ...
  // If multiple variables specified copy data type and DEFAULT for Oracle, SQL
  // Server, PostgreSQL
  ...
  // Remove DECLARE keyword for Oracle, declarations will be in DECLARE section
  // after IS
  if (_target == SQL_ORACLE)
    Token::Remove(declare);
  ...
  if (Target(SQL_DB2) == false &&
      (Token::Compare(name, "SQLCODE", L"SQLCODE", 7) ||
       Token::Compare(name, "SQLSTATE", L"SQLSTATE", 8))) {
    /*Token *semi */ (void)GetNextCharToken(';', L';');
...
bool SqlParser::ParseForStatement(Token *for_, int scope) {
  ...
  // DB2, Teradata do not allow to use a declared cursor, but a cursor name and
  // attributes can be assigned before the SELECT statement
  if (Source(SQL_DB2, SQL_TERADATA) == true &&
      first->Compare("SELECT", L"SELECT", 6) == false) {
  ...
  if (Token::Compare(first, "SELECT", L"SELECT", 6) == true) {
    ParseSelectStatement(first, 0, SQL_SEL_FOR, NULL, NULL, NULL, &out_cols,
                         NULL, NULL, NULL, NULL);

    // Save output columns and corresponding record variable name (DB2 allows
    // referencing output columns without qualified record name, while Oracle
    // not)
  ...
  if (select == NULL && !range_loop) {
    ...
    if (cursor_loop == true) {
      ...
      if (open != NULL &&
          Target(SQL_SQL_SERVER, SQL_MARIADB, SQL_MYSQL) == true) {
...
bool SqlParser::ParseForeachStatement(Token *foreach_, int scope) {
  ...
  // In Informix cursor name can be specified before SELECT for further
  // references WHERE CURRENT OF cur inside the loop body
  ...
  // Save FOREACH number in order of apperance since it can be changed by
  // recursive calls
  ...
  // Informix uses INTO in SELECT, so FOREACH fetches into existing local
  // variables
  ParseSelectStatement(select, 0, SQL_SEL_FOREACH, NULL, NULL, NULL, &out_cols,
                       &into_cols, NULL, NULL, NULL);

  Token *select_end = GetLastToken();

  // Set frontier although there is not start keyword, but the block terminates
  // with END FOREACH;
  bool frontier = true;
  ...
  Token *end = GetNextWordToken("END", L"END", 3);
  Token *foreach2 =
      (end != NULL) ? GetNextWordToken("FOREACH", L"FOREACH", 7) : NULL;

  // Semicolon is optional in Informix, but required for Oracle, PostgreSQL
  Token *semi = GetNextCharToken(';', L';');

  if (foreach2 != NULL && semi == NULL &&
      Target(SQL_ORACLE, SQL_POSTGRESQL) == true)
  ...
  if (Target(SQL_ORACLE, SQL_POSTGRESQL) == true) {
    ...
    if (_target == SQL_ORACLE) {
      ...
      // In Oracle values fetched into record variable, while in Informix into
      // local variables
      for (ListwItem *i = into_cols.GetFirst(); i != NULL; i = i->next) {
      ...
      // For PostgreSQL a list of variable can be used instead of record
      // variable
      if (_target == SQL_POSTGRESQL) {
...
bool SqlParser::ParseIfStatement(Token *if_, int scope) {
  ...
  // Oracle, DB2, MySQL, PostgreSQL, Informix, Sybase ASA, Sybase ADS IF THEN
  // ELSEIF/ELSIF ELSE END IF;
  else {
    ...
    while (true) {
      ...
      if (_target_app == APP_JAVA)
        ...
        // For SQL Server use BEGIN unless true block is defined as THEN BEGIN
        // ... END ELSE ...
        if (_target == SQL_SQL_SERVER) {
      ...
      if (_target_app == APP_JAVA)
      ...
      else if (_target == SQL_ORACLE && elseif != NULL)
        ...
        } else if (Target(SQL_DB2, SQL_MYSQL) == true && elsif != NULL)
    ...
    if (end != NULL || endif != NULL) {
      ...
      if (_target_app == APP_JAVA) {
        ...
        if (_target == SQL_SQL_SERVER) {
          ...
          // Remove last END as well if BEGIN END is also used around true/false
          // blocks
          ...
          // Semicolon is optional in Informix, but required for Oracle,
          // PostgreSQL
...
bool SqlParser::ParseWhileStatement(Token *while_, int scope) {
  ...
  Token *begin =
      (loop == NULL && do_ == NULL) ? GetNext("BEGIN", L"BEGIN", 5) : NULL;
...
bool SqlParser::ParseWhileStatementEnd() {
  ...
  Token *while_ = (loop == NULL && !Source(SQL_SQL_SERVER, SQL_SYBASE))
                      ? GetNextWordToken("WHILE", L"WHILE", 5)
                      : NULL;
  ...
  if (_target == SQL_SQL_SERVER) {
    ...
    if (Target(SQL_ORACLE, SQL_POSTGRESQL) == true) {
      ...
      else if (loop == NULL && Source(SQL_ORACLE, SQL_DB2, SQL_INFORMIX,
                                      SQL_TERADATA, SQL_MYSQL) == false)
      ...
      if (Target(SQL_DB2, SQL_MYSQL, SQL_MARIADB, SQL_TERADATA)) {
        if (loop != NULL)
          Token::Change(loop, "WHILE", L"WHILE", 5);
                 ...
                 Source(SQL_ORACLE, SQL_DB2, SQL_INFORMIX, SQL_TERADATA,
                        SQL_MYSQL, SQL_MARIADB) == false)
...
bool SqlParser::ParseInsertStatement(Token *insert) {
  ...
  if (values != NULL) {
    ...
    while (true) {
      ...
      while (true) {
        ...
        if (exp->type == TOKEN_IDENT &&
            exp->subtype == TOKEN_SUB_IDENT_SEQNEXTVAL) {
  ...
  // SELECT clause
    ...
    ParseSelectStatement(select, 0, SQL_SEL_INSERT, NULL, NULL, NULL, NULL,
                         NULL, NULL, NULL, NULL);
...
bool SqlParser::ParseLetStatement(Token *let) {
  ...
  // Check for pattern LET var = DBINFO('sqlca.sqlerrd1') that gets the last
  // SERIAL value
...
bool SqlParser::ParseLeaveStatement(Token *leave) {
  ...
  } else if (_target == SQL_SQL_SERVER && label == NULL)
...
bool SqlParser::ParseGetStatement(Token *get) {
  ...
  if (type->Compare("ROW_COUNT", L"ROW_COUNT", 9) == true) {
    ...
    if (type->Compare("MESSAGE_TEXT", L"MESSAGE_TEXT", 12) == true) {
      ...
      if (type->Compare("DB2_RETURN_STATUS", L"DB2_RETURN_STATUS", 17) ==
          true) {
...
bool SqlParser::ParseGrantStatement(Token *grant) {
  ...
  if (priv->Compare("EXECUTE", L"EXECUTE", 7) == true) {
    ...
    if (open != NULL) {
      // Parameter mode (IN, OUT) and name are optional, datatype is required in
...
bool SqlParser::ParseMergeStatement(Token *merge) {
  ...
  if (TOKEN_CMPC(next, '(')) {
    ParseSelectStatement(next, 0, SQL_SEL_MERGE, NULL, NULL, NULL, NULL, NULL,
                         NULL, NULL, NULL);
  ...
  while (true) {
    ...
    if (TOKEN_CMP(operation, "UPDATE"))
      ...
      // Note INSERT clause slightly differs from standard INSERT statment, so
      // we parse it individually
...
bool SqlParser::ParseExceptionBlock(Token *exception) {
  ...
  while (true) {
    ...
    if (_target_app == APP_JAVA) {
      ...
      Token::Change(when, "DECLARE EXIT HANDLER FOR",
                    L"DECLARE EXIT HANDLER FOR", 24);
      Token::Change(then, "BEGIN", L"BEGIN", 5);
    ...
    if (declare != NULL) {
      ...
      if (begin != NULL) {
        // For MySQL declaration put inside BEGIN-END and THEN was converted to
        // BEGIN keyword
        if (Target(SQL_MARIADB, SQL_MYSQL))
          Token::Remove(begin);
      }
...
bool SqlParser::ParseOpenStatement(Token *open) {
  ...
  if (for_ != NULL) {
    ...
    // FOR SELECT
    if (select != NULL) {
      ParseSelectStatement(select, 0, SQL_SEL_OPEN, NULL, NULL, NULL, NULL,
                           NULL, NULL, NULL, NULL);
      ...
      if (_target == SQL_SQL_SERVER)
        ...
        if (_target == SQL_NETEZZA) {
          ...
          // LOOP statement must follow and now it is part of FOR IN EXECUTE
          // statement
          if (semi != NULL)
            Token::Remove(semi);
        }
    ...
    // FOR variable
    {
      // Variable or string containing SQL statement; string literal can be part
      // of concatenation expression
      ...
      if (_target == SQL_SQL_SERVER) {
        ...
        if (exp == exp_end) {
          ...
          // Variable; EXECUTE (@variable) in SQL Server, must be enclosed with
          // () unless stored procedure name
          else {
            TOKEN_CHANGE(open, "EXECUTE");
  ...
  // FOR is not specified
    ...
    // Cursor can be open for a prepared statement in Informix, find its
    // declaration
    ...
    if (_target_app == APP_JAVA) {
      ...
      if (_target == SQL_NETEZZA) {
        ...
        // LOOP statement must follow and now it is part of FOR IN SELECT
        // statement
        if (semi != NULL)
          Token::Remove(semi);
      }
...
bool SqlParser::ParsePromptStatement(Token *prompt) {
  ...
  while (cur) {
    ...
    // Skip spaces
    while (prev && (Token::Compare(prev, ' ', L' ') == true ||
                    Token::Compare(prev, '\t', L'\t') == true))
      prev = prev->prev;

    if (Token::Compare(prev, '\r', L'\r') == true ||
        Token::Compare(prev, '\n', L'\n') == true) {
      PushBack(cur);
      break;
    }
...
bool SqlParser::ParseRaiseStatement(Token *raise) {
  ...
  if (notice != NULL) {
    ...
    if (_target == SQL_ORACLE) {
      Token::Change(raise, "DBMS_OUTPUT.PUT_LINE(", L"DBMS_OUTPUT.PUT_LINE(",
                    21);
      Token::Remove(notice);
    ...
    if (exception != NULL) {
      ...
      if (_target == SQL_ORACLE) {
        Token::Change(raise, "RAISE_APPLICATION_ERROR(",
                      L"RAISE_APPLICATION_ERROR(", 24);
        Token::Remove(exception);
...
bool SqlParser::ParseReturnStatement(Token *return_) {
  ...
  // In SQL Server, Sybase ASE RETURN can be without expression and semicolon,
  // check that it is not followed by END
  if (Source(SQL_SQL_SERVER, SQL_SYBASE)) {
    Token *end = TOKEN_GETNEXTW("END");
  ...
  if (_spl_return_with_resume == 0) {
    ...
    if (_spl_returning_datatypes.GetCount() > 1) {
      ...
      if (_spl_scope == SQL_SCOPE_PROC && num > 0 &&
          Target(SQL_ORACLE) == true) {
    ...
    // Single value is returned
      ...
      // SQL Server procedure can return the status code (integer value), Oracle
      // does not allow this
      if (_spl_scope == SQL_SCOPE_PROC && num > 0 &&
          Target(SQL_ORACLE) == true) {
...
bool SqlParser::ParseRollbackStatement(Token *rollback) {
  ...
  if (_spl_scope == SQL_SCOPE_PROC &&
      Target(SQL_POSTGRESQL, SQL_GREENPLUM) == true)
...
bool SqlParser::ParseSetStatement(Token *set) {
  ...
  if (Token::Compare(set, "SET", L"SET", 3) == true &&
      ParseSetOptions(set) == true)
    return true;

  STATS_DECL
  PL_STMS_STATS(set);

  // MySQL allows to have SELECT inside an expression as a parameter of a
  // function SET var = IFNULL((SELECT ...), 0);
  bool select_deep_inside = false;

  // Comma-separated list of assignments can be specified
  while (true) {
    // SET (var, var2, ...) = (value, value2, ...) list or (SELECT c1, c2, ...)
    // can be specified in DB2 LET var, var2, ... = value, value2, ... or
    // (SELECT c1, c2, ...) in Informix
    Token *open = GetNextCharToken('(', L'(');
    ...
    // Add @ for parameter names for SQL Server and Sybase (variables already
    // converted)
    if (Target(SQL_SQL_SERVER, SQL_SYBASE) == true)
      ConvertToTsqlVariable(var);

    // In DB2 trigger, column can be assigned without NEW correlation name, make
    // sure it is not an existing local variable
    if (_source == SQL_DB2 && _spl_scope == SQL_SCOPE_TRIGGER &&
        GetVariable(var) == NULL)
    ...
    while (true) {
      ...
      // Add @ for parameter names for SQL Server and Sybase (variable is
      // already declared)
      if (Target(SQL_SQL_SERVER, SQL_SYBASE) == true)
    ...
    if (select == NULL) {
      // SET var = (var2 + var3)/3 is possible so check the number of
      // assignments
      if (open2 != NULL && num == 1) {
    ...
    // SELECT statement is specified
      ...
      ParseSelectStatement(select, 0, SQL_SEL_EXP, NULL, &list_end, &sel_exp,
    ...
    // In SQL Server convert to separate SET statements, so braces can be
    // removed
    if (_target == SQL_SQL_SERVER) {
      if (select != NULL) {
        Token::Remove(set);
      ...
      if (Target(SQL_ORACLE, SQL_POSTGRESQL) == true) {
        ...
        // There is a SELECT deep inside assignment expression, it is not
        // allowed in Oracle Convert to SELECT exp INTO var FROM dual
        else {
...
bool SqlParser::ParseShowStatement(Token *show) {
  ...
  if (option->Compare("ERRORS", L"ERRORS", 6) == true ||
      option->Compare("ERR", L"ERR", 3) == true) {
    if (_target != SQL_ORACLE)
...
bool SqlParser::ParseSignalStatement(Token *signal) {
  ...
  if (_target == SQL_SQL_SERVER) {
    ...
    if (_target == SQL_ORACLE) {
      Token::Change(signal, "RAISE_APPLICATION_ERROR",
                    L"RAISE_APPLICATION_ERROR", 23);
...
bool SqlParser::ParseSystemStatement(Token *system) {
  ...
  if (_target == SQL_ORACLE) {
    Token::Change(system, "DBMS_SCHEDULER.CREATE_JOB(",
                  L"DBMS_SCHEDULER.CREATE_JOB(", 26);

    AppendNoFormat(system, "job_name => '", L"job_name => '", 13);
    AppendCopy(system, _spl_name);
    AppendNoFormat(system, "_job', job_type => 'EXECUTABLE', job_action =>",
                   L"_job', job_type => 'EXECUTABLE', job_action =>", 46);
...
bool SqlParser::ParseTruncateStatement(Token *truncate, int scope) {
  ...
  // In PostgreSQL can be executed inside function body, Oracle requires EXECUTE
  // IMMEDIATE
  if (_target == SQL_ORACLE &&
      (scope == SQL_SCOPE_PROC || scope == SQL_SCOPE_FUNC)) {
...
bool SqlParser::ParseUpdateStatement(Token *update) {
  ...
  if (Source(SQL_SQL_SERVER, SQL_SYBASE) &&
      ParseSqlServerUpdateStatement(update)) {
    STMS_STATS(update);
    return true;
  }
  ...
  if (TOKEN_CMP(next, "FROM") == true) {
    ...
    while (true) {
      ...
      if (open != NULL) {
        ...
        if (sel != NULL)
          ParseSelectStatement(sel, 0, SQL_SEL_UPDATE_FROM, NULL, NULL, NULL,
                               NULL, NULL, NULL, NULL, NULL);
  ...
  while (true) {
    ...
    // List of single column assignments or (c1, c2, ...) = (v1, v2, ...) can be
    // specified
    Token *open = GetNextCharToken('(', L'(');
    ...
    if (open2 != NULL) {
      ...
      if (select != NULL)
        ParseSelectStatement(select, 0, SQL_SEL_UPDATE_SET, NULL, NULL, NULL,
                             NULL, NULL, NULL, NULL, NULL);
    }
...
bool SqlParser::ParseUseStatement(Token *use) {
  ...
  if (_target == SQL_ORACLE)
    Token::Change(use, "ALTER SESSION SET CURRENT_SCHEMA =",
                  L"ALTER SESSION SET CURRENT_SCHEMA =", 34);
...
bool SqlParser::ParseProcedureParameters(Token *proc_name, int *count,
                                         Token **end) {
  // SQL Server and Sybase have optional parentheses around parameters
  // Informix, Teradata require () if there are no parameters, Oracle does not
  // allow empty ()
  Token *open = GetNextCharToken('(', L'(');
  ...
  if (open == NULL) {
    ...
    // AS in Oracle, SQL Server, Sybase ASE; IS in Oracle, BEGIN in DB2, MySQL,
    // Teradata
    if (Token::Compare(next, "AS", L"AS", 2) == true ||
        Token::Compare(next, "IS", L"IS", 2) == true ||
  ...
  while (true) {
    ...
    // AS or IS keyword marks the end in SQL Server, Sybase when there are ()
    // around parameters
    ...
    // Oracle refcursor
    if (_source == SQL_ORACLE && Token::Compare(data_type, "SYS_REFCURSOR",
                                                L"SYS_REFCURSOR", 13) == true) {
      sys_refcursor = true;
    ...
    // Netezza does not allow specifying parameter name and type (only IN
    // assumed), only data type
    if (_target == SQL_NETEZZA) {
    ...
    // In SQL Server OUT or OUTPUT go after the default; Sybase ASE, Sybase ADS
    // use OUTPUT
    ...
    if (_target_app == APP_JAVA) {
      ...
      if (_source == SQL_ORACLE && (in != NULL || out != NULL) &&
        ...
        if (Source(SQL_DB2, SQL_MYSQL, SQL_TERADATA) == true &&
            (in != NULL || out != NULL || inout != NULL)) {
          // In MySQL OUT/INOUT go before name, in SQL Server after default
          if (_target == SQL_SQL_SERVER) {
            // SQL Server does not support INOUT, always use OUT
  ...
  // If parameters are not enclosed with (), add them for all dataabases except
...
bool SqlParser::ParseProcedureBody(Token *create, Token *procedure, Token *name,
                                   Token *as, int *result_sets) {
  Token *body_start = GetLastToken();

  // In DB2, Teradata, MySQL block can start with a label
  ParseLabelDeclaration(NULL, true);

  // In SQL Server and Sybase procedural block can start without BEGIN; In
  // Informix BEGIN not used
  Token *begin = GetNextWordToken("BEGIN", L"BEGIN", 5);
  ...
  if (_source == SQL_ORACLE && begin == NULL) {
    ...
    // In SQL Server, MySQL declaration is after BEGIN, append BEGIN after AS,
    // and remove BEGIN
    if (Target(SQL_SQL_SERVER, SQL_MARIADB, SQL_MYSQL) == true &&
        declare == true && begin != NULL) {
  ...
  if (as != NULL) {
    ...
    if (_source != SQL_POSTGRESQL && _target == SQL_POSTGRESQL)
      ...
      if (_source != SQL_NETEZZA && _target == SQL_NETEZZA)
        Token::Change(
            as,
            "RETURNS INT EXECUTE AS CALLER\nLANGUAGE NZPLSQL AS\nBEGIN_PROC",
            L"RETURNS INT EXECUTE AS CALLER\nLANGUAGE NZPLSQL AS\nBEGIN_PROC",
            60);
  }
  ...
  if (Target(SQL_MARIADB, SQL_MYSQL)) {
    ...
    // If source SQL Server then standalone DECLARE statements can go in block,
    // and if target is Oracle BEGIN must be after all declarations, not after
    // AS
    if (begin == NULL &&
        Target(SQL_ORACLE, SQL_SQL_SERVER, SQL_SYBASE) == false)
      Append(as, "\nBEGIN", L"\nBEGIN", 6);
  ...
  // If BEGIN END is not specified in SQL Server and Sybase ASE, body is
  // terminated with GO, it acts as the frontier Informix does not use BEGIN
  // keyword, but body always terminated with END PROCEDURE, set frontier
  ...
  if (Source(SQL_SQL_SERVER, SQL_SYBASE) == true &&
      Target(SQL_SQL_SERVER, SQL_SYBASE) == false) {
    // In SQL Server GO can be used to terminate procedure block without using
    // BEGIN END
    ...
    // Informix does not use any special delimiter after END PROCEDURE; Teradata
    // also after END;
    if (Source(SQL_INFORMIX, SQL_TERADATA) == true) {
      if (_target == SQL_ORACLE)
        Append(semi, "\n/", L"\n/", 2);
      else
        // In SQL Server procedure block can be within AS and GO, no BEGIN END
        // required
        if (_target == SQL_SQL_SERVER) {
  ...
  // For target Oracle and PostgreSQL, if there declarations in the body move
  // BEGIN after last DECLARE
  if (!Source(SQL_ORACLE, SQL_POSTGRESQL) &&
      Target(SQL_ORACLE, SQL_POSTGRESQL) && _target_app != APP_JAVA)
    OracleMoveBeginAfterDeclare(create, as, begin, body_start);

  // Transform DB2, MySQL, Teradata EXIT HANDLERs, Informix ON EXCEPTION to
  // Oracle and PostgreSQL EXCEPTION
  if (Source(SQL_DB2, SQL_MYSQL, SQL_INFORMIX, SQL_TERADATA) &&
      Target(SQL_ORACLE, SQL_POSTGRESQL))
    OracleExitHandlersToException(end);

  // Add label for target MySQL, MariaDB if there is a RETURN inside the
  // procedure (converted to LEAVE lbl)
...
bool SqlParser::ParseSplEndName(Token *name, Token * /*end*/) {
  ...
  // Compare with procedure name, here it can be specified without schema name,
  // or one can be quoted another unquoted
  if (CompareIdentifiersExistingParts(name, next) == true) {
    ...
    if (_spl_outer_label != NULL &&
        Token::Compare(_spl_outer_label, next, _spl_outer_label->len - 1) ==
            true) {
      // Remove for other databases
      if (_target != SQL_DB2)
        Token::Remove(next);
  ...
  while (true) {
    ...
    if (next->Compare("DETERMINISTIC", L"DETERMINISTIC", 13) == true) {
      ...
      if (next->Compare("NOT", L"NOT", 3) == true) {
        Token *deterministic =
            GetNextWordToken("DETERMINISTIC", L"DETERMINISTIC", 13);
            ...
            if (next->Compare("NO", L"NO", 2) == true) {
              ...
              if (next->Compare("EXTERNAL", L"EXTERNAL", 8) == true) {
                ...
                if (next->Compare("READS", L"READS", 5) == true) {
                  Token *sql = GetNextWordToken("SQL", L"SQL", 3);
                  Token *data = (sql != NULL)
                                    ? GetNextWordToken("DATA", L"DATA", 4)
                                    : NULL;
                        ...
                        if (next->Compare("CALLED", L"CALLED", 6) == true) {
                          ...
                          if (next->Compare("RETURNS", L"RETURNS", 7) == true) {
                            ...
                            if (null != NULL) {
                              Token *on = GetNextWordToken("ON", L"ON", 2);
                              Token *null2 =
                                  GetNextWordToken(on, "NULL", L"NULL", 4);
                              Token *input =
                                  GetNextWordToken(null2, "INPUT", L"INPUT", 5);
                              ;
                            ...
                            if (next->Compare("INHERIT", L"INHERIT", 7) ==
                                true) {
                              Token *special =
                                  GetNextWordToken("SPECIAL", L"SPECIAL", 7);
                              Token *registers = NULL;

                              if (special != NULL)
                                registers = GetNextWordToken("REGISTERS",
                                                             L"REGISTERS", 9);
                              ...
                              if (next->Compare("SQL", L"SQL", 3) == true) {
                                Token *security = GetNextWordToken(
                                    "SECURITY", L"SECURITY", 8);
                                Token *type =
                                    (security != NULL) ? GetNextToken() : NULL;
                                ...
                                if (_target == SQL_ORACLE && security != NULL) {
                                  ...
                                  if (Token::Compare(type, "INVOKER",
                                                     L"INVOKER", 7) == true)
                                    Token::Change(type, "CURRENT_USER",
                                                  L"CURRENT_USER", 12);
                                }
...
bool SqlParser::ParseProcedureOptions(Token *create) {
        ...
        if (next->Compare("LANGUAGE", L"LANGUAGE", 8) == true) {
          ...
          if (next->Compare("CONTAINS", L"CONTAINS", 8) == true) {
            ...
            if (next->Compare("NO", L"NO", 2) == true) {
              ...
              // EXTERNAL ACTION in DB2; EXTERNAL NAME in DB2 (C/Java etc.
              // procedures)
              if (next->Compare("EXTERNAL", L"EXTERNAL", 8) == true) {
                Token *action = GetNextWordToken("ACTION", L"ACTION", 6);
                Token *name = NULL;
                  ...
                  if (next->Compare("MODIFIES", L"MODIFIES", 8) == true) {
                    ...
                    if (next->Compare("COMMENT", L"COMMENT", 7) == true) {
                      ...
                      if (next->Compare("SPECIFIC", L"SPECIFIC", 8) == true) {
                        ...
                        if (Source(SQL_DB2, SQL_TERADATA) == true &&
                            (next->Compare("DYNAMIC", L"DYNAMIC", 7) == true ||
                             next->Compare("RESULT", L"RESULT", 6) == true)) {
                          Token *result = NULL;
                          Token *sets = NULL;
                          Token *num = NULL;
                          ...
                          if (_source == SQL_SYBASE_ASA &&
                            ...
                            // Variable comma separated list of columns and data
                            // types
                            while (open != NULL) {
                            ...
                            if (next->Compare("CALLED", L"CALLED", 6) == true) {
                              ...
                              if (next->Compare("INHERIT", L"INHERIT", 7) ==
                                ...
                                if (next->Compare("PARAMETER", L"PARAMETER",
                                                  9) == true) {
                                  Token *style =
                                      GetNextWordToken("STYLE", L"STYLE", 5);

                                  // Style DB2GENERAL, DB2SQL, GENERAL, GENERAL
                                  // WITH NULLS, JAVA, SQL
                                  ...
                                  if (next->Compare("FENCED", L"FENCED", 6) ==
                                      true) {
                                    // Optional NOT THREADSAFE, THREADSAFE
                                    /*Token *not_ */ (void)GetNextWordToken(
                                        "NOT", L"NOT", 3);

                                    Token *fenced_type = GetNextWordToken(
                                        "THREADSAFE", L"THREADSAFE", 10);
                                    ...
                                    if (next->Compare("DBINFO", L"DBINFO", 6) ==
                                        true) {
                                      if (_target == SQL_ORACLE)
                                        Token::Remove(next);
                                      ...
                                      if (next->Compare("COLLID", L"COLLID",
                                                        6) == true) {
                                        Token *name = GetNextToken();
                                        ...
                                        // WLM ENVIRONMENT name in DB2 (workload
                                        // manager)
                                        if (next->Compare("WLM", L"WLM", 3) ==
                                            true) {
                                          Token *environment = GetNextWordToken(
                                              "ENVIRONMENT", L"ENVIRONMENT",
                                              11);
                                          Token *name =
                                              GetNextToken(environment);
                                          ...
                                          if (next->Compare("RUN", L"RUN", 3) ==
                                              true) {
                                            Token *options = GetNextWordToken(
                                                "OPTIONS", L"OPTIONS", 7);
                                            ...
                                            // } in MySQL
                                            if (next->Compare("SQL", L"SQL",
                                                              3) == true) {
                                              Token *security =
                                                  GetNextWordToken("SECURITY",
                                                                   L"SECURITY",
                                                                   8);
                                              Token *type = (security != NULL)
                                                                ? GetNextToken()
                                                                : NULL;
                                              ...
                                              // WITH EXECUTE AS { OWNER | name
                                              // } or WITH RECOMPILE in SQL
                                              // Server
                                              if (next->Compare("WITH", L"WITH",
                                                                4) == true) {
                                                Token *execute = GetNext(
                                                    "EXECUTE", L"EXECUTE", 7);
                                                Token *as =
                                                    (execute != NULL)
                                                        ? GetNext("AS", L"AS",
                                                ...
                                                // RETURNS or RETURNING in
                                                // Informix
                                                if (next->Compare("RETURNS",
                                                                  L"RETURNS",
                                                                  7) == true ||
                                                    next->Compare("RETURNING",
                                                                  L"RETURNING",
                                                                  9) == true) {
                                                  _spl_returns = next;
  ...
  // PostgreSQL requires RETURNS VOID (set if there is no any other RETURN
  // keyword as in Informix i.e)
  if (_spl_returns == NULL && _target == SQL_POSTGRESQL) {
    ...
    // If REFCURSOR is returned then this must be specified in RETURNS clause,
    // not VOID
    if (_spl_refcursor_params_num > 0) {
...
bool SqlParser::ParseFunctionBody(Token *create, Token *function, Token *name,
                                  Token *as) {
  Token *body_start = GetLastToken();
  ...
  // Oracle, SQL Server require BEGIN for functions; DB2 and MySQL not (if
  // single statement); In Informix BEGIN not used
  Token *begin = GetNextWordToken("BEGIN", L"BEGIN", 5);

  // In Oracle and PostgreSQL, variable declaration goes before BEGIN
  if (Source(SQL_ORACLE, SQL_POSTGRESQL) == true && begin == NULL) {
    // In PostgreSQL DECLARE keyword goes before variable block, but not in
    // Oracle
  ...
  // DB2, MySQL, Informix do not use AS; in SQL Server, Sybase ASA it is
  // optional
  if (as == NULL) {
    // AS required for PostgreSQL
    if (_target == SQL_POSTGRESQL) {
  ...
  // MySQL function may not have BEGIN, add it for other databases including SQL
  // Server; for Oracle added in OracleMoveBeginAfterDeclare
  if (begin == NULL &&
      Target(SQL_MARIADB, SQL_MYSQL, SQL_ORACLE, SQL_POSTGRESQL) == false)
    Append(Nvl(as, GetLastToken()), "\nBEGIN", L"\nBEGIN", 6);

  bool frontier = (begin != NULL) ? true : false;

  // Informix does not use BEGIN keyword, but body always terminated with END
  // FUNCTION, set frontier
  if (_source == SQL_INFORMIX)
    frontier = true;
  ...
  // SQL Server require last statement in function to be RETURN even if the
  // previous is IF RETURN n ELSE RETURN k
  if (_target == SQL_SQL_SERVER && !_spl_func_to_proc &&
      !TOKEN_CMP(_spl_last_stmt, "RETURN"))
  ...
  if (_spl_monday_1 && _target == SQL_SQL_SERVER)
    APPEND_NOFMT(begin, "\n/* Make sure SET DATEFIRST 1; is set for session. "
                        "Use SELECT @@DATEFIRST; to see the current value. */");
  ...
  if (begin == NULL && end == NULL &&
      Target(SQL_SQL_SERVER, SQL_ORACLE) == true)
    Append(GetLastToken(), "\nEND;", L"END;", 5);

  // In Oracle ; must follow END name, in SQL Server, DB2, MySQL ; is optional
  // after END
  Token *semi = GetNextCharToken(';', L';');
  ...
  // For target Oracle, if there declarations in the body move BEGIN after last
  // DECLARE
  if (Target(SQL_ORACLE, SQL_POSTGRESQL) == true)
    OracleMoveBeginAfterDeclare(create, as, begin, body_start);
