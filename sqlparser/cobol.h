...
// #define CBL_GETNEXTW(string)     _sqlparser->GetNext(string, L##string,
// sizeof(string) - 1)
...
public:
  ...
  bool ParseVariableSection(int section_scope, Token *parent_record = NULL,
                            ListWM *record_items = NULL);
  bool ParseRecordDeclaration(int section_scope, Token *parent_record,
                              Token *num, Token *name, Token *dot);
  bool ParseVariableDeclaration(int section_scope, Token *num, Token *name,
                                ListWM *record_items = NULL);
  bool ParsePicFormat(Token *pic, Token **type = NULL, Token **len = NULL,
                      Token **var_end = NULL);
  bool ParseDb2RecordDeclaration(int section_scope, Token *num, Token *name,
                                 Token *dot, Token *end_dot,
                                 ListWM *record_items);
  ...
  void SetTypes(short s, short t, short ta) {
    _source = s;
    _target = t;
    _target_app = ta;
  }
  ...
  Token *GetNext(const char ch, const wchar_t wch) {
    return _sqlparser->GetNext(ch, wch);
  }
  Token *GetNext(const char *str, const wchar_t *wstr, size_t len) {
    return _sqlparser->GetNext(str, wstr, len);
  }
  ...
  Token *LookNext(const char *str, const wchar_t *wstr, size_t len) {
    return _sqlparser->LookNext(str, wstr, len);
  }
  ...
  void Append(Token *token, TokenStr *str, Token *format = NULL) {
    return _sqlparser->Append(token, str, format);
  }
  Token *AppendNoFormat(Token *token, const char *str, const wchar_t *wstr,
                        size_t len) {
    return _sqlparser->AppendNoFormat(token, str, wstr, len);
  }
  void AppendFirstNoFormat(Token *token, const char *str, const wchar_t *wstr,
                           size_t len) {
    _sqlparser->AppendFirstNoFormat(token, str, wstr, len);
  }
  ...
  Token *PrependNoFormat(Token *token, const char *str, const wchar_t *wstr,
                         size_t len) {
    return _sqlparser->PrependNoFormat(token, str, wstr, len);
  }
};
