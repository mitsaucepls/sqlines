...
#define BOOK_CTC_ALL_END                                                       \
  4 // CREATE TABLE end of all column definitions, pointer to ) i.e. before
...
#define COPY_SCOPE_PROC                                                        \
  1 // Procedure, function, trigger or outer anonymous block
...
#define TOKEN_GETNEXTWP(prev, string)                                          \
  GetNext(prev, string, L##string, sizeof(string) - 1)
...
#define TOKEN_CMP(token, string)                                               \
  Token::Compare(token, string, L##string, sizeof(string) - 1)
#define TOKEN_CMP_PART0(token, string)                                         \
  Token::Compare(token, string, L##string, 0, sizeof(string) - 1)
#define TOKEN_CHANGE(token, string)                                            \
  Token::Change(token, string, L##string, sizeof(string) - 1)
#define TOKEN_CHANGE_FMT(token, string, format)                                \
  Token::Change(token, string, L##string, sizeof(string) - 1, format)
#define TOKEN_CHANGE_NOFMT(token, string)                                      \
  Token::ChangeNoFormat(token, string, L##string, sizeof(string) - 1)

#define APPEND(token, string)                                                  \
  Append(token, string, L##string, sizeof(string) - 1)
#define APPEND_FMT(token, string, format)                                      \
  Append(token, string, L##string, sizeof(string) - 1, format)
#define APPEND_FIRST_FMT(token, string, format)                                \
  AppendFirst(token, string, L##string, sizeof(string) - 1, format)
#define APPEND_FIRST_NOFMT(token, string)                                      \
  AppendFirstNoFormat(token, string, L##string, sizeof(string) - 1)
#define APPEND_NOFMT(token, string)                                            \
  AppendNoFormat(token, string, L##string, sizeof(string) - 1)
#define APPENDS(token, tokenstr) Append(token, tokenstr)

#define APPENDSTR(tokenstr, string)                                            \
  tokenstr.Append(string, L##string, sizeof(string) - 1)

#define PREPEND(token, string)                                                 \
  Prepend(token, string, L##string, sizeof(string) - 1)
#define PREPEND_FMT(token, string, format)                                     \
  Prepend(token, string, L##string, sizeof(string) - 1, format)
#define PREPEND_NOFMT(token, string)                                           \
  PrependNoFormat(token, string, L##string, sizeof(string) - 1)

#define LOOKNEXT(string) LookNext(string, L##string, sizeof(string) - 1)

#define COMMENT(string, start, end)                                            \
  Comment(string, L##string, sizeof(string) - 1, start, end)
...
class SqlParser {
  ...
  Token *_spl_name;
  // Start token of procedure, function or trigger (usually it is CREATE
  // keyword)
  Token *_spl_start;
  ...
  // Standalone BEGIN levels in the current procedure (each BEGIN entry is
  ...
  // Last token of last declared variable (not cursor) in the outer PL/SQL
  // DECLARE block
  ...
  // Result set generated cursor names (for standalone SELECT returning a result
  // set)
  ...
  // Temporary table replacing their content each time they are declared (DB2
  // WITH REPLACE)
  ...
  // Loops level in the current procedure (each loop entry is cleared on loop
  // exit)
  ListW _spl_loops;
  ...
  // Variable is added to hold dynamic SQL (in SQL Server, it is @sql for
  // sp_executesql that does not allow expressions)
...
public:
  ...
  void SetTypes(short source, short target) {
    _source = source;
    _target = target;
  }
  ...
  // Perform conversion
  int Convert(const char *input, int size, const char **output, int *out_size,
              int *lines);
  ...
  Token *GetNextSpecialCharToken(Token *prev, const char *str,
                                 const wchar_t *wstr, size_t len);
  Token *GetNextSpecialCharToken(const char *str, const wchar_t *wstr,
                                 size_t len);
  Token *GetNextWordToken(const char *str, const wchar_t *wstr, size_t len);
  Token *GetNextWordToken(Token *prev, const char *str, const wchar_t *wstr,
                          size_t len);
  ...
  Token *GetNext(const char *str, const wchar_t *wstr, size_t len) {
    return GetNextWordToken(str, wstr, len);
  }
  Token *GetNext(Token *prev, const char *str, const wchar_t *wstr,
                 size_t len) {
    return GetNextWordToken(prev, str, wstr, len);
  }
  Token *GetNext(const char ch, const wchar_t wch) {
    return GetNextCharToken(ch, wch);
  }
  Token *GetNext(Token *prev, const char ch, const wchar_t wch) {
    return GetNextCharToken(prev, ch, wch);
  }
  ...
  Token *Append(Token *token, const char *str, const wchar_t *wstr, size_t len,
                Token *format = NULL);
  Token *AppendWithSpaceAfter(Token *token, const char *str,
                              const wchar_t *wstr, size_t len,
                              Token *format = NULL);
  void Append(Token *token, int value);
  void Append(Token *token, TokenStr *str, Token *format = NULL);
  void AppendFirst(Token *token, const char *str, const wchar_t *wstr,
                   size_t len, Token *format = NULL);
  Token *AppendNoFormat(Token *token, const char *str, const wchar_t *wstr,
                        size_t len);
  void AppendNoFormat(Token *token, TokenStr *str);
  void AppendFirstNoFormat(Token *token, const char *str, const wchar_t *wstr,
                           size_t len);
  Token *AppendCopy(Token *token, Token *append);
  Token *AppendCopy(Token *token, Token *first, Token *last,
                    bool append_removed = true);
  void AppendSpaceCopy(Token *token, Token *append);
  void AppendSpaceCopy(Token *token, Token *first, Token *last,
                       bool append_removed = true);
  void AppendNewlineCopy(Token *token, Token *first, Token *last,
                         size_t newlines = 1, bool append_removed = true);
  void Append(Token *token, Token *append);

  // Prepend the token with the specified value
  Token *Prepend(Token *token, const char *str, const wchar_t *wstr, size_t len,
                 Token *format = NULL);
  Token *Prepend(Token *token, TokenStr *str, Token *format = NULL);
  Token *PrependNoFormat(Token *token, const char *str, const wchar_t *wstr,
                         size_t len);
  Token *PrependNoFormat(Token *token, TokenStr *str);
  void PrependFirstNoFormat(Token *token, const char *str, const wchar_t *wstr,
                            size_t len);
  Token *PrependCopy(Token *token, Token *prepend);
  Token *PrependCopy(Token *token, Token *first, Token *last,
                     bool prepend_removed = true);
  void PrependSpaceCopy(Token *token, Token *first, Token *last = NULL,
                        bool prepend_removed = true);
  void Prepend(Token *token, Token *prepend);

  void ChangeWithSpacesAround(Token *token, const char *new_str,
                              const wchar_t *new_wstr, size_t len,
                              Token *format = NULL);

  // Check for the source and target type
  bool Source(short s1, short s2 = -1, short s3 = -1, short s4 = -1,
              short s5 = -1, short s6 = -1);
  bool Target(short t1, short t2 = -1, short t3 = -1, short t4 = -1,
              short t5 = -1, short t6 = -1);
  ...
  bool ParseBooleanExpression(int scope, Token *stmt_start = NULL,
                              int *conditions_count = NULL,
                              int *rowlimit = NULL, Token *prev_open = NULL,
                              bool *bool_operator_not_exists = NULL);
  bool ParseBooleanAndOr(int scope, Token *stmt_start, int *conditions_count,
                         int *rowlimit);
  ...
  bool ParseCreateFunction(Token *create, Token *or_, Token *replace,
                           Token *function);
  ...
  bool ParseCreatePackage(Token *create, Token *or_, Token *replace,
                          Token *package);
  bool ParseCreatePackageSpec(Token *create, Token *or_, Token *replace,
                              Token *package);
  bool ParseCreatePackageBody(Token *create, Token *or_, Token *replace,
                              Token *package, Token *body);
  bool ParseCreateProcedure(Token *create, Token *or_, Token *replace,
                            Token *procedure, int *result_sets);
  bool ParseCreateTrigger(Token *create, Token *or_, Token *trigger);
  bool ParseCreateTriggerBody(Token *create, Token *name, Token *table,
                              Token *when, Token *insert, Token *update,
                              Token *delete_, Token **end);
  bool ParseCreateTriggerOraclePattern(Token *create, Token *table, Token *when,
                                       Token *insert);
  ...
  bool ParseSequenceOptions(Token **start_with, Token **increment_by,
                            StatsSummaryItem &sti);
  ...
  bool ParseSelectStatement(Token *token, int block_scope, int select_scope,
                            int *result_sets, Token **list_end,
                            ListW *exp_starts, ListW *out_cols,
                            ListW *into_cols, int *appended_subquery_aliases,
                            Token **from_end, Token **where_end);
  ...
  bool ParseProcedureSpAddExtendedProperty(Token *execute,
                                           Token *sp_extendedproperty);
  ...
  bool ParseCreateTableColumns(Token *create, Token *table_name, ListW &pkcols,
                               Token **id_col, Token **id_start, Token **id_inc,
                               bool *id_default, ListWM *inline_indexes,
                               Token **last_colname);
  bool ParseColumnConstraints(Token *create, Token *table_name, Token *column,
                              Token *type, Token *type_end, Token **id_col,
  ...
  bool ParseDeclareVariable(Token *declare, Token *name, Token *type,
                            int cnt_list);
  bool ParseDefaultExpression(Token *type, Token *type_end, Token *token,
                              int pos);
  bool FormatDefaultExpression(Token *datatype, Token *exp, Token *default_);
  bool ParseStandaloneColumnConstraints(Token *alter, Token *table_name,
                                        ListW &pkcols, ListWM *inline_indexes);
  bool ParseInlineColumnConstraint(Token *type, Token *type_end,
                                   Token *constraint, int num);
  bool ParseKeyConstraint(Token *alter, Token *name, Token *primary,
                          ListW &pkcols, bool inline_cns,
  ...
  bool ParseProcedureBody(Token *create, Token *procedure, Token *name,
  ...
  bool ParseFunctionBody(Token *create, Token *function, Token *name,
                         Token *as);
  bool ParseSubSelect(Token *open, int select_scope);
  Token *GetNextSelectStartKeyword();
  bool ParseSelectCteClause(Token *with);
  bool ParseSelectList(Token *select, int select_scope, bool *into,
                       bool *dummy_not_required, bool *agg_func,
                       bool *agg_list_func, ListW *exp_starts, ListW *out_cols,
                       ListW *into_cols, Token **rowlimit_slist,
                       bool *rowlimit_percent);
  bool ParseSelectListPredicate(Token **rowlimit_slist, bool *rowlimit_percent);
  bool ParseSelectFromClause(Token *select, bool nested_from, Token **from,
                             Token **from_end, int *appended_subquery_aliases,
                             bool dummy_not_required, ListWM *from_table_end);
  bool ParseJoinClause(Token *first, Token *second, bool first_is_subquery,
                       ListWM *from_table_end);
  bool GetJoinKeywords(Token *token, Token **left_right_full,
                       Token **outer_inner, Token **join);
  bool ParseWhereClause(int stmt_scope, Token **where_, Token **where_end,
                        int *rowlimit = NULL);
  ...
  bool ParseSelectOptions(Token *select, Token *from_end, Token *where_,
                          Token *order, Token **rowlimit_soptions,
                          int *rowlimit);
  void SelectSetOutColsDataTypes(ListW *out_cols, ListWM *from_table_end);
  void SelectConvertRowlimit(Token *select, Token *from, Token *from_end,
                             Token *where_, Token *where_end, Token *pre_order,
                             Token *order, Token *rowlimit_slist,
                             Token *rowlimit_soptions, int rowlimit,
                             bool rowlimit_percent);

  bool ParseTempTableOptions(Token *table_name, Token **start, Token **end,
                             bool *no_data);
  bool ParseStorageClause(Token *table_name, Token **id_start, Token **comment,
                          Token *last_colname, Token *last_colend);
  bool ParseCreateIndexOptions();
  bool OpenWithReturnCursor(Token *name);

  void SqlServerConvertRowLevelTrigger(Token *table, Token *when, Token *insert,
                                       Token *update, Token *delete_,
                                       Token *end);
  void SqlServerAppendSubqueryAlias(Token *append,
                                    int *appended_subquery_aliases);
  void SqlServerDelimiter();
  void SqlServerAddStmtDelimiter(bool force = false);
  bool SqlServerGoDelimiter(bool just_remove = false);
  void SqlServerToDateAdd(Token *op, Token *first, Token *first_end,
                          Token *second, Token *second_end);
  ...
  bool ParseOracleRownumCondition(Token *first, Token *op, Token *second,
                                  int *rowlimit);
  bool RecognizeOracleDateFormat(Token *str, TokenStr &format);
  void OracleEmulateIdentity(Token *create, Token *table, Token *column,
                             Token *last, Token *id_start, Token *id_inc,
                             bool id_default);
  ...
  void OracleMoveBeginAfterDeclare(Token *create, Token *as, Token *begin,
                                   Token *body_start);
  ...
  void OracleMoveSelectOutofIf(Token *stmt_start, Token *select,
                               Token *select_list_end, Token *open,
                               Token *close);
  ...
  bool ParseMysqlStorageClause(Token *table_name, Token **id_start,
                               Token **comment);
  ...
  bool ParseDb2GeneratedClause(Token *create, Token *table_name, Token *column,
                               Token *generated, Token **id_col,
                               Token **id_start, Token **id_inc,
                               bool *id_default);
  bool ParseDb2FetchFirstRowOnly(Token *fetch, Token **rowlimit_soptions,
                                 int *rowlimit);
  bool ParseDb2StogroupClause(Token *objname, Token *using_, Token *stogroup,
                              int scope);
  ...
  bool ParseDb2ResultSetLocatorDeclaration(Token *declare, Token *name,
  ...
  void InformixGetReturningName(Token *retvalue, TokenStr &name,
                                const char *prefix, const wchar_t *wprefix,
                                size_t plen, int num, bool always_prefix);
  ...
  bool ParseInformixMatchesPredicate(Token *pre_first, Token *first,
                                     Token *not_, Token *matches);
  ...
  bool ParseTeradataPrimaryIndex(Token *unique, Token *primary,
                                 Token *last_colname, Token *last_colend);
  ...
  void AddGreenplumDistributedBy(Token *create, Token *close, ListW &pkcols,
                                 Book *col_end);
  ...
  void SplitIdentifierByLastPart(Token *token, TokenStr &lead, TokenStr &trail,
                                 int parts = -1);
  ...
  Token *AppendIdentifier(Token *source, const char *word,
                          const wchar_t *w_word, size_t len);
  void AppendIdentifier(TokenStr &source, const char *word,
                        const wchar_t *w_word, size_t len);
  ...
  void Comment(const char *word, const wchar_t *w_word, size_t len,
