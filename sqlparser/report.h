...
#define SQLINES_REPORT_OK_MARK                                                 \
  "<span style=\"color:green\"><strong>&#x2713;</strong></span>"
#define SQLINES_REPORT_WARN_MARK                                               \
  "<span style=\"color:red\"><strong>!</strong></span>"
#define SQLINES_REPORT_ERROR_MARK                                              \
  "<span style=\"color:red\"><strong>&#x2715;</strong></span>"
...
private:
  ...
  void GetReportSection(Stats *stats, std::string &macro, std::string &data,
                        std::string &snippets);
  void GetReportSectionRows(std::map<std::string, int> &values,
                            std::string &data, int *distinct, int *occurrences);
  void GetReportSectionRows(std::map<std::string, StatsItem> &values,
                            std::string &data, int *distinct, int *occurrences);
  void GetReportSectionRows(std::string section,
                            std::map<std::string, StatsSummaryItem> &values,
                            std::string &data, std::string &snippets,
                            int *distinct, int *occurrences, bool desc = true,
                            bool note = true, bool complexity = true);
  void GetReportSectionRows(std::string section,
                            std::map<std::string, StatsDetailItem> &values,
                            std::string &data, std::string &snippets,
                            int *distinct, int *occurrences, bool desc = true,
                            bool note = true, bool complexity = true);
  ...
  const char *ProcessReportIfExists(Stats *stats, const char *cur_tpl,
                                    const char *macro);
