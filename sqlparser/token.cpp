...
bool Token::Compare(const char *word, const wchar_t *w_word, size_t start,
                    size_t len) {
  return Token::Compare(this, word, w_word, start, len);
}

bool Token::Compare(Token *token, const char *word, const wchar_t * /*w_word*/,
                    size_t len) {
  if (token == NULL)
    return false;

  if (token->str != NULL && token->len == len &&
      !_strnicmp(token->str, word, len))
    return true;
...
bool Token::Compare(Token *token, const char *word, const wchar_t * /*w_word*/,
  ...
  if (token->str != NULL && token->len >= start + len &&
      !_strnicmp(token->str + start, word, len))
    return true;
...
bool Token::CompareCS(Token *token, const char *word,
                      const wchar_t * /*w_word*/, size_t start, size_t len) {
  if (token == NULL)
    return false;
...
bool Token::Compare(Token *first, Token *second, size_t len) {
  ...
  if (first->str != NULL && second->str != NULL && first->len >= len &&
      second->len >= len && !_strnicmp(first->str, second->str, len))
    return true;
...
bool Token::Compare(Token *first, TokenStr *second) {
  ...
  // Use NULL pointers for empty strings for correct comparison (since only str
  // or wstr can be set at a time)
...
bool Token::CompareTarget(Token *token, const char *word,
                          const wchar_t * /*w_word*/, size_t len) {
  if (token == NULL || token->t_len != len)
    return false;

  if (token->t_str != NULL && word != NULL &&
      _strnicmp(token->t_str, word, len) == 0)
    return true;
...
bool Token::IsBlank() { return Token::IsBlank(this); }

bool Token::IsBlank(Token *token) {
  if (token == NULL)
    return false;

  if (token->chr == ' ' || token->chr == '\t' || token->chr == '\r' ||
      token->chr == '\n' || token->wchr == L' ' || token->wchr == L'\t' ||
      token->wchr == L'\r' || token->wchr == L'\n')
    return true;

  return false;
}

void Token::Change(Token *token, const char *new_str,
                   const wchar_t * /*new_wstr*/, size_t len, Token *format) {
  if (token == NULL)
    return;
...
void Token::Change(Token *token, Token *values) {
  ...
  } else if (values->str != NULL) {
    token->t_str = Str::GetCopy(values->str, values->len);
    token->t_len = values->len;
  }
}

void Token::ChangeNoFormat(Token *token, const char *new_str,
...
void Token::ChangeNoFormat(Token *token, Token *source, size_t start,
                           size_t len) {
...
void Token::FormatTargetValue(Token *token, Token *format) {
  ...
  for (size_t i = 0; i < len; i++) {
    // Check for a letter (English ASCII only), src is signed so check for
    // negative values
...
void Token::Remove(Token *token, bool remove_spaces_before) {
  ...
  if (remove_spaces_before == true) {
    // Remove spaces token before the removed word (recursively), but only if
    // there is a blank or ) , ; after removed token (otherwise tokens can be
    // merged)
    if (prev->Compare(' ', L' ') == true ||
        prev->Compare('\t', L'\t') == true) {
      if (next != NULL) {
  ...
  // In other cases better to remove spaces after: "SET var = 1" to "var := 1"
  // when SET is removed
  else {
...
void TokenStr::Append(TokenStr &tokenstr, size_t start, size_t l) {
  ...
  else if (tokenstr.wstr.size() > start + l)
...
bool TokenStr::Compare(TokenStr *tokenstr, const char *word,
                       const wchar_t * /*w_word*/, size_t len) {
...
bool TokenStr::Compare(TokenStr *tokenstr, const char ch, const wchar_t /*wch*/,
                       size_t pos) {
