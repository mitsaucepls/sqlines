...
bool SqlParser::ParseSystemProcedure(Token *execute, Token *name) {
  ...
  else if (TOKEN_CMP(name, "SP_ADDTYPE") || TOKEN_CMP(name, "SYS.SP_ADDTYPE"))
    exists = ParseProcedureSpAddType(execute, name);
  else if (TOKEN_CMP(name, "SP_BINDRULE") || TOKEN_CMP(name, "SYS.SP_BINDRULE"))
    exists = ParseProcedureSpBindRule(execute, name);
  else if (TOKEN_CMP(name, "SP_ADDEXTENDEDPROPERTY") ||
           TOKEN_CMP(name, "SYS.SP_ADDEXTENDEDPROPERTY"))
...
bool SqlParser::ParseProcedureSpAddType(Token *execute, Token *sp_addtype) {
  ...
  // In Sybase ASE, data type must be enclosed with ' or " if it contains
  // spaces, () i.e.
...
bool SqlParser::ParseProcedureSpAddExtendedProperty(
    Token *execute, Token *sp_extendedproperty) {
  ...
  while (true) {
    ...
    if (TOKEN_CMP_PART0(next, "@") && equal != NULL) {
      ...
      if (TOKEN_CMP(next, "@name"))
        name = val;
      else if (TOKEN_CMP(next, "@value"))
        value = val;
      else if (TOKEN_CMP(next, "@level0type"))
        level0_type = val;
      else if (TOKEN_CMP(next, "@level0name"))
        level0_name = val;
      else if (TOKEN_CMP(next, "@level1type"))
        level1_type = val;
      else if (TOKEN_CMP(next, "@level1name"))
        level1_name = val;
      else if (TOKEN_CMP(next, "@level2type"))
        level2_type = val;
      else if (TOKEN_CMP(next, "@level2name"))
        level2_name = val;
  ...
  if (TOKEN_CMP(name, "N'MS_Description'") ||
      TOKEN_CMP(name, "'MS_Description'")) {
