...
bool SqlParser::Source(short s1, short s2, short s3, short s4, short s5,
                       short s6) {
  if (_source == s1 || _source == s2 || _source == s3 || _source == s4 ||
      _source == s5 || _source == s6)
    return true;
...
bool SqlParser::Target(short t1, short t2, short t3, short t4, short t5,
                       short t6) {
  if (_target == t1 || _target == t2 || _target == t3 || _target == t4 ||
      _target == t5 || _target == t6)
    return true;
...
Book *SqlParser::GetBookmark(int type, Token *name, Token *name2) {
  ...
  while (bookmark != NULL) {
    if (bookmark->type == type &&
        Token::Compare(bookmark->name, name) == true) {
...
Token *SqlParser::AppendIdentifier(Token *source, const char *word,
...
void SqlParser::AppendIdentifier(TokenStr &source, const char *word,
                                 const wchar_t *w_word, size_t len) {
...
bool SqlParser::MapObjectName(Token *token) {
  ...
  // Find an identifier
  for (StringMap::iterator i = _object_map.begin(); i != _object_map.end();
       i++) {
  ...
  *len = *len + 1;
  ...
  if (_source == SQL_INFORMIX &&
    ...
    if (Source(SQL_SQL_SERVER, SQL_SYBASE) &&
        ...
        (schema.Compare("[dbo].", L"[dbo].", 6) == true ||
         schema.Compare("dbo.", L"dbo.", 4) == true))
      schema.Clear();
    else
      // Remove `` in MySQL
      if (_source == SQL_MYSQL && _target != SQL_MYSQL) {
        ...
        if (_source != SQL_MYSQL && Target(SQL_MARIADB, SQL_MYSQL)) {
          ...
          if (_source == SQL_SYBASE_ASA && _target != SQL_SYBASE_ASA &&
              schema.Compare("dba.", L"dba.", 4) == true)
            schema.Clear();
...
void SqlParser::GetNextIdentItem(Token *token, TokenStr &ident, size_t *len) {
  ...
  while (i < token->len) {
    if (end_del == 0) {
      if (token->Compare('.', L'.', i) == true)
        break;
    } else if (token->Compare(end_del, end_delw, i) == true) {
      ident.Append(token, i, 1);
      i++;
...
void SqlParser::SplitIdentifierByLastPart(Token *token, TokenStr &lead,
                                          TokenStr &trail, int parts) {
  if (token == NULL)
    return;
...
void SqlParser::DiscloseImplicitRecordVariables(Token *format) {
  ...
  // Add column list to cursor declaration that use SELECT * FROM instead of
...
// Cut operation
void SqlParser::Cut(int scope, int type, Token *name, Token *start,
                    Token *end) {
...
void SqlParser::AddReturnRefcursors(Token *end) {
  if (!Source(SQL_ORACLE) || !Target(SQL_POSTGRESQL) ||
      _spl_refcursor_params_num == 0 || end == NULL)
    return;

  // Iterate REFCURSORS parameters and add RETURN statement
  for (ListwItem *i = _spl_refcursor_params.GetFirst(); i != NULL;
       i = i->next) {
...
void SqlParser::AddGeneratedVariables() {
  // Add not hound handler that must go after all variables and cursors; and
  ...
  // Oracle uses DECLARE block with the keyword at the beginning so use the
  // variable name to define the indention
...
void SqlParser::Enter(int scope) { _scope.Add(scope); }
...
bool SqlParser::OpenWithReturnCursor(Token *name) {
  ...
  while (cur != NULL) {
    if (cur->type != COPY_CURSOR_WITH_RETURN ||
        Token::Compare(name, cur->name) == false) {
      cur = cur->next;
      continue;
    }
