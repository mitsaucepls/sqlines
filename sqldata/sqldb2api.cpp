...
SqlDb2Api::~SqlDb2Api() { Deallocate(); }

// Initialize API
int SqlDb2Api::Init() {
#if defined(WIN32) || defined(_WIN64)

  // Force UTF-8 codepage at the client side. By default, if DB2CODEPAGE
  // environment variable is not set, DB2 uses regional settings
  ...
  if (_dll == NULL) {
    ...
    for (std::list<std::string>::iterator i = paths.begin(); i != paths.end();
         i++) {
      // For DB2 10.1 and 11 path already includes terminating \ and DLL is
      // located in BIN
  ...
  if (_dll != NULL) {
    ...
    _SQLBindParameter =
        (SQLBindParameterFunc)Os::GetProcAddress(_dll, "SQLBindParameter");
    ...
    if (_SQLAllocHandle == NULL || _SQLBindCol == NULL ||
        _SQLBindParameter == NULL || _SQLConnect == NULL ||
        _SQLDescribeCol == NULL || _SQLDisconnect == NULL ||
        _SQLDriverConnect == NULL || _SQLEndTran == NULL ||
        _SQLExecDirect == NULL || _SQLExecute == NULL || _SQLFetch == NULL ||
        _SQLFreeHandle == NULL || _SQLGetDiagRec == NULL ||
        _SQLPrepare == NULL || _SQLSetEnvAttr == NULL ||
        _SQLSetConnectAttr == NULL || _SQLSetStmtAttr == NULL)
      return -1;
  } else {
...
int SqlDb2Api::ExecuteScalar(const char *query, int *result,
                             size_t *time_spent) {
  SQLHANDLE stmt;
  int q_result = 0;
...
int SqlDb2Api::OpenCursor(const char *query, size_t buffer_rows,
  ...
  for (int i = 0; i < _cursor_cols_count; i++) {
    ...
    // On 64-bit platform SQLULEN column_size is 64-bit, but DB2 does sets upper
    // 4 bytes to 0 for some reason, we so must initialize
  ...
  for (int i = 0; i < _cursor_cols_count; i++) {
    ...
    if (_cursor_cols[i]._native_dt == SQL_CHAR ||
      ...
      if (_cursor_cols[i]._native_dt == -1) {
        ...
        // Data type GRAPHIC with code -95 (fixed-length UTF-16) or VARGRAPHIC
        // with code -96 (variable-length UTF-16)
        if (_cursor_cols[i]._native_dt == -95 ||
            _cursor_cols[i]._native_dt == -96) {
                    ...
                    if (_cursor_cols[i]._native_dt == SQL_TYPE_TIME) {
                      ...
                      if (_cursor_cols[i]._native_dt == SQL_DECIMAL ||
                        ...
                        if (_cursor_cols[i]._native_dt == SQL_REAL) {
                          ...
                          // Driver returns 7 as len for FLOAT, it is too short
                          ...
                          if (_cursor_cols[i]._native_dt == -99 ||
                              _cursor_cols[i]._native_dt == -98) {
                            _cursor_cols[i]._native_fetch_dt = SQL_C_CHAR;

                            if (_cursor_cols[i]._native_dt == -98)
                              _cursor_cols[i]._native_fetch_dt = SQL_C_BINARY;

                            // DB2 allows specifying any length for BLOB, so it
                            // not necessary to large and we can fetch as a
                            // regular type in arrays
    ...
    if (_cursor_cols[i]._data != NULL) {
      ...
      rc = _SQLBindCol(
          _hstmt_cursor, (SQLUSMALLINT)(i + 1),
          (SQLSMALLINT)_cursor_cols[i]._native_fetch_dt, _cursor_cols[i]._data,
...
int SqlDb2Api::InitBulkTransfer(const char *table, size_t col_count,
                                size_t allocated_array_rows, SqlCol *s_cols,
                                SqlCol ** /*t_cols*/) {
  // Set AUTOCOMMIT OFF
  int rc =
      _SQLSetConnectAttr(_hdbc, SQL_ATTR_AUTOCOMMIT, SQL_AUTOCOMMIT_OFF, 0);
  ...
  rc = _SQLSetStmtAttr(_hstmt_insert, SQL_ATTR_PARAM_BIND_TYPE,
                       SQL_PARAM_BIND_BY_COLUMN, 0);

  // Specify the array size
  rc = _SQLSetStmtAttr(_hstmt_insert, SQL_ATTR_PARAMSET_SIZE,
                       (SQLPOINTER)allocated_array_rows, 0);

  // Number of processed rows
  rc = _SQLSetStmtAttr(_hstmt_insert, SQL_ATTR_PARAMS_PROCESSED_PTR,
                       &_rows_processed, 0);
  ...
  for (int i = 0; i < col_count; i++) {
    ...
    if (((_source_api_type == SQLDATA_SQL_SERVER ||
      ...
      rc = _SQLBindParameter(
          _hstmt_insert, (SQLUSMALLINT)(i + 1), SQL_PARAM_INPUT, SQL_C_CHAR,
          SQL_CHAR, s_cols[i]._len, 0, s_cols[i]._data,
          (SQLLEN)s_cols[i]._fetch_len, (SQLLEN *)s_cols[i].ind);
    } else
      ...
      if ((_source_api_type == SQLDATA_ORACLE &&
        ...
        rc = _SQLBindParameter(
            _hstmt_insert, (SQLUSMALLINT)(i + 1), SQL_PARAM_INPUT, SQL_C_CHAR,
            SQL_CHAR, s_cols[i]._len, 0, s_cols[i]._data,
            (SQLLEN)s_cols[i]._fetch_len, (SQLLEN *)_ins_cols[i].ind);
      } else
        ...
        if ((_source_api_type == SQLDATA_SQL_SERVER ||
          ...
          // Bind to source buffer directly
          rc = _SQLBindParameter(_hstmt_insert, (SQLUSMALLINT)(i + 1),
                                 SQL_PARAM_INPUT, SQL_C_LONG, SQL_INTEGER,
                                 sizeof(int), 0, s_cols[i]._data, sizeof(int),
                                 (SQLLEN *)s_cols[i].ind);
        } else
          // Sybase INT
          if (_source_api_type == SQLDATA_SYBASE &&
            ...
            if ((_source_api_type == SQLDATA_SQL_SERVER ||
              ...
              // Bind to source buffer directly
              rc = _SQLBindParameter(_hstmt_insert, (SQLUSMALLINT)(i + 1),
                                     SQL_PARAM_INPUT, SQL_C_SHORT, SQL_SMALLINT,
                                     sizeof(short), 0, s_cols[i]._data,
                                     sizeof(short), (SQLLEN *)s_cols[i].ind);
            } else
              // Sybase SMALLINT
              if (_source_api_type == SQLDATA_SYBASE &&
                ...
                // Bind data buffer directly
                rc = _SQLBindParameter(
                    _hstmt_insert, (SQLUSMALLINT)(i + 1), SQL_PARAM_INPUT,
                    SQL_C_SHORT, SQL_SMALLINT, sizeof(short), 0,
                    s_cols[i]._data, sizeof(short), (SQLLEN *)_ins_cols[i].ind);
              }
  }
...
int SqlDb2Api::TransferRows(SqlCol *s_cols, int rows_fetched, int *rows_written,
...
// Drop the table
int SqlDb2Api::DropTable(const char *table, size_t *time_spent,
...
int SqlDb2Api::GetLobLength(size_t /*row*/, size_t /*column*/,
                            size_t * /*length*/) {
  return -1;
}

// Get LOB content
int SqlDb2Api::GetLobContent(size_t /*row*/, size_t /*column*/, void * /*data*/,
                             size_t /*length*/, int * /*len_ind*/) {
  return -1;
}

// Get partial LOB content
int SqlDb2Api::GetLobPart(size_t /*row*/, size_t /*column*/, void * /*data*/,
                          size_t /*length*/, int * /*len_ind*/) {
  /*int rc = SQLGetData(_hstmt_cursor, (SQLUSMALLINT)(column + 1), SQL_C_BINARY,
  data, (SQLLEN)length, (SQLLEN*)len_ind);
...
int SqlDb2Api::GetAvailableTables(std::string &table_template,
                                  std::string & /*exclude*/,
  ...
  GetSelectionCriteria(table_template.c_str(), "tabschema", "tabname",
                       condition, _user.c_str(), true);
  GetSelectionCriteria(table_template.c_str(), "creator", "name", condition,
                       _user.c_str(), true);
...
int SqlDb2Api::ReadSchema(const char *select, const char *exclude,
  ...
  GetSelectionCriteria(select, exclude, "t.tabschema", "t.tabname", condition,
                       _user.c_str(), false);
...
int SqlDb2Api::GetKeyConstraintColumns(SqlConstraints &cns,
...
int SqlDb2Api::ReadConstraintTable(const char * /*schema*/,
...
int SqlDb2Api::ReadConstraintColumns(const char * /*schema*/,
                                     const char * /*table*/,
                                     const char * /*constraint*/,
                                     std::string & /*cols*/) {
  return -1;
...
void SqlDb2Api::GetCatalogSelectionCriteria(
    std::string & /*selection_template*/, std::string & /*output*/) {}

// Set version of the connected database
void SqlDb2Api::SetVersion() {
  // Tested on DB2 10.1, 11 LUW
  std::string query = "SELECT ii.service_level || ', ' || ii.inst_ptr_size || "
                      "'-bit, ' || si.os_name || ' ' ";
  query += "|| si.os_version || ', ' || si.total_cpus || ' CPU', ";
  query += "	(SELECT pi.installed_prod FROM "
           "TABLE(SYSPROC.ENV_GET_PROD_INFO()) AS pi ";
  query +=
      "	WHERE pi.installed_prod IN ('EXP', 'PE', 'WSE', 'WSUE', 'ESE'))";
  query += "FROM TABLE(SYSPROC.ENV_GET_INST_INFO()) AS ii, "
           "TABLE(SYSPROC.ENV_GET_SYS_INFO()) AS si";
  ...
  if (rc >= 0 && cols != NULL && cols[0].ind != NULL && cols[1].ind != NULL) {
      ...
      if (!IsIndNull(len) && data != NULL) {
        ...
        if (len == 3 && strncmp(data, "EXP", 3) == 0)
          ...
          if (len == 2 && strncmp(data, "PE", 2) == 0)
            ...
            if ((len == 3 && strncmp(data, "WSE", 3) == 0) ||
                (len == 4 && strncmp(data, "WSUE", 4) == 0))
    ...
    rc = OpenCursor(query2.c_str(), 1, 0, &col_count, &allocated_rows,
                    &rows_fetched, &cols, &time_read, true);
...
void SqlDb2Api::FindDb2Paths(std::list<std::string> &paths) {
  ...
  int rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\IBM\\DB2", 0, KEY_READ,
                        &hKey);
  ...
  // For 10 and 11 "DB2 Path Name" parameter contains installation directory
  // WITH terminating '\'
  rc = RegQueryValueEx(hKey, "DB2 Path Name", NULL, NULL, (LPBYTE)location,
                       (LPDWORD)&value_size);
