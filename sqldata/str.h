...
public:
  ...
  static bool IsSet(const char *in) {
    if (in != NULL && *in != '\x0')
      return true;
    else
      return false;
  ...
  static bool CompareNumberAsString(const char *first, int flen,
                                    const char *second, int slen);
  ...
  static void SqlTs2Str(short year, short month, short day, short hour,
                        short minute, short second, long fraction, char *out);
  // Convert 7-byte packed Oracle DATE to string (non-null terminated, exactly
  // 19 characters)
  static void OraDate2Str(unsigned char *ora_date, char *out);

  // Get integer or fractional part of number in string
  static const char *GetIntegerPart(const char *str, int slen, char *out,
                                    int *out_len);
