...
#define TRACE_DIFF_DMP(data, len)                                              \
  {                                                                            \
    if (_trace_valdiff_data)                                                   \
      File::Append(_valdiff_file.c_str(), data, len);                          \
  }
...
struct SqlDataReply {
  ...
  // For tables, name of the index, sequence i.e for which operation is
  // performed
  char t_o_name[1024];
  ...
  SqlDataReply() {
    ...
    *t_o_name = '\x0';
    ...
    s_error = 0;
    t_error = 0;
    *s_native_error_text = '\x0';
    *t_native_error_text = '\x0';
    s_sql = NULL;
    t_sql = NULL;
    data = NULL;
    *data2 = '\x0';
...
public:
  ...
  int InitStatic(int db_types, const char *source_conn, const char *target_conn,
                 int *s_rc, int *t_rc);
  // Set database configuration
  int Init(int db_types, const char *source_conn, const char *target_conn,
           int *s_rc, int *t_rc);
  ...
  // Test connection
  int TestConnection(std::string &conn, std::string &error,
  ...
  void SetDataTypeMapping(std::list<SqlDataTypeMap> *dtmap) {
    _datatype_map = dtmap;
  }
  void SetTableSelectExpressions(std::map<std::string, std::string> *tselmap) {
    _tsel_exp_map = tselmap;
  }
  void SetTableSelectExpressionsAll(std::string *tselall) {
    _tsel_exp_all = tselall;
  }
  void SetTableWhereConditions(std::map<std::string, std::string> *twheremap) {
    _twhere_cond_map = twheremap;
  }

  // Get the list of available tables
  int GetAvailableTables(int db_types, std::string &table_template,
                         std::string &exclude, std::list<std::string> &tables);

  // Read schema information
  int ReadSchema(int db_types, std::string &select, std::string &exclude,
                 bool read_cns = true, bool read_idx = true);

  // Transfer table rows
  int TransferRows(SqlDataReply &reply, int options, bool create_tables,
                   bool data);
  ...
  int ValidateCompareRows(SqlCol *s_cols, SqlCol *t_cols, int s_col_count,
                          int *s_bytes, int t_col_count, int s_rows, int t_rows,
                          int *t_bytes, int running_rows,
                          int running_not_equal_rows);

  // Compare string representations of numbers .5 and 0.50
  bool ValidateCompareNumbers(SqlCol *s_col, const char *s_string, int s_len,
                              SqlCol *t_col, const char *t_string, int t_len,
                              int *equal);
  // Compare datetime values fetched as strings
  bool ValidateCompareDatetimes(SqlCol *s_col, const char *s_string, int s_len,
                                SqlCol *t_col, const char *t_string, int t_len,
                                int *equal);
  // Compare strings
  bool ValidateCompareStrings(SqlCol *s_col, const char *s_string, int s_len,
                              SqlCol *t_col, const char *t_string, int t_len,
                              int *equal);
  // Compare LOB values
  bool ValidateCompareLobs(SqlCol *s_col, int s_len, SqlCol *t_col, int t_len,
                           int *equal);
  // Dump differences in column
  void ValidateDiffDump(SqlCol *s_col, SqlCol *t_col, int row, int s_len,
  ...
  int GetColumnDataLen(SqlCol *cols, int row, int column, int db_type,
                       SqlApiBase *db_api);
  // Get column data from the buffer
  int GetColumnData(SqlCol *cols, int row, int column, int db_type,
                    SqlApiBase *db_api, char **str, bool *int_set, int *int_v,
                    SQL_TIMESTAMP_STRUCT **ts, char **ora_date);

  // Build transfer and data validation query
  int BuildQuery(std::string &s_query, std::string &t_query,
                 const char *s_table, const char *t_table, bool transfer);
  int BuildQueryAddOrder(std::string &s_query, std::string &s_schema,
                         std::string &s_object, std::string &t_query,
                         std::string &t_schema, std::string &t_object);
  ...
  int CreateSequence(int db_types, SqlDataReply &reply, const char *query,
                     const char *seq_name);

  // Drop foreign keys to the
  int DropReferences(int db_types, const char *table, size_t *time_spent,
                     int *keys);

  // Read specific catalog information
  int ReadConstraintTable(int db_types, const char *schema,
                          const char *constraint, std::string &table);
  int ReadConstraintColumns(int db_types, const char *schema, const char *table,
                            const char *constraint, std::string &cols);
  int GetKeyConstraintColumns(int db_types, SqlConstraints &cns,
                              std::list<std::string> &key_cols);
  int GetForeignKeyConstraintColumns(int db_types, SqlConstraints &cns,
                                     std::list<std::string> &fcols,
                                     std::list<std::string> &pcols,
                                     std::string &ptable);
  int GetIndexColumns(int db_types, SqlIndexes &idx,
  ...
  void MapColumn(const char *s_table, const char *s_name, std::string &t_name,
                 std::string &t_type);
...
private:
  ...
  int PrepareTransfer(SqlCol *s_cols, const char *s_table, const char *t_table,
                      size_t col_count, int options, SqlDataReply &reply);

  bool IsSpecialIdentifier(const char *s_name);

  // Generate SQL CREATE TABLE statement
  int GenerateCreateTable(SqlCol *s_cols, const char *s_table,
                          const char *t_table, int col_count, std::string &sql);
  ...
  int GetDefaultClause(const char *s_table, char *column,
                       std::string &default_clause);
  // Get IDENTITY clause to be added to column definition
  int GetInlineIdentityClause(const char *s_table, char *column,
  ...
  int Execute(int cmd);
  // Execute one command for source, another for target in concurrent threads
  // and wait
  ...
  int CopyColumns(SqlCol *cols, SqlCol **cols_copy, size_t col_count,
                  size_t allocated_array_rows);
  // Copy data
  int CopyColumnData(SqlCol *s_cols, SqlCol *t_cols, size_t col_count,
                     int rows_fetched);
