...
int SqlDataCmd::Run() {
  ...
  if (!_parameters.IsCfgReadOk())
    _log.LogFile("\n\nError reading the configuration file: %s",
                 _parameters.GetCfgReadError());
  ...
  int workers =
      (_command != SQLDATA_CMD_TRANSFER_OBJ)
          ? _sqlData.SetConcurrentSessions(_concurrent_sessions, _total_tables)
          : 1;

  if (_command == SQLDATA_CMD_TRANSFER) {
    // Remove foreign keys for selected tables if DROP or TRUNCATE options are
    // set
    rc = DropReferences();

    _log.Log("\n\nTransferring database ");
    _transfer_table_num = 1;
  } else if (_command == SQLDATA_CMD_TRANSFER_OBJ) {
    _log.Log("\n\nTransferring database objects ");
    _transfer_obj_num = 1;
  } else if (_command == SQLDATA_CMD_VALIDATE) {
    if (_command_options == SQLDATA_OPT_ROWCOUNT)
      _log.Log("\n\nValidating table row count ");
    else if (_command_options == SQLDATA_OPT_ROWS)
      _log.Log("\n\nValidating table data ");

    _validate_table_num = 1;
  } else if (_command == SQLDATA_CMD_ASSESS) {
    _log.Log("\n\nRunning assessment ");
    _assess_table_num = 1;
  }

  _log.Log("(%d %s):\n", workers,
           (workers == 1) ? "session" : "concurrent sessions");
...
void SqlDataCmd::Callback(SqlDataReply *reply) {
  ...
  } else if (reply->_cmd == SQLDATA_CMD_ASSESS)
...
void SqlDataCmd::CallbackTransfer(SqlDataReply *reply) {
  ...
  if (reply->_cmd_subtype == SQLDATA_CMD_STARTED) {
    _log.Log("Started (%d of %d, session %d)", _transfer_table_num,
             _total_tables, reply->session_id);
    _transfer_table_num++;
    ...
    if (reply->rc != -1) {
      _log.Log("%d row%s read, %s, session %d)", reply->_s_int1,
               SUFFIX(reply->_s_int1), time_fmt, reply->session_id);
      ...
      _log.Log("Failed, %s, session %d)\n    %s", time_fmt, reply->session_id,
               reply->s_native_error_text);
    ...
    else if (reply->_cmd_subtype == SQLDATA_CMD_DROP_SEQUENCE)
    ...
    if (reply->_cmd_subtype == SQLDATA_CMD_DROP_FOREIGN_KEY) {
      ...
      _log.Log("\n  %s (%d key%s, %s)", reply->_t_name, reply->_int1,
               SUFFIX(reply->_int1), time_fmt);
      ...
      if (reply->_s_int1 != 0 && reply->_s_int2 != 0)
        r_speed_rows =
            ((double)reply->_s_int1) / ((double)reply->_s_int2) * 1000.0;

      if (reply->_t_int2 != 0) {
        if (reply->_t_int1 != 0)
          w_speed_rows =
              ((double)reply->_t_int1) / ((double)reply->_t_int2) * 1000.0;
      ...
      _log.Log("\n     Rows read:     %d (%.0lf rows/sec)", reply->_s_int1,
               r_speed_rows);
      _log.Log("\n     Rows written:  %d (%.0lf rows/sec, %s, %s/sec)",
               reply->_t_int1, w_speed_rows, w_bytes_fmt, w_speed_bytes_fmt);
      _log.Log("\n     Transfer time: %s (%s read, %s write)", tr_time_fmt,
               r_time_fmt, w_time_fmt);
      ...
      // Save information about the tables with different number of read and
      // written rows
      if (reply->_cmd_subtype == SQLDATA_CMD_COMPLETE && reply->rc == 0 &&
        ...
        table_diff +=
            Str::IntToString(abs(reply->_s_int1 - reply->_t_int1), row_fmt);
      ...
      if (reply->_cmd_subtype == SQLDATA_CMD_COMPLETE_WITH_DDL_ERROR) {
      ...
      } else if (reply->_cmd_subtype == SQLDATA_CMD_ADD_DEFAULT ||
        ...
        else if (reply->_cmd_subtype == SQLDATA_CMD_ADD_CHECK_CONSTRAINT)
          _log.Log("Add CHECK constraint (");
        else if (reply->_cmd_subtype == SQLDATA_CMD_ADD_PRIMARY_KEY)
          _log.Log("Add PRIMARY KEY constraint (");
        else if (reply->_cmd_subtype == SQLDATA_CMD_ADD_UNIQUE_KEY)
          _log.Log("Add UNIQUE constraint (");
        else if (reply->_cmd_subtype == SQLDATA_CMD_ADD_FOREIGN_KEY)
          _log.Log("Add FOREIGN KEY constraint (");
        else if (reply->_cmd_subtype == SQLDATA_CMD_ADD_COMMENT)
          _log.Log("Add comment (");
        else if (reply->_cmd_subtype == SQLDATA_CMD_CREATE_INDEX)
          _log.Log("Create index %s (", reply->t_o_name);
        else if (reply->_cmd_subtype == SQLDATA_CMD_CREATE_SEQUENCE)
          _log.Log("Create sequence (");
        else if (reply->_cmd_subtype == SQLDATA_CMD_CREATE_TRIGGER)
        ...
        _total_target_ddl++;
        LogSql(reply, time_fmt);
      } else if (reply->_cmd_subtype == SQLDATA_CMD_ALL_COMPLETE) {
        ...
        if (_failed_tables_list.empty() == false) {
          ...
          for (std::list<std::string>::iterator i =
                   _failed_tables_names_list.begin();
               i != _failed_tables_names_list.end(); i++)
            _failed_tables_log.Log("%s\n", i->c_str());
        }

        // Output information about tables with different number of read and
        // written rows
        if (_tables_read_write_diff_list.empty() == false) {
          _log.Log("\n\nRead/write row count differences (%d table%s):\n\n",
                   read_write_diff_count, SUFFIX(read_write_diff_count));

          int k = 1;
          for (std::list<std::string>::iterator i =
                   _tables_read_write_diff_list.begin();
               i != _tables_read_write_diff_list.end(); i++, k++)
            _log.Log("%4d. %s\n", k, i->c_str());
        }

        if (_failed_tables_list.empty() == false ||
            _tables_read_write_diff_list.empty() == false)
          _log.Log("\nPlease contact us at support@sqlines.com for any "
                   "assistance.\n");
        ...
        if (reply->_int1 != 0) {
          speed_rows =
              ((double)reply->_t_int1) / ((double)reply->_int1) * 1000.0;
          speed_bytes =
        ...
        if (_failed_tables_list.empty() == true &&
            _tables_read_write_diff_list.empty() == true)
          _log.Log("\n");

        _log.Log("\nSummary:\n");
        _log.Log("\n  Tables:        %d (%d Ok, %d failed)", _total_tables,
                 _total_tables - _failed_tables, _failed_tables);
        _log.Log("\n  Target DDL:    %d (%d Ok, %d failed)", _total_target_ddl,
                 _total_target_ddl - _failed_target_ddl, _failed_target_ddl);
        _log.Log("\n  Rows read:     %d", reply->_s_int1);
        _log.Log("\n  Rows written:  %d", reply->_t_int1);

        if (reply->_s_int1 != reply->_t_int1)
          _log.Log(" (%d row difference)",
                   abs(reply->_s_int1 - reply->_t_int1));

        _log.Log("\n  Transfer time: %s (%.0lf rows/sec, %s, %s/sec)", time_fmt,
                 speed_rows, bytes_fmt, speed_bytes_fmt);
        ...
        if (_failed_target_ddl > 0)
          _log.Log("\n  Failed DDL SQL statements: %s",
                   SQLDATA_FAILED_DDL_LOGFILE);
      } else if (reply->_cmd_subtype == SQLDATA_CMD_DUMP_DDL) {
        ...
        for (int i = 0; i < reply->_int1; i++) {
          _log.LogFile(
              ...
              i + 1, cols[i]._name, cols[i]._native_dt, cols[i]._len,
              cols[i]._precision, cols[i]._scale);
        }
      }
}
...
void SqlDataCmd::CallbackAssess(SqlDataReply *reply) {
  ...
  if (reply->_cmd_subtype == SQLDATA_CMD_STARTED) {
    _log.Log("Started (%d of %d, session %d)", _assess_table_num, _total_tables,
             reply->session_id);
    _assess_table_num++;
  } else if (reply->_cmd_subtype == SQLDATA_CMD_OPEN_CURSOR) {
    Str::FormatTime((size_t)reply->_s_int2, time_fmt);
    ...
    if (reply->_s_int1 != 0 && reply->_s_int2 != 0) {
      ...
      if (reply->_s_bigint1 != 0)
        r_speed_bytes =
            ((double)reply->_s_bigint1) / ((double)reply->_s_int2) * 1000.0;
    }
    ...
    if (reply->_int1 != 0) {
      ...
      speed_bytes =
          ((double)reply->_s_bigint1) / ((double)reply->_int1) * 1000.0;
    }
    ...
    _log.Log("\n  Assessment time: %s (%.0lf rows/sec, %s, %s/sec)", time_fmt,
             speed_rows, bytes_fmt, speed_bytes_fmt);
...
void SqlDataCmd::CallbackValidationRowCount(SqlDataReply *reply) {
  ...
  if (reply->_cmd_subtype == SQLDATA_CMD_STARTED) {
    _log.Log("Started (%d of %d, session %d)", _validate_table_num,
             _total_tables, reply->session_id);
    _validate_table_num++;
  } else
    ...
    if (reply->_cmd_subtype == SQLDATA_CMD_COMPLETE) {
      ...
      if (reply->_cmd_subtype == SQLDATA_CMD_ALL_COMPLETE) {
        if (_tables_diff == 0)
          ...
          int k = 1;
          for (std::list<std::string>::iterator i = _tables_diff_list.begin();
               i != _tables_diff_list.end(); i++, k++)
            _log.Log("%5d. %s\n", k, i->c_str());
        }
        ...
        _log.Log(
            "  Total number of rows:           %d in source, %d in target\n",
            reply->_s_int1, reply->_t_int1);
  ...
  if (reply->_cmd_subtype == SQLDATA_CMD_STARTED) {
    ...
    if (reply->_cmd_subtype == SQLDATA_CMD_COMPLETE) {
      ...
      if (reply->_s_int1 != 0 && reply->_s_time_spent != 0)
        s_speed_rows =
            ((double)reply->_s_int1) / ((double)reply->_s_time_spent) * 1000.0;

      if (reply->_t_int1 != 0 && reply->_t_time_spent != 0)
        t_speed_rows =
            ((double)reply->_t_int1) / ((double)reply->_t_time_spent) * 1000.0;

      double s_speed_bytes = 0, t_speed_bytes = 0;

      if (reply->_s_bigint1 != 0 && reply->_s_time_spent != 0)
        s_speed_bytes = ((double)reply->_s_bigint1) /
                        ((double)reply->_s_time_spent) * 1000.0;

      if (reply->_t_bigint1 != 0 && reply->_t_time_spent != 0)
        t_speed_bytes = ((double)reply->_t_bigint1) /
                        ((double)reply->_t_time_spent) * 1000.0;
      ...
      if (reply->_t_int1 > 0)
        _log.Log(", %.0lf rows/sec, %s, %s/sec)", t_speed_rows, t_bytes_fmt,
                 t_speed_bytes_fmt);
      else
        _log.Log(")");
      ...
      // Show differences
      if (reply->_int1 != 0)
        _log.Log("\n    Not Equal: %d row%s, %d of %d column%s (%s)",
                 reply->_int1, SUFFIX(reply->_int1), reply->_int2, reply->_int3,
                 SUFFIX(reply->_int3), reply->data2);
...
int SqlDataCmd::Connect() {
  ...
  _log.Log(" (%s)\n  ", time_fmt);
  LogConnection(_source_type, reply._s_rc, reply._s_name,
                reply.s_native_error_text, reply._s_time_spent);

  if (_command != SQLDATA_CMD_ASSESS) {
    _log.Log("\n  ");
    LogConnection(_target_type, reply._t_rc, reply._t_name,
                  reply.t_native_error_text, reply._t_time_spent);
  }
...
int SqlDataCmd::TestConnection(std::string &conn, std::string &error,
                               std::string &loaded_path,
                               std::list<std::string> &search_paths,
                               size_t *time_spent) {
  _sqlData.SetParameters(&_parameters);

  int rc = _sqlData.TestConnection(conn, error, loaded_path, search_paths,
                                   time_spent);
...
void SqlDataCmd::LogConnection(int type, int rc, const char *version,
                               const char *error, size_t time_spent) {
      ...
      if (type == SQLDATA_POSTGRESQL && (version == NULL || *version == '\x0'))
        ...
        if (type == SQLDATA_INFORMIX && (version == NULL || *version == '\x0'))
          ...
          if (type == SQLDATA_DB2 && (version == NULL || *version == '\x0'))
            _log.Log("IBM DB2 ");
          else if (type == SQLDATA_ASA)
            _log.Log("Sybase SQL Anywhere ");
          else if (type == SQLDATA_STDOUT)
...
int SqlDataCmd::ReadMetadata() {
  ...
  if (!_qf.empty()) {
    ...
    _log.Log(" (%d %s)", _total_tables,
             SUFFIX2(_total_tables, "query", "queries"));
  } else
    // Tables
    if (_command != SQLDATA_CMD_TRANSFER_OBJ) {
      // Add all tables to the list
      for (std::list<std::string>::iterator i = avail_tables.begin();
           i != avail_tables.end(); i++)
        _tables.push_back(*i);
      ...
      _log.Log(" (%d table%s, %s)", _total_tables, SUFFIX(_total_tables),
               time_fmt);
      ...
      // Non-table objects
      if (_command == SQLDATA_CMD_TRANSFER_OBJ) {
        _sqlData.CreateObjectQueue(_t, _texcl, &_total_objects,
                                   &_total_object_lines, &_total_object_bytes);
        ...
        _log.Log(" (%d object%s, %d line%s, %s, %s)", _total_objects,
                 SUFFIX(_total_objects), _total_object_lines,
                 SUFFIX(_total_object_lines), bytes_fmt, time_fmt);
      }
...
int SqlDataCmd::GetQueriesFromFile(
    std::string &file, std::map<std::string, std::string> &queries) {
...
int SqlDataCmd::SetParameters() {
  ...
  if (_license.IsLicenseCheckRequired()) {
    if (_license.IsEmpty())
      ...
      _log.Log("\n\nThe product is licensed to %s.",
               _license.GetName().c_str());
  }
  ...
  // Some mandatory parameters must be set, and if not print how to use and
  // exist
  if (_sd.empty() || _td.empty() || _parameters.Get(HELP_PARAMETER) ||
      (_t.empty() && _tf.empty() && _qf.empty())) {
    _log.Log("\n\nCommand line error:\n");
...
void SqlDataCmd::LogSql(SqlDataReply *reply, const char *time_str) {
  ...
  if (first_ddl == true) {
    _ddl_log.LogFile("-- %s - %s.\n-- %s", SQLDATA_VERSION, SQLDATA_DESC,
                     SQLDATA_COPYRIGHT);
    _ddl_log.LogFile(
  ...
  // SQL statement failed
    ...
    if (first_failed == true && drop_not_existent == false) {
      _failed_ddl_log.LogFile("-- %s - %s.\n-- %s", SQLDATA_VERSION,
                              SQLDATA_DESC, SQLDATA_COPYRIGHT);
...
void SqlDataCmd::DefineCommand() {
  ...
  if (_strnicmp(cur, "transfer_obj", 12) == 0)
    _command = SQLDATA_CMD_TRANSFER_OBJ;
  else if (_strnicmp(cur, "transfer", 8) == 0)
    _command = SQLDATA_CMD_TRANSFER;
  else if (_strnicmp(cur, "validate", 8) == 0)
    _command = SQLDATA_CMD_VALIDATE;
  else if (_strnicmp(cur, "assess", 6) == 0)
  ...
  if (_command == SQLDATA_CMD_TRANSFER) {
    if (_topt.empty() == false) {
      ...
      while (*cur) {
        if (_strnicmp(cur, "create", 6) == 0) {
          ...
          cur += 6;
        } else if (_strnicmp(cur, "recreate", 8) == 0) {
          _command_options |= SQLDATA_OPT_DROP;
          _command_options |= SQLDATA_OPT_CREATE;
          cur += 8;
        } else if (_strnicmp(cur, "truncate", 8) == 0) {
          _command_options |= SQLDATA_OPT_TRUNCATE;
          cur += 8;
        } else if (_strnicmp(cur, "none", 4) == 0) {
    ...
    if (_command == SQLDATA_CMD_VALIDATE) {
      ...
      if (_strnicmp(cur, "rowcount", 8) == 0) {
        ...
        cur += 8;
      } else if (_strnicmp(cur, "rows", 4) == 0) {
...
void SqlDataCmd::PrintHowToUse() {
  ...
  printf("\n\n   sqldata.exe -sd=oracle,scott/tiger@orcl "
         "-td=sql,trusted@srv1,hr -t=cities");
  printf("\n");
  printf("\nRun sqldata_w.exe to launch a GUI version of SQLData.\n");
  printf("\nPress any key to continue...\n");
  _getch();
#else
  printf("\n\n   ./sqldata -sd=oracle,scott/tiger@orcl -td=sql,trusted@srv1,hr "
         "-t=cities");
  printf("\n\n");
#endif
}
...
void SqlDataCmd::PrintCurrentTimestamp() { PrintCurrentTimestamp(_log, ""); }
