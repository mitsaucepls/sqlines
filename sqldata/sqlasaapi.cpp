...
int SqlAsaApi::Connect(size_t *time_spent) {
  ...
  // Set ODBC 3 version
  if (rc == SQL_SUCCESS)
    rc = SQLSetEnvAttr(_henv, SQL_ATTR_ODBC_VERSION, (SQLPOINTER)SQL_OV_ODBC3,
                       0);
  ...
  rc = SQLDriverConnect(_hdbc, NULL, (SQLCHAR *)conn.c_str(), SQL_NTS, NULL, 0,
...
int SqlAsaApi::ExecuteScalar(const char *query, int *result,
                             size_t *time_spent) {
  SQLHANDLE stmt;
  int q_result = 0;
...
int SqlAsaApi::OpenCursor(const char *query, size_t buffer_rows,
  ...
  for (int i = 0; i < _cursor_cols_count; i++) {
    ...
    rc = SQLDescribeCol(_hstmt_cursor, (SQLUSMALLINT)(i + 1),
                        (SQLCHAR *)_cursor_cols[i]._name, 256, NULL, &native_dt,
    ...
    if ((native_dt == SQL_CHAR || native_dt == SQL_VARCHAR) && !catalog_query &&
        _char_as_wchar) {
      _cursor_cols[i]._native_dt =
          (native_dt == SQL_CHAR) ? SQL_WCHAR : SQL_WVARCHAR;
    } else
      ...
      if (native_dt == SQL_DECIMAL || native_dt == SQL_NUMERIC) {
        ...
        // LONG VARCHAR and SQL_LONGVARBINARY (size is 2,000,000,000 returned
        // for ASA 9)
        if (native_dt == SQL_LONGVARCHAR || native_dt == SQL_LONGVARBINARY ||
            native_dt == SQL_WLONGVARCHAR) {
          // Fecthed as LOB (CHECK constraints and DEFAULT values defined as
          // LONG VARCHAR in catalog)
          ...
          // TIME data type (fractional precision is always 6, no option to
          // specify other value)
  ...
  for (int i = 0; i < _cursor_cols_count; i++) {
              ...
              if (_cursor_cols[i]._native_dt == SQL_TINYINT) {
                ...
                if (_cursor_cols[i]._native_dt == SQL_BIT) {
                  ...
                  if (_cursor_cols[i]._native_dt == SQL_REAL) {
                    ...
                    // Driver returns 7 as len for FLOAT, it is too short for
                    ...
                    if (_cursor_cols[i]._native_dt == SQL_DOUBLE) {
                      ...
                      // Driver returns 15 as len for DOUBLE, it is too short
                      ...
                      if (_cursor_cols[i]._native_dt == SQL_TYPE_DATE) {
                        ...
                        if (_cursor_cols[i]._native_dt == SQL_TYPE_TIMESTAMP) {
                          ...
                          // TIME (len is 6, bind to string as ODBC TIME_STRUCT
                          // does not support fractions)
                          if (_cursor_cols[i]._native_dt == SQL_TYPE_TIME) {
                            ...
                            if (_cursor_cols[i]._native_dt == SQL_DECIMAL ||
                              ...
                              // Include sign, decimal point and terminating
                              // zero
                              _cursor_cols[i]._fetch_len =
                                  _cursor_cols[i]._len +
    ...
    if (_cursor_cols[i]._data != NULL) {
      ...
      if (_cursor_cols[i]._lob == false) {
        rc = SQLBindCol(_hstmt_cursor, (SQLUSMALLINT)(i + 1),
  ...
  rc = SQLSetStmtAttr(_hstmt_cursor, SQL_ATTR_ROW_ARRAY_SIZE,
                      (SQLPOINTER)_cursor_allocated_rows, 0);

  if (rc == -1)
    SetError(SQL_HANDLE_STMT, _hstmt_cursor);

  rc = SQLSetStmtAttr(_hstmt_cursor, SQL_ATTR_ROWS_FETCHED_PTR,
                      &_cursor_fetched, 0);
...
int SqlAsaApi::GetLobPart(size_t /*row*/, size_t column, void * /*data*/,
                          size_t /*length*/, int * /*len_ind*/) {
  SQLLEN ind = 0;

  int rc = SQLGetData(_hstmt_cursor, (SQLUSMALLINT)(column + 1),
...
int SqlAsaApi::InitBulkTransfer(const char * /*table*/, size_t /*col_count*/,
...
int SqlAsaApi::TransferRows(SqlCol * /*s_cols*/, int /*rows_fetched*/,
...
int SqlAsaApi::CloseBulkTransfer() { return -1; }

// Drop the table
int SqlAsaApi::DropTable(const char *table, size_t *time_spent,
...
int SqlAsaApi::GetLobLength(size_t /*row*/, size_t /*column*/,
                            size_t * /*length*/) {
  return -1;
}

// Get LOB content
int SqlAsaApi::GetLobContent(size_t /*row*/, size_t /*column*/, void * /*data*/,
                             size_t /*length*/, int * /*len_ind*/) {
  return -1;
}

// Get the list of available tables
int SqlAsaApi::GetAvailableTables(std::string &select, std::string &exclude,
                                  std::list<std::string> &tables) {
  std::string condition;

  // Get a condition to select objects from the catalog
  // * means all user tables, *.* means all tables accessible by the user
  GetSelectionCriteria(select.c_str(), exclude.c_str(), "USER_NAME(creator)",
                       "table_name", condition, _user.c_str(), false);

  // Tested on ASA 9
  std::string query = "SELECT USER_NAME(creator), table_name FROM systable "
  ...
  if (strcmp(select.c_str(), "*.*") == 0 ||
      (strcmp(select.c_str(), "*") == 0 && _user.empty()))
...
int SqlAsaApi::ReadSchema(const char *select, const char *exclude,
  ...
  // Get a condition to select objects from the catalog
  // * means all user tables, *.* means all tables accessible by the user
  GetSelectionCriteria(select, exclude, "USER_NAME(t.creator)", "t.table_name",
...
int SqlAsaApi::ReadTableColumns(std::string &condition) {
  ...
  std::string query = "SELECT USER_NAME(t.creator), t.table_name, "
                      "c.column_name, c.column_id, c.table_id,";
  query += " c.pkey, c.\"default\", CASE WHEN c.max_identity < 2000000000 THEN "
           "CAST(c.max_identity AS INT) WHEN c.max_identity > 2000000000 THEN "
           "CAST(1000000 AS INT) ELSE CAST(0 AS INT) END";
  ...
  while (rc >= 0) {
    ...
    for (int i = 0; i < rows_fetched; i++) {
      ...
      if (len != -1) {
        std::string tab;
        std::string name(cols[1]._data + cols[1]._fetch_len * i, 0,
                         (size_t)len);
      ...
      // Y if a primary key column (there is no any information on PK columns
      // order if it distinguishes from column order)
      char pkey =
          (len != -1) ? *(cols[5]._data + cols[5]._fetch_len * i) : '\x0';
...
int SqlAsaApi::ReadTableConstraints(std::string &condition) {
  ...
  query +=
      " c.table_id = t.table_id AND c.constraint_type in ('P', 'U', 'F', 'T') ";

  // Query for ASA 10, 12 and 16 (join by object_id, but table_id is returned
  // since it is used in syscolumn)
  std::string query2 = "SELECT USER_NAME(t.creator), t.table_name, "
                       "c.constraint_name, c.constraint_type,";
  query2 += " cast(t.table_id as int), c.constraint_id, ";
  query2 +=
      "(SELECT index_id FROM sysindex where c.ref_object_id = object_id), ";
  // fkey_id is stored as index_id in sysidx, not sysindex
  query2 += "(SELECT cast(index_id as smallint) FROM sysidx where "
            "c.ref_object_id = object_id) ";
  query2 += " FROM sysconstraint c, systable t WHERE ";
  query2 += " c.table_object_id = t.object_id AND c.constraint_type in ('P', "
            "'U', 'F', 'T') ";
  ...
  if (rc < 0)
    rc = OpenCursor(query2.c_str(), 100, 0, &col_count, &allocated_rows,
                    &rows_fetched, &cols, &time_read, true);
...
int SqlAsaApi::ReadIndexes(std::string &condition) {
  // Tested on ASA 9, index_type is 'U' for UNIQUE constraints, 'Y' for unique
  // indexes, 'N' for non-unique indexes
  std::string query = "SELECT NULL, i.index_name, USER_NAME(t.creator), "
                      "t.table_name, t.table_id, i.index_id,";
  query += " i.\"unique\"";
...
int SqlAsaApi::ReadIndexColumns(std::string &condition) {
  // Tested on ASA 9
  std::string query =
      "SELECT ic.table_id, ic.index_id, ic.column_id, ic.\"order\"";
  ...
  while (rc >= 0) {
    ...
    for (int i = 0; i < rows_fetched; i++) {
      ...
      // Find column name
      for (std::list<SqlColMeta>::iterator i = _table_columns.begin();
           i != _table_columns.end(); i++) {
...
int SqlAsaApi::ReadReferences(std::string &condition) {
  // Tested on ASA 9, 16
  std::string query =
      "SELECT fk.foreign_table_id, fk.foreign_key_id, fk.primary_table_id";
  ...
  while (rc >= 0) {
    ...
    for (int i = 0; i < rows_fetched; i++) {
      ...
      // Now find foreign key constraint and set primary key constraint
      // reference
      for (std::list<SqlConstraints>::iterator i = _table_constraints.begin();
           i != _table_constraints.end(); i++) {
        if ((*i).tabid == fk_tabid && (*i).fkid == fk_keyid &&
            (*i).type == 'R') {
          (*i).r_cnsid = pk_cnsid;
          break;
        }
...
int SqlAsaApi::ReadForeignKeyColumns(std::string &condition) {
  // Tested on ASA 16
  std::string query =
      "SELECT fk.foreign_table_id, fk.foreign_key_id, fk.foreign_column_id";
  query += " FROM sysfkcol fk, systable t, sysidxcol i WHERE";
  query += " fk.foreign_table_id = t.table_id AND fk.foreign_table_id = "
           "i.table_id AND";
  query +=
      " fk.foreign_key_id = i.index_id AND fk.foreign_column_id = i.column_id";
...
int SqlAsaApi::ReadForeignKeyActions() {
  ...
  std::string query =
      "SELECT foreign_table_id, foreign_key_id, event, referential_action";
...
void SqlAsaApi::SetUniqueConstraintColumn(SqlIndColumns &idx_col) {
  ...
  for (std::list<SqlConstraints>::iterator i = _table_constraints.begin();
       i != _table_constraints.end(); i++) {
    if ((*i).type == 'U' && (*i).tabid == idx_col.tabid &&
        (*i).idxid == idx_col.idxid) {
      SqlConsColumns col_cns;
...
// Set IDENTITY column
// Note that CHECKPOINT command maybe required to update max_identity in
// SYSTABCOL
...
int SqlAsaApi::GetKeyConstraintColumns(SqlConstraints &cns,
...
int SqlAsaApi::GetForeignKeyConstraintColumns(SqlConstraints &cns,
...
int SqlAsaApi::GetIndexColumns(SqlIndexes &idx,
...
int SqlAsaApi::ReadConstraintTable(const char * /*schema*/,
...
int SqlAsaApi::ReadConstraintColumns(const char * /*schema*/,
                                     const char * /*table*/,
                                     const char * /*constraint*/,
                                     std::string & /*cols*/) {
  return -1;
...
void SqlAsaApi::SetError(SQLSMALLINT handle_type, SQLHANDLE handle) {
  ...
  *sqlstate = '\x0';
  ...
  SQLGetDiagRec(handle_type, handle, 1, sqlstate, (SQLINTEGER *)&_native_error,
