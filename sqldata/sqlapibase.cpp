...
bool SqlApiBase::IsReservedWord(const char *name) {
  ...
  for (std::list<std::string>::iterator i = _reserved_words_ddl.begin();
       i != _reserved_words_ddl.end(); i++) {
...
SqlConstraints *SqlApiBase::GetConstraint(const char *schema, const char *table,
                                          const char *name) {
...
void SqlApiBase::MapDataType(SqlCol *col, std::list<SqlDataTypeMap> *dtmap) {
  ...
  for (std::list<SqlDataTypeMap>::iterator i = dtmap->begin();
       i != dtmap->end(); i++) {
...
void SqlApiBase::SplitQualifiedName(const char *qualified, std::string &schema,
                                    std::string &object) {
  if (qualified == NULL)
    return;
...
void SqlApiBase::SplitConnectionString(const char *conn, std::string &user,
...
// Build a condition to select objects from the catalog and exclude from
// selection
void SqlApiBase::GetSelectionCriteria(const char *select, const char *exclude,
  ...
  GetSelectionCriteria(exclude, schema, object, excl, default_schema,
                       upper_case);
...
void SqlApiBase::GetSelectionCriteria(const char *selection_template,
...
int SqlApiBase::GetKeyConstraintColumns(SqlConstraints &cns,
                                        std::string &output) {
  ...
  for (std::list<std::string>::iterator i = columns.begin(); i != columns.end();
       i++) {
    if (c > 0)
      output += ", ";
