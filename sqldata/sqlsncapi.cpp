...
SqlSncApi::~SqlSncApi() {
  // When other ODBC drivers are used crash can happen on exit when not
  // disconnected and not deallocated
  Deallocate();
}
...
int SqlSncApi::Init() {
  ...
  _dll_odbc =
      LoadLibraryEx(SQLSRV_ODBC_DLL, NULL, LOAD_WITH_ALTERED_SEARCH_PATH);
  ...
  if (_dll_odbc != NULL) {
    _SQLAllocHandle =
        (SQLAllocHandleFunc)Os::GetProcAddress(_dll_odbc, "SQLAllocHandle");
    _SQLBindCol = (SQLBindColFunc)Os::GetProcAddress(_dll_odbc, "SQLBindCol");
    _SQLDescribeCol =
        (SQLDescribeColFunc)Os::GetProcAddress(_dll_odbc, "SQLDescribeCol");
    _SQLDisconnect =
        (SQLDisconnectFunc)Os::GetProcAddress(_dll_odbc, "SQLDisconnect");
    _SQLDriverConnect =
        (SQLDriverConnectFunc)Os::GetProcAddress(_dll_odbc, "SQLDriverConnect");
    _SQLExecDirect =
        (SQLExecDirectFunc)Os::GetProcAddress(_dll_odbc, "SQLExecDirect");
    _SQLFetch = (SQLFetchFunc)Os::GetProcAddress(_dll_odbc, "SQLFetch");
    _SQLFreeHandle =
        (SQLFreeHandleFunc)Os::GetProcAddress(_dll_odbc, "SQLFreeHandle");
    _SQLGetData = (SQLGetDataFunc)Os::GetProcAddress(_dll_odbc, "SQLGetData");
    _SQLGetDiagRec =
        (SQLGetDiagRecFunc)Os::GetProcAddress(_dll_odbc, "SQLGetDiagRec");
    _SQLNumResultCols =
        (SQLNumResultColsFunc)Os::GetProcAddress(_dll_odbc, "SQLNumResultCols");
    _SQLSetConnectAttr = (SQLSetConnectAttrFunc)Os::GetProcAddress(
        _dll_odbc, "SQLSetConnectAttr");
    _SQLSetEnvAttr =
        (SQLSetEnvAttrFunc)Os::GetProcAddress(_dll_odbc, "SQLSetEnvAttr");
    _SQLSetStmtAttr =
        (SQLSetStmtAttrFunc)Os::GetProcAddress(_dll_odbc, "SQLSetStmtAttr");
    ...
    if (_dll != NULL) {
      ...
      _bcp_moretext =
          (bcp_moretextFunc)Os::GetProcAddress(_dll, "bcp_moretext");
      _bcp_sendrow = (bcp_sendrowFunc)Os::GetProcAddress(_dll, "bcp_sendrow");
    }

    if (_SQLAllocHandle == NULL || _SQLBindCol == NULL ||
        _SQLDescribeCol == NULL || _SQLDisconnect == NULL ||
        _SQLDriverConnect == NULL || _SQLExecDirect == NULL ||
        _SQLFetch == NULL || _SQLFreeHandle == NULL || _SQLGetData == NULL ||
        _SQLGetDiagRec == NULL || _SQLSetConnectAttr == NULL ||
        _SQLSetEnvAttr == NULL || _SQLSetStmtAttr == NULL ||
        _bcp_batch == NULL || _bcp_bind == NULL || _bcp_control == NULL ||
        _bcp_init == NULL || _bcp_moretext == NULL || _bcp_sendrow == NULL)
      return -1;
  } else {
  ...
  if (_parameters != NULL)
    _bcp_codepage =
        _parameters->GetInt("-bcp_codepage", -2); // -1 is used by BCPFILECP_RAW
...
int SqlSncApi::Connect(size_t *time_spent) {
  ...
  // Set ODBC 3 version
  if (rc == SQL_SUCCESS)
    rc = _SQLSetEnvAttr(_henv, SQL_ATTR_ODBC_VERSION, (SQLPOINTER)SQL_OV_ODBC3,
                        0);
  ...
  rc = _SQLSetConnectAttr(_hdbc, SQL_COPT_SS_BCP, (SQLPOINTER)SQL_BCP_ON,
                          SQL_IS_INTEGER);

  // Set the size of network packets (rc = 1 with message Package Size changed)
  rc = _SQLSetConnectAttr(_hdbc, SQL_ATTR_PACKET_SIZE, (SQLPOINTER)(1024 * 128),
                          0);
  ...
  // Connection string must not contain spaces between parameters, =, values
  // etc.
  rc = _SQLDriverConnect(_hdbc, NULL, (SQLCHAR *)conn.c_str(), SQL_NTS, NULL, 0,
...
int SqlSncApi::ExecuteScalar(const char *query, int *result,
                             size_t *time_spent) {
  SQLHANDLE stmt;
  int q_result = 0;
...
int SqlSncApi::OpenCursor(const char *query, size_t buffer_rows,
  ...
  for (size_t i = 0; i < _cursor_cols_count; i++) {
    ...
    rc = _SQLDescribeCol(_hstmt_cursor, (SQLUSMALLINT)(i + 1),
                         (SQLCHAR *)_cursor_cols[i]._name, 256, NULL,
                         &native_dt, &column_size, &decimal_digits, &nullable);

    _cursor_cols[i]._native_dt = native_dt;
    _cursor_cols[i]._len = column_size;

    // VARCHAR(MAX), NVARCHAR(MAX) has 0 length
    if ((native_dt == SQL_VARCHAR || native_dt == SQL_WVARCHAR ||
         native_dt == SQL_VARBINARY) &&
        column_size == 0) {
      ...
      if (native_dt == SQL_DECIMAL || native_dt == SQL_NUMERIC) {
        ...
        if (native_dt == SQL_LONGVARCHAR || native_dt == SQL_LONGVARBINARY ||
            native_dt == SQL_WLONGVARCHAR) {
          // Do not bind LOB column, read LOB value in chunks
          if (_lob_bind_buffer == 0) {
            // Change 2G column size to estimated average size (chunk size of 3
            // MB)
  ...
  if (buffer_rows > 0) {
    // Row length can be large for bound LOB buffers so inforce hard limit of
    // buffer size
  ...
  for (size_t i = 0; i < _cursor_cols_count; i++) {
    ...
    if (_cursor_cols[i]._native_dt == SQL_CHAR ||
        _cursor_cols[i]._native_dt == SQL_VARCHAR) {
      ...
      if (_cursor_cols[i]._native_dt == SQL_WCHAR ||
          _cursor_cols[i]._native_dt == SQL_WVARCHAR) {
        // MySQL does not support UCS2 in LOAD DATA INFILE, let's convert to
        // ASCII; also convert to ASCII for target Oracle
        if (_target_api_provider != NULL &&
            (_target_api_provider->GetType() == SQLDATA_MYSQL ||
             _target_api_provider->GetType() == SQLDATA_ORACLE)) {
          _cursor_cols[i]._native_fetch_dt = SQL_C_CHAR;
          _cursor_cols[i]._fetch_len = _cursor_cols[i]._len + 1;
          _cursor_cols[i]._data =
              new char[_cursor_cols[i]._fetch_len * _cursor_allocated_rows];
          ...
          _cursor_cols[i]._fetch_len =
              _cursor_cols[i]._len * sizeof(wchar_t) + 2 /* U+0000 */;
          _cursor_cols[i]._data =
              new char[_cursor_cols[i]._fetch_len * _cursor_allocated_rows];
        }
      } else
        ...
        if (_cursor_cols[i]._native_dt == SQL_BIGINT) {
          ...
          if (_cursor_cols[i]._native_dt == SQL_WCHAR ||
            ...
            if (_cursor_cols[i]._native_dt == SQL_INTEGER) {
              // Check whether target supports bind to INT or string has to be
              // used
              if ((_target_api_provider != NULL &&
                   _target_api_provider->IsIntegerBulkBindSupported() ==
                       true) ||
                  catalog_query == true) {
              ...
              if (_cursor_cols[i]._native_dt == SQL_SMALLINT) {
                // Check whether target supports bind to SMALLINT or string has
                // to be used
                if ((_target_api_provider != NULL &&
                     _target_api_provider->IsSmallintBulkBindSupported() ==
                         true) ||
                    catalog_query == true) {
                ...
                if (_cursor_cols[i]._native_dt == SQL_TINYINT) {
                  ...
                  if (_cursor_cols[i]._native_dt == SQL_DECIMAL ||
                      _cursor_cols[i]._native_dt == SQL_NUMERIC) {
                    _cursor_cols[i]._native_fetch_dt = SQL_C_CHAR;

                    // Include sign, decimal point and terminating zero
                    _cursor_cols[i]._fetch_len =
                        _cursor_cols[i]._len + _cursor_cols[i]._scale + 3;
                    _cursor_cols[i]._data =
                    ...
                    if (_cursor_cols[i]._native_dt == SQL_FLOAT) {
                      ...
                      // Driver returns column len 53 for FLOAT without
                      // parameters
                      _cursor_cols[i]._fetch_len = _cursor_cols[i]._len + 1;
                      _cursor_cols[i]._data =
                          new char[_cursor_cols[i]._fetch_len *
                      ...
                      if (_cursor_cols[i]._native_dt == SQL_REAL) {
                        ...
                        if (_cursor_cols[i]._native_dt == SQL_TYPE_DATE) {
                          ...
                          // TIMESTAMP
                          if (_cursor_cols[i]._native_dt ==
                              SQL_TYPE_TIMESTAMP) {
                            // Check whether target supports bind to TIMESTAMP
                            // or string has to be used
                            if (_target_api_provider != NULL &&
                                _target_api_provider
                              ...
                              // SMALLDATETIME len is 16 (YYYY-MM-DD HH24:MI
                              // with spaces), but bind to 16 causes truncation
                              // errors, add :SS (seconds always zero)
                              if (_cursor_cols[i]._len == 16)
                                _cursor_cols[i]._fetch_len = 19 + 1;
                              else
                                _cursor_cols[i]._fetch_len =
                                    _cursor_cols[i]._len + 1;
                            }
                                ...
                                if (_cursor_cols[i]._native_dt == SQL_BIT) {
                                  ...
                                  if (_cursor_cols[i]._native_dt ==
                                    ...
                                    if (_cursor_cols[i]._native_dt ==
                                      ...
                                      // Will not bound if fecthed as LOB, but
                                      // still allocate the space for the chunk
                                      _cursor_cols[i]._fetch_len =
                                          _cursor_cols[i]._len;
                                      _cursor_cols[i]._data =
                                          new char[_cursor_cols[i]._fetch_len];
    ...
    if (_cursor_cols[i]._data != NULL) {
      ...
      if (_cursor_cols[i]._lob == false) {
        rc = _SQLBindCol(_hstmt_cursor, SQLUSMALLINT(i + 1),
                         (SQLSMALLINT)_cursor_cols[i]._native_fetch_dt,
                         _cursor_cols[i]._data,
                         (SQLLEN)_cursor_cols[i]._fetch_len,
  ...
  // Prepare array fetch
  rc = _SQLSetStmtAttr(_hstmt_cursor, SQL_ATTR_ROW_ARRAY_SIZE,
                       (SQLPOINTER)_cursor_allocated_rows, 0);
  rc = _SQLSetStmtAttr(_hstmt_cursor, SQL_ATTR_ROWS_FETCHED_PTR,
                       &_cursor_fetched, 0);

  // Perform initial fetch, return SQL_SUCCESS even if there are less rows than
  // array size
  rc = Fetch(NULL, NULL);
...
int SqlSncApi::GetLobPart(size_t /*row*/, size_t column, void * /*data*/,
                          size_t /*length*/, int * /*len_ind*/) {
  SQLLEN ind = 0;

  int rc = _SQLGetData(_hstmt_cursor, (SQLUSMALLINT)(column + 1),
                       (SQLSMALLINT)_cursor_cols[column]._native_fetch_dt,
                       _cursor_cols[column]._data,
                       (SQLLEN)_cursor_cols[column]._fetch_len, &ind);
  ...
  if (ind == -1)
    ...
    if (_cursor_cols[column]._native_dt == SQL_LONGVARCHAR ||
        _cursor_cols[column]._native_dt == SQL_VARCHAR) {
      // ind contains the full length if the buffer is smaller than LOB (tested
      // on ASA 9)
      ...
      if (_cursor_cols[column]._native_dt == SQL_WLONGVARCHAR ||
          _cursor_cols[column]._native_dt == SQL_WVARCHAR) {
        if (ind >= (SQLLEN)_cursor_cols[column]._fetch_len)
          _cursor_cols[column].ind[0] = _cursor_cols[column]._fetch_len - 1;
        // For last chunk u0000 not included (!) to ind (tested on ASA 16)
        else
          _cursor_cols[column].ind[0] = (size_t)ind;
      } else if (_cursor_cols[column]._native_dt == SQL_LONGVARBINARY ||
                 _cursor_cols[column]._native_dt == SQL_VARBINARY) {
        if (ind >= (SQLLEN)_cursor_cols[column]._fetch_len)
...
int SqlSncApi::InitBulkTransfer(const char *table, size_t col_count,
  ...
  // By default Windows locale defines input data codepage that can cause
  // problems when Windows and table locale does not match For some reason
  // specifying BCPFILECP_RAW (-1) still causes conversion and cannot be used as
  // default
  if (_bcp_codepage != -2) {
  ...
  for (int i = 0; i < col_count; i++) {
    ...
    if ((_source_api_type == SQLDATA_SYBASE &&
        ...
        (_source_api_type == SQLDATA_MYSQL &&
         s_cols[i]._native_fetch_dt != MYSQL_TYPE_BLOB)) {
      _bcp_cols[i]._native_dt = SQLCHARACTER;
    } else
      // SQL Server, Sybase ASA NCHAR, NVARCHAR less than 8000 bytes (4000
      // characters), DB2 GRAPHIC and VARGRAPHIC
      if ((_source_api_type == SQLDATA_SQL_SERVER ||
        ...
        // Oracle VARCHAR2, TIMESTAMP or NUMBER/NUMBER(p,s) where s != 0 mapped
        // to string
        if ((_source_api_type == SQLDATA_ORACLE &&
             s_cols[i]._native_fetch_dt == SQLT_STR) ||
          ...
          if ((_source_api_type == SQLDATA_ORACLE &&
               s_cols[i]._native_fetch_dt == SQLT_INT) ||
              // Sybase ASE INT
            ...
            // SMALLINT
            if ((_source_api_type == SQLDATA_SYBASE &&
                 s_cols[i]._native_fetch_dt == CS_SMALLINT_TYPE) ||
              ...
              if (_source_api_type == SQLDATA_ORACLE &&
                  s_cols[i]._native_fetch_dt == SQLT_DAT) {
                _bcp_cols[i]._native_dt = SQLCHARACTER;

                // Allocate also space for terminating 0
                _bcp_cols[i]._len = 19 + 1;
              } else
                // TIMESTAMP fetched as SQL_TIMESTAMP_STRUCT to SQLCHARACTER 26
                // bytes (yyyy-mm-dd hh:mi:ss.ffffff)
                if ((_source_api_type == SQLDATA_SQL_SERVER ||
                     _source_api_type == SQLDATA_DB2 ||
                     _source_api_type == SQLDATA_INFORMIX ||
                     _source_api_type == SQLDATA_ASA ||
                     _source_api_type == SQLDATA_ODBC) &&
                    s_cols[i]._native_fetch_dt == SQL_C_TYPE_TIMESTAMP) {
                  _bcp_cols[i]._native_dt = SQLCHARACTER;
                  ...
                  if (_source_api_type == SQLDATA_ORACLE &&
                      s_cols[i]._native_fetch_dt == SQLT_BIN) {
                    _bcp_cols[i]._native_dt = SQLVARBINARY;
                  } else
                    // ODBC FLOAT
                    if (_source_api_type == SQLDATA_ODBC &&
                        s_cols[i]._native_fetch_dt == SQL_C_FLOAT) {
                      _bcp_cols[i]._native_dt = SQLFLT4;
                    } else
                      // Map BLOB and CLOB to use bcp_moretext
                      if ((_source_api_type == SQLDATA_ORACLE &&
                           (s_cols[i]._native_dt == SQLT_BLOB ||
                            s_cols[i]._native_dt == SQLT_CLOB)) ||
                          ((_source_api_type == SQLDATA_SQL_SERVER ||
                            _source_api_type == SQLDATA_INFORMIX ||
                            _source_api_type == SQLDATA_ASA ||
                            _source_api_type == SQLDATA_ODBC) &&
                          ...
                          (_source_api_type == SQLDATA_SYBASE &&
                           s_cols[i]._native_fetch_dt == CS_CHAR_TYPE &&
                           s_cols[i]._fetch_len > 8000) ||
                          // SQL Server VARCHAR(max) and NVARCHAR(max), DB2
                          // CLOB, XML
                          ((_source_api_type == SQLDATA_SQL_SERVER ||
                            _source_api_type == SQLDATA_DB2) &&
                           (s_cols[i]._native_fetch_dt == SQL_C_CHAR ||
                          ...
                          ((_source_api_type == SQLDATA_SQL_SERVER ||
                           ...
                           s_cols[i]._fetch_len > 8000) ||
                          // Sybase ASA CHAR/VARCHAR, NCHAR/NVARCHAR greater
                          // than 8000 bytes
                          (_source_api_type == SQLDATA_ASA &&
                           (s_cols[i]._native_fetch_dt == SQL_C_CHAR ||
                            s_cols[i]._native_fetch_dt == SQL_C_WCHAR) &&
                        ...
                        if ((_source_api_type == SQLDATA_ORACLE &&
                          ...
                          if ((_source_api_type == SQLDATA_SQL_SERVER ||
                            ...
                            if (_source_api_type == SQLDATA_DB2 &&
                                s_cols[i]._native_dt == -370)
                              _bcp_cols[i]._native_dt = SQLXML;

                        _bcp_cols[i]._len = 0;

                        // Allocate space for indicators to store source LOB
                        // values
                        ...
                        rc = _bcp_bind(_hdbc, NULL, 0, SQL_VARLEN_DATA, NULL, 0,
                                       _bcp_cols[i]._native_dt, i + 1);
    ...
    // Allocating space for 1 item in buffer and the null/length indicator that
    // goes before data
    if (_bcp_cols[i]._len > 0) {
      _bcp_cols[i]._data = new char[_bcp_cols[i]._len + 4];

      // Bind the data buffer
      rc = _bcp_bind(_hdbc, (LPCBYTE)_bcp_cols[i]._data, 4,
                     (DBINT)_bcp_cols[i]._len, NULL, 0, _bcp_cols[i]._native_dt,
                     i + 1);
...
int SqlSncApi::TransferRows(SqlCol *s_cols, int rows_fetched, int *rows_written,
                            size_t *bytes_written, size_t *time_spent) {
  if (s_cols == NULL || _bcp_cols == NULL || _bcp_cols_count <= 0 ||
      rows_fetched < 0)
    return -1;
  ...
  for (size_t i = 0; i < rows_fetched; i++) {
    ...
    for (size_t k = 0; k < _bcp_cols_count; k++) {
      ...
      *ind = SQL_NULL_DATA;
        ...
        if (_source_api_type == SQLDATA_SYBASE && s_cols[k]._ind2 != NULL) {
          ...
          if ((_source_api_type == SQLDATA_SQL_SERVER ||
            ...
            *ind = (SQLINTEGER)s_cols[k].ind[i];
            ...
            if (_source_api_type == SQLDATA_DB2 &&
                s_cols[k].ind[0] & 0xFFFFFFFF00000000)
      ...
      if ((_source_api_type == SQLDATA_ORACLE &&
           s_cols[k]._native_dt != SQLT_BLOB &&
           s_cols[k]._native_dt != SQLT_CLOB) ||
          (_source_api_type == SQLDATA_SYBASE &&
           s_cols[k]._native_dt != CS_TEXT_TYPE) ||
          _source_api_type == SQLDATA_INFORMIX ||
          _source_api_type == SQLDATA_DB2 ||
          _source_api_type == SQLDATA_MYSQL ||
      ...
      // Oracle VARCHAR2, RAW
      if ((_source_api_type == SQLDATA_ORACLE &&
           (s_cols[k]._native_fetch_dt == SQLT_STR ||
            s_cols[k]._native_fetch_dt == SQLT_BIN)) ||
          // Sybase CHAR
          (_source_api_type == SQLDATA_SYBASE &&
           s_cols[k]._native_fetch_dt == CS_CHAR_TYPE &&
           s_cols[k]._fetch_len <= 8000) ||
          // MySQL all data types except BLOB
          (_source_api_type == SQLDATA_MYSQL &&
           s_cols[k]._native_fetch_dt != MYSQL_TYPE_BLOB) ||
          // Informix, Sybase ASA, ODBC CHAR/VARCHAR and other data types
          // fetched as string
          ((_source_api_type == SQLDATA_SQL_SERVER ||
            _source_api_type == SQLDATA_DB2 ||
            _source_api_type == SQLDATA_INFORMIX ||
          ...
          ((_source_api_type == SQLDATA_INFORMIX ||
           ...
           s_cols[k]._lob == false)) {
        memcpy(_bcp_cols[k]._data + 4,
               s_cols[k]._data + s_cols[k]._fetch_len * i, (size_t)*ind);
      } else
        // Oracle NUMBER fetched as INT
        if ((_source_api_type == SQLDATA_ORACLE &&
             s_cols[k]._native_fetch_dt == SQLT_INT) ||
            // Sybase ASE INT
            (_source_api_type == SQLDATA_SYBASE &&
             s_cols[k]._native_fetch_dt == CS_INT_TYPE) ||
          ...
          // Sybase ASE SMALLINT
          if ((_source_api_type == SQLDATA_SYBASE &&
               s_cols[k]._native_fetch_dt == CS_SMALLINT_TYPE) ||
            ...
            // ODBC FLOAT
            if (_source_api_type == SQLDATA_ODBC &&
                s_cols[k]._native_fetch_dt == SQL_C_FLOAT) {
              ...
              if (_source_api_type == SQLDATA_ORACLE &&
                  s_cols[k]._native_fetch_dt == SQLT_DAT) {
                *ind = 19;

                // Unsigned required for proper conversion to int
                unsigned char *data =
                    (unsigned char *)(s_cols[k]._data +
                                      s_cols[k]._fetch_len * i);
                ...
                if ((_source_api_type == SQLDATA_SQL_SERVER ||
                  ...
                  *ind = 26;
                  ...
                  // Informix returns '1200-01-01 11:11:00.0' for 11:11 (HOUR TO
                  // MINUTE)
                  ...
                  // In ODBC, fraction is stored in nanoseconds, but now we
                  // support only microseconds
                  fraction = fraction / 1000;

                  // Convert SQL_TIMESTAMP_STRUCT to string
                  Str::SqlTs2Str((short)ts->year, (short)ts->month,
                                 (short)ts->day, (short)ts->hour,
                                 (short)ts->minute, (short)ts->second, fraction,
                                 _bcp_cols[k]._data + 4);
                }
    }
...
int SqlSncApi::WriteLob(SqlCol *s_cols, size_t row, size_t *lob_bytes) {
  if (s_cols == NULL || _bcp_lob_exists == false ||
  ...
  for (size_t k = 0; k < _bcp_cols_count; k++) {
    ...
    if (_source_api_type == SQLDATA_ORACLE) {
      ...
      if (_source_api_type == SQLDATA_SYBASE) {
        ...
        rc = _bcp_moretext(_hdbc, (DBINT)len,
                           (LPCBYTE)s_cols[k]._data +
                               s_cols[k]._fetch_len * row);
        row_len += len;
      } else if (_source_api_type == SQLDATA_SQL_SERVER ||
                 _source_api_type == SQLDATA_DB2 ||
                 _source_api_type == SQLDATA_INFORMIX ||
                 _source_api_type == SQLDATA_ASA ||
                 _source_api_type == SQLDATA_ODBC) {
        // First chunk already read, BCP indicator is 4-byte on both 32-bit and
        // 64-bit platforms
        ...
        while (more) {
          ...
          rc = _bcp_moretext(
              _hdbc, (DBINT)len,
              (LPCBYTE)(s_cols[k]._data + s_cols[k]._fetch_len * row));
      ...
      } else if (_source_api_type == SQLDATA_MYSQL) {
...
// Drop the table
int SqlSncApi::DropTable(const char *table, size_t *time_spent,
...
int SqlSncApi::DropReferences(const char *table, size_t *time_spent) {
  ...
  // NOLOCK is used to prevent "[Microsoft][SQL Server Native Client 11.0][SQL
  // Server]Transaction (Process ID 54) was deadlocked on lock resources with
  // another process and has been chosen as the deadlock victim. Rerun the
  // transaction."
  std::string drop =
      "BEGIN "
      "DECLARE @stmt VARCHAR(300);"
      "DECLARE cur CURSOR FOR SELECT 'ALTER TABLE [' + "
...
int SqlSncApi::GetLobLength(size_t /*row*/, size_t /*column*/,
                            size_t * /*length*/) {
  return -1;
}

// Get LOB content
int SqlSncApi::GetLobContent(size_t /*row*/, size_t /*column*/, void * /*data*/,
                             size_t /*length*/, int * /*len_ind*/) {
  return -1;
}

// Get the list of available tables
int SqlSncApi::GetAvailableTables(std::string &table_template,
                                  std::string & /*exclude*/,
                                  std::list<std::string> &tables) {
  std::string condition, condition2;

  // Get a condition to select objects from the catalog
  GetSelectionCriteria(table_template.c_str(), "SCHEMA_NAME(uid)", "name",
                       condition, NULL, false);
  GetSelectionCriteria(table_template.c_str(), "USER_NAME(uid)", "name",
                       condition2, NULL, false);

  // Build the query (sys.objects is available since 2005, sysobject is
  // available in 2000-2012 but will be removed SCHEMA_NAME() is not available
  // in SQL Server 2000 USER_NAME() is available in all versions but does not
  // return schema name (!), so it returns NULL for a table in a schema other
  // than dbo
  std::string query =
      "SELECT SCHEMA_NAME(uid), name FROM sysobjects WHERE type = 'U'";
  std::string query2 =
...
int SqlSncApi::ReadSchema(const char *select, const char * /*exclude*/,
...
int SqlSncApi::ReadIndexes(const char *select) {
  ...
  GetSelectionCriteria(select, "USER_NAME(o.uid)", "o.name", condition, NULL,
                       false);

  // Indexes except PK and UNIQUE constraint indexes, tested on SQL Server 2008,
  // 2000
  std::string query = "SELECT NULL, i.name, USER_NAME(o.uid), o.name, o.id, "
                      "CAST(i.indid AS INT),";
  query += " CASE INDEXPROPERTY(i.id,i.name,'IsUnique') WHEN 1 THEN 'Y' ELSE "
           "'N' END";
  query += " FROM sysindexes i, sysobjects o";
  query += " WHERE i.id = o.id AND i.name NOT IN (SELECT name FROM sysobjects "
           "where parent_obj = i.id and xtype in ('PK','UQ'))";

  // SQL Server 2000 return indexes on system tables
  query += " AND o.name NOT IN "
           "('syscolumns','syscomments','sysdepends','sysfiles','sysfiles1',";
  query += "'sysforeignkeys','sysfulltextcatalogs','sysfulltextnotify','"
           "sysindexes','sysindexkeys',";
  query += "'sysmembers','sysobjects','syspermissions','sysproperties','"
           "sysprotects','sysreferences',";
  query += "'systypes','sysusers')";

  // skip indid=0, this is not index, just a record for table itself (heap
  // index?), index name is NULL in SQL Server in this case; index name = table
  // name in SQL Server 2000
  query += " AND indid > 0";

  // SQL Server 2008 returns indexes on system objects in sys schema, skip them
  // as well
  ...
  int rc = OpenCursor(query.c_str(), 100, 0, &col_count, &allocated_rows,
                      &rows_fetched, &cols, &time_read, true);
  ...
  while (rc >= 0) {
    ...
    for (int i = 0; i < rows_fetched; i++) {
      ...
      // sysindex does not includes indexes on primary keys; indexes on UNIQUE
      // constraints were excluded in the query
  ...
  while (rc >= 0) {
    ...
    for (int i = 0; i < rows_fetched; i++) {
      ...
      if (len != -1) {
        ...
        strncpy(idx_col.column, cols[2]._data + cols[2]._fetch_len * i,
                (size_t)len);
...
int SqlSncApi::GetIndexColumns(SqlIndexes &idx,
...
int SqlSncApi::ReadConstraintTable(const char * /*schema*/,
...
int SqlSncApi::ReadConstraintColumns(const char * /*schema*/,
                                     const char * /*table*/,
                                     const char * /*constraint*/,
                                     std::string & /*cols*/) {
  return -1;
...
void SqlSncApi::GetCatalogSelectionCriteria(
    std::string & /*selection_template*/, std::string & /*output*/) {}
...
void SqlSncApi::DefineDriver(std::string &driver, std::string &dll) {
  ...
  // Keys: SQL Server, SQL Server Native Client 10.0, SQL Server Native
  // Client 11.0
  int rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\ODBC\\ODBCINST.INI", 0,
  ...
  // Return the highest available version (this driver is also used for 2014 and
  // 2016)
  if (sql2012) {
...
void SqlSncApi::SetVersion() {
  ...
  // Microsoft SQL Server 2008 R2 (RTM) - 10.50.1617.0 (Intel X86) Apr 22 2011
  // 11:57:00 Copyright (c) Microsoft Corporation Express Edition with Advanced
  // Services on Windows NT 6.1 <X86> (Build 7600: ) Microsoft SQL Server 2014
  // (SP2) (KB3171021) - 12.0.5000.0 (X64) Jun 17 2016 19:14:09 Copyright (c)
  // Microsoft Corporation	Express Edition (64-bit) on Windows NT 6.3 <X64>
  // (Build 10586: )
...
void SqlSncApi::SetError(SQLSMALLINT handle_type, SQLHANDLE handle) {
  ...
  *sqlstate = '\x0';
  ...
  _SQLGetDiagRec(handle_type, handle, 1, sqlstate, (SQLINTEGER *)&_native_error,
                 (SQLCHAR *)_native_error_text, 1024, NULL);
  ...
  *_error_text = '\x0';
  ...
  if (_native_error == 208)
    ...
    // Cannot find the object "%.*ls" because it does not exist or you do not
    // have permissions. (Returned by TRUNCATE TABLE for non-existing table)
    if (_native_error == 4701)
      _error = SQL_DBAPI_NOT_EXISTS_ERROR;
    else
      // Cannot %S_MSG the %S_MSG '%.*ls', because it does not exist or you do
      // not have permission (Returned by DROP TABLE).
