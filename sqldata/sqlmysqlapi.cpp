...
mysql_set_local_infile_handlerFunc
    SqlMysqlApi::_mysql_set_local_infile_handler = NULL;
...
SqlMysqlApi::~SqlMysqlApi() { Deallocate(); }

// Initialize API for process (MySQL C library can be compiled as non
// thread-safe)
int SqlMysqlApi::InitStatic() {
  ...
  if (_dll != NULL) {
    ...
    _mysql_fetch_fields =
        (mysql_fetch_fieldsFunc)Os::GetProcAddress(_dll, "mysql_fetch_fields");
    _mysql_fetch_row =
        (mysql_fetch_rowFunc)Os::GetProcAddress(_dll, "mysql_fetch_row");
    _mysql_fetch_lengths = (mysql_fetch_lengthsFunc)Os::GetProcAddress(
        _dll, "mysql_fetch_lengths");
    _mysql_free_result =
        (mysql_free_resultFunc)Os::GetProcAddress(_dll, "mysql_free_result");
    _mysql_num_fields =
        (mysql_num_fieldsFunc)Os::GetProcAddress(_dll, "mysql_num_fields");
    _mysql_options =
        (mysql_optionsFunc)Os::GetProcAddress(_dll, "mysql_options");
    _mysql_real_connect =
        (mysql_real_connectFunc)Os::GetProcAddress(_dll, "mysql_real_connect");
    _mysql_query = (mysql_queryFunc)Os::GetProcAddress(_dll, "mysql_query");
    _mysql_set_character_set = (mysql_set_character_setFunc)Os::GetProcAddress(
        _dll, "mysql_set_character_set");
    _mysql_set_local_infile_handler =
        (mysql_set_local_infile_handlerFunc)Os::GetProcAddress(
            _dll, "mysql_set_local_infile_handler");
    _mysql_thread_init =
        (mysql_thread_initFunc)Os::GetProcAddress(_dll, "mysql_thread_init");
    _mysql_use_result =
        (mysql_use_resultFunc)Os::GetProcAddress(_dll, "mysql_use_result");

    // mysql_set_character_set is available since 5.0.7, check for NULL when use

    if (_mysql_init == NULL || _mysql_close == NULL || _mysql_errno == NULL ||
        _mysql_error == NULL || _mysql_fetch_fields == NULL ||
        _mysql_fetch_row == NULL || _mysql_fetch_lengths == NULL ||
        _mysql_free_result == NULL || _mysql_num_fields == NULL ||
        _mysql_options == NULL || _mysql_real_connect == NULL ||
  ...
  // mysql_library_init is not exported DLL function it is define for
  // mysql_server_init but by calling mysql_init we force mysql_library_init()
  // call to initialize the library for the process Must be called before
  // threads are launched, otherwise the application crashes on some systems
  if (_mysql_init != NULL) {
...
int SqlMysqlApi::Connect(size_t *time_spent) {
  ...
  // Connect is called in a new thread so we must call mysql_init again but now
  // for the thread (calls mysql_thread_init) If not mysqk_init not called in
  // each thread working with MySQL, the application crashes
  MYSQL *mysql = _mysql_init(&_mysql);

  if (mysql == NULL)
    return -1;

  mysql = _mysql_real_connect(&_mysql, _server.c_str(), _user.c_str(),
                              _pwd.c_str(), _db.c_str(), (unsigned int)_port,
  ...
  _mysql_set_local_infile_handler(&_mysql, local_infile_initS,
                                  local_infile_readS, local_infile_endS,
...
int SqlMysqlApi::InitGlobalOptions() {
  ...
  if (max_allowed_packet != NULL) {
    ...
    if (rc != -1) {
      rc = ExecuteNonQuery(std::string("SET GLOBAL max_allowed_packet=")
                               .append(max_allowed_packet)
                               .c_str(),
                           NULL);
      Disconnect();
    }
...
int SqlMysqlApi::InitSession() {
  ...
  // Set INNODB_STATS_ON_METADATA=0 to significantly speed up access to
  // INFORMATION_SCHEMA views (about 5-10 times faster from 5-10 sec to 0.5-1
  // sec). But this requires SUPER privilege Available since 5.1.17
  ...
  if (_subtype == SQLDATA_SUBTYPE_MARIADB) {
    ...
    if (Str::IsSet(value))
      rc = ExecuteNonQuery(
          std::string("SET sql_log_bin=").append(value).c_str(), NULL);
  } else
  ...
  if (_subtype == SQLDATA_SUBTYPE_MARIADB) {
    ...
    if (Str::IsSet(value))
      rc = ExecuteNonQuery(
          std::string("SET unique_checks=").append(value).c_str(), NULL);
  }

  // If AUTOCOMMIT is set to 0 in the database or client side, LOAD DATA INFILE
  // requires a COMMIT statement to put data to the table, so we force
  // AUTOCOMMIT to 1 for connection
  rc = ExecuteNonQuery("SET autocommit=1", NULL);

  // Reset initialization error text so it will not be later associated with the
  // data transfer
...
void SqlMysqlApi::Deallocate() { Disconnect(); }

// Get row count for the specified object
int SqlMysqlApi::GetRowCount(const char *object, int *count,
                             size_t *time_spent) {
  if (object == NULL)
    return -1;
...
int SqlMysqlApi::ExecuteScalar(const char *query, int *result,
                               size_t *time_spent) {
...
int SqlMysqlApi::OpenCursor(const char *query, size_t buffer_rows,
  ...
  for (int i = 0; i < _cursor_cols_count; i++) {
    ...
    // DECIMAL or NUMERIC
    if (_cursor_cols[i]._native_dt == MYSQL_TYPE_NEWDECIMAL ||
        _cursor_cols[i]._native_dt == MYSQL_TYPE_DECIMAL) {
      _cursor_cols[i]._scale = (int)fields[i].decimals;

      // If decimals is 0 length contain precision + 1, if <> 0, length contains
      // precision + scale
      if (fields[i].decimals == 0)
        _cursor_cols[i]._precision = (int)_cursor_cols[i]._len - 1;
      else
        _cursor_cols[i]._precision =
            (int)(_cursor_cols[i]._len - _cursor_cols[i]._scale);
    } else
      // MYSQL_TYPE_BLOB returned for BLOB, TEXT, LONGTEXT (MySQL 5.5);
      // BINARY_FLAG is set for BLOBs
    ...
    // NOT NULL attribute
    _cursor_cols[i]._nullable =
        (fields[i].flags & NOT_NULL_FLAG) ? false : true;
  }
  ...
  if (buffer_rows > 0)
    ...
    // Avoid 0 rows size when only CHAR(0), BINARY(0) or LONGTEXT columns in
    // table
    if (row_size == 0)
      row_size++;
  ...
  for (int i = 0; i < _cursor_cols_count; i++) {
    ...
    // Do not allocate space for LONGTEXT, LONGBLOB columns, but allocate for
    // BLOB and TEXT (65,535 bytes)
    if (_cursor_cols[i]._len != -1 && _cursor_cols[i]._len < 1000000000) {
...
int SqlMysqlApi::Fetch(int *rows_fetched, size_t *time_spent) {
  ...
  for (int i = 0; i < _cursor_allocated_rows; i++) {
    ...
    unsigned long *lengths =
        (unsigned long *)_mysql_fetch_lengths(_cursor_result);
    ...
    for (int k = 0; k < _cursor_cols_count; k++) {
      ...
      if (row[k] != NULL && _cursor_cols[k]._native_dt == MYSQL_TYPE_DATE) {
        ...
        if (row[k] != NULL &&
            _cursor_cols[k]._native_dt == MYSQL_TYPE_DATETIME) {
          // Change to NULL
      ...
      // Copy data (LONGBLOB and LONGTEXT are not copied if they transferred as
      // LOBs to the target)
...
int SqlMysqlApi::InitBulkTransfer(const char *table, size_t col_count,
  ...
  _load_command =
      "LOAD DATA LOCAL INFILE 'sqldata.in_memory' IGNORE INTO TABLE ";
  _load_command += table;
  ...
  // Return code typically 0 as init function waits for LDI thread to start
  // only, but in case cases init can catch LDI errors
  return _ldi_rc;
}

// Transfer rows between databases
int SqlMysqlApi::TransferRows(SqlCol *s_cols, int rows_fetched,
...
// Write LOB data
int SqlMysqlApi::WriteLob(SqlCol * /*s_cols*/, int /*row*/,
                          int * /*lob_bytes*/) {
  return -1;
}
...
int SqlMysqlApi::local_infile_read(char *buf, unsigned int buf_len) {
  TRACE_P("MySQL/C LOAD DATA INFILE Read callback() Entered, buffer size is %d "
          "bytes",
          buf_len);

  // Wait until a new portion of data is available (_ldi_cols is NULL when first
  // callback is called before TransferRows called)
  if (_ldi_cols == NULL || (_ldi_current_row == 0 && _ldi_current_col == 0 &&
                            _ldi_current_col_len == 0)) {
  ...
  // Copy rows
  for (size_t i = _ldi_current_row; i < _ldi_rows_count;
       i++, _ldi_current_row++) {
    // Copy column data
    for (size_t k = _ldi_current_col; k < _ldi_cols_count;
         k++, _ldi_current_col++) {
      ...
      if (k > 0 && _ldi_current_col_len == 0) {
        if (remain_len >= 1) {
          *cur = '\t';
          ...
          TRACE_P("MySQL/C LOAD DATA INFILE Read callback() Left - middle of "
                  "batch, %d bytes chunk, %d bytes all",
                  (int)(cur - buf), _ldi_bytes_all);
      ...
      if (_source_api_type == SQLDATA_ORACLE && _ldi_cols[k]._ind2 != NULL) {
        if (_ldi_cols[k]._ind2[i] != -1) {
          // LOB column
          if (_ldi_cols[k]._native_fetch_dt == SQLT_BLOB ||
              _ldi_cols[k]._native_fetch_dt == SQLT_CLOB) {
            ...
            if (_ldi_current_col_len == 0) {
              ...
              int lob_rc =
                  _source_api_provider->GetLobLength(i, k, &_ldi_lob_size);
              ...
              if (lob_rc != -1 && _ldi_lob_size > 0) {
                ...
                _ldi_lob_data = _source_api_provider->GetLobBuffer(
                    i, k, _ldi_lob_size, &alloc_size);

                // Get LOB content
                lob_rc = _source_api_provider->GetLobContent(
                    i, k, _ldi_lob_data, alloc_size, &read_size);
        ...
        if (_source_api_type == SQLDATA_SYBASE && _ldi_cols[k]._ind2 != NULL) {
          ...
          if ((_source_api_type == SQLDATA_INFORMIX ||
            ...
            if (_source_api_type ==
                SQLDATA_DB2 /*&& _ldi_cols[k].ind[0] & 0xFFFFFFFF00000000*/)
              len = ((int *)(_ldi_cols[k].ind))[i];
#endif
            // DB2 supports TIMESTAMP(12), it cause Data truncation warning, so
            // trim it
            if (_source_api_type == SQLDATA_DB2 &&
                _ldi_cols[k]._native_dt == SQL_TYPE_TIMESTAMP && len > 26)
              len = 26;
          }
      ...
      if (len == -1) {
        ...
        if ((_source_api_type == SQLDATA_ORACLE &&
             (_ldi_cols[k]._native_fetch_dt == SQLT_STR ||
              _ldi_cols[k]._native_fetch_dt == SQLT_BLOB ||
              _ldi_cols[k]._native_fetch_dt == SQLT_CLOB ||
              _ldi_cols[k]._native_fetch_dt == SQLT_BIN ||
              _ldi_cols[k]._native_fetch_dt == SQLT_LNG)) ||
            // Sybase CHAR
            (_source_api_type == SQLDATA_SYBASE &&
             _ldi_cols[k]._native_fetch_dt == CS_CHAR_TYPE) ||
            // ODBC CHAR
            ((_source_api_type == SQLDATA_ODBC ||
          ...
          if (_source_api_type == SQLDATA_ORACLE &&
              _ldi_cols[k]._native_fetch_dt == SQLT_DAT) {
            if (remain_len >= 19) {
              // Unsigned required for proper conversion to int
              unsigned char *data =
                  (unsigned char *)(_ldi_cols[k]._data +
                                    _ldi_cols[k]._fetch_len * i);
            ...
            if ((_source_api_type == SQLDATA_SQL_SERVER ||
                 ...
                 _source_api_type == SQLDATA_ODBC) &&
                _ldi_cols[k]._native_fetch_dt == SQL_C_TYPE_TIMESTAMP) {
              if (remain_len >= 26) {
                ...
                SQL_TIMESTAMP_STRUCT *ts =
                    (SQL_TIMESTAMP_STRUCT *)(_ldi_cols[k]._data + offset);
      ...
      if (no_space) {
        // If it is non-first column and 0 bytes were written, remove column
        // delimiter
        if (k > 0 && _ldi_current_col_len == 0) {
          cur--;
          remain_len++;
  ...
  TRACE_P("MySQL/C LOAD DATA INFILE Read callback() Left - Batch fully loaded, "
          "%d bytes last chunk, %d bytes batch, %d bytes all",
...
int SqlMysqlApi::local_infile_error(void * /*ptr*/, char * /*error_msg*/,
                                    unsigned int /*error_msg_len*/) {
  TRACE("MySQL/C LOAD DATA INFILE - Error callback()");
  return 0;
}

int SqlMysqlApi::local_infile_initS(void **ptr, const char * /*filename*/,
                                    void *userdata) {
  TRACE_S(((SqlMysqlApi *)userdata),
          "MySQL/C LOAD DATA INFILE Init callback()");
...
int SqlMysqlApi::local_infile_readS(void *ptr, char *buf,
...
int SqlMysqlApi::local_infile_errorS(void *ptr, char *error_msg,
                                     unsigned int error_msg_len) {
  SqlMysqlApi *mysqlApi = (SqlMysqlApi *)ptr;
...
void *SqlMysqlApi::StartLoadDataInfileS(void *object) {
  ...
  mysqlApi->_ldi_rc =
      mysqlApi->ExecuteNonQuery(mysqlApi->_load_command.c_str(), NULL);

  mysqlApi->_ldi_terminated = 1;

  // Release thread called CloseBulkTransfer, so this connection can be used by
  // other functions If the table does not exist ExecuteNonQuery returns
  // immediately, so notify TransferRows
#if defined(WIN32) || defined(WIN64)
  SetEvent(mysqlApi->_completed_event);
#else
  Os::SetEvent(&mysqlApi->_completed_event);
...
void SqlMysqlApi::ShowWarnings(const char *prefix) {
  ...
  int rc = OpenCursor("SHOW WARNINGS", 100, 0, &col_count, &allocated_rows,
                      &rows_fetched, &cols, NULL, true);
...
// Drop the table
int SqlMysqlApi::DropTable(const char *table, size_t *time_spent,
...
int SqlMysqlApi::DropReferences(const char *table, size_t *time_spent) {
  ...
  // Query to find foreign keys on the table (this query takes 5-8 sec on first
  // call even if there are 3 rows in the whole table)

  // INNODB_STATS_ON_METADATA=0 is set to significantly speed up access to
  // INFORMATION_SCHEMA views (about 5-10 times faster from 5-10 sec to 0.5-1
  // sec). But this requires SUPER privilege and may be OFF

  std::string query = "SELECT constraint_name, table_name FROM "
  ...
  for (std::list<std::string>::iterator i = drop_fk.begin(); i != drop_fk.end();
       i++) {
...
int SqlMysqlApi::GetLobLength(size_t /*row*/, size_t /*column*/,
                              size_t * /*length*/) {
  return -1;
}

// Get LOB content
int SqlMysqlApi::GetLobContent(size_t /*row*/, size_t /*column*/,
...
int SqlMysqlApi::GetAvailableTables(std::string &table_template,
                                    std::string & /*exclude*/,
  ...
  GetSelectionCriteria(table_template.c_str(), "table_schema", "table_name",
                       condition, _db.c_str(), false);

  // Build the query
  std::string query =
      "SELECT table_schema, table_name FROM information_schema.tables";
  query += " WHERE table_type='BASE TABLE'";
  query += " AND table_schema NOT IN ('information_schema', 'mysql', "
           "'performance_schema')";
...
int SqlMysqlApi::ReadSchema(const char * /*select*/, const char * /*exclude*/,
                            bool /*read_cns*/, bool /*read_idx*/) {
  return -1;
}

// Get table name by constraint name
int SqlMysqlApi::ReadConstraintTable(const char * /*schema*/,
...
int SqlMysqlApi::ReadConstraintColumns(const char * /*schema*/,
                                       const char * /*table*/,
                                       const char * /*constraint*/,
                                       std::string & /*cols*/) {
  return -1;
...
void SqlMysqlApi::SetVersion() {
  ...
  while (more) {
    ...
    if (row[0] != NULL && strcmp(row[0], "version") == 0) {
      sscanf(row[1], "%d.%d.%d", &_version_major, &_version_minor,
             &_version_release);
    }
...
void SqlMysqlApi::FindMysqlPaths() {
  ...
  // MySQL Installer for MySQL 5.7 puts keys even for 64-bit version to
  ...
  for (int k = 0; k < 2; k++) {
    int rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, keys[k], 0,
    ...
    while (more) {
      ...
      rc = RegQueryValueEx(hSubkey, "Location", NULL, NULL, (LPBYTE)location,
                           (LPDWORD)&value_size);
