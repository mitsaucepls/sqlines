...
void SqlData::SetConnectionStrings(std::string &source, std::string &target,
                                   short *s_type, short *t_type) {
  _source_conn = source;
  _target_conn = target;
...
short SqlData::DefineDbType(const char *conn) {
  ...
  else if (_strnicmp(conn, "sql", 3) == 0)
    return SQLDATA_SQL_SERVER;
  else if (_strnicmp(conn, "mysql", 5) == 0)
    return SQLDATA_MYSQL;
  else if (_strnicmp(conn, "mariadb", 7) == 0)
    return SQLDATA_MYSQL;
  else if (_strnicmp(conn, "pg", 2) == 0)
    return SQLDATA_POSTGRESQL;
  else if (_strnicmp(conn, "sybase", 6) == 0)
    return SQLDATA_SYBASE;
  else if (_strnicmp(conn, "informix", 8) == 0)
    return SQLDATA_INFORMIX;
  else if (_strnicmp(conn, "db2", 3) == 0)
    return SQLDATA_DB2;
  else if (_strnicmp(conn, "asa", 3) == 0)
    return SQLDATA_ASA;
  else if (_strnicmp(conn, "odbc", 4) == 0)
    return SQLDATA_ODBC;
  else if (_strnicmp(conn, "stdout", 6) == 0)
...
short SqlData::DefineSqlParserType(short type) {
  ...
  if (type == SQLDATA_SQL_SERVER)
    rt = SQL_SQL_SERVER;
  else if (type == SQLDATA_ORACLE)
    rt = SQL_ORACLE;
  else if (type == SQLDATA_DB2)
    rt = SQL_DB2;
  else if (type == SQLDATA_MYSQL)
    rt = SQL_MYSQL;
  else if (type == SQLDATA_POSTGRESQL)
    rt = SQL_POSTGRESQL;
  else if (type == SQLDATA_SYBASE)
    rt = SQL_SYBASE;
  else if (type == SQLDATA_INFORMIX)
    rt = SQL_INFORMIX;
...
int SqlData::ConnectMetaDb(SqlDataReply &reply, int db_types) {
  ...
  if (rc == -1) {
    ...
    // Exist only if both source and target initialized with errors, otherwise
    // try to connect to one of the databases (at least test connection)
    if (s_rc == -1 && t_rc == -1)
      return -1;
  }
...
int SqlData::GetAvailableTables(std::string &select, std::string &exclude,
...
int SqlData::TestConnection(std::string &conn, std::string &error,
...
int SqlData::CreateMetadataQueues(std::string &select, std::string &exclude) {
  ...
  int db_types =
      (_command != SQLDATA_CMD_ASSESS) ? SQLDB_BOTH : SQLDB_SOURCE_ONLY;

  // Read source and target schemas
  int rc = _db.ReadSchema(db_types, select, exclude, _migrate_constraints,
                          _migrate_indexes);

  // Do not perform any DDL tasks if validation or assessment is performed, or
  // load to existing or truncate is set
  ...
  if (table_columns != NULL) {
    ...
    for (std::list<SqlColMeta>::iterator i = table_columns->begin();
      ...
      if ((*i).identity == true) {
        // For Oracle create sequence and trigger, for PostgreSQL sequence and
        // default
        if (_target_type == SQLDATA_ORACLE ||
            _target_type == SQLDATA_POSTGRESQL)
          GetIdentityMetaTask((*i));
      }
    }
  }

  std::list<SqlConstraints> *table_constraints =
      _db.GetTableConstraints(SQLDB_SOURCE_ONLY);
  std::list<SqlConsColumns> *table_cons_columns =
      _db.GetConstraintColumns(SQLDB_SOURCE_ONLY);

  if (table_constraints != NULL) {
    for (std::list<SqlConstraints>::iterator i = table_constraints->begin();
         i != table_constraints->end(); i++) {
      SqlMetaTask task;
      ...
      if ((*i).type == 'C' && (*i).condition != NULL) {
        ...
        // Convert condition to the target syntax seperately (not entire ALTER
        // TABLE statement) to avoid unintended changes in identifiers
        ...
        if ((*i).type == 'P' || (*i).type == 'U') {
          if ((*i).type == 'P') {
            // For target MySQL, MariaDB a primary key is defined in CREATE
            // TABLE if the table has an identity column, so skip PK in ALTER
            // TABLE
            if (_target_type == SQLDATA_MYSQL &&
                IsIdentityDefined(schema, table))
              continue;
          ...
          if ((*i).type == 'R' && _source_type == SQLDATA_ORACLE) {
            ...
            for (std::list<SqlConsColumns>::iterator k =
                     table_cons_columns->begin();
                 k != table_cons_columns->end(); k++) {
              // Compare schema, table and constraint names (columns are already
              // ordered)
              if ((*k).schema != NULL &&
                  strcmp((*k).schema, (*i).schema) == 0 && (*k).table != NULL &&
                  strcmp((*k).table, (*i).table) == 0 &&
                  (*k).constraint != NULL &&
                  strcmp((*k).constraint, (*i).constraint) == 0) {
              ...
              if ((*k).schema != NULL &&
                  strcmp((*k).schema, (*i).r_schema) == 0 &&
                  (*k).constraint != NULL &&
                  strcmp((*k).constraint, (*i).r_constraint) == 0) {
            ...
            // If primary key table not selected, its columns will not be found,
            // so read them from the database
            if (ptable.empty() == true) {
              _db.ReadConstraintTable(SQLDB_SOURCE_ONLY, (*i).r_schema,
                                      (*i).r_constraint, ptable);
              _db.ReadConstraintColumns(SQLDB_SOURCE_ONLY, (*i).r_schema, NULL,
                                        (*i).r_constraint, pcols);
            }
      ...
      MapConstraintName((*i).constraint, cns_name, (*i).type, task.s_name,
                        ptable);
  ...
  std::list<SqlComments> *table_comments =
      _db.GetTableComments(SQLDB_SOURCE_ONLY);

  if (table_comments != NULL && _target_type != SQLDATA_MYSQL) {
    for (std::list<SqlComments>::iterator i = table_comments->begin();
         i != table_comments->end(); i++) {
      SqlMetaTask task;
  ...
  if (table_indexes != NULL) {
    for (std::list<SqlIndexes>::iterator i = table_indexes->begin();
         i != table_indexes->end(); i++) {
      SqlMetaTask task;
  ...
  if (sequences != NULL && _target_type != SQLDATA_MYSQL) {
    for (std::list<SqlSequences>::iterator i = sequences->begin();
         i != sequences->end(); i++) {
      SqlMetaTask task;
...
int SqlData::CreateObjectQueue(std::string &select, std::string &exclude,
                               int *total_obj, int *total_lines,
                               int *total_bytes) {
  size_t obj_cnt = 0;
  size_t obj_lines = 0;
  size_t obj_bytes = 0;
  ...
  if (procedures != NULL) {
    ...
    for (std::list<SqlObjMeta>::iterator i = procedures->begin();
         i != procedures->end(); i++) {
      SqlObjMetaTask task;
      task.source = &(*i);
...
int SqlData::DropReferences(int *all_keys, size_t *time_spent) {
  ...
  for (std::list<std::string>::iterator i = _tables->begin();
       i != _tables->end(); i++) {
    std::string t_table;
    int keys = 0;
    size_t time_s = 0;
...
// Initialize local (in-process) database interface
int SqlData::InitLocal(SqlDataReply & /*reply*/, int db_types, int *s_rc,
                       int *t_rc) {
  ...
  int rc = _db.InitStatic(db_types, _source_conn.c_str(), _target_conn.c_str(),
                          s_rc, t_rc);

  // Initialize source and target database interfaces (continue connecting even
  // if static init failed for one of the database, helps test connections)
  rc = _db.Init(db_types, _source_conn.c_str(), _target_conn.c_str(), s_rc,
                t_rc);
...
// Connect to source and target databases through in-process interface
int SqlData::ConnectLocal(SqlDataReply &reply, int db_types, int *s_rc,
                          int *t_rc) {
...
void SqlData::SetConstraintMappingFromFile(std::string &file) {
  ...
  while (*cur) {
    ...
    _cns_map.insert(std::pair<std::string, std::string>(
        table + ',' + source_cns, target_cns));
  }
}
...
void SqlData::SetTableWhereConditionsFromFile(std::string &file) {
  ...
  while (*cur) {
    ...
    _twhere_cond_map.insert(
        std::pair<std::string, std::string>(table, where_cond));
  }
}
...
int SqlData::StartSessions() {
  ...
  HANDLE hp =
      CreateNamedPipe(SQLDATA_NAMED_PIPE, PIPE_ACCESS_DUPLEX,
  ...
  while (more) {
    ...
    if (cn == FALSE) {
      ...
      // if ERROR_PIPE_CONNECTED is returned, it is normal (client connected
      // before ConnectNamedPipe called)
      if (err == ERROR_PIPE_CONNECTED)
        connected = true;
    }

    // If connection failed, for example, ERROR_NO_DATA means that client closed
    // pipe on its end, reopen
    if (connected == false)
      CloseHandle(hp);
...
int SqlData::StartLocalWorkers() {
  ...
  for (int i = 1; i < local; i++) {
    ...
    sqlDb->Init(db_types, _source_conn.c_str(), _target_conn.c_str(), NULL,
                NULL);
...
int SqlData::StartLocalWorker(SqlDb *sqlDb) {
  ...
  while (more) {
    ...
    if (_command == SQLDATA_CMD_TRANSFER) {
      if (_migrate_tables || _migrate_data)
        reply.rc = sqlDb->TransferRows(reply, _command_options, _migrate_tables,
                                       _migrate_data);
      else {
        reply.rc = 0;
  ...
  *reply._t_name = '\x0';
  ...
  while (more) {
    ...
    if (task.type == SQLDATA_CMD_ADD_DEFAULT ||
        task.type == SQLDATA_CMD_ADD_PRIMARY_KEY ||
        task.type == SQLDATA_CMD_ADD_UNIQUE_KEY ||
        task.type == SQLDATA_CMD_ADD_FOREIGN_KEY ||
        task.type == SQLDATA_CMD_ADD_COMMENT ||
      ...
      reply.rc = sqlDb->ExecuteNonQuery(SQLDB_TARGET_ONLY, reply,
                                        task.statement.c_str());
    } else
      // Add a CHECK constraint (expression conversion was already performed)
      if (task.type == SQLDATA_CMD_ADD_CHECK_CONSTRAINT) {
        reply.rc = sqlDb->ExecuteNonQuery(SQLDB_TARGET_ONLY, reply,
                                          task.statement.c_str());
      } else if (task.type == SQLDATA_CMD_CREATE_SEQUENCE) {
        reply.rc = sqlDb->CreateSequence(SQLDB_TARGET_ONLY, reply,
                                         task.statement.c_str(),
                                         task.t_o_name.c_str());
      }
...
void SqlData::NotifyMetaCompletion(SqlMetaTask &task) {
  ...
  for (std::list<SqlMetaTask>::iterator i = _meta_in_progress.begin();
       i != _meta_in_progress.end(); i++) {
...
bool SqlData::GetNextMetaTask(SqlMetaTask &task) {
  ...
  while (_meta_tasks.empty() == false) {
    ...
    if (task.type == SQLDATA_CMD_CREATE_TRIGGER && data_complete == true &&
        pk_created == true)
      pk_created = IsSequencesCreated();

    // Due to locks, do not perform DDL operations on the same object in
    // parallel
    ...
    if (data_complete == false || pk_created == false || ddl_exists) {
      ...
      if (first == true) {
        ...
        if (first_task.type == task.type && first_task.s_name == task.s_name &&
            first_task.statement == task.statement) {
          // Temporary leave the critical section to allow other threads to do
          // work
          Os::LeaveCriticalSection(&_task_queue_critical_section);
          Os::Sleep(1);
          Os::EnterCriticalSection(&_task_queue_critical_section);
...
void SqlData::ConvertSql(std::string &in, std::string &out) {
  ...
  // Convert the file
  int rc = ::ConvertSql(_sqlParser, in.c_str(), (int)in.size(), &output,
                        &out_size, NULL);
...
bool SqlData::IsDataTransferred(std::string &table) {
  ...
  std::list<std::string>::iterator i =
      std::find(_tables_in_progress.begin(), _tables_in_progress.end(), table);
...
// For foreign key task check if the primary key was already created (already in
// critical session)
bool SqlData::IsPrimaryKeyCreated(SqlMetaTask &task) {
  ...
  for (std::list<SqlMetaTask>::iterator i = _meta_tasks.begin();
       i != _meta_tasks.end(); i++) {
    // Check any primary key
    if ((*i).type == SQLDATA_CMD_ADD_PRIMARY_KEY)
      return false;
...
// Check is a DDL task is running for the specified object (already in critical
// session)
...
// For trigger task, check that all sequences already created (already in
// critical session)
...
void SqlData::MapObjectName(std::string &s_schema, std::string &s_table,
                            std::string &t_name, bool tmap) {
  std::string t_schema;
  std::string t_table;
...
void SqlData::MapObjectName(std::string &s_schema, std::string &s_table,
                            std::string &t_schema, std::string &t_table) {
  // Map schema name
  if (s_schema.empty() == false) {
    ...
    if (_schema_map.empty() == false) {
      // If source is Oracle, convert schema name to upper case
      if (_source_type == SQLDATA_ORACLE || source_subtype == SQLDATA_ORACLE)
        std::transform(s_schema2.begin(), s_schema2.end(), s_schema2.begin(),
                       ::toupper);

      // Find the mapped name for this schema
      std::map<std::string, std::string>::iterator i =
          _schema_map.find(s_schema2);
...
void SqlData::MapConstraintName(const char *source, std::string &target,
                                char type, std::string &table,
                                std::string &p_table) {
  if (source == NULL)
    return;
  ...
  std::map<std::string, std::string>::iterator i =
      _cns_map.find(std::string(table) + ',' + std::string(source));
  ...
  if ((strncmp(source, "SYS_C0", 6) == 0) ||
      (strncmp(source, "ASA", 3) == 0 && len <= 8))
    return;

  // Foreign key constraint can be equal to PK table name (by default in ASA),
  // while in SQL Server constraints, tables must be unique in the database
...
void SqlData::MapColumn(const char *s_table, std::string &s_name,
...
void SqlData::ConvertIdentifier(const char *in, std::string &out) {
  ...
  // Informix index can start with a space following by a digit, replace space
  // with a alphabetic char
  if (*cur == ' ') {
    out = 'n';
    cur++;
  } else
...
void SqlData::SetCallback(void *object, SqlDataCallbackFunc func) {
  _callback_object = object, _callback = func;
...
int SqlData::SetConcurrentSessions(int max_sessions, int table_count) {
  // Max sessions cannot exceed the number of tables (do not count metadata
  // tasks as they typicaly wait data load completion)
...
// Get primary or unique key columns
void SqlData::GetKeyConstraintColumns(SqlMetaTask &task, SqlConstraints &cns,
                                      std::string &cols) {
  ...
  for (std::list<std::string>::iterator i = lcols.begin(); i != lcols.end();
       i++) {
    if (c > 0)
      cols += ", ";
...
// Get foreign key columns
void SqlData::GetForeignKeyConstraintColumns(SqlMetaTask &task,
                                             SqlConstraints &cns,
                                             std::string &fcols,
                                             std::string &pcols,
                                             std::string &ptable) {
  ...
  _db.GetForeignKeyConstraintColumns(SQLDB_SOURCE_ONLY, cns, lfcols, lpcols,
                                     ptable);

  int c = 0;

  for (std::list<std::string>::iterator i = lfcols.begin(); i != lfcols.end();
       i++) {
    if (c > 0)
      fcols += ", ";
  ...
  for (std::list<std::string>::iterator i = lpcols.begin(); i != lpcols.end();
       i++) {
    if (c > 0)
      pcols += ", ";
...
// Generate statement to emulate identity in Oracle (sequence and trigger) and
// PostgreSQL (sequence and default)
...
void SqlData::GetErrors(SqlDataReply &reply) { _db.GetErrors(reply); }
