...
int SqlDb::Init(int db_types, const char *source_conn, const char *target_conn,
                int *s_rc_inout, int *t_rc_inout) {
  ...
  // Initialize the source database API (do not exit in case of error, allow
  // initializing the target API)
  if (s_db_api != NULL) {
...
int SqlDb::InitStatic(int db_types, const char *source_conn,
                      const char *target_conn, int *s_rc_out, int *t_rc_out) {
...
int SqlDb::Connect(int db_types, SqlDataReply &reply, int *s_rc_in,
                   int *t_rc_in) {
...
// Test connection
int SqlDb::TestConnection(std::string &conn, std::string &error,
...
int SqlDb::GetAvailableTables(int db_types, std::string &table_template,
                              std::string &exclude,
...
int SqlDb::ReadSchema(int db_types, std::string &select, std::string &exclude,
                      bool read_cns, bool read_idx) {
  int rc = 0;

  // Read source and target schemas
  if (db_types == SQLDB_BOTH && _source_ca.db_api != NULL &&
      _target_ca.db_api != NULL) {
    if (target_type == SQLDATA_ORACLE) {
      ...
      rc = Execute(SQLDATA_CMD_READ_SCHEMA,
                   SQLDATA_CMD_READ_SCHEMA_FOR_TRANSFER_TO);
    } else
      rc = _source_ca.db_api->ReadSchema(select.c_str(), exclude.c_str(),
                                         read_cns, read_idx);
  } else
...
int SqlDb::ReadObjects(int db_types, std::string &select,
                       std::string &exclude) {
  int rc = 0;
...
int SqlDb::ReadConstraintTable(int /*db_types*/, const char *schema,
                               const char *constraint, std::string &table) {
  if (_source_ca.db_api == NULL)
    return -1;
...
int SqlDb::ReadConstraintColumns(int /*db_types*/, const char *schema,
  ...
  int rc =
      _source_ca.db_api->ReadConstraintColumns(schema, table, constraint, cols);

  return rc;
}

// Check if identity column defined for the table
bool SqlDb::IsIdentityDefined(int /*db_types*/, std::string &schema,
                              std::string &table) {
  if (_source_ca.db_api == NULL)
    return false;
...
int SqlDb::GetKeyConstraintColumns(int /*db_types*/, SqlConstraints &cns,
                                   std::list<std::string> &key_cols) {
  if (_source_ca.db_api == NULL)
    return -1;
...
int SqlDb::GetForeignKeyConstraintColumns(int /*db_types*/, SqlConstraints &cns,
                                          std::list<std::string> &fcols,
                                          std::list<std::string> &pcols,
                                          std::string &ptable) {
  if (_source_ca.db_api == NULL)
    return -1;

  int rc = _source_ca.db_api->GetForeignKeyConstraintColumns(cns, fcols, pcols,
                                                             ptable);
...
int SqlDb::GetIndexColumns(int /*db_types*/, SqlIndexes &idx,
...
// Transfer table rows
int SqlDb::TransferRows(SqlDataReply &reply, int options, bool create_tables,
                        bool data) {
  ...
  if (reply.s_sql_l.empty()) {
    if (_metaSqlDb != NULL)
      _metaSqlDb->BuildQuery(select, t_select, reply._s_name, reply._t_name,
                             true);
  } else
  ...
  int rc = _source_ca.db_api->OpenCursor(
      select.c_str(), buffer_rows, 1024 * 1024 * 32, &col_count,
      &allocated_array_rows, &rows_fetched, &s_cols, &time_read, false,
      _datatype_map);
  ...
  if (_callback != NULL) {
    ...
    // There can be warnings (rc = 1) such as truncation error, get exact
    // message
    if (rc != 0) {
      reply.s_error = _source_ca.db_api->GetError();
      strcpy(reply.s_native_error_text,
             _source_ca.db_api->GetNativeErrorText());
    }
  ...
  // Check whether target bound the source buffers, so the same buffers must be
  // passed to TransferRows
  ...
  if (create_tables && options != 0) {
    rc = PrepareTransfer(s_cols, reply._s_name, reply._t_name, col_count,
                         options, reply);
    ...
    if (rc == -1) {
      ...
      // Notify on transfer completion with DDL error (call back will be called
      // in the caller)
  ...
  if (data && rc != -1 && rows_fetched != 0) {
    rc = _target_ca.db_api->InitBulkTransfer(
        reply._t_name, col_count, allocated_array_rows, cur_cols, &t_cols);
    bulk_init = true;
  }
  ...
  while (data && rc != -1) {
    ...
    if (no_more_data == true) {
      // Sybase CT-lib and ODBC return NO DATA after all rows fetched so check
      // for number of rows
      if (rows_fetched != 0) {
        ...
        rc = _target_ca.db_api->TransferRows(
            cur_cols, rows_fetched, &rows_written, &bytes_written, &time_write);
      ...
      // Use concurrent threads
        ...
        // Fetch the next set of data, and insert the current set in 2
        // concurrent threads
  ...
  if (data && ddl_error == false) {
    ...
    if (rc == -1) {
      ...
      strcpy(reply.s_native_error_text,
             _source_ca.db_api->GetNativeErrorText());
      strcpy(reply.t_native_error_text,
             _target_ca.db_api->GetNativeErrorText());
    }
  }
...
int SqlDb::AssessRows(SqlDataReply &reply) {
  ...
  // select = "select well_te from afmss.well_remarks where
  // well_rmks_id=100827";
  ...
  // Open cursor allocating 10M buffer
  int rc = _source_ca.db_api->OpenCursor(select.c_str(), 0, 1024 * 1024 * 10,
                                         &col_count, &allocated_array_rows,
...
// Check whether we need to drop, create or truncate the table
int SqlDb::PrepareTransfer(SqlCol *s_cols, const char *s_table,
                           const char *t_table, size_t col_count, int options,
                           SqlDataReply &reply) {
  if (s_table == NULL || t_table == NULL || _target_ca.db_api == NULL ||
      s_cols == NULL)
    return -1;
...
// Generate SQL CREATE TABLE statement
int SqlDb::GenerateCreateTable(SqlCol *s_cols, const char *s_table,
                               const char *t_table, int col_count,
                               std::string &sql) {
  ...
  for (int i = 0; i < col_count; i++) {
    ...
    // Get IDENTITY clause to be added to column definition, it may affect the
    ...
    if (coltype.empty() == false)
      ...
      if ((source_type == SQLDATA_ORACLE && s_cols[i]._native_dt == SQLT_CHR) ||
          // Sybase ASE CHAR and VARCHAR (CS_CHAR_TYPE for VARCHAR <= 255, and
          // CS_LONGCHAR_TYPE for VARCHAR < 32K)
          (source_type == SQLDATA_SYBASE &&
           (s_cols[i]._native_dt == CS_CHAR_TYPE ||
            s_cols[i]._native_dt == CS_LONGCHAR_TYPE)) ||
          // MySQL VARCHAR
          (source_type == SQLDATA_MYSQL &&
           s_cols[i]._native_dt == MYSQL_TYPE_VAR_STRING) ||
        ...
        if (target_type == SQLDATA_SQL_SERVER) {
          ...
          if (target_type == SQLDATA_ORACLE) {
            if (s_cols[i]._len <= 4000) {
              ...
              sql += "CLOB";
          } else if (target_type == SQLDATA_MYSQL) {
            ...
            if (s_cols[i]._len <= 8000) {
              ...
              sql += "LONGTEXT";
          } else if (target_type == SQLDATA_POSTGRESQL) {
            sql += "TEXT";
          } else {
            sql += "VARCHAR(";
            sql += int1;
        ...
        if ((source_type == SQLDATA_ORACLE &&
             s_cols[i]._native_dt == SQLT_AFC) ||
            // SQL Server CHAR
            (source_type == SQLDATA_SQL_SERVER &&
             s_cols[i]._native_dt == SQL_CHAR) ||
            // Informix CHAR
            (source_type == SQLDATA_INFORMIX &&
             s_cols[i]._native_dt == SQL_CHAR) ||
            // DB2 CHAR
            ...
            // MySQL CHAR
            (source_type == SQLDATA_MYSQL &&
             s_cols[i]._native_dt == MYSQL_TYPE_STRING)) {
          ...
          if (target_type == SQLDATA_MYSQL) {
            ...
            // SQL Server allows max size 8000 for CHAR (while Informix allows
            // 32,767)
            if (target_type == SQLDATA_SQL_SERVER) {
              if (s_cols[i]._len <= 8000) {
                sql += "CHAR(";
              ...
              // Add CHAR length semantic for Oracle as many databases specify
              // size in characters, while BYTE is default in Oracle
              if (target_type == SQLDATA_ORACLE)
                sql += " CHAR";
          ...
          if (((source_type == SQLDATA_SQL_SERVER ||
                source_type == SQLDATA_ASA) &&
            ...
            if (((source_type == SQLDATA_SQL_SERVER ||
              ...
              if ((source_type == SQLDATA_ORACLE &&
                   s_cols[i]._native_dt == SQLT_NUM &&
                   s_cols[i]._precision != 0 && s_cols[i]._precision < 10 &&
                   s_cols[i]._scale == 0) ||
                  // Oracle NUMBER as INTEGER through ODBC
                  (source_type == SQLDATA_ODBC &&
                   source_subtype == SQLDATA_ORACLE &&
                   s_cols[i]._native_dt == SQL_DECIMAL &&
                   s_cols[i]._precision < 10 && s_cols[i]._scale == 0) ||
                  // SQL Server, DB2, Informix, Sybase ASA INTEGER
                  ((source_type == SQLDATA_SQL_SERVER ||
                    source_type == SQLDATA_DB2 ||
                    source_type == SQLDATA_INFORMIX ||
                    source_type == SQLDATA_ASA) &&
                   s_cols[i]._native_dt == SQL_INTEGER) ||
                  // Sybase ASE INT
                  (source_type == SQLDATA_SYBASE &&
                   s_cols[i]._native_dt == CS_INT_TYPE) ||
                  // MySQL INT
                  (source_type == SQLDATA_MYSQL &&
                   s_cols[i]._native_dt == MYSQL_TYPE_LONG))

              {
                ...
                if (((source_type == SQLDATA_SQL_SERVER ||
                    ...
                    // MySQL BIGINT
                    (source_type == SQLDATA_MYSQL &&
                     s_cols[i]._native_dt == MYSQL_TYPE_LONGLONG) ||
                    // Sybase ASE BIGINT
                    (source_type == SQLDATA_SYBASE &&
                     s_cols[i]._native_dt == CS_BIGINT_TYPE) ||
                    // Informix SQL_INFX_BIGINT (-114)
                    (source_type == SQLDATA_INFORMIX &&
                     s_cols[i]._native_dt == -114)) {
                  if (target_type == SQLDATA_ORACLE)
                    sql += "NUMBER(19)";
                  else
                    sql += "BIGINT";
                  ...
                  if ((source_type == SQLDATA_SYBASE &&
                       s_cols[i]._native_dt == CS_SMALLINT_TYPE) ||
                      (source_type == SQLDATA_MYSQL &&
                       s_cols[i]._native_dt == MYSQL_TYPE_SHORT) ||
                      ((source_type == SQLDATA_SQL_SERVER ||
                        source_type == SQLDATA_DB2 ||
                        source_type == SQLDATA_INFORMIX ||
                        source_type == SQLDATA_ASA) &&
                    ...
                    if (((source_type == SQLDATA_SQL_SERVER ||
                        ...
                        // Sybase TINYINT
                        (source_type == SQLDATA_SYBASE &&
                         s_cols[i]._native_dt == CS_TINYINT_TYPE) ||
                        // MySQL TINYINT
                        (source_type == SQLDATA_MYSQL &&
                         s_cols[i]._native_dt == MYSQL_TYPE_TINY)) {
                      if (target_type == SQLDATA_ORACLE)
                        sql += "NUMBER(3)";
                      else if (target_type == SQLDATA_SQL_SERVER ||
                      ...
                      if (source_type == SQLDATA_ORACLE &&
                          s_cols[i]._native_dt == SQLT_NUM &&
                          ((s_cols[i]._precision == 0 &&
                            s_cols[i]._scale == 129) ||
                           s_cols[i]._precision >= 38)) {
                        if (target_type == SQLDATA_SQL_SERVER)
                          sql += "FLOAT";
                        else if (target_type == SQLDATA_ORACLE)
                          sql += "NUMBER";
                        else if (target_type == SQLDATA_MYSQL)
                          sql += "DOUBLE";
                        else
                        ...
                        // MySQL DOUBLE
                        if ((source_type == SQLDATA_MYSQL &&
                             s_cols[i]._native_dt == MYSQL_TYPE_DOUBLE) ||
                          ...
                          if ((source_type == SQLDATA_SQL_SERVER ||
                            ...
                            // Informix single-precision floating point number
                            // SMALLFLOAT, REAL
                            if ((source_type == SQLDATA_INFORMIX &&
                                 s_cols[i]._native_dt == SQL_REAL) ||
                                // DB2 FLOAT, SQL Server REAL, Sybase ASA
                              ...
                              // Informix double-precision floating point number
                              // FLOAT, DOUBLE PRECISION Sybase ASA DOUBLE
                              if ((source_type == SQLDATA_INFORMIX ||
                                   source_type == SQLDATA_ASA) &&
                                  s_cols[i]._native_dt == SQL_DOUBLE) {
                                if (target_type == SQLDATA_SQL_SERVER)
                                ...
                                if ((source_type == SQLDATA_MYSQL &&
                                    ...
                                    // Sybase ASE FLOAT
                                    (source_type == SQLDATA_SYBASE &&
                                     s_cols[i]._native_dt == CS_FLOAT_TYPE)) {
                                  if (target_type == SQLDATA_SQL_SERVER)
                                    sql += "REAL";
                                  else if (target_type == SQLDATA_ORACLE)
                                    sql += "NUMBER";
                                  ...
                                  // DB2 DECFLOAT(16 | 34)
                                  if (source_type == SQLDATA_DB2 &&
                                      s_cols[i]._native_dt == -360) {
                                    sql += "DOUBLE";
                                  } else
                                    ...
                                    if ((source_type == SQLDATA_ORACLE &&
                                        ...
                                        (source_type == SQLDATA_SYBASE &&
                                         (s_cols[i]._native_dt ==
                                          ...
                                          s_cols[i]._native_dt ==
                                              CS_DECIMAL_TYPE)) ||
                                        // SQL Server, DB2, Informix, Sybase ASA
                                        // DECIMAL/NUMERIC
                                        ((source_type == SQLDATA_SQL_SERVER ||
                                          source_type == SQLDATA_DB2 ||
                                          source_type == SQLDATA_INFORMIX ||
                                        ...
                                        (source_type == SQLDATA_MYSQL &&
                                         (s_cols[i]._native_dt ==
                                          ...
                                          s_cols[i]._native_dt ==
                                              MYSQL_TYPE_DECIMAL))) {
                                      // Sybase ASE allows NUMERIC/DECIMAL(x,0)
                                      // for identity while MySQL/MariaDB allows
                                      // only integers
                                      if (source_type == SQLDATA_SYBASE &&
                                          target_type == SQLDATA_MYSQL &&
                                          s_cols[i]._scale == 0 &&
                                          !identity_clause.empty()) {
                                        ...
                                        sql += Str::IntToString(
                                            s_cols[i]._precision, int1);
                                        sql += ",";
                                        sql += Str::IntToString(
                                            s_cols[i]._scale, int2);
                                        sql += ")";
                                      }
                                    } else
                                      // Sybase ASE MONEY
                                          ...
                                          if ((source_type == SQLDATA_ORACLE &&
                                            ...
                                            if ((source_type ==
                                              ...
                                              if (target_type ==
                                                ...
                                                // In DB2 fraction can be up to
                                                // 12, in some other databases
                                                // up to 9 while MySQL and
                                                // MariaDB support up to 6
    ...
    // Get DEFAULT if it is a literal (MySQL i.e. requires full column
    // specification in ALTER TABLE)
    GetDefaultClause(s_table, s_cols[i]._name, default_clause);

    if (source_type == SQLDATA_DB2 && target_type == SQLDATA_MYSQL &&
        default_clause.empty() == false) {
      sql += " DEFAULT ";
      sql += default_clause;
    }

    // Check NOT NULL attribute
    if (s_cols[i]._nullable == false ||
        IsPrimaryKeyColumn(s_table, s_cols[i]._name))
      sql += " NOT NULL";
  ...
  // For MySQL add table comment, character set etc. as a part of CREATE TABLE
  // statement
  if (target_type == SQLDATA_MYSQL)
...
// Get DEFAULT if it is a literal (MySQL i.e. requires full column specification
// in ALTER TABLE)
int SqlDb::GetDefaultClause(const char *s_table, char *column,
                            std::string &default_clause) {
  ...
  if (_metaSqlDb != NULL)
    _metaSqlDb->_source_ca.db_api->GetColumnDefault(s_table, column,
                                                    def_clause);
  ...
  if (def_clause.empty() == false) {
    ...
    if (*d == '\'' || (*d >= '0' && *d <= '9') ||
        !_stricmp(d, "CURRENT_TIMESTAMP"))
      default_clause = def_clause;
    else if (!_stricmp(d, "CURRENT TIMESTAMP"))
...
int SqlDb::GetInlineIdentityClause(const char *s_table, char *column,
                                   std::string &identity_clause) {
  if (s_table == NULL || column == NULL)
    return -1;
  ...
  std::list<SqlColMeta> *table_columns =
      _metaSqlDb->GetTableColumns(SQLDB_SOURCE_ONLY);
  ...
  for (std::list<SqlColMeta>::iterator i = table_columns->begin();
       i != table_columns->end(); i++) {
...
void SqlDb::MapColumn(const char *s_table, const char *s_name,
                      std::string &t_name, std::string &t_type) {
  ...
  if (_column_map != NULL && _column_map->size() > 0) {
    ...
    // Find mapping record for this table and column
    for (std::list<SqlColMap>::iterator i = _column_map->begin();
         i != _column_map->end(); i++) {
      if (_stricmp((*i).schema.c_str(), schema.c_str()) == 0 &&
          _stricmp((*i).table.c_str(), table.c_str()) == 0 &&
          _stricmp((*i).name.c_str(), s_name) == 0) {
  ...
  if (target_type == SQLDATA_SQL_SERVER) {
    ...
    // Check for space (Sybase ASE, ASA i.e. return unqouted identifier when
    // space inside)
...
// Check is the identifier contains a special character such as a space, dot
// etc.
...
int SqlDb::CopyColumns(SqlCol *cols, SqlCol **cols_copy, size_t col_count,
                       size_t allocated_array_rows) {
  if (cols == NULL || cols_copy == NULL || col_count <= 0 ||
      allocated_array_rows <= 0)
    return -1;
...
// Copy data
int SqlDb::CopyColumnData(SqlCol *s_cols, SqlCol *t_cols, size_t col_count,
                          int rows_fetched) {
  ...
  for (int i = 0; i < col_count; i++) {
    // Copy data
    memcpy(t_cols[i]._data, s_cols[i]._data,
           s_cols[i]._fetch_len * rows_fetched);
    ...
    if (s_cols[i]._len_ind2 != NULL)
      memcpy(t_cols[i]._len_ind2, s_cols[i]._len_ind2,
             sizeof(short) * rows_fetched);

    if (s_cols[i]._len_ind4 != NULL)
      memcpy(t_cols[i]._len_ind4, s_cols[i]._len_ind4,
             sizeof(int) * rows_fetched);
  }
...
int SqlDb::ValidateRows(SqlDataReply &reply) {
  ...
  _metaSqlDb->BuildQuery(s_select, t_select, reply._s_name, reply._t_name,
                         false);
  ...
  // Set trace file name containing possible differences (it will be created
  // only if any difference found)
  if (_trace_diff_data)
    _trace_diff.SetLogfile(
        std::string(reply._s_name).append("_diff.txt").c_str(), NULL);
  ...
  while (more) {
    ...
    not_equal_rows = ValidateCompareRows(
        s_cols, t_cols, s_col_count, &s_bytes, t_col_count, s_fetched_rows,
    ...
    // Exit if the number of fetched rows is less than allocated rows, or we
    // reached the number of errors
    if ((s_fetched_rows < s_alloc_rows || t_fetched_rows < t_alloc_rows) ||
        (_validation_not_equal_max_rows != -1 &&
         all_not_equal_rows >= _validation_not_equal_max_rows)) {
      more = false;
      break;
    }
    ...
    // No more row, 0 can be returned if total rows divisible buffer size (10K,
    // 100K, 10M etc.)
...
int SqlDb::ValidateCompareRows(SqlCol *s_cols, SqlCol *t_cols, int s_col_count,
                               int *s_bytes, int t_col_count, int s_rows,
                               int t_rows, int *t_bytes, int running_rows,
                               int running_not_equal_rows) {
  ...
  for (int i = 0; i < s_rows; i++) {
    ...
    for (int k = 0; k < s_col_count; k++) {
      ...
      int s_len =
          GetColumnDataLen(s_cols, i, k, source_type, _source_ca.db_api);
      int t_len =
          GetColumnDataLen(t_cols, i, k, target_type, _target_ca.db_api);
      ...
      if (s_len == -1 && t_len == -1)
        ...
        if ((source_type == SQLDATA_ORACLE && s_len == -1 &&
             target_type != SQLDATA_ORACLE && t_len == 0) ||
            (source_type != SQLDATA_ORACLE && s_len == 0 &&
             target_type == SQLDATA_ORACLE && t_len == -1))
          continue;
        else
          ...
          if ((s_len == -1 && t_len != -1) || (s_len != -1 && t_len == -1)) {
            ...
            if (_trace_diff_data)
              ValidateDiffDump(&s_cols[k], &t_cols[k], i + running_rows, s_len,
                               t_len, s_string, t_string);
      ...
      bool lob =
          ValidateCompareLobs(&s_cols[k], s_len, &t_cols[k], t_len, &equal);

      // Get the source and target values
      if (!lob) {
        GetColumnData(s_cols, i, k, source_type, _source_ca.db_api, &s_string,
                      &s_int_set, &s_int, &s_ts_struct, &s_ora_date);
        GetColumnData(t_cols, i, k, target_type, _target_ca.db_api, &t_string,
                      &t_int_set, &t_int, &t_ts_struct, &t_ora_date);
      }

      // Compare 2 string values
      if (s_string != NULL && t_string != NULL && !lob) {
        // Compare string representations of numbers .5 and 0.50
        bool num = ValidateCompareNumbers(&s_cols[k], s_string, s_len,
                                          &t_cols[k], t_string, t_len, &equal);
        bool datetime = false;

        if (!num)
          datetime = ValidateCompareDatetimes(
              &s_cols[k], s_string, s_len, &t_cols[k], t_string, t_len, &equal);

        if (!num && !datetime) {
          if (s_len == t_len)
            equal = memcmp(s_string, t_string, (size_t)s_len);
          else
            ValidateCompareStrings(&s_cols[k], s_string, s_len, &t_cols[k],
                                   t_string, t_len, &equal);
        }
      } else
        // Compare string and integer
        if (((s_int_set && t_string != NULL) ||
             (s_string != NULL && t_int_set)) &&
            !lob) {
          ...
          if (((s_ts_struct != NULL && t_string != NULL) ||
               (t_ts_struct != NULL && s_string != NULL)) &&
              !lob) {
            SQL_TIMESTAMP_STRUCT *ts =
                (s_ts_struct != NULL) ? s_ts_struct : t_ts_struct;
            char *str = (s_string != NULL) ? s_string : t_string;
            ...
            Str::SqlTs2Str((short)ts->year, (short)ts->month, (short)ts->day,
                           (short)ts->hour, (short)ts->minute,
                           (short)ts->second, (long)ts->fraction, ts_str);

            equal = memcmp(str, ts_str, (size_t)str_len);
          } else
            // Compare Oracle DATE and string
            if (((s_ora_date != NULL && t_string != NULL) ||
                 (t_ora_date != NULL && s_string != NULL)) &&
                !lob) {
              ...
              // Convert 7-byte packed Oracle DATE to string (non-null
              // terminated, exactly 19 characters)
              Str::OraDate2Str((unsigned char *)ora_date, ora_date_str);

              // Target SQL Server i.e. may have trailing .000 in string, so
              // let's use Oracle len
    ...
    if (equal_row == false) {
      ...
      if (_validation_not_equal_max_rows != -1 &&
          (not_equal_rows + running_not_equal_rows) >=
              _validation_not_equal_max_rows)
        break;
    }
  }
...
// Compare string representations of numbers .5 and 0.50
bool SqlDb::ValidateCompareNumbers(SqlCol *s_col, const char *s_string,
                                   int s_len, SqlCol *t_col,
                                   const char *t_string, int t_len,
                                   int *equal) {
  ...
  if ((source_type == SQLDATA_ORACLE && s_col->_native_dt == SQLT_NUM &&
       s_col->_scale != 129 /*floating*/) ||
      (source_type == SQLDATA_SYBASE &&
       s_col->_native_fetch_dt == CS_CHAR_TYPE &&
       (s_col->_native_dt == CS_NUMERIC_TYPE ||
        s_col->_native_dt == CS_DECIMAL_TYPE ||
        s_col->_native_dt == CS_MONEY_TYPE ||
        s_col->_native_dt == CS_MONEY4_TYPE)) ||
      (source_type == SQLDATA_INFORMIX &&
       s_col->_native_fetch_dt == SQL_C_CHAR &&
    ...
    // Compare floating point numbers that can be different due to rounding
    // issues Sybase ASE can return 6.7000000000000002 for value 6.7
    if ((source_type == SQLDATA_ORACLE && s_col->_native_dt == SQLT_NUM &&
         s_col->_scale == 129 /*floating*/) ||
        (source_type == SQLDATA_SYBASE &&
         s_col->_native_fetch_dt == CS_CHAR_TYPE &&
         (s_col->_native_dt == CS_REAL_TYPE ||
          s_col->_native_dt == CS_FLOAT_TYPE))) {
...
bool SqlDb::ValidateCompareDatetimes(SqlCol *s_col, const char *s_string,
                                     int s_len, SqlCol *t_col,
                                     const char *t_string, int t_len,
                                     int *equal) {
  if (s_col == NULL || s_string == NULL || t_col == NULL || t_string == NULL)
  ...
  if ((source_type == SQLDATA_ORACLE && s_col->_native_dt == SQLT_TIMESTAMP) ||
      (source_type == SQLDATA_SYBASE &&
       (s_col->_native_dt == CS_DATETIME_TYPE ||
        s_col->_native_dt == CS_DATETIME4_TYPE ||
        s_col->_native_dt == CS_DATE_TYPE)) &&
          (target_type == SQLDATA_MYSQL)) {
    int s_len_new = s_len;
    int t_len_new = t_len;

    // All datetime data types are fetched with 26 length from Sybase ASE, so we
    ...
    // If length are not equal allow only difference in number of 0s in
    // fractional part, so consider .0 and .000 equal
...
// Compare strings
bool SqlDb::ValidateCompareStrings(SqlCol *s_col, const char *s_string,
                                   int s_len, SqlCol *t_col,
                                   const char *t_string, int t_len,
                                   int *equal) {
  if (s_col == NULL || s_string == NULL || t_col == NULL || t_string == NULL)
...
// Compare LOB values
bool SqlDb::ValidateCompareLobs(SqlCol *s_col, int s_len, SqlCol *t_col,
                                int t_len, int *equal) {
  ...
  if (source_type == SQLDATA_ORACLE &&
      (s_col->_native_dt == SQLT_CLOB || s_col->_native_dt == SQLT_BLOB)) {
    if (s_len == t_len)
      comp = 0;
...
void SqlDb::ValidateDiffDump(SqlCol *s_col, SqlCol *t_col, int row, int s_len,
                             int t_len, char *s_string, char *t_string) {
  if (s_col == NULL || t_col == NULL)
    return;

  _trace_diff.LogFile(
      "Row %d, Column %s (src native type code %d, tgt native type code %d)\n",
...
int SqlDb::GetColumnDataLen(SqlCol *cols, int row, int column, int db_type,
                            SqlApiBase *db_api) {
  ...
  if (db_type == SQLDATA_ORACLE) {
    ...
    if (cols[column]._ind2[row] == -1)
      ...
      if (cols[column]._native_dt == SQLT_CLOB ||
          cols[column]._native_dt == SQLT_BLOB) {
        // LOB contains data
  ...
  } else if (db_type == SQLDATA_SYBASE) {
  ...
  } else if (db_type == SQLDATA_MYSQL) {
    if (cols[column].ind != NULL && cols[column].ind[row] != -1)
      len = cols[column].ind[row];
  } else if (db_type == SQLDATA_POSTGRESQL) {
    if (cols[column].ind != NULL && cols[column].ind[row] != -1)
      len = cols[column].ind[row];
  } else if (db_type == SQLDATA_SQL_SERVER || db_type == SQLDATA_INFORMIX) {
    if (cols[column].ind != NULL && cols[column].ind[row] != -1)
      len = cols[column].ind[row];
  }
...
int SqlDb::GetColumnData(SqlCol *cols, int row, int column, int db_type,
                         SqlApiBase *db_api, char **str, bool *int_set,
                         int *int_value, SQL_TIMESTAMP_STRUCT **ts_struct,
                         char **ora_date) {
  if (cols == NULL || db_api == NULL)
    return -1;
  ...
  if (db_type == SQLDATA_ORACLE) {
    if (cols[column]._native_fetch_dt == SQLT_STR ||
        cols[column]._native_fetch_dt == SQLT_BIN)
      s = cols[column]._data + cols[column]._fetch_len * row;
    else if (cols[column]._native_fetch_dt == SQLT_INT) {
      int_v = *((int *)(cols[column]._data + cols[column]._fetch_len * row));
      int_s = true;
    } else if (cols[column]._native_fetch_dt == SQLT_DAT)
    ...
    if (cols[column]._native_fetch_dt == SQL_C_CHAR ||
        cols[column]._native_fetch_dt == SQL_C_BINARY)
      s = cols[column]._data + cols[column]._fetch_len * row;
    else if (cols[column]._native_fetch_dt == SQL_C_LONG) {
      int_v = *((int *)(cols[column]._data + cols[column]._fetch_len * row));
      int_s = true;
    } else if (cols[column]._native_fetch_dt == SQL_TYPE_TIMESTAMP)
      ts = (SQL_TIMESTAMP_STRUCT *)(cols[column]._data +
                                    cols[column]._fetch_len * row);
  }
...
int SqlDb::BuildQuery(std::string &s_query, std::string &t_query,
                      const char *s_table, const char *t_table, bool transfer) {
  if (_source_ca.db_api == NULL)
    return -1;
  ...
  if (!select_list && source_type != SQLDATA_ORACLE &&
      source_type != SQLDATA_INFORMIX && source_type != SQLDATA_SYBASE &&
      source_type != SQLDATA_ASA) {
    if (s_table != NULL) {
      s_query += "* FROM ";
      s_query += s_table;
      ...
      if (s != NULL && t != NULL && c != NULL &&
        ...
        if (d != NULL &&
            ...
            ((source_type == SQLDATA_ORACLE && !_stricmp(d, "CHAR")) ||
             ...
             (source_type == SQLDATA_SYBASE &&
              (!_stricmp(d, "CHAR") || !_stricmp(d, "ID") ||
               !_stricmp(d, "TID"))))) {
          // MySQL by default trims spaces in CHAR, but PAD_CHAR_TO_FULL_LENGTH
          // can be set, so trim both
          ...
          if (source_type == SQLDATA_ORACLE && d != NULL &&
              strstr(d, "WITH TIME ZONE") != NULL) {
            ...
            if (source_type == SQLDATA_ORACLE && d != NULL &&
              ...
              // Oracle stores quoted columns without " but in the same case as
              // they specified in CREATE TABLE
              if (source_type == SQLDATA_ORACLE) {
                // Double quotes will not be returned when describing column
                // name after open cursor
                s_query += "\"";
                s_query += c;
                s_query += "\"";
                ...
                // Sybase ASE stores columns without [] even if they are
                // reserved words or contain spaces
                if (source_type == SQLDATA_SYBASE) {
                  s_query += '[';
                  s_query += c;
                  s_query += ']';
                  ...
                  // Sybase ASA stores columns that are reserved words without
                  // any quotes in catalog
                  if (source_type == SQLDATA_ASA &&
                      _source_ca.db_api->IsReservedWord(c)) {
                    s_query += '"';
                    s_query += c;
                    s_query += '"';
  ...
  if (transfer == false)
    BuildQueryAddOrder(s_query, s_schema, s_object, t_query, t_schema,
                       t_object);
...
int SqlDb::BuildQueryAddOrder(std::string &s_query, std::string &s_schema,
                              std::string &s_object, std::string &t_query,
                              std::string & /*t_schema*/,
                              std::string & /*t_object*/) {
  if (_metaSqlDb == NULL || _metaSqlDb->_source_ca.db_api == NULL ||
      _metaSqlDb->_target_ca.db_api == NULL)
    return -1;
  ...
  if (table_cns != NULL && table_cns_cols != NULL) {
    ...
    for (std::list<SqlConstraints>::iterator i = table_cns->begin();
         i != table_cns->end(); i++) {
  ...
  if (pk_exists == false && idx != NULL) {
    ...
    for (std::list<SqlIndexes>::iterator i = idx->begin(); i != idx->end();
         i++) {
      if ((*i).unique == false)
        continue;
  ...
  if (pk_exists == false && unique_exists == false && table_cols != NULL) {
    ...
    // Find table columns
    for (std::list<SqlColMeta>::iterator i = table_cols->begin();
         i != table_cols->end(); i++) {
      char *s = (*i).schema;
      char *t = (*i).table;
      char *c = (*i).column;
      ...
      if (d != NULL) {
        // A numeric column
        if (!_stricmp(d, "NUMBER") || !_stricmp(d, "INT") ||
            !_stricmp(d, "SMALLINT") || !_stricmp(d, "BIGINT") ||
            !_stricmp(d, "NUMERIC") || !_stricmp(d, "DECIMAL") ||
            !_stricmp(d, "FLOAT") || !_stricmp(d, "MONEY") ||
            !_stricmp(d, "SMALLMONEY")) {
          s_order.push_back(c);
          ...
          if (!_stricmp(d, "CHAR") || !_stricmp(d, "VARCHAR")) {
            ...
            // A datetime column
            if (strstr(d, "TIMESTAMP") != NULL || !_stricmp(d, "DATE") ||
                !_stricmp(d, "DATETIME")) {
              s_order.push_back(c);
        ...
        if (source_type == SQLDATA_INFORMIX) {
          // A numeric column SMALLINT - 1, INTEGER - 2, FLOAT - 3, SMALLFLOAT -
          // 4, DECIMAL - 5 SERIAL - 6 (if NOT NULL +256)
          if ((dc >= 1 && dc <= 6) || (dc >= 257 && dc <= 262)) {
            s_order.push_back(c);
            s_order_types.push_back("Numeric");
          } else
            // A string column CHAR - 0, CHAR NOT NULL - 256, VARCHAR - 13,
            // VARCHAR NOT NULL 269
  ...
  if (s_order.empty() == false) {
    ...
    for (std::list<std::string>::iterator i = s_order.begin();
         i != s_order.end(); i++) {
      std::string type;
      ...
      if (source_type == SQLDATA_ORACLE && target_type == SQLDATA_MYSQL)
        s_query += " NULLS FIRST";
      else if ((source_type == SQLDATA_MYSQL ||
                source_type == SQLDATA_INFORMIX) &&
               target_type == SQLDATA_ORACLE)
        t_query += " NULLS FIRST";
      else
        // MySQL/MariaDB uses collation defined in the table, for some reason it
        // does not use COLLATION_CONNECTION session value so we redefine
        // COLLATE in ORDER BY for strings
        if (target_type == SQLDATA_MYSQL) {
          ...
          if (source_type == SQLDATA_INFORMIX &&
              target_type == SQLDATA_POSTGRESQL) {
            // Informix sorts strings in ASCII order - space, digit, A,B,a, b;
            // while PostgreSQL space, digit, a, A, b, B when collation is not
            // "C"
            if (type == "String") {
              // Collate "C" to use ASCII code sorting is available since
              // PostgreSQL 9.1
              if (t_db_api->GetMajorVersion() >= 9 &&
                  t_db_api->GetMinorVersion() >= 1)
...
int SqlDb::ExecuteNonQuery(int db_types, SqlDataReply &reply,
                           const char *query) {
...
int SqlDb::CreateSequence(int db_types, SqlDataReply &reply, const char *query,
                          const char *seq_name) {
  if (seq_name == NULL)
    return -1;
...
int SqlDb::DropReferences(int db_types, const char *table, size_t *time_spent,
                          int *keys) {
  SqlApiBase *api =
      (db_types == SQLDB_SOURCE_ONLY) ? _source_ca.db_api : _target_ca.db_api;
...
int SqlDb::Execute(int cmd) { return Execute(cmd, cmd); }

// Execute one command for source, another for target in concurrent threads and
// wait
...
#if defined(WIN32) || defined(_WIN64)
  HANDLE handles[2] = {_source_ca._completed_event,
                       _target_ca._completed_event};
  ...
  while (more) {
    ...
    if (ca->_cmd == SQLDATA_CMD_CONNECT) {
      ...
      if (ca->_cmd == SQLDATA_CMD_VALIDATE_ROW_COUNT) {
        ca->cmd_rc = (short)ca->db_api->GetRowCount(ca->_name, &ca->_int1,
                                                    &ca->_time_spent);
      } else
        ...
        if (ca->_cmd == SQLDATA_CMD_OPEN_CURSOR) {
          ...
          ca->cmd_rc = (short)ca->db_api->OpenCursor(
              (const char *)ca->_void1, (size_t)ca->_int4, 0, &col_count,
              &allocated_rows, &ca->_int3, (SqlCol **)&ca->_void2,
              &ca->_time_spent);
          ...
          if (ca->_cmd == SQLDATA_CMD_FETCH_NEXT) {
            ...
            if (ca->_cmd == SQLDATA_CMD_INSERT_NEXT) {
              ...
              ca->cmd_rc = (short)ca->db_api->TransferRows(
                  (SqlCol *)ca->_void1, ca->_int2, &ca->_int1, &bytes_written,
                  &time_spent);
              ...
              if (ca->_cmd == SQLDATA_CMD_CLOSE_CURSOR) {
                ...
                if (ca->_cmd == SQLDATA_CMD_READ_SCHEMA) {
                  ca->cmd_rc = (short)ca->db_api->ReadSchema(
                      (const char *)ca->_void1, (const char *)ca->_void2);
                } else
                  // Read the schema of the database where transfer is performed
                  // to
                  if (ca->_cmd == SQLDATA_CMD_READ_SCHEMA_FOR_TRANSFER_TO) {
                    ca->cmd_rc = (short)ca->db_api->ReadSchemaForTransferTo(
                        (const char *)ca->_void1, (const char *)ca->_void2);
                  }
...
void SqlDb::SetParameters(Parameters *p) {
  ...
  _validation_not_equal_max_rows =
      _parameters->GetInt("-validation_not_equal_max_rows", -1);
  _validation_datetime_fraction =
      _parameters->GetInt("-validation_datetime_fraction", -1);
