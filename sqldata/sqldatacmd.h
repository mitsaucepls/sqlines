...
#define WORKERS_OPTION                                                         \
  "-wrk" // Number of separate worker processes (undocumented)
...
class SqlDataCmd {
  ...
  // List of tables with different number of read and written rows (includes
  // table name and row count information)
  std::list<std::string> _tables_read_write_diff_list;
  // List of tables with different row count (includes table name and row count
  // information)
...
public:
  ...
  void SetConsoleFunc(void *object, AppLogConsoleFunc console) {
    _log.SetConsoleFunc(object, console);
  }

  void SetUiCallback(void *o, SqlDataCallbackFunc f) {
    _ui_callback = f;
    _ui_callback_object = o;
  }
...
private:
  ...
  int GetQueriesFromFile(std::string &file,
  ...
  void LogConnection(int type, int rc, const char *version, const char *error,
                     size_t time_spent);
