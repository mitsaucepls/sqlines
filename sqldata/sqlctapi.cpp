...
int SqlCtApi::Init() {
  ...
  if (_ct_dll == NULL) {
    ...
    for (std::list<std::string>::iterator i = paths.begin(); i != paths.end();
         i++) {
      // For Sybase 15 and 16, DDLs are in \ASE-15_0\bin, and cannot be loaded
      // from OCS-15_0 as some dependent DDLs are missed (sybcsi_core27.dll)
  ...
  if (_ct_dll != NULL && _cs_dll != NULL) {
    _cs_ctx_alloc =
        (cs_ctx_allocFunc)Os::GetProcAddress(_cs_dll, "cs_ctx_alloc");
    ...
    _cs_loc_alloc =
        (cs_loc_allocFunc)Os::GetProcAddress(_cs_dll, "cs_loc_alloc");
    ...
    if (_cs_ctx_alloc == NULL || _cs_config == NULL || _cs_ctx_drop == NULL ||
        _cs_dt_info == NULL || _cs_locale == NULL || _cs_loc_alloc == NULL ||
        _ct_bind == NULL || _ct_cancel == NULL || _ct_command == NULL ||
        _ct_con_alloc == NULL || _ct_con_drop == NULL ||
        _ct_con_props == NULL || _ct_connect == NULL || _ct_close == NULL ||
        _ct_cmd_alloc == NULL || _ct_cmd_drop == NULL || _ct_describe == NULL ||
        _ct_diag == NULL || _ct_exit == NULL || _ct_fetch == NULL ||
        _ct_init == NULL || _ct_res_info == NULL || _ct_results == NULL ||
        _ct_send == NULL)
      return -1;
  } else {
  ...
  if (_cs_ctx_alloc != NULL) {
    ...
    _cs_locale(_context, CS_SET, _locale, CS_LC_ALL, (CS_CHAR *)NULL, CS_UNUSED,
               (CS_INT *)NULL);
    ...
    if (Str::IsSet(codepage))
      ret = _cs_locale(_context, CS_SET, _locale, CS_SYB_CHARSET, codepage,
                       strlen(codepage), (CS_INT *)NULL);

    // Define the string format for all data types
    CS_INT dt_fmt_type = CS_DATES_YMDHMSUS_YYYY;
    _cs_dt_info(_context, CS_SET, _locale, CS_DT_CONVFMT, CS_UNUSED,
                (CS_VOID *)&dt_fmt_type, CS_SIZEOF(CS_INT), NULL);
...
int SqlCtApi::Connect(size_t *time_spent) {
  ...
  rc = _ct_con_props(_connection, CS_SET, CS_USERNAME, (CS_VOID *)_user.c_str(),
                     CS_NULLTERM, NULL);
  TRACE_P("Sybase CTLIB ct_con_props() CS_USERNAME, retcode %d", rc);

  rc = _ct_con_props(_connection, CS_SET, CS_PASSWORD, (CS_VOID *)_pwd.c_str(),
                     CS_NULLTERM, NULL);
  ...
  if (_parameters->GetTrue("-sybase_encrypted_password") != NULL) {
    ...
    // Sybase uses extended password encryption as the first preference, if the
    // server cannot support extended password encryption, it uses normal
    // password encryption
    rc = _ct_con_props(_connection, CS_SET, CS_SEC_EXTENDED_ENCRYPTION,
                       (CS_VOID *)&sec_encryption, CS_UNUSED, NULL);
    rc = _ct_con_props(_connection, CS_SET, CS_SEC_ENCRYPTION,
                       (CS_VOID *)&sec_encryption, CS_UNUSED, NULL);
  }
  ...
  if (!_port.empty()) {
    ...
    rc = _ct_con_props(_connection, CS_SET, CS_SERVERADDR,
                       (CS_VOID *)serveraddr.c_str(), CS_NULLTERM, NULL);
  ...
  if (_db.empty() == false) {
    ...
    rc = _ct_command(cmd, CS_LANG_CMD, (CS_CHAR *)sql.c_str(), CS_NULLTERM,
                     CS_UNUSED);
...
int SqlCtApi::ExecuteScalar(const char *query, int *result,
                            size_t *time_spent) {
...
int SqlCtApi::OpenCursor(const char *query, size_t buffer_rows,
  ...
  rc = _ct_command(_cursor_cmd, CS_LANG_CMD, (CS_CHAR *)query, CS_NULLTERM,
                   CS_UNUSED);
  TRACE_P("Sybase CTLIB ct_command(), retcode %d", rc);
  ...
  // Even if SELECT is incorrect (syntax errors i.e.) ct_command, ct_send and
  // ct_results return CS_SUCCEED, but ct_results returns CS_CMD_FAIL (4048) in
  // type (second param)
  if ((rc = _ct_results(_cursor_cmd, &type)) != CS_SUCCEED ||
      type != CS_ROW_RESULT) {
    SetError();

    // We must cancel of fetch all ct_results before dropping the command;
    // otherwise this drop fails and all subsequent statements will fail as well
  ...
  rc = _ct_res_info(_cursor_cmd, CS_NUMDATA, &_cursor_cols_count, CS_UNUSED,
                    NULL);
  ...
  for (int i = 0; i < _cursor_cols_count; i++) {
    // Note that CHAR and VARCHAR both have CS_CHAR_TYPE, and you cannot
    // distinguish them using CS_FMT_PADBLANK and CS_FMT_PADNULL in fmt.format
    // as it is always 0 (CS_FMT_UNUSED) in ct_describe (Not applicable)

    // CS_LONGCHAR_TYPE is returned for VARCHAR > 255 (max length is 32K since
    // ASE 12.5)

    rc = _ct_describe(_cursor_cmd, i + 1, &fmt[i]);
    TRACE_P("Sybase CTLIB ct_describe(), retcode %d", rc);
    TRACE_P("Sybase CTLIB data type %d, len %d, name len %d",
            (int)fmt[i].datatype, (int)fmt[i].maxlength, (int)fmt[i].namelen);
    ...
    if (_cursor_cols[i]._native_dt == CS_TEXT_TYPE ||
        _cursor_cols[i]._native_dt == CS_UNITEXT_TYPE) {
  ...
  for (int i = 0; i < _cursor_cols_count; i++) {
    // CHAR and VARCHAR data types; CS_CHAR_TYPE for VARCHAR <= 255, and
    // CS_LONGCHAR_TYPE for VARCHAR < 32K
    if (_cursor_cols[i]._native_dt == CS_CHAR_TYPE ||
        _cursor_cols[i]._native_dt == CS_LONGCHAR_TYPE) {
      // Do not bind to null-terminating string as zero byte will be included to
      // length indicator
          ...
          if (_cursor_cols[i]._native_dt == CS_INT_TYPE) {
            ...
            if (_cursor_cols[i]._native_dt == CS_SMALLINT_TYPE) {
              ...
              if (_cursor_cols[i]._native_dt == CS_TINYINT_TYPE) {
                ...
                // NUMERIC and DECIMAL
                if (_cursor_cols[i]._native_dt == CS_NUMERIC_TYPE ||
                    _cursor_cols[i]._native_dt == CS_DECIMAL_TYPE) {
                  _cursor_cols[i]._native_fetch_dt = CS_CHAR_TYPE;

                  // Include sign, decimal point and terminating zero
                  _cursor_cols[i]._fetch_len =
                      (size_t)(_cursor_cols[i]._precision +
                      ...
                      if (_cursor_cols[i]._native_dt == CS_DATETIME_TYPE) {
                        ...
                        if (_cursor_cols[i]._native_dt == CS_DATETIME4_TYPE) {
                          ...
                          if (_cursor_cols[i]._native_dt == CS_DATE_TYPE) {
                            ...
                            // All date/time data types are fetched in full
                            // length, since ANSI ISO format cannot be specified
                            ...
                            fmt[i].datatype = CS_CHAR_TYPE;
                            fmt[i].maxlength =
                                (CS_INT)_cursor_cols[i]._fetch_len;
                          } else
                            // TIME, length is 4
                            if (_cursor_cols[i]._native_dt == CS_TIME_TYPE) {
                              ...
                              // All date/time data types are fetched in full
                              // length, since ANSI ISO format cannot be
                              // specified separately for each type
                              _cursor_cols[i]._fetch_len = 26;
                              _cursor_cols[i]._data =
                                  new char[_cursor_cols[i]._fetch_len *
                                           _cursor_allocated_rows];
                              ...
                              if (_cursor_cols[i]._native_dt == CS_FLOAT_TYPE) {
                                ...
                                if (_cursor_cols[i]._native_dt == CS_BIT_TYPE) {
                                  ...
                                  if (_cursor_cols[i]._native_dt ==
                                    ...
                                    if (_cursor_cols[i]._native_dt ==
                                        CS_UNITEXT_TYPE) {
                                      // Data fetched as UTF-16 i.e. 0x00 byte
                                      // goes first for first 127 ASCII
                                      // characters
                                      _cursor_cols[i]._native_fetch_dt =
                                          CS_UNICHAR_TYPE;
    ...
    if (_cursor_cols[i]._data != NULL) {
      ...
      rc = _ct_bind(_cursor_cmd, i + 1, &fmt[i], _cursor_cols[i]._data,
                    (CS_INT *)_cursor_cols[i]._len_ind4, _cursor_cols[i]._ind2);
  ...
  rc = _ct_fetch(_cursor_cmd, CS_UNUSED, CS_UNUSED, CS_UNUSED,
                 (CS_INT *)&fetched);
...
int SqlCtApi::Fetch(int *rows_fetched, size_t *time_spent) {
  ...
  // Fetch the data
  int rc = _ct_fetch(_cursor_cmd, CS_UNUSED, CS_UNUSED, CS_UNUSED,
                     (CS_INT *)&fetched);
...
int SqlCtApi::CloseCursor() {
  ...
  // Close cursor can be called when not all rows are fetched (error creating
  // table in the target database i.e.), so we need to cancel command
...
int SqlCtApi::InitBulkTransfer(const char * /*table*/, size_t /*col_count*/,
...
int SqlCtApi::TransferRows(SqlCol * /*s_cols*/, int /*rows_fetched*/,
...
int SqlCtApi::CloseBulkTransfer() { return -1; }

// Drop the table
int SqlCtApi::DropTable(const char *table, size_t *time_spent,
...
int SqlCtApi::GetLobLength(size_t /*row*/, size_t /*column*/,
                           size_t * /*length*/) {
  return -1;
}

// Get LOB content
int SqlCtApi::GetLobContent(size_t /*row*/, size_t /*column*/, void * /*data*/,
                            size_t /*length*/, int * /*len_ind*/) {
  return -1;
}

// Get the list of available tables
int SqlCtApi::GetAvailableTables(std::string &table_template,
                                 std::string & /*exclude*/,
...
int SqlCtApi::ReadSchema(const char *select, const char *exclude, bool read_cns,
                         bool read_idx) {
  std::string condition;
  ...
  GetSelectionCriteria(select, exclude, "u.name", "o.name", condition, "dbo",
                       false);
...
int SqlCtApi::ReadTableColumns(std::string &condition) {
  ...
  // Do **not** join on 2 columns: c.type = t.type AND c.usertype = t.usertype -
  // it does not match (tested on pubs3 and some columns can be lost)
  std::string query =
      "select u.name, o.name, c.name, t.name, c.colid, o.id, c.status";
  query += " FROM sysusers u, sysobjects o, syscolumns c, systypes t";
  query += " WHERE o.type = 'U' AND o.uid = u.uid AND o.id = c.id AND "
...
int SqlCtApi::ReadIndexes(std::string &condition) {
  // Tested on Sybase ASE 16
  std::string query = "select u.name, o.name, i.name, i.status, i.id, i.indid, "
                      "index_col(object_name(i.id), indid, 1), "
                      "index_col(object_name(i.id), indid, 2),";
  query +=
      " index_col(object_name(i.id), indid, 3), index_col(object_name(i.id), "
      "indid, 4), index_col(object_name(i.id), indid, 5),";
  query +=
      " index_col(object_name(i.id), indid, 6), index_col(object_name(i.id), "
      "indid, 7), index_col(object_name(i.id), indid, 8)";
  query += " from sysindexes i, sysobjects o, sysusers u";
  query +=
      " where i.indid <> 0 and i.id = o.id and o.type = 'U' and indid <> 255 "
      "and substring(object_name(i.id),1,3) <> 'sys' and o.uid = u.uid ";
  ...
  while (rc >= 0) {
    ...
    for (int i = 0; i < rows_fetched; i++) {
      ...
      // Status (SMALLINT) - bits: 0x2 - Unique index, 0x800 - Index on primary
      // key
      if (len == 2) {
      ...
      for (int k = 0; k < 8; k++) {
        ...
        strncpy(idx_col.column, cols[6 + k]._data + cols[6 + k]._fetch_len * i,
                (size_t)len);
...
int SqlCtApi::ReadReferences(std::string &condition) {
  // Tested on Sybase ASE 16
  std::string query = "select pu.name, po.name, u.name, o.name, co.name, "
                      "r.tableid, r.constrid,";
  query += " col_name(r.tableid,fokey1), col_name(r.tableid,fokey2), "
           "col_name(r.tableid,fokey3), col_name(r.tableid,fokey4), "
           "col_name(r.tableid,fokey5),";
  query += " col_name(r.tableid,fokey6), col_name(r.tableid,fokey7), "
           "col_name(r.tableid,fokey8), col_name(r.tableid,fokey9), "
           "col_name(r.tableid,fokey10),";
  query += " col_name(r.tableid,fokey11), col_name(r.tableid,fokey12), "
           "col_name(r.tableid,fokey13), col_name(r.tableid,fokey14), "
           "col_name(r.tableid,fokey15),";
  query += " col_name(r.tableid,fokey16),";
  query += " col_name(r.reftabid,refkey1), col_name(r.reftabid,refkey2), "
           "col_name(r.reftabid,refkey3), col_name(r.reftabid,refkey4), "
           "col_name(r.reftabid,refkey5),";
  query += " col_name(r.reftabid,refkey6), col_name(r.reftabid,refkey7), "
           "col_name(r.reftabid,refkey8), col_name(r.reftabid,refkey9), "
           "col_name(r.reftabid,refkey10),";
  query += " col_name(r.reftabid,refkey11), col_name(r.reftabid,refkey12), "
           "col_name(r.reftabid,refkey13), col_name(r.reftabid,refkey14), "
           "col_name(r.reftabid,refkey15),";
  query += " col_name(r.reftabid,refkey16)";
  query += " from sysreferences r, sysobjects po, sysusers pu, sysobjects o, "
           "sysusers u, sysobjects co";
  query += " where r.reftabid = po.id and po.uid = pu.uid and r.tableid = o.id "
           "and o.uid = u.uid and r.constrid = co.id";
  ...
  while (rc >= 0) {
    ...
    for (int i = 0; i < rows_fetched; i++) {
      ...
      if (len != -1) {
        ...
        strncpy(cns.pk_schema, cols[0]._data + cols[0]._fetch_len * i,
                (size_t)len);
      ...
      if (len != -1) {
        ...
        strncpy(cns.pk_table, cols[1]._data + cols[1]._fetch_len * i,
                (size_t)len);
        cns.pk_table[len] = '\x0';
      }
      ...
      if (len != -1) {
        ...
        strncpy(cns.schema, cols[2]._data + cols[2]._fetch_len * i,
                (size_t)len);
        cns.schema[len] = '\x0';
      }
      ...
      if (len != -1) {
        ...
        strncpy(cns.constraint, cols[4]._data + cols[4]._fetch_len * i,
                (size_t)len);
      ...
      for (int k = 0; k < 16; k++) {
        ...
        // No more columns in the foreign key - COL_NAME() returns NULL when
        // applied for column ID 0
        if (len == -1)
          break;
        ...
        strncpy(cns_col.column, cols[7 + k]._data + cols[7 + k]._fetch_len * i,
                (size_t)len);
        ...
        if (len != -1) {
          ...
          strncpy(cns_col.pk_column,
                  cols[7 + 16 + k]._data + cols[7 + 16 + k]._fetch_len * i,
                  (size_t)len);
    ...
    for (int i = 0; i < rows_fetched; i++) {
      ...
      if (prev_id == -1 || obj_meta.id != id) {
        ...
        if (len != -1) {
          ...
          strncpy(obj_meta.schema, cols[2]._data + cols[2]._fetch_len * i,
                  (size_t)len);
        ...
        if (len != -1) {
          ...
          strncpy(obj_meta.name, cols[3]._data + cols[3]._fetch_len * i,
                  (size_t)len);
...
int SqlCtApi::ReadConstraintTable(const char * /*schema*/,
...
int SqlCtApi::ReadConstraintColumns(const char * /*schema*/,
                                    const char * /*table*/,
                                    const char * /*constraint*/,
                                    std::string & /*cols*/) {
  return -1;
...
int SqlCtApi::GetKeyConstraintColumns(SqlConstraints &cns,
...
int SqlCtApi::GetForeignKeyConstraintColumns(SqlConstraints &cns,
...
void SqlCtApi::GetCatalogSelectionCriteria(std::string & /*selection_template*/,
                                           std::string & /*output*/) {}
...
void SqlCtApi::SetVersion() {
  ...
  if (rc >= 0 && cols != NULL && cols[0]._ind2 != NULL &&
      cols[0]._ind2[0] != -1)
...
void SqlCtApi::FindSybasePaths(std::list<std::string> &paths) {
  ...
  int rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Sybase\\Setup", 0,
                        KEY_READ, &hKey);

  if (rc == ERROR_SUCCESS) {
    // For 15.0 "Sybase" parameter contains installation directory without
    // terminating '\'
    rc = RegQueryValueEx(hKey, "Sybase", NULL, NULL, (LPBYTE)location,
                         (LPDWORD)&value_size);
  ...
  rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Sybase\\SQLServer", 0,
                    KEY_READ, &hKey);

  if (rc == ERROR_SUCCESS) {
    // For 16 "RootDir" parameter contains installation directory without
    // terminating '\'
    rc = RegQueryValueEx(hKey, "RootDir", NULL, NULL, (LPBYTE)location,
                         (LPDWORD)&value_size);
