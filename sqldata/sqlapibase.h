...
#define TRACE_P(mess, ...)                                                     \
  {                                                                            \
    if (_trace && _log != NULL)                                                \
      _log->Trace(mess, ##__VA_ARGS__);                                        \
  }
#define TRACE_S(obj, mess)                                                     \
  {                                                                            \
    if (obj->_trace && obj->_log != NULL)                                      \
      obj->_log->Trace(mess);                                                  \
  }

#define TRACE_DMP_INIT(file)                                                   \
  {                                                                            \
    if (_trace_data && _log != NULL) {                                         \
      File::Truncate(file);                                                    \
      _trace_data_file = file;                                                 \
    }                                                                          \
  }
#define TRACE_DMP(data, len)                                                   \
  {                                                                            \
    if (_trace_data)                                                           \
      File::Append(_trace_data_file.c_str(), data, len);                       \
  }
...
struct SqlCol {
  ...
  // Precision and scale for numeric data types (scale also specifies fraction
  // for TIMESTAMP)
  int _precision;
  int _scale;
  ...
  // NULL and length indicator for ODBC (4-byte for 32-bit Windows, and 8-byte
  // for 64-bit Windows) size_t is 32-bit on 32-bit Windows and 64-bit on 64-bit
  // Windows
  size_t *ind;
  ...
  // For data validation, the total number of rows where the column values are
  // different
  int _diff_rows;
...
struct SqlConstraints {
  ...
  // Constraint type P for primary key, C for check, R for foreign key, U for
  // unique
  char type;
  ...
  // For foreign keys, reference to the primary key or unique constraint
  // (Oracle)
  char *r_schema;
  char *r_constraint;
...
protected:
  ...
  // The maximum number of rows in read/write batch, by default it is defined by
  // the buffer size divided by the table row size
  size_t _batch_max_rows;

  // Fixed size buffer to read LOB values by binding not reading by parts (can
  // cause truncation error if the buffer is less than the maximum LOB value,
  // default is to read LOBs by separate calls
  int _lob_bind_buffer;

  // When converting from ASCII or UTF16/UCS-2 character sets in the source
  // database to UTF8 i.e. in the target database depending on the actual data
  // you may need greater storage size. And vice versa converting in opposite
  // direction you may require smaller storage size. This parameter specifies
  // the length change ratio. If the source length is 100, and ratio is 1.1 then
  // the target length will be 110
...
public:
  ...
  virtual int GetRowCount(const char *object, int *count,
                          size_t *time_spent) = 0;

  // Execute the statement and get scalar result
  virtual int ExecuteScalar(const char *query, int *result,
                            size_t *time_spent) = 0;
  ...
  virtual int InitBulkTransfer(const char *table, size_t col_count,
                               size_t allocated_array_rows, SqlCol *s_cols,
                               SqlCol **t_cols) = 0;

  // Transfer rows between databases
  virtual int TransferRows(SqlCol *s_cols, int rows_fetched, int *rows_written,
                           size_t *bytes_written, size_t *time_spent) = 0;

  // Specifies whether API allows to parallel reading from this API and write to
  // another API
  virtual bool CanParallelReadWrite() = 0;

  // Specifies whether target bound source buffers, so the same buffers must be
  ...
  // Drop the table
  virtual int DropTable(const char *table, size_t *time_spent,
                        std::string &drop_stmt) = 0;
  ...
  virtual int DropReferencesChild(const char * /*table*/,
                                  size_t * /*time_spent*/, int * /*keys*/) {
    return -1;
  }
  ...
  virtual char *GetLobBuffer(size_t /*row*/, size_t /*column*/,
                             size_t /*length*/, size_t * /*alloc_len*/) {
    return NULL;
  }
  virtual void FreeLobBuffer(char * /*buf*/) { return; }
  // Get LOB content
  virtual int GetLobContent(size_t row, size_t column, void *data,
                            size_t length, int *len_ind) = 0;
  // Get partial LOB content
  virtual int GetLobPart(size_t row, size_t column, void *data, size_t length,
                         int *len_ind) = 0;

  // Get the list of available tables
  virtual int GetAvailableTables(std::string &table_template,
                                 std::string &exclude,
                                 std::list<std::string> &tables) = 0;

  // Read schema information
  virtual int ReadSchema(const char *select, const char *exclude,
                         bool read_cns = true, bool read_idx = true) = 0;
  virtual int ReadSchemaForTransferTo(const char * /*select*/,
                                      const char * /*exclude*/) {
    return 0;
  }
  void ClearSchema();
  ...
  virtual int ReadConstraintTable(const char *schema, const char *constraint,
                                  std::string &table) = 0;
  virtual int ReadConstraintColumns(const char *schema, const char *table,
                                    const char *constraint,
                                    std::string &cols) = 0;

  // Get a list of columns for specified primary or unique key
  virtual int GetKeyConstraintColumns(SqlConstraints & /*cns*/,
  ...
  virtual int GetForeignKeyConstraintColumns(SqlConstraints & /*cns*/,
  ...
  virtual int GetIndexColumns(SqlIndexes & /*idx*/,
  ...
  virtual SqlConstraints *GetConstraint(const char *schema, const char *table,
                                        const char *name);

  // Split qualified name to schema and object name
  static void SplitQualifiedName(const char *qualified, std::string &schema,
                                 std::string &object);

  // Split connection string to user, password and database
  void SplitConnectionString(const char *conn, std::string &user,
                             std::string &pwd, std::string &db);
  void SplitConnectionString(const char *conn, std::string &user,
                             std::string &pwd, std::string &server,
                             std::string &db, std::string &port);

  // Build a condition to select objects from the catalog
  static void GetSelectionCriteria(const char *select, const char *exclude,
                                   const char *schema, const char *object,
                                   std::string &output,
                                   const char *default_schema, bool upper_case);
  static void GetSelectionCriteria(const char *selection_template,
                                   const char *schema, const char *object,
                                   std::string &output,
                                   const char *default_schema, bool upper_case);
  ...
  std::list<SqlConstraints> *GetTableConstraints() {
    return &_table_constraints;
  }
  std::list<SqlConsColumns> *GetConstraintColumns() {
    return &_table_cons_columns;
  }
  ...
  std::list<SqlIndExp> *GetIndexExpressions() {
    return &_table_ind_expressions;
  }
  ...
  bool IsReservedWord(std::string &name) {
    return IsReservedWord(name.c_str());
  }

  // Get column default by table and column name
  virtual int GetColumnDefault(const char * /*table*/, const char * /*column*/,
                               std::string & /*col_default*/) {
    return -1;
  }
  // Get table or column comment by table and column name
  virtual int GetComment(const char * /*table*/, const char * /*column*/,
                         std::string & /*comment*/) {
    return -1;
  }
