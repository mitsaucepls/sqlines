...
SqlOdbcApi::~SqlOdbcApi() {
  // Note that some drivers Oracle ODBC i.e. cause application crash on exit
  // when not deallocated
  Deallocate();
}
...
int SqlOdbcApi::Connect(size_t * /*time_spent*/) {
  ...
  rc =
      SQLDriverConnect(_hdbc, NULL, (SQLCHAR *)conn.c_str(), SQL_NTS, full_conn,
                       1024, &full_conn_out_len, SQL_DRIVER_NOPROMPT);
...
int SqlOdbcApi::GetRowCount(const char *object, int *count,
                            size_t *time_spent) {
  if (object == NULL)
    return -1;
...
int SqlOdbcApi::ExecuteScalar(const char *query, int *result,
                              size_t *time_spent) {
  SQLHANDLE stmt;
  int q_result = 0;
...
int SqlOdbcApi::OpenCursor(const char *query, size_t buffer_rows,
  ...
  for (int i = 0; i < _cursor_cols_count; i++) {
    ...
    // Save precision and scale for decimal type (returned for Oracle
    // NUMBER(p,s)
  ...
  for (int i = 0; i < _cursor_cols_count; i++) {
    ...
    if (_cursor_cols[i]._native_dt == SQL_CHAR ||
      ...
      if (_db_type == SQLDATA_ORACLE &&
          _cursor_cols[i]._native_dt == SQL_DECIMAL &&
          _cursor_cols[i]._precision <= 9 && _cursor_cols[i]._scale == 0) {
    ...
    if (_cursor_cols[i]._data != NULL) {
      ...
      rc = SQLBindCol(_hstmt_cursor, (SQLUSMALLINT)(i + 1),
                      ...
                      _cursor_cols[i]._data, (SQLLEN)_cursor_cols[i]._fetch_len,
...
int SqlOdbcApi::InitBulkTransfer(const char * /*table*/, size_t /*col_count*/,
...
int SqlOdbcApi::TransferRows(SqlCol * /*s_cols*/, int /*rows_fetched*/,
...
int SqlOdbcApi::WriteLob(SqlCol * /*s_cols*/, int /*row*/,
                         int * /*lob_bytes*/) {
  return -1;
}

// Complete bulk transfer
int SqlOdbcApi::CloseBulkTransfer() { return -1; }
...
// Drop the table
int SqlOdbcApi::DropTable(const char * /*table*/, size_t * /*time_spent*/,
                          std::string & /*drop_stmt*/) {
  return -1;
}

// Remove foreign key constraints referencing to the parent table
int SqlOdbcApi::DropReferences(const char * /*table*/,
                               size_t * /*time_spent*/) {
  return -1;
}

// Get the length of LOB column in the open cursor
int SqlOdbcApi::GetLobLength(size_t /*row*/, size_t /*column*/,
                             size_t * /*length*/) {
  return -1;
}

// Get LOB content
int SqlOdbcApi::GetLobContent(size_t /*row*/, size_t /*column*/,
...
int SqlOdbcApi::GetAvailableTables(std::string &table_template,
                                   std::string & /*exclude*/,
...
int SqlOdbcApi::ReadSchema(const char * /*select*/, const char * /*exclude*/,
                           bool /*read_cns*/, bool /*read_idx*/) {
  return -1;
}

// Get table name by constraint name
int SqlOdbcApi::ReadConstraintTable(const char * /*schema*/,
...
int SqlOdbcApi::ReadConstraintColumns(const char * /*schema*/,
                                      const char * /*table*/,
                                      const char * /*constraint*/,
                                      std::string & /*cols*/) {
  return -1;
...
void SqlOdbcApi::GetCatalogSelectionCriteria(
