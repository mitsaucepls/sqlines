...
SqlIfmxApi::~SqlIfmxApi() { Deallocate(); }
...
int SqlIfmxApi::Init() {
  ...
  if (_dll == NULL) {
    ...
    for (std::list<std::string>::iterator i = paths.begin(); i != paths.end();
         i++) {
      // For Informix 11.7 path already includes terminating \ and DLL is
      // located in BIN
  ...
  if (_dll != NULL) {
    ...
    _SQLAllocHandle =
        (SQLAllocHandleFunc)Os::GetProcAddress(_dll, "SQLAllocHandle");
    _SQLBindCol = (SQLBindColFunc)Os::GetProcAddress(_dll, "SQLBindCol");
    _SQLDescribeCol =
        (SQLDescribeColFunc)Os::GetProcAddress(_dll, "SQLDescribeCol");
    _SQLDisconnect =
        (SQLDisconnectFunc)Os::GetProcAddress(_dll, "SQLDisconnect");
    _SQLDriverConnect =
        (SQLDriverConnectFunc)Os::GetProcAddress(_dll, "SQLDriverConnect");
    _SQLExecDirect =
        (SQLExecDirectFunc)Os::GetProcAddress(_dll, "SQLExecDirect");
    _SQLFetch = (SQLFetchFunc)Os::GetProcAddress(_dll, "SQLFetch");
    _SQLFreeHandle =
        (SQLFreeHandleFunc)Os::GetProcAddress(_dll, "SQLFreeHandle");
    _SQLGetData = (SQLGetDataFunc)Os::GetProcAddress(_dll, "SQLGetData");
    _SQLGetDiagRec =
        (SQLGetDiagRecFunc)Os::GetProcAddress(_dll, "SQLGetDiagRec");
    _SQLNumResultCols =
        (SQLNumResultColsFunc)Os::GetProcAddress(_dll, "SQLNumResultCols");
    _SQLSetConnectAttr =
        (SQLSetConnectAttrFunc)Os::GetProcAddress(_dll, "SQLSetConnectAttr");
    _SQLSetEnvAttr =
        (SQLSetEnvAttrFunc)Os::GetProcAddress(_dll, "SQLSetEnvAttr");
    _SQLSetStmtAttr =
        (SQLSetStmtAttrFunc)Os::GetProcAddress(_dll, "SQLSetStmtAttr");
...
int SqlIfmxApi::Connect(size_t *time_spent) {
  ...
  // Informix ODBC ignores CLIENT_LOCALE set in SETNET32 and environment
  // variables
  ...
  if (client_locale != NULL && *client_locale != '\x0') {
    ...
    // DB_LOCALE must be also set, for example if database is en_US.57372
    // (UTF-8), and only CLIENT_LOCATE is set to this value, still got Locale
    // mismatch error on connection
    conn += "Db_Locale=";
    conn += client_locale;
    conn += ";";
  }
  // Informix crashes on conversion errors, so set default database locale that
  // is set on Informix servers (both Windows and Unix)
  else
  ...
  rc = _SQLDriverConnect(_hdbc, NULL, (SQLCHAR *)conn.c_str(), SQL_NTS,
                         full_conn, 1024, &full_conn_out_len,
                         SQL_DRIVER_NOPROMPT);
  ...
  // Will fail if transactions are not available in the database (256:
  // Transaction not available)
  _SQLSetConnectAttr(_hdbc, SQL_ATTR_TXN_ISOLATION,
...
int SqlIfmxApi::GetRowCount(const char *object, int *count,
                            size_t *time_spent) {
  if (object == NULL)
    return -1;
...
int SqlIfmxApi::ExecuteScalar(const char *query, int *result,
                              size_t *time_spent) {
  SQLHANDLE stmt;
  int q_result = 0;
...
int SqlIfmxApi::OpenCursor(const char *query, size_t buffer_rows,
                           int buffer_memory, size_t *col_count,
                           size_t *allocated_array_rows, int *rows_fetched,
                           SqlCol **cols, size_t *time_spent,
                           bool catalog_query,
  ...
  // Informix can return 0 column if the fetch of the previous table failed with
  // character conversion error (Informix return NO_DATA after an error in one
  // of rows of the previous table)
  ...
  for (int i = 0; i < _cursor_cols_count; i++) {
    ...
    // For INTEGER column size is 10, for SMALLINT 5, for DECIMAL contains only
    // p (s is in decimal digits)
    _cursor_cols[i]._len = column_size;

    // For DATETIME save fractional precision (column size already includes
    ...
    if (native_dt == SQL_TYPE_TIMESTAMP)
      ...
      if (native_dt == SQL_DECIMAL || native_dt == SQL_NUMERIC) {
        ...
        // If DECIMAL created without parameters, it created as floating point
        // with p=16, s=255, convert it to DOUBLE
        if (decimal_digits == 255)
          _cursor_cols[i]._native_dt = SQL_DOUBLE;
      } else
        // TEXT (SQL_LONGVARCHAR), CLOB (-103), BYTE (SQL_LONGVARBINARY) and
        // BLOB (-102)
        if (native_dt == SQL_LONGVARCHAR || native_dt == -103 ||
            native_dt == SQL_LONGVARBINARY || native_dt == -102) {
          // Fecthed as LOB
          ...
          // Fetched as VARCHAR, max length is 32K, SUCCESS_WITH_INFO is
          // returned if column has a large value and error "Data truncated" is
          // set, but length indicator will contain the total size (!) of LOB,
          // not fetched size
          else
  ...
  if (buffer_rows > 0)
    ...
    // On some systems unexpected behaviour (long waits or hang) when buffer is
    // too large (100K rows i.e)
      ...
      if (_cursor_cols[i]._native_dt == SQL_BIGINT ||
        ...
        if (_cursor_cols[i]._native_dt == SQL_INTEGER) {
          ...
          if (_cursor_cols[i]._native_dt == SQL_SMALLINT) {
            // Check whether target supports bind to SMALLINT or string has to
            // be used
            if ((_target_api_provider != NULL &&
                 _target_api_provider->IsSmallintBulkBindSupported() == true) ||
                catalog_query == true) {
              ...
              if (_cursor_cols[i]._native_dt == SQL_TYPE_DATE) {
                ...
                if (_cursor_cols[i]._native_dt == SQL_TYPE_TIMESTAMP) {
                  ...
                  if (_cursor_cols[i]._native_dt == SQL_DECIMAL ||
                    ...
                    // FLOAT (SQL_DOUBLE), SMALLFLOAT (SQL_REAL), REAL
                    // (SQL_REAL) and DOUBLE (SQL_DOUBLE)
                    if (_cursor_cols[i]._native_dt == SQL_REAL ||
                        _cursor_cols[i]._native_dt == SQL_DOUBLE) {
                      _cursor_cols[i]._native_fetch_dt = SQL_C_CHAR;

                      // Driver returns 15 as len for FLOAT, it is too short for
                      ...
                      // TEXT (SQL_LONGVARCHAR), CLOB (-103), BYTE
                      // (SQL_LONGVARBINARY) and BLOB (-102)
                      if (_cursor_cols[i]._native_dt == SQL_LONGVARCHAR ||
                          _cursor_cols[i]._native_dt == -103 ||
                          _cursor_cols[i]._native_dt == SQL_LONGVARBINARY ||
                          _cursor_cols[i]._native_dt == -102) {
                        _cursor_cols[i]._native_fetch_dt = SQL_C_CHAR;
                        ...
                        // Fecthed as LOB
                        if (_cursor_fetch_lob_as_varchar == false) {
                          // Will not bound if fecthed as LOB, but still
  ...
  // Perform initial fetch, returns SQL_SUCCESS even if there are less rows than
  // array size
  rc = Fetch(NULL, NULL);
...
int SqlIfmxApi::Fetch(int *rows_fetched, size_t *time_spent) {
  ...
  // SQL_SUCCESS_WITH_INFO is set when data truncated, and error message "Data
  // truncated"
  if (rc != 0 && rc != 100)
    SetError(SQL_HANDLE_STMT, _hstmt_cursor);
  ...
  if ((rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) &&
    ...
    for (size_t i = 0; i < _cursor_cols_count; i++) {
      ...
      // SQLGetData crashes on next call for this table if SQLGetData for the
      // previous row failed
      if (rc == -1)
        break;
    }
...
int SqlIfmxApi::InitBulkTransfer(const char * /*table*/, size_t /*col_count*/,
                                 size_t /*allocated_array_rows*/,
                                 SqlCol * /*s_cols*/, SqlCol ** /*t_cols*/) {
  return -1;
}

// Transfer rows between databases
int SqlIfmxApi::TransferRows(SqlCol * /*s_cols*/, int /*rows_fetched*/,
                             int * /*rows_written*/, size_t * /*bytes_written*/,
                             size_t * /*time_spent*/) {
  return -1;
}

// Write LOB data using BCP API
int SqlIfmxApi::WriteLob(SqlCol * /*s_cols*/, int /*row*/,
                         int * /*lob_bytes*/) {
  return -1;
}

// Complete bulk transfer
int SqlIfmxApi::CloseBulkTransfer() { return -1; }
...
// Drop the table
int SqlIfmxApi::DropTable(const char * /*table*/, size_t * /*time_spent*/,
                          std::string & /*drop_stmt*/) {
  return -1;
}

// Remove foreign key constraints referencing to the parent table
int SqlIfmxApi::DropReferences(const char * /*table*/,
                               size_t * /*time_spent*/) {
  return -1;
}

// Get the length of LOB column in the open cursor
int SqlIfmxApi::GetLobLength(size_t /*row*/, size_t /*column*/,
                             size_t * /*length*/) {
  return -1;
}

// Get LOB content
int SqlIfmxApi::GetLobContent(size_t /*row*/, size_t /*column*/,
...
int SqlIfmxApi::GetLobPart(size_t /*row*/, size_t column, void * /*data*/,
  ...
  // If SQLGetData returns -1, for example for error 21005 (Inexact character
  // conversion) next call to SQLGetData crashes (!), so the processing of this
  // table must be stopped
  if (rc == -1) {
    SetError(SQL_HANDLE_STMT, _hstmt_cursor);
    return rc;
  }
  ...
  if (ind == -1)
    ...
    if (_cursor_cols[column]._native_dt == SQL_LONGVARCHAR ||
        _cursor_cols[column]._native_dt == -103) {
      ...
      // When BLOB value larger then the buffer Informix returns full LOB size
      // in ind
      if (_cursor_cols[column]._native_dt == SQL_LONGVARBINARY ||
          _cursor_cols[column]._native_dt == -102) {
...
int SqlIfmxApi::GetAvailableTables(std::string &select, std::string &exclude,
                                   std::list<std::string> &tables) {
  std::string condition;

  // Get a condition to select objects from the catalog
  // * means all user tables, *.* means all tables accessible by the user
  GetSelectionCriteria(select.c_str(), exclude.c_str(), "owner", "tabname",
                       condition, _user.c_str(), false);
...
int SqlIfmxApi::ReadSchema(const char *select, const char *exclude,
  ...
  // * means all user tables, *.* means all tables accessible by the user
  GetSelectionCriteria(select, exclude, "t.owner", "t.tabname", condition,
                       _user.c_str(), false);
...
int SqlIfmxApi::ReadTableColumns(std::string &condition) {
  std::string query =
      "SELECT t.owner, tabname, colname, c.colno, c.tabid, c.coltype";
  query += " FROM syscolumns c, systables t WHERE c.tabid = t.tabid AND "
           "t.tabid >= 100";
  ...
  while (rc >= 0) {
    ...
    for (int i = 0; i < rows_fetched; i++) {
      ...
      if (len == 2)
        col_meta.data_type_code =
            *((short *)(cols[5]._data + cols[5]._fetch_len * i));
...
int SqlIfmxApi::ReadIdentityColumns(std::string &condition) {
  // If SERIAL is NOT NULL, code is 262 (NOT NULL added automatically by
  // DB-Access)
...
int SqlIfmxApi::ReadDefaults(std::string &condition) {
  ...
  query += " FROM sysdefaults d, systables t WHERE d.tabid = t.tabid AND "
           "t.tabid >= 100 ";
...
void SqlIfmxApi::SetDefault(SqlColMeta &col, char type, char *value,
                            size_t len) {
  ...
  if (type == 'L') {
    ...
    if (dc == 0 || dc == 256) {
      ...
      // A number SMALLINT - 1, INTEGER - 2, FLOAT - 3, SMALLFLOAT - 4, DECIMAL
      // - 5 (if NOT NULL +256)
      if ((dc >= 1 && dc <= 5) || (dc >= 257 && dc <= 261)) {
        const char *cur = value;
        int i = 0;

        // Default value contains binary representation, a space and then ASCII
        // value of the number
        while (i < len) {
          if (cur[i] == ' ') {
            i++;
...
int SqlIfmxApi::ReadTableConstraints(std::string &condition) {
  std::string query = "SELECT t.owner, tabname, constrname, constrtype, "
                      "c.tabid, c.idxname, c.constrid";
  ...
  while (rc >= 0) {
    ...
    for (int i = 0; i < rows_fetched; i++) {
      ...
      if (len != -1) {
        ...
        strncpy(col_cns.idxname, cols[5]._data + cols[5]._fetch_len * i,
                (size_t)len);
...
int SqlIfmxApi::ReadReferences(std::string & /*condition*/) {
  ...
  while (rc >= 0) {
    ...
    for (int i = 0; i < rows_fetched; i++) {
      ...
      for (std::list<SqlConstraints>::iterator m = _table_constraints.begin();
           m != _table_constraints.end(); m++) {
        if ((*m).cnsid == cnsid) {
          (*m).r_cnsid = r_cnsid;
          break;
        }
...
int SqlIfmxApi::ReadIndexes(std::string &condition) {
  // Note that i.owner for indexes created for FK is 'informix' even if table is
  // not in informix schema
  std::string query =
      "SELECT i.owner, i.idxname, t.owner, t.tabname, i.tabid, i.idxtype, ";
  ...
  while (rc >= 0) {
    ...
    for (int i = 0; i < rows_fetched; i++) {
      ...
      for (int k = 0; k < 16; k++) {
        ...
        short colno =
            *((short *)(cols[6 + k]._data + cols[6 + k]._fetch_len * i));
...
int SqlIfmxApi::ReadConstraintTable(const char * /*schema*/,
...
int SqlIfmxApi::ReadConstraintColumns(const char * /*schema*/,
                                      const char * /*table*/,
                                      const char * /*constraint*/,
                                      std::string & /*cols*/) {
  return -1;
...
void SqlIfmxApi::AddSystemTablesFilter(const char *alias, std::string &query) {
  // Systems views are in 'informix' schema and start with 'sys%', but user
  // objects also be in this schema and start with 'sys%', but their tabid
  // starts with 100
  query += "AND ";
  query += alias;
  query += "tabid >= 100 ";
...
int SqlIfmxApi::GetKeyConstraintColumns(SqlConstraints &cns,
  ...
  for (std::list<SqlIndColumns>::iterator i = _table_ind_columns.begin();
    ...
    if ((*i).index != NULL && cns.idxname != NULL &&
        strcmp((*i).index, cns.idxname) == 0) {
...
int SqlIfmxApi::GetForeignKeyConstraintColumns(SqlConstraints &cns,
                                               std::list<std::string> &fcols,
                                               std::list<std::string> &pcols,
                                               std::string &ptable) {
  // In Informix, each foreign key creates an index, so we can find columns the
  // same way as for primary key
...
int SqlIfmxApi::GetIndexColumns(SqlIndexes &idx,
  ...
  for (std::list<SqlIndColumns>::iterator i = _table_ind_columns.begin();
    ...
    if ((*i).index != NULL && idx.index != NULL &&
        strcmp((*i).index, idx.index) == 0) {
...
void SqlIfmxApi::SetVersion() {
  // Tested on Informix 11.7 (Includes product name - IBM Informix Dynamic
  // Server Version 11.70.TC5DE) T means 32-bit Windows, U - 32-bit Unix, F -
  // 64-bit Windows/Unix
  const char *query =
...
void SqlIfmxApi::FindInformixPaths(std::list<std::string> &paths) {
  ...
  int rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Informix\\Environment",
                        0, KEY_READ, &hKey);
  ...
  // For 11.7 "INFORMIXDIR" parameter contains installation directory WITH
  // terminating '\'
  rc = RegQueryValueEx(hKey, "INFORMIXDIR", NULL, NULL, (LPBYTE)location,
                       (LPDWORD)&value_size);
