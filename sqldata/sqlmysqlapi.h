...
#define MARIADB_64BIT_DLL_LOAD_ERROR                                           \
  "Loading MariaDB C API " MARIADB_C_DLL                                       \
  ": 64-bit library loaded, use 32-bit client or 64-bit version of SQLines "   \
  "Data"

#define MYSQL_DLL_LOAD_ERROR "Loading MySQL C API " MYSQL_C_DLL ": "
#define MYSQL_64BIT_DLL_LOAD_ERROR                                             \
  "Loading MySQL C API " MYSQL_C_DLL                                           \
  ": 64-bit library loaded, use 32-bit client or 64-bit version of SQLines "   \
  "Data"

#define MYSQL_C_FUNC_LOAD_ERROR                                                \
  "Library " MYSQL_C_DLL " loaded, but API functions cannot be found"
#define MARIADB_C_FUNC_LOAD_ERROR                                              \
  "Library " MARIADB_C_DLL " loaded, but API functions cannot be found"
...
typedef MYSQL *(STDCALL *mysql_real_connectFunc)(
...
typedef int(STDCALL *mysql_optionsFunc)(MYSQL *mysql, enum mysql_option option,
                                        const void *arg);
typedef int(STDCALL *mysql_set_character_setFunc)(MYSQL *mysql,
                                                  const char *csname);
typedef void (*mysql_set_local_infile_handlerFunc)(
...
private:
  ...
  static int local_infile_initS(void **ptr, const char *filename,
                                void *userdata);
  static int local_infile_readS(void *ptr, char *buf, unsigned int buf_len);
  static void local_infile_endS(void *ptr);
  static int local_infile_errorS(void *ptr, char *error_msg,
                                 unsigned int error_msg_len);
