...
int SqlPgApi::Init() {
  ...
  if (_dll != NULL) {
    ...
    _PQerrorMessage =
        (PQerrorMessageFunc)Os::GetProcAddress(_dll, "PQerrorMessage");
    ...
    _PQputCopyData =
        (PQputCopyDataFunc)Os::GetProcAddress(_dll, "PQputCopyData");
    _PQputCopyEnd = (PQputCopyEndFunc)Os::GetProcAddress(_dll, "PQputCopyEnd");
    _PQresultErrorMessage = (PQresultErrorMessageFunc)Os::GetProcAddress(
        _dll, "PQresultErrorMessage");
    _PQresultStatus =
        (PQresultStatusFunc)Os::GetProcAddress(_dll, "PQresultStatus");
    _PQsetClientEncoding = (PQsetClientEncodingFunc)Os::GetProcAddress(
        _dll, "PQsetClientEncoding");
    _PQsetdbLogin = (PQsetdbLoginFunc)Os::GetProcAddress(_dll, "PQsetdbLogin");
    _PQstatus = (PQstatusFunc)Os::GetProcAddress(_dll, "PQstatus");

    if (_PQclear == NULL || _PQerrorMessage == NULL || _PQexec == NULL ||
        _PQfmod == NULL || _PQfname == NULL || _PQfsize == NULL ||
        _PQftype == NULL || _PQgetisnull == NULL || _PQgetlength == NULL ||
        _PQgetResult == NULL || _PQgetvalue == NULL || _PQnfields == NULL ||
        _PQntuples == NULL || _PQputCopyData == NULL || _PQputCopyEnd == NULL ||
        _PQresultErrorMessage == NULL || _PQresultStatus == NULL ||
        _PQsetClientEncoding == NULL || _PQsetdbLogin == NULL ||
        _PQstatus == NULL)
      return -1;
  } else {
...
// Free connection and environment handles (When other ODBC drivers are used
// crash can happen on exit when not disconnected and not deallocated)
...
int SqlPgApi::ExecuteScalar(const char *query, int *result,
                            size_t *time_spent) {
...
int SqlPgApi::OpenCursor(const char *query, size_t buffer_rows,
  ...
  for (int i = 0; i < _cursor_cols_count; i++) {
    ...
    if (len == -1) {
      ...
      if (_cursor_cols[i]._native_dt == PG_DT_BPCHAR ||
          _cursor_cols[i]._native_dt == PG_DT_VARCHAR) {
        if (len > 4)
          len -= 4;
      } else
    ...
    // NOT NULL attribute
    //_cursor_cols[i]._nullable = (fields[i].flags & NOT_NULL_FLAG) ? false :
    //true;
  }
...
int SqlPgApi::Fetch(int *rows_fetched, size_t *time_spent) {
  ...
  for (int i = 0; i < _cursor_allocated_rows; i++) {
    ...
    for (int k = 0; k < _cursor_cols_count; k++) {
      ...
      size_t len =
          (size_t)_PQgetlength(_cursor_result, _cursor_rows_fetched, k);
...
int SqlPgApi::InitBulkTransfer(const char *table, size_t col_count,
...
int SqlPgApi::TransferRows(SqlCol *s_cols, int rows_fetched, int *rows_written,
  ...
  for (size_t i = 0; i < rows_fetched; i++) {
    ...
    for (size_t k = 0; k < _copy_cols_count; k++) {
      ...
      if (_source_api_type == SQLDATA_ORACLE && s_cols[k]._ind2 != NULL) {
        if (s_cols[k]._ind2[i] != -1) {
          ...
          if (s_cols[k]._native_fetch_dt == SQLT_DAT)
            ...
            // LOB column
            if (s_cols[k]._native_fetch_dt == SQLT_BLOB ||
                s_cols[k]._native_fetch_dt == SQLT_CLOB) {
              size_t lob_size = 0;
              ...
              if (lob_rc != -1 && lob_size > 0) {
                ...
                lob_data = _source_api_provider->GetLobBuffer(i, k, lob_size,
                                                              &alloc_size);

                // Get LOB content
                lob_rc = _source_api_provider->GetLobContent(
                    i, k, lob_data, alloc_size, &read_size);
      ...
      // Check if we still have space to write column data, NULL value and
      // delimiters
      ...
      if (len == -1) {
        ...
        if ((_source_api_type == SQLDATA_ORACLE &&
             (s_cols[k]._native_fetch_dt == SQLT_STR ||
              s_cols[k]._native_fetch_dt == SQLT_BLOB ||
              s_cols[k]._native_fetch_dt == SQLT_CLOB ||
              s_cols[k]._native_fetch_dt == SQLT_BIN ||
              s_cols[k]._native_fetch_dt == SQLT_LNG)) ||
            ...
            // MySQL CHAR
            (_source_api_type == SQLDATA_MYSQL &&
             s_cols[k]._native_fetch_dt == MYSQL_TYPE_STRING)) {
          ...
          for (int m = 0; m < len; m++) {
            ...
            // Check if we still have space to write the next byte of column
            // data (can explode due to escape sequences)
          ...
          if (_source_api_type == SQLDATA_ORACLE &&
            ...
            unsigned char *data =
                (unsigned char *)(s_cols[k]._data + s_cols[k]._fetch_len * i);
            ...
            if ((_source_api_type == SQLDATA_SQL_SERVER ||
              ...
              // Sybase ASA 9 stores 6-digit fraction multiplied by 1000, i.e.
              // 123456 stored as 123456000
...
// Drop the table
int SqlPgApi::DropTable(const char *table, size_t *time_spent,
...
int SqlPgApi::GetLobLength(size_t /*row*/, size_t /*column*/,
                           size_t * /*length*/) {
  return -1;
}

// Get LOB content
int SqlPgApi::GetLobContent(size_t /*row*/, size_t /*column*/, void * /*data*/,
                            size_t /*length*/, int * /*len_ind*/) {
  return -1;
}

// Get the list of available tables
int SqlPgApi::GetAvailableTables(std::string & /*table_template*/,
                                 std::string & /*exclude*/,
...
int SqlPgApi::ReadSchema(const char * /*select*/, const char * /*exclude*/,
                         bool /*read_cns*/, bool /*read_idx*/) {
  return -1;
}

// Get table name by constraint name
int SqlPgApi::ReadConstraintTable(const char * /*schema*/,
...
int SqlPgApi::ReadConstraintColumns(const char * /*schema*/,
                                    const char * /*table*/,
                                    const char * /*constraint*/,
                                    std::string & /*cols*/) {
  return -1;
...
void SqlPgApi::GetCatalogSelectionCriteria(std::string & /*selection_template*/,
                                           std::string & /*output*/) {}
...
void SqlPgApi::InitSession() {
  ...
  // Client encoding is UTF-8 by default while data in ASCII, read encoding
  // values
...
void SqlPgApi::SetVersion() {
  ...
  if (value != NULL && strlen(value) > 11) {
    // Skip "PostgreSQL "
    sscanf(value + 11, "%d.%d.%d", &_version_major, &_version_minor,
           &_version_release);
  }
...
void SqlPgApi::FindPgPaths(std::list<std::string> &paths) {
  ...
  int rc =
      RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\PostgreSQL\\Installations", 0,
                   KEY_READ, &hKey);
  ...
  while (more) {
    ...
    rc = RegQueryValueEx(hSubkey, "Base Directory", NULL, NULL,
                         (LPBYTE)location, (LPDWORD)&value_size);
