...
typedef int (*PQgetisnullFunc)(const PGresult *res, int row_number,
                               int column_number);
typedef int (*PQgetlengthFunc)(const PGresult *res, int row_number,
                               int column_number);
typedef PGresult *(*PQgetResultFunc)(PGconn *conn);
typedef char *(*PQgetvalueFunc)(const PGresult *res, int row_number,
                                int column_number);
...
typedef PGconn *(*PQsetdbLoginFunc)(const char *pghost, const char *pgport,
                                    const char *pgoptions, const char *pgtty,
                                    const char *dbName, const char *login,
                                    const char *pwd);
typedef ConnStatusType (*PQstatusFunc)(const PGconn *conn);

// PostgreSQL built-in data types (SELECT oid, typname FROM pg_type system
// table)
