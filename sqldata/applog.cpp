...
void AppLog::LogFileVaList(const char *format, va_list args) {
  ...
  // During the first call destroy the log file content that may exists from the
  // previous runs
  if (first_write == true)
    openMode = "w";
  ...
  if (file) {
    ...
    if (first_write == true)
      printf("\n\nError:\n Opening log file %s - %s", _filename.c_str(),
             strerror(errno));
  }
...
void AppLog::TraceFileVaList(const char *format, va_list args) {
  ...
  if (file) {
    ...
    // Show error message during the first call only
    if (first_write == true)
      printf("\n\nError:\n Opening trace file %s - %s", _filename_trc.c_str(),
             strerror(errno));
  }
