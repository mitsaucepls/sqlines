...
class SqlData {
  ...
  // Local in-process database interface (to retrieve metadata and process
  // tasks)
  SqlDb _db;
...
public:
  ...
  void SetConnectionStrings(std::string &source, std::string &target,
  ...
  int GetAvailableTables(std::string &table_template, std::string &exclude,
                         std::list<std::string> &tables);

  // Set tables or queries for processing
  void SetTables(std::list<std::string> *tables) { _tables = tables; }
  void SetQueries(std::map<std::string, std::string> *queries) {
    _queries = queries;
  }
  ...
  int CreateObjectQueue(std::string &select, std::string &exclude,
                        int *total_obj, int *total_lines, int *total_bytes);
...
private:
  ...
  void MapObjectName(std::string &s_schema, std::string &s_table,
                     std::string &t_name, bool tmap = true);
  void MapObjectName(std::string &s_schema, std::string &s_table,
                     std::string &t_schema, std::string &t_table);
  void MapConstraintName(const char *source, std::string &target, char type,
                         std::string &table, std::string &p_table);
  void MapColumn(const char *s_table, std::string &s_name, std::string &t_name,
                 std::string &t_type);

  // Convert identifier between databases
  void ConvertIdentifier(const char *in, std::string &out);

  // Generate statement to emulate identity in Oracle (create sequence and
  // trigger), sequence and default in PostgreSQL
  ...
  void GetKeyConstraintColumns(SqlMetaTask &task, SqlConstraints &cns,
                               std::string &output);
  // Get foreign key columns
  void GetForeignKeyConstraintColumns(SqlMetaTask &task, SqlConstraints &cns,
                                      std::string &fcols, std::string &pcols,
                                      std::string &ptable);
  // Get index columns
