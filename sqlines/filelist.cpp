...
/* Define the list of files
        ...
        last wildcard is only for files, use \ at the end to specify last
   wildcard for directories

        if path is not specified for an item, the path from the previous item is
   taken if it exists d:\project\*.sql, *.ddl d:\project\file1.sql, file2.sql
...
int FileList::Load(const char *inputPath) {
  ...
  while (*cur) {
    ...
    // Find files, the directory list may grow(!) as new sub-directories can be
    // found
    for (std::list<std::string>::iterator i = dirs.begin(); i != dirs.end();
         i++) {
...
int FileList::GetDirectoriesByWildcard(const char *wildcard,
                                       std::list<std::string> &dirs) {
  if (wildcard == NULL)
    return -1;
...
// Find files in the specified directory matching the file wildcard
int FileList::FindFiles(std::string dir, std::string file,
                        std::list<std::string> &dirs,
  ...
  do {
    ...
    if (File::IsFile(&fileInfo)) {
      ...
      // if a directory found (skipping "." and "..") and to the directory list
