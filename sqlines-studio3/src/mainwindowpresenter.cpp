...
MainWindowPresenter::MainWindowPresenter(const model::Settings &settings,
  ...
  connect(view, SIGNAL(tabTitleChanged(int, QString)), this,
          SLOT(tabTitleChanged(int, QString)));

  connect(view, SIGNAL(sourceDataChanged(int, QString)), this,
          SLOT(sourceDataChanged(int, QString)));

  connect(view, SIGNAL(targetDataChanged(int, QString)), this,
          SLOT(targetDataChanged(int, QString)));

  connect(view, SIGNAL(sourceModeChanged(int, QString)), this,
          SLOT(sourceModeChanged(int, QString)));

  connect(view, SIGNAL(targetModeChanged(int, QString)), this,
          SLOT(targetModeChanged(int, QString)));

  connect(view, SIGNAL(fileOpened(int, QString, QString)), this,
          SLOT(fileOpened(int, QString, QString)));
  ...
  connect(view, SIGNAL(convertPressed(int)), this, SLOT(convert(int)));

  connect(view, SIGNAL(openNewTabPressed()), this, SLOT(openNewTab()));

  connect(view, SIGNAL(closeTabPressed(int)), this, SLOT(closeTab(int)));

  connect(view, SIGNAL(tabChanged(int)), this, SLOT(tabChanged(int)));

  connect(view, SIGNAL(tabMoved(int, int)), this, SLOT(tabMoved(int, int)));

  connect(view, SIGNAL(openFilePressed(int)), this, SLOT(openFile(int)));

  connect(view, SIGNAL(saveFilePressed(int)), this, SLOT(saveFile(int)));

  connect(view, SIGNAL(saveFileAsPressed(int)), this, SLOT(saveFileAs(int)));

  connect(view, SIGNAL(openAboutPressed()), this, SIGNAL(openAboutPressed()));

  connect(view, SIGNAL(openConvertUtilityPressed()), this,
          SIGNAL(openConvertUtilityPressed()));

  connect(view, SIGNAL(openSettingsPressed()), this,
          SIGNAL(openSettingsPressed()));

  connect(view, SIGNAL(openOnlineHelpPressed()), this, SLOT(openOnlineHelp()));

  connect(view, SIGNAL(openContactUsPressed()), this, SLOT(openContactUs()));

  connect(view, SIGNAL(openSitePressed()), this, SLOT(openSite()));

  connect(view, SIGNAL(openSourceCodePressed()), this, SLOT(openSourceCode()));
}
...
void MainWindowPresenter::conversionCrashed(
    int tabIndex, const std::exception_ptr &exception) noexcept {
  try {
    ...
    if (error.fileType() ==
        model::Converter::fileCreationError::FileType::Source) {
      errorMsg += "source file.";
    } else if (error.fileType() ==
               model::Converter::fileCreationError::FileType::Target) {
      errorMsg += "target file.";
    } else if (error.fileType() ==
               model::Converter::fileCreationError::FileType::Log) {
      errorMsg += "log file.";
    }
    errorMsg += "\nPath: " + error.filePath();
    errorMsg += "\n\n" + QString(error.what()) + ".";
    ...
    if (error.fileType() == model::Converter::fileOpenError::FileType::Source) {
      errorMsg += "source file.";
               ...
               model::Converter::fileOpenError::FileType::Target) {
      errorMsg += "target file.";
    } else if (error.fileType() ==
               model::Converter::fileOpenError::FileType::Log) {
      errorMsg += "log file.";
    }
    errorMsg += "\nPath: " + error.filePath();
    errorMsg += "\n\n" + QString(error.what()) + ".";
...
void MainWindowPresenter::remodeled() noexcept { this->mainWindow.reset(); }
...
void MainWindowPresenter::tabAdded() noexcept {
  ...
  bool highlighterEnabled =
      this->settings["ui.input-field.highlighter"].toInt();
  bool wrappingEnabled = this->settings["ui.input-field.wrapping"].toInt();

  this->mainWindow.openNewTab(lineNumberEnabled, highlighterEnabled,
                              wrappingEnabled);
}
...
void MainWindowPresenter::sourceModeUpdated(
    int, const QString &sourceMode) noexcept {
  this->mainWindow.showSourceMode(sourceMode);
}

void MainWindowPresenter::targetModeUpdated(
    int, const QString &targetMode) noexcept {
  this->mainWindow.showTargetMode(targetMode);
}

void MainWindowPresenter::sourceDataUpdated(
    int tabIndex, const QString &sourceData) noexcept {
  this->mainWindow.showSourceData(tabIndex, sourceData);
}

void MainWindowPresenter::targetDataUpdated(
    int tabIndex, const QString &targetData) noexcept {
  this->mainWindow.showTargetData(tabIndex, targetData);
}

void MainWindowPresenter::sourceFilePathUpdated(
    int tabIndex, const QString &filePath) noexcept {
  if (this->mainWindow.currentTabIndex() == tabIndex) {
    this->mainWindow.showFilePath(filePath);
  }
}

void MainWindowPresenter::targetFilePathUpdated(
    int tabIndex, const QString &filePath) noexcept {
  if (this->mainWindow.currentTabIndex() == tabIndex) {
    this->mainWindow.showFilePath(filePath);
  }
...
void MainWindowPresenter::sourceModeChanged(
    int tabIndex, const QString &currentMode) noexcept {
  this->converter.updateSourceMode(tabIndex, currentMode);
}

void MainWindowPresenter::targetModeChanged(
    int tabIndex, const QString &currentMode) noexcept {
...
void MainWindowPresenter::fileOpened(int tabIndex, const QString &sourceData,
...
void MainWindowPresenter::focusChanged(
    int tabIndex, view::IMainWindow::FieldInFocus inFocus) noexcept {
...
void MainWindowPresenter::openSourceCode() const noexcept {
  QDesktopServices::openUrl(
      QUrl("https://github.com/dmtolpeko/sqlines/tree/master/sqlines-studio3"));
}
