...
SettingsLoader::SettingsLoader(const QString &orgName, const QString &appName,
                               model::Settings &settings,
                               model::Snapshot &snapshot) noexcept
    : settingsFile(orgName, appName), settings(settings), state(snapshot) {
...
void SettingsLoader::saveAppState() noexcept { saveUiState(); }

void SettingsLoader::clearSettings() noexcept { this->settingsFile.clear(); }
...
void SettingsLoader::loadGeneralSettings() noexcept {
  ...
  if (this->settingsFile.contains("save-last-session")) {
    QString saveSession =
        this->settingsFile.value("save-last-session").toString();
...
void SettingsLoader::saveTabsState(const QString &prefix) noexcept {
  ...
  for (auto i = 0; i < tabsNumber.toInt(); i++) {
    ...
    const QString &sourcePath =
        this->state[prefix + index + ".source-filePath"];
    this->settingsFile.setValue("source-filePath", sourcePath);

    const QString &targetPath =
        this->state[prefix + index + ".target-filePath"];
