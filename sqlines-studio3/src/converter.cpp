...
Converter::filesystemError::filesystemError(filesystemError::FileType fileType,
    ...
    : std::runtime_error(errorString.toStdString()), type(fileType),
      path(filePath) {}

Converter::filesystemError::FileType
Converter::filesystemError::fileType() const noexcept {
  return this->type;
}
...
Converter::fileCreationError::fileCreationError(
    filesystemError::FileType fileType, const QString &filePath,
    const QString &errorString) noexcept
...
Converter::fileDoesNotExist::fileDoesNotExist(
    filesystemError::FileType fileType, const QString &filePath) noexcept
...
Converter::Converter(const model::Settings &settings,
  ...
  connect(this, &Converter::conversionStartedInDetachedThread, this,
          conversionStarted);
  ...
  connect(this, &Converter::conversionEndedInDetachedThread, this,
          conversionEnded);

  auto conversionCrashed = [this](int tabIndex,
                                  const std::exception_ptr &exception) {
    for (auto observer : this->observers) {
      observer.get().conversionCrashed(tabIndex, exception);
    }
  };
  connect(this, &Converter::conversionCrashedInDetachedThread, this,
          conversionCrashed);
  ...
  connect(this, &Converter::sourceDataChangedInDetachedThread, this,
          sourceDataChanged);
  ...
  connect(this, &Converter::targetDataChangedInDetachedThread, this,
          targetDataChanged);
  ...
  connect(this, &Converter::sourceFilePathChangedInDetachedThread, this,
          sourceFilePathChanged);
  ...
  connect(this, &Converter::targetFilePathChangedInDetachedThread, this,
          targetFilePathChanged);
}
...
void Converter::makeSnapshot(model::Snapshot &snapshot) noexcept {
  ...
  for (auto i = 0; i < tabsNumber; i++) {
    ...
    snapshot["ui.tabs." + index + ".source-mode"] =
        this->tabsData[i].sourceMode;
    snapshot["ui.tabs." + index + ".target-mode"] =
        this->tabsData[i].targetMode;
...
void Converter::convert(int tabIndex) {
  ...
  auto runConversion = [this](int tabIndex) {
    ...
    try {
      ...
      model::CoreProcess process(
          this->settings["general.dirs.process-filePath"]);
      ...
      if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        throw fileOpenError(filesystemError::FileType::Target, targetFilePath,
                            file.errorString());
      }
...
void Converter::openFile(int tabIndex, const QString &filePath) {
  ...
  if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    throw fileOpenError(filesystemError::FileType::Source, filePath,
                        file.errorString());
  }
...
void Converter::saveTargetFile(int tabIndex) {
  ...
  if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    throw fileOpenError(filesystemError::FileType::Target, filePath,
                        file.errorString());
  }
...
void Converter::restoreSourceData(int tabIndex,
                                  const QString &filePath) noexcept {
...
void Converter::restoreTargetData(int tabIndex,
                                  const QString &filePath) noexcept {
  if (!QFileInfo::exists(filePath)) {
    return;
  }
...
QString Converter::createSourceFile(int tabIndex) const {
  ...
  QFile file(filePath);
  if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    throw fileCreationError(filesystemError::FileType::Source, filePath,
                            file.errorString());
  }
...
QString Converter::createTargetFile(int tabIndex) const {
  ...
  QFile file(filePath);
  if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    throw fileCreationError(filesystemError::FileType::Target, filePath,
                            file.errorString());
  }
...
QString Converter::createLogFile() const {
  ...
  QFile file(filePath);
  if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    throw fileCreationError(filesystemError::FileType::Log, filePath,
                            file.errorString());
  }
...
void Converter::monitorFileChanges(
    const std::atomic<bool> &programIsRunning) noexcept {
  while (programIsRunning) {
    ...
    // Check target files
    for (auto i = this->targetFiles.cbegin(); i != this->targetFiles.cend();
         ++i) {
...
void Converter::updateSourceFileData(int tabIndex) noexcept {
  ...
  if (!file.exists() && !tabData.sourceFilePath.isEmpty()) {
    ...
    emit sourceFilePathChangedInDetachedThread(tabIndex,
                                               tabData.sourceFilePath);
    return;
  }
...
void Converter::updateTargetFileData(int tabIndex) noexcept {
  ...
  if (!file.exists() && !tabData.targetFilePath.isEmpty()) {
    ...
    emit targetFilePathChangedInDetachedThread(tabIndex,
                                               tabData.targetFilePath);
    return;
  }
