...
Highlighter::Highlighter(QTextDocument &parent) noexcept
    : QSyntaxHighlighter(&parent), keywords(setKeywords()),
...
void Highlighter::highlightBlock(const QString &text) {
  ...
  while (startIndex >= 0) {
    ...
    auto endIndex =
        text.indexOf(this->commentEndExpression, startIndex, &endMatch);
    auto length = 0;
    if (endIndex == -1) {
    ...
    startIndex =
        text.indexOf(this->commentStartExpression, startIndex + length);
  }
}
...
void Highlighter::setKeywordHighlighting() noexcept {
  ...
  for (const auto &function : this->functions) {
    ...
    this->highlightingRules.emplace_back(std::move(pattern), std::move(format));
  }
}
...
QStringList Highlighter::setKeywords() const noexcept {
  ...
  keywords << "ADD" << "ADD CONSTRAINT" << "ALL" << "ALTER" << "ALTER COLUMN"
           ...
           << "IF" << "IN" << "INDEX" << "INNER JOIN" << "INSERT"
           << "INSERT INTO"
           << "INSERT INTO SELECT" << "IS" << "IS NULL" << "IS NOT NULL"
           << "INT"
           << "INTO"
...
QStringList Highlighter::setFunctions() const noexcept {
  ...
  functions << "ABS" << "ACOS" << "ADDDATE" << "ADDTIME" << "ASCII" << "ASIN"
            << "ATAN" << "ATAN2" << "ATN2" << "AVG"
            ...
            << "DATALENGTH" << "DATE" << "DATEADD" << "DATEDIFF"
            << "DATEFROMPARTS"
            << "DATENAME" << "DATEPART" << "DATE_ADD" << "DATE_FORMAT"
            << "DATE_SUB"
