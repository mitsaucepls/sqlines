...
SettingsPresenter::SettingsPresenter(
    model::Settings &settings, model::SettingsLoader &settingsLoader,
    model::License &license, view::ISettingsWidget &settingsWidget,
    view::IMainWindowSettings &mainWindow) noexcept
    : settings(settings), settingsLoader(settingsLoader), license(license),
  ...
  if (!this->license.exists() || !this->license.isActive()) {
    ...
    this->settingsWidget.showLicenseInfo(
  ...
  connect(view, SIGNAL(setDefaultsPressed()), this, SLOT(setDefaults()));

  connect(view, SIGNAL(changeSessionSavingPressed(bool)), this,
          SLOT(changeSessionSaving(bool)));

  connect(view, SIGNAL(changeWorkingDirPressed(QString)), this,
          SLOT(changeWorkingDir(QString)));

  connect(view, SIGNAL(changeNumberAreaPressed(bool)), this,
          SLOT(changeNumberArea(bool)));

  connect(view, SIGNAL(changeHighlighterPressed(bool)), this,
          SLOT(changeHighlighter(bool)));

  connect(view, SIGNAL(changeWrappingPressed(bool)), this,
          SLOT(changeWrapping(bool)));

  connect(view, SIGNAL(commitLicenseChangePressed(QString, QString)), this,
          SLOT(changeLicense(QString, QString)));
}

SettingsPresenter::~SettingsPresenter() { saveSettings(); }
...
void SettingsPresenter::restoreGeneralSettings() noexcept {
  ...
  } else if (currDir == QStandardPaths::writableLocation(
...
void SettingsPresenter::restoreEditorSettings() noexcept {
  ...
  const QString &numberAreaEnabled =
      this->settings["ui.input-field.number-area"];
  ...
  const QString &highlighterEnabled =
      this->settings["ui.input-field.highlighter"];
...
void SettingsPresenter::changeWorkingDir(
    const QString &newWorkingDir) noexcept {
  ...
  if (newWorkingDir == "Documents") {
    ...
    this->settings["general.dirs.dirs-number"] =
        QString::number(dirsNumber + 1);
