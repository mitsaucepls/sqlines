...
ConvertUtilityPresenter::ConvertUtilityPresenter(
    const model::Settings &settings, const std::map<QString, QString> &cmdModes,
  ...
  connect(widget, SIGNAL(convertPressed()), this, SLOT(convert()));

  connect(widget, SIGNAL(selectFilePressed()), this, SLOT(selectFile()));

  connect(widget, SIGNAL(sourceModeSelected(QString)), this,
          SLOT(sourceModeSelected(QString)));

  connect(widget, SIGNAL(targetModeSelected(QString)), this,
          SLOT(targetModeSelected(QString)));
}
...
void ConvertUtilityPresenter::sourceModeSelected(
    const QString &sourceMode) noexcept {
  this->sourceMode = sourceMode;
}

void ConvertUtilityPresenter::targetModeSelected(
    const QString &targetMode) noexcept {
...
QString ConvertUtilityPresenter::createTargetFile() const {
  ...
  if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    throw fileCreationError(fileCreationError::FileType::Target, filePath,
                            file.errorString());
  }
...
QString ConvertUtilityPresenter::createLogFile() const {
  ...
  QFile file(filePath);
  if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    throw fileCreationError(fileCreationError::FileType::Log, filePath,
                            file.errorString());
  }
...
ConvertUtilityPresenter::fileCreationError::fileCreationError(
...
const QString &
ConvertUtilityPresenter::fileCreationError::filePath() const noexcept {
  return this->path;
}
