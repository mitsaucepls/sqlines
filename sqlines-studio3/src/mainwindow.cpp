...
MainWindow::MainWindow(const QStringList &sourceModes,
                       const QStringList &targetModes, QWidget *parent) noexcept
    : QMainWindow(parent) {
  this->sourceComboBox.addItems(sourceModes);
  this->targetComboBox.addItems(targetModes);

  connect(&this->tabWidget, &ui::TabWidget::tabCloseRequested, this,
          &ui::MainWindow::closeTabPressed);

  connect(this->tabWidget.tabBar(), &QTabBar::tabMoved, this,
          &ui::MainWindow::tabMoved);

  connect(&this->tabWidget, &ui::TabWidget::currentChanged, this,
          &ui::MainWindow::tabChanged);

  connect(&this->sourceComboBox, &QComboBox::currentTextChanged, this, [this] {
    emit sourceModeChanged(this->tabWidget.currentIndex(),
                           this->sourceComboBox.currentText());
  });

  connect(&this->targetComboBox, &QComboBox::currentTextChanged, this, [this] {
...
void MainWindow::openNewTab(bool lineNumberEnabled, bool highlighterEnabled,
                            bool wrappingEnabled) noexcept {
  auto centralWidget = new CentralWidget(lineNumberEnabled, highlighterEnabled,
                                         wrappingEnabled, this);

  connect(centralWidget, &ui::CentralWidget::sourceDataChanged, this,
          [this](const QTextDocument &data) {
            emit sourceDataChanged(this->tabWidget.currentIndex(),
                                   data.toPlainText());
          });

  connect(centralWidget, &ui::CentralWidget::targetDataChanged, this,
          [this](const QTextDocument &data) {
            emit targetDataChanged(this->tabWidget.currentIndex(),
                                   data.toPlainText());
          });

  connect(centralWidget, &ui::CentralWidget::fileOpened, this,
          [this](const QString &filePath) {
            emit fileOpened(
                this->tabWidget.currentIndex(),
  ...
  connect(centralWidget, &ui::CentralWidget::fileOpened, &this->tabWidget,
          [this](const QString &filePath) {
  ...
  connect(centralWidget, &ui::CentralWidget::focusChanged, this, focusChange);
...
void MainWindow::closeTab(int tabIndex) noexcept {
  ...
  if (this->tabWidget.count() != 0) {
    this->tabWidget.currentWidget().focusOn(
        CentralWidget::FieldInFocus::SourceField);
  }
}
...
view::IMainWindow::FieldInFocus MainWindow::inFocus() const noexcept {
  CentralWidget::FieldInFocus inFocus =
      this->tabWidget.currentWidget().inFocus();
...
int MainWindow::height() const noexcept { return QMainWindow::height(); }

int MainWindow::width() const noexcept { return QMainWindow::width(); }

int MainWindow::posX() const noexcept { return x(); }

int MainWindow::posY() const noexcept { return y(); }

void MainWindow::resize(int width, int height) noexcept {
  QMainWindow::resize(width, height);
}

void MainWindow::move(int x, int y) noexcept { QMainWindow::move(x, y); }
...
void MainWindow::makeTabsSection(QToolBar *parent) noexcept {
  ...
  connect(newTab, &QAction::triggered, this, &MainWindow::openNewTabPressed);
...
void MainWindow::makeFileSection(QToolBar *parent) noexcept {
  ...
  connect(openFile, &QAction::triggered, this,
          [this] { emit openFilePressed(this->tabWidget.currentIndex()); });
  ...
  connect(saveFile, &QAction::triggered, this,
          [this] { emit saveFilePressed(this->tabWidget.currentIndex()); });
...
void MainWindow::makeToolsSection(QToolBar *parent) noexcept {
  ...
  connect(convert, &QAction::triggered, this,
          [this] { emit convertPressed(this->tabWidget.currentIndex()); });
...
QMenu *MainWindow::makeMainMenuTab(QMenuBar *parent) noexcept {
  ...
  connect(about, &QAction::triggered, this, &ui::MainWindow::openAboutPressed);
  ...
  connect(preferences, &QAction::triggered, this,
          &ui::MainWindow::openSettingsPressed);
...
QMenu *MainWindow::makeFileMenuTab(QMenuBar *parent) noexcept {
  ...
  connect(saveFileAs, &QAction::triggered, this,
          [this] { emit saveFileAsPressed(this->tabWidget.currentIndex()); });
...
QMenu *MainWindow::makeToolsMenuTab(QMenuBar *parent) noexcept {
  ...
  connect(runConversion, &QAction::triggered, this,
          [this] { emit convertPressed(this->tabWidget.currentIndex()); });

  auto conversionUtility = new QAction("Conversion utility...", toolsMenu);
  connect(conversionUtility, &QAction::triggered, this,
          &MainWindow::openConvertUtilityPressed);
...
QMenu *MainWindow::makeViewMenuTab(QMenuBar *parent) noexcept {
  ...
  connect(zoomIn, &QAction::triggered, this,
          [this] { this->tabWidget.currentWidget().zoomIn(); });

  auto zoomOut = new QAction("Zoom out", viewMenu);
  zoomOut->setShortcut(QKeySequence::ZoomOut);
  connect(zoomOut, &QAction::triggered, this,
          [this] { this->tabWidget.currentWidget().zoomOut(); });
...
QMenu *MainWindow::makeHelpMenuTab(QMenuBar *parent) noexcept {
  ...
  connect(onlineHelp, &QAction::triggered, this,
          &ui::MainWindow::openOnlineHelpPressed);

  auto contactUs = new QAction("Contact us...", helpMenu);
  connect(contactUs, &QAction::triggered, this,
          &ui::MainWindow::openContactUsPressed);

  auto openSite = new QAction("Open official site...", helpMenu);
  connect(openSite, &QAction::triggered, this,
          &ui::MainWindow::openSitePressed);

  auto openSourceCode = new QAction("View source code...", helpMenu);
  connect(openSourceCode, &QAction::triggered, this,
          &ui::MainWindow::openSourceCodePressed);
