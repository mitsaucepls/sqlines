...
Application::Application(int &argc, char **argv) noexcept
    : QApplication(argc, argv),
      ...
      configFilePath(":/config.txt"), sourceModes(loadSourceModes()),
      targetModes(loadTargetModes()), cmdModes(loadCmdModes()),

      programIsRunning(true),

      settingsLoader("SQLines", "SQLines Studio", this->settings,
                     this->snapshot),
      ...
      convertUtilityPresenter(this->settings, this->cmdModes,
                              this->convertUtilityWidget),
      settingsPresenter(this->settings, this->settingsLoader, this->license,
                        this->settingsWidget, this->mainWindow) {
  ...
  connect(&this->mainWindowPresenter,
          &presenter::MainWindowPresenter::openAboutPressed, &this->aboutWidget,
          &ui::AboutWidget::show);

  connect(&this->mainWindowPresenter,
          &presenter::MainWindowPresenter::openSettingsPressed,
          &this->settingsWidget, &ui::SettingsWidget::show);

  connect(&this->mainWindowPresenter,
          &presenter::MainWindowPresenter::openConvertUtilityPressed,
...
int Application::execute() noexcept {
  std::thread thread([this]() { makeCheckpoint(this->programIsRunning); });
  thread.detach();
