...
License::License(const model::Settings &settings,
  ...
  connect(this, &License::statusChangedInDetachedThread, this, statusChanged);
}

License::~License() { this->mutex.lock(); }
...
void License::monitorLicenseFile(
    const std::atomic<bool> &programIsRunning) noexcept {
  while (programIsRunning) {
    ...
    auto needsUpdate = [this, &licensePath] {
      return (!QFileInfo::exists(licensePath) ||
              QFileInfo(licensePath).lastModified() !=
                  this->licenseFileLastModified);
    };
