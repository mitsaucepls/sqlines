...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(ui->sourceDirButton, SIGNAL(pressed()), this,
          SLOT(openSourceDirectory()));
  connect(ui->targetDirButton, SIGNAL(pressed()), this,
          SLOT(openTargetDirectory()));
  ...
  connect(&sqlinesProcess, SIGNAL(readyReadStandardOutput()), this,
          SLOT(showSqlinesOutput()));
...
void MainWindow::startSqlines() {
  // Save options as some edit lines may be not in comboboxes yet (currentText
  // will return null)
  saveOptions();
...
void MainWindow::showSqlinesOutput() {
  ...
  for (int i = 0; i < fullOutput.size(); i++) {
    ...
    if (cur == '\b') {
      ...
      ui->logPlainTextEdit->moveCursor(QTextCursor::Left,
...
void MainWindow::sqlinesError(QProcess::ProcessError) {
  ...
  QMessageBox::critical(this, "SQLines",
                        "Starting sqlines command line - " +
                            sqlinesProcess.errorString());
}

// Open dialog to select a source directory
void MainWindow::openSourceDirectory() {
  QString dir = QFileDialog::getExistingDirectory(
      this, tr("Select a Source Directory"), lastSelectedSourceDir);

  if (!dir.isEmpty()) {
    lastSelectedSourceDir = dir;
    ui->sourceDirComboBox->insertItem(0,
...
void MainWindow::openTargetDirectory() {
  QString dir = QFileDialog::getExistingDirectory(
      this, tr("Select a Target Directory"), lastSelectedTargetDir);
...
void MainWindow::loadOptions() {
  ...
  if (!prevSourceType.isEmpty() &&
      typesList.contains(prevSourceType, Qt::CaseInsensitive))
    ui->sourceTypeComboBox->setCurrentIndex(
        ui->sourceTypeComboBox->findText(prevSourceType));

  QString prevTargetType = settings->value(INI_TARGET_TYPE).toString();
  // restore the value only if exists in the list
  if (!prevTargetType.isEmpty() &&
      typesList.contains(prevTargetType, Qt::CaseInsensitive))
    ui->targetTypeComboBox->setCurrentIndex(
        ui->targetTypeComboBox->findText(prevTargetType));
  ...
  ui->sourceDirComboBox->setCurrentIndex(
      settings->value(INI_LAST_CUR_SOURCE_DIR).toInt());
  ...
  ui->targetDirComboBox->setCurrentIndex(
      settings->value(INI_LAST_CUR_TARGET_DIR).toInt());
}
...
void MainWindow::saveOptions() {
  ...
  settings->setValue(INI_LAST_CUR_SOURCE_DIR,
                     ui->sourceDirComboBox->currentIndex());
  ...
  settings->setValue(INI_LAST_CUR_TARGET_DIR,
                     ui->targetDirComboBox->currentIndex());
  ...
  // settings->setValue(INI_CUR_COMMAND_LINE,
  // ui->commandLineComboBox->currentIndex());

  // other options
  // settings->setValue(INI_OPEN_REPORT, ui->openReportCheckBox->isChecked());
  // settings->setValue(INI_SWITCH_TO_LOG,
  // ui->switchToLogCheckBox->isChecked());
  // settings->setValue(INI_ITEMS_IN_HISTORY,
  // ui->itemsInHistoryLineEdit->text());
}
...
QString MainWindow::getCmdType(QString type) {
  ...
  if (type == "Oracle")
    ...
    cmd = "sql";
  else if (type == "IBM DB2")
    cmd = "db2";
  else if (type == "Sybase ASE")
    cmd = "sybase";
  else if (type == "Sybase SQL Anywhere")
    cmd = "asa";
  else if (type == "Sybase Advantage")
    cmd = "ads";
  else if (type == "Informix")
    cmd = "informix";
    ...
    cmd = "greenplum";
  else if (type == "Netezza")
    cmd = "netezza";
  else if (type == "Hive")
    cmd = "hive";
  else if (type == "EsgynDB")
    cmd = "esgyndb";
