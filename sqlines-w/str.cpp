...
char *Str::FormatTime(int time_ms, char *output) {
  ...
  if (time_ms < 1000)
    ...
    sprintf(output, "%d min %d sec", time_ms / 60 / 1000,
            (time_ms % (60 * 1000)) / 1000);
...
void Str::SqlTs2Str(short year, short month, short day, short hour,
                    short minute, short second, long fraction, char *out) {
  if (out == NULL)
    return;
...
// Convert 7-byte packed Oracle DATE to string (non-null terminated, exactly 19
// characters)
...
bool Str::CompareNumberAsString(const char *first, int flen, const char *second,
                                int slen) {
...
const char *Str::GetIntegerPart(const char *str, int slen, char *out,
                                int *out_len) {
...
struct abbreviations abbr[] = {{"active", "ACTIVE", 6, "act", "ACT"},
