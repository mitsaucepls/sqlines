...
void MainWindow::initializeComponents() {
  ...
  connect(ui->actionOnline_conversion, SIGNAL(triggered()), this,
          SLOT(openOnlineConversion()));
  connect(ui->actionOnline_help, SIGNAL(triggered()), this,
          SLOT(openOnlineHelp()));
  connect(ui->actionContact_us, SIGNAL(triggered()), this,
          SLOT(openContactUs()));
  connect(ui->actionExit, SIGNAL(triggered()), this, SLOT(exitApp()));

  // UI style menu items (View menu)
  connect(ui->actionWindows_7, SIGNAL(triggered()), this,
          SLOT(setWindows7Style()));
  connect(ui->actionWindows_XP, SIGNAL(triggered()), this,
          SLOT(setWindowsXPStyle()));
  connect(ui->actionWindows, SIGNAL(triggered()), this,
          SLOT(setWindowsStyle()));
  connect(ui->actionMacintosh, SIGNAL(triggered()), this,
          SLOT(setMacintoshStyle()));
  ...
  connect(ui->actionPlastigue, SIGNAL(triggered()), this,
          SLOT(setPlastiqueStyle()));
  connect(ui->actionCleanlooks, SIGNAL(triggered()), this,
          SLOT(setCleanlooksStyle()));
  connect(ui->actionDefaultStyle, SIGNAL(triggered()), this,
...
void MainWindow::sqlinesError(QProcess::ProcessError) {
  QMessageBox::critical(this, "SQLines Studio",
                        "Starting sqlines.exe - " +
...
void MainWindow::openFileDialog() {
  QStringList fileNames =
      QFileDialog::getOpenFileNames(this, tr("Open File(s)"), lastOpenDir, "");
